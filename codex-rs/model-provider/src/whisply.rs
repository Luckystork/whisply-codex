...
const WHISPLY_MODEL_INSTRUCTIONS: &str = "You are Whisply, a helpful general assistant. Follow the user's instructions and the runtime's permission and tool policies.";
...
impl WhisplyCatalogModelsEndpoint {
    ...
    async fn fetch_models(
        ...
        envelope
            ...
            .map_err(|_| Self::unavailable_catalog_error())?;
        let catalog_revision = envelope.catalog.catalog_revision.clone();
        let models = model_infos_from_catalog(&envelope.catalog)?;
...
fn model_info_from_catalog_model(index: usize, model: &CatalogModel) -> CoreResult<ModelInfo> {
    ...
    Ok(ModelInfo {
        ...
        description: Some("Whisply hosted model.".to_string()),
        ...
        model_messages: Some(ModelMessages {
            instructions_template: Some(WHISPLY_MODEL_INSTRUCTIONS.to_string()),
