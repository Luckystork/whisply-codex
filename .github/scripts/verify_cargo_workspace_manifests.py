...
"""Verify that whisply-rs Cargo manifests follow workspace manifest policy.

Checks:
- Crates inherit `[workspace.package]` metadata.
- Crates opt into `[lints] workspace = true`.
- Crate names follow the whisply-rs directory naming conventions.
...
MANIFEST_FEATURE_EXCEPTIONS = {
    "whisply-rs/code-mode/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
    "whisply-rs/v8-poc/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
}
...
def main() -> int:
    ...
    print(
        "Cargo manifests under whisply-rs must inherit workspace package metadata, "
    ...
    print(
        "Cargo only applies `whisply-rs/Cargo.toml` `[workspace.lints.clippy]` "
    ...
    print()
    print(
        "Package-name checks apply to `whisply-rs/<crate>/Cargo.toml` and "
        "`whisply-rs/utils/<crate>/Cargo.toml`."
    )
    print(
