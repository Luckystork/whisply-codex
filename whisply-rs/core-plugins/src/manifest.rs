...
pub type PluginManifest = whisply_plugin::manifest::PluginManifest<AbsolutePathBuf>;
pub type PluginManifestHooks = whisply_plugin::manifest::PluginManifestHooks<AbsolutePathBuf>;
pub type PluginManifestInterface =
    whisply_plugin::manifest::PluginManifestInterface<AbsolutePathBuf>;
pub type PluginManifestMcpServers =
    whisply_plugin::manifest::PluginManifestMcpServers<AbsolutePathBuf>;
pub type PluginManifestPaths = whisply_plugin::manifest::PluginManifestPaths<AbsolutePathBuf>;

pub type UriPluginManifest = whisply_plugin::manifest::PluginManifest<PathUri>;
...
fn resolve_raw_plugin_manifest(
    ...
    let interface = interface.and_then(|interface| {
        ...
        let interface = whisply_plugin::manifest::PluginManifestInterface {
            display_name,
            short_description,
            long_description,
            developer_name,
    ...
    Ok(whisply_plugin::manifest::PluginManifest {
        name,
        version,
        description,
        keywords,
        paths: whisply_plugin::manifest::PluginManifestPaths {
...
fn resolve_manifest_hooks(
    plugin_root: &PathUri,
    hooks: Option<RawPluginManifestHooks>,
) -> Option<whisply_plugin::manifest::PluginManifestHooks<PathUri>> {
    match hooks? {
        RawPluginManifestHooks::Path(path) => {
            resolve_manifest_path(plugin_root, "hooks", Some(&path))
                .map(|path| whisply_plugin::manifest::PluginManifestHooks::Paths(vec![path]))
        }
        RawPluginManifestHooks::Paths(paths) => {
            let hooks = paths
                ...
                .collect::<Vec<_>>();
            (!hooks.is_empty())
                .then_some(whisply_plugin::manifest::PluginManifestHooks::Paths(hooks))
        }
        RawPluginManifestHooks::Inline(hooks) => {
            Some(whisply_plugin::manifest::PluginManifestHooks::Inline(vec![
                *hooks,
            ]))
        }
        RawPluginManifestHooks::InlineList(hooks) => (!hooks.is_empty())
            .then_some(whisply_plugin::manifest::PluginManifestHooks::Inline(hooks)),
...
fn resolve_manifest_mcp_servers(
    plugin_root: &PathUri,
    mcp_servers: Option<RawPluginManifestMcpServers>,
) -> Option<whisply_plugin::manifest::PluginManifestMcpServers<PathUri>> {
    match mcp_servers? {
        RawPluginManifestMcpServers::Path(path) => {
            resolve_manifest_path(plugin_root, "mcpServers", Some(&path))
                .map(whisply_plugin::manifest::PluginManifestMcpServers::Path)
        }
        RawPluginManifestMcpServers::Object(servers) => match serde_json::to_string(&servers) {
            Ok(servers) => Some(whisply_plugin::manifest::PluginManifestMcpServers::Object(
                servers,
            )),
            Err(err) => {
...
mod tests {
    ...
    use whisply_plugin::PluginProvider;
    use whisply_plugin::ResolvedPlugin;
    use whisply_plugin::manifest::PluginManifest as GenericPluginManifest;
    use whisply_plugin::manifest::PluginManifestHooks;
    use whisply_plugin::manifest::PluginManifestInterface;
    use whisply_plugin::manifest::PluginManifestMcpServers;
    use whisply_plugin::manifest::PluginManifestPaths;
