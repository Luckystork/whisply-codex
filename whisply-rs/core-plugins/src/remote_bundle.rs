...
use whisply_http_client::HttpResponse;
use whisply_http_client::RouteAwareRequestError;
use whisply_plugin::PluginId;
use whisply_plugin::PluginIdError;
use whisply_utils_absolute_path::AbsolutePathBuf;
use whisply_utils_plugins::AGENT_PLUGIN_MANIFEST_RELATIVE_PATH;
use whisply_utils_plugins::AgentPluginSchemaStatus;
use whisply_utils_plugins::agent_plugin_schema_status;
...
pub enum RemotePluginBundleInstallError {
    ...
    DownloadBody {
        url: String,
        #[source]
        source: whisply_http_client::HttpError,
    },
...
fn install_remote_plugin_bundle(
    ...
    record_remote_bundle_package_hash(&codex_home, &result);
    Ok(result)
}

/// Measure the bundle that was just written, so a later edit to the unpacked
/// tree is a change rather than the only thing anyone ever knew about it.
///
/// A validated download says the bytes arrived intact; it says nothing about
/// the tree an hour later. Without this the remote install path is unpinned by
/// construction and its MCP servers start whatever the directory holds.
///
/// Only the package is recorded, not the launch pins: nobody was shown a
/// command line here, and claiming otherwise would report a review that did not
/// happen.
fn record_remote_bundle_package_hash(codex_home: &Path, installed: &PluginInstallResult) {
    let plugin_key = installed.plugin_id.as_key();
    let Some(package_hash) = plugin_package_hash_blocking(installed.installed_path.as_path())
    else {
        warn!(
            plugin = %plugin_key,
            "could not measure installed remote plugin bundle; its files are not pinned"
        );
        return;
    };
    // A pin that cannot be written leaves the plugin exactly as unpinned as it
    // was before the install, which is not a reason to fail the install.
    if let Err(err) = apply_user_plugin_config_edits_blocking(
        codex_home,
        ...
        warn!("failed to record installed remote plugin package hash: {err}");
    }
}
...
mod tests {
    ...
    /// A remote bundle arrives without anyone reading it, so nothing but the
    /// install itself can say what its files were. Without that record the
    /// unpacked tree can be rewritten and its MCP servers started forever.
    #[test]
    fn installing_a_remote_bundle_records_the_files_it_unpacked() {
        let codex_home = tempdir().expect("tempdir");
        ...
        let result = install_remote_plugin_bundle(
            ...
            tar_gz_bytes(&[
                ...
                ("server.js", b"console.log('linear');", /*mode*/ 0o644),
            ]),
        )
        .expect("install bundle");

        let recorded = std::fs::read_to_string(codex_home.path().join("config.toml"))
            .expect("the install should have written a config");
        assert!(
            recorded.contains("trusted_package_hash = \"sha256:"),
            "a remote bundle install left its files unpinned:\n{recorded}"
        );

        // The recorded hash has to be of the tree that was installed, not of
        // some other directory that happens to be measurable.
        let expected = plugin_package_hash_blocking(result.installed_path.as_path())
            .expect("measure the installed bundle");
        assert!(
            recorded.contains(&expected),
            "the recorded hash is not the installed tree:\n{recorded}"
        );
    }

    /// Editing the unpacked tree afterwards has to be visible as a change, or
    /// the recorded hash is decoration.
    #[test]
    fn a_remote_bundle_rewritten_after_install_no_longer_matches_what_was_recorded() {
        let codex_home = tempdir().expect("tempdir");
        ...
        .expect("install bundle");
        let recorded = std::fs::read_to_string(codex_home.path().join("config.toml"))
            .expect("read the recorded pin");

        fs::write(
            result.installed_path.as_path().join("server.js"),
            "require('child_process');",
        )
        .expect("rewrite the unpacked program");

        let rewritten = plugin_package_hash_blocking(result.installed_path.as_path())
            .expect("measure the rewritten bundle");
        assert!(
            !recorded.contains(&rewritten),
            "a rewritten bundle still measures as the one that was installed"
        );
    }
