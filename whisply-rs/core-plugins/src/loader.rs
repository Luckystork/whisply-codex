...
use crate::plugin_package_pin::PluginPackageTrust;
use crate::plugin_package_pin::plugin_package_hash_blocking;
use crate::plugin_package_pin::plugin_package_trust;
...
use whisply_config::HooksFile;
use whisply_config::PluginConfigEdit;
use whisply_config::PluginMcpLaunchTrust;
use whisply_config::apply_user_plugin_config_edits_blocking;
use whisply_config::plugin_mcp_launch_hash;
use whisply_config::plugin_mcp_launch_trust;
...
use whisply_config::types::PluginConfig;
use whisply_config::types::PluginMcpServerConfig;
use whisply_config::types::PluginPackageOrigin;
use whisply_connectors::parse_plugin_app_config;
use whisply_connectors::parse_plugin_app_config_value;
...
use whisply_mcp::parse_agent_plugin_mcp_config;
use whisply_mcp::parse_plugin_mcp_config;
use whisply_plugin::AppDeclaration;
use whisply_plugin::LoadedPlugin;
use whisply_plugin::PluginCapabilitySummary;
use whisply_plugin::PluginHookSource;
use whisply_plugin::PluginId;
use whisply_plugin::PluginIdError;
use whisply_plugin::app_connector_ids_from_declarations;
use whisply_protocol::auth::AuthMode;
use whisply_protocol::protocol::Product;
use whisply_protocol::protocol::SkillScope;
use whisply_skills::SkillConfigRules;
...
use whisply_utils_plugins::find_plugin_manifest_path;
...
pub fn refresh_curated_plugin_cache(
    ...
    let mut package_hash_edits = Vec::<PluginConfigEdit>::new();
    ...
    for plugin_id in configured_curated_plugin_ids {
        ...
        record_installed_package_hash(&mut package_hash_edits, plugin_id, &installed);
        cache_refreshed = true;
    }

    persist_installed_package_hashes(codex_home, package_hash_edits);

    Ok(cache_refreshed)
}

/// Measure a tree the moment it lands, so that an edit made to it afterwards is
/// a change against something.
///
/// The curated cache and remote bundles arrive without anyone reading them, so
/// this records what was installed rather than what was agreed to. That is
/// worth less than an approval and much more than nothing: it is the difference
/// between a cached plugin whose files can be rewritten and launched forever,
/// and one where rewriting them stops the launch.
///
/// Only the package is recorded. The launch pins stay absent because no command
/// line was shown to anyone, and writing them here would report a review that
/// never happened.
fn record_installed_package_hash(
    edits: &mut Vec<PluginConfigEdit>,
    plugin_id: &PluginId,
    installed: &PluginInstallResult,
) {
    let Some(package_hash) = plugin_package_hash_blocking(installed.installed_path.as_path())
    else {
        // A tree too large or too strange to measure is still installed; it is
        // simply not pinned, which is where every plugin started.
        warn!(
            plugin = %plugin_id.as_key(),
            "could not measure installed plugin package; its files are not pinned"
        );
        return;
    };
    edits.push(PluginConfigEdit::SetTrustedPackageHash {
        plugin_key: plugin_id.as_key(),
        package_hash,
    });
}

/// A pin that cannot be written leaves the plugin running unpinned, which is
/// what it was a moment ago; refusing the whole refresh over it would break a
/// working install to protect against an edit nobody has made.
fn persist_installed_package_hashes(codex_home: &Path, edits: Vec<PluginConfigEdit>) {
    if edits.is_empty() {
        return;
    }
    if let Err(err) = apply_user_plugin_config_edits_blocking(codex_home, edits) {
        warn!("failed to record installed plugin package hashes: {err}");
    }
}
...
/// The settings recorded for one plugin, including the launch pins its MCP
/// servers were approved with.
pub fn configured_plugin_from_codex_home(
    codex_home: &Path,
    plugin_key: &str,
) -> Option<PluginConfig> {
    configured_plugins_from_codex_home(
        codex_home,
        "failed to read user config while reviewing plugin MCP launches",
        "failed to parse user config while reviewing plugin MCP launches",
    )
    .remove(plugin_key)
}
...
async fn load_plugin(
    ...
    // Answered once for the whole plugin: the same files back both the servers
    // it starts and the hooks it runs without being asked.
    let package_trust = plugin_package_trust(
        plugin_root.as_path(),
        plugin.trusted_package_hash.as_deref(),
    )
    .await;
    match scope {
        PluginLoadScope::AllCapabilities {
            ...
            let plugin_key = loaded_plugin_id.as_key();
            loaded_plugin.mcp_servers = load_plugin_mcp_servers_from_manifest_with_format(
                plugin_root.as_path(),
                manifest_paths,
                PluginMcpPolicy {
                    plugin_id: Some(plugin_key.as_str()),
                    servers: Some(&plugin.mcp_servers),
                    launches_pinned: plugin.mcp_launch_pinned.unwrap_or(false),
                    package_trust,
                },
    ...
    let (hook_sources, hook_load_warnings) = if !package_trust.may_launch() {
        // Hooks are the part of a plugin that runs without being asked, so a
        // package nobody has agreed to must not be reached through them either.
        (
            Vec::new(),
            vec![package_refusal(
                &loaded_plugin_id.as_key(),
                package_trust,
                plugin_package_origin(
                    plugin.mcp_launch_pinned.unwrap_or(false),
...
/// Why none of a plugin's executable parts are being used, in the words the
/// person can act on.
fn package_refusal(
    plugin_key: &str,
    package_trust: PluginPackageTrust,
    origin: PluginPackageOrigin,
) -> String {
    let reason = match package_trust {
        PluginPackageTrust::Unreadable => McpServerDisabledReason::PluginPackageUnreadable {
            plugin: plugin_key.to_string(),
            origin,
        },
        _ => McpServerDisabledReason::PluginPackageChanged {
            plugin: plugin_key.to_string(),
            origin,
        },
    };
    format!("Hooks from `{plugin_key}` are not loaded: {reason}")
}

/// A plugin's own settings for the MCP servers it contributes: the per-server
/// policy the person set, and whether its launches have ever been reviewed.
///
/// The two travel together because a launch pin is only meaningful next to the
/// plugin it belongs to. Loads that have no plugin identity to attribute a
/// refusal to — the declaration-only reads used to describe a plugin — pass the
/// default, which enforces nothing.
#[derive(Clone, Copy)]
pub(crate) struct PluginMcpPolicy<'a> {
    pub(crate) plugin_id: Option<&'a str>,
    pub(crate) servers: Option<&'a HashMap<String, PluginMcpServerConfig>>,
    pub(crate) launches_pinned: bool,
    /// Whether the files behind those launches are still the approved ones.
    pub(crate) package_trust: PluginPackageTrust,
}

impl Default for PluginMcpPolicy<'_> {
    fn default() -> Self {
        Self {
            plugin_id: None,
            servers: None,
            launches_pinned: false,
            package_trust: PluginPackageTrust::Unpinned,
        }
    }
}

impl<'a> PluginMcpPolicy<'a> {
    fn server(&self, name: &str) -> Option<&'a PluginMcpServerConfig> {
        self.servers.and_then(|servers| servers.get(name))
    }

    /// Once a plugin has been pinned, a server with no pin of its own did not
    /// exist when the person approved the plugin. Before that, no pin means the
    /// installation simply predates pinning.
    fn pins_recorded(&self) -> bool {
        plugin_launches_reviewed(self.launches_pinned, self.servers)
    }

    fn package_origin(&self) -> PluginPackageOrigin {
        plugin_package_origin(self.launches_pinned, self.servers)
    }
}

fn plugin_launches_reviewed(
    launches_pinned: bool,
    servers: Option<&HashMap<String, PluginMcpServerConfig>>,
) -> bool {
    launches_pinned
        || servers.is_some_and(|servers| {
            servers
                .values()
                .any(|server| server.trusted_launch_hash.is_some())
        })
}

/// Launch pins are only written where a person was shown the command lines and
/// agreed to them, so their presence is what separates a package somebody
/// approved from one the product installed and measured on its own.
fn plugin_package_origin(
    launches_pinned: bool,
    servers: Option<&HashMap<String, PluginMcpServerConfig>>,
) -> PluginPackageOrigin {
    if plugin_launches_reviewed(launches_pinned, servers) {
        PluginPackageOrigin::Approved
    } else {
        PluginPackageOrigin::Installed
    }
}

/// Refuse to start a plugin MCP server that does not launch what was approved.
///
/// This runs on every load rather than at install, because a local plugin's
/// manifest is re-read every time and an installed tree can be edited in place.
/// A server with no recorded pin is left alone while the plugin itself has
/// never been pinned: every installation that predates pins would otherwise
/// stop working to guard against a substitution that has not happened.
///
/// The package is answered before the launch definition, because an unchanged
/// command line starting rewritten code is the case a launch pin cannot see.
fn enforce_plugin_mcp_launch_pin(
    policy: PluginMcpPolicy<'_>,
    server_name: &str,
    config: &mut McpServerConfig,
) {
    let Some(plugin_id) = policy.plugin_id else {
        return;
    };
    let trusted = policy
        .server(server_name)
        .and_then(|server| server.trusted_launch_hash.as_deref());
    let reason = match policy.package_trust {
        PluginPackageTrust::Changed => McpServerDisabledReason::PluginPackageChanged {
            plugin: plugin_id.to_string(),
            origin: policy.package_origin(),
        },
        PluginPackageTrust::Unreadable => McpServerDisabledReason::PluginPackageUnreadable {
            plugin: plugin_id.to_string(),
            origin: policy.package_origin(),
        },
        PluginPackageTrust::Unpinned | PluginPackageTrust::Trusted => {
            match plugin_mcp_launch_trust(config, trusted) {
                PluginMcpLaunchTrust::Trusted => return,
                PluginMcpLaunchTrust::Unpinned if !policy.pins_recorded() => return,
                PluginMcpLaunchTrust::Unpinned => McpServerDisabledReason::PluginServerAdded {
                    plugin: plugin_id.to_string(),
                },
                PluginMcpLaunchTrust::Changed => McpServerDisabledReason::PluginLaunchChanged {
                    plugin: plugin_id.to_string(),
                },
            }
        }
    ...
    warn!(
        ...
        server = server_name,
        reason = %reason,
        "plugin MCP server is not what was approved; not starting it"
    );
    config.enabled = false;
    config.disabled_reason = Some(reason);
}
...
pub async fn plugin_capability_summary_from_root(
    ...
    let mut mcp_server_names = load_plugin_mcp_servers_from_manifest_with_format(
        plugin_root.as_path(),
        manifest_paths,
        // Names only: a summary that hid a pinned-out server would make the
        // plugin look like it never declared one.
        PluginMcpPolicy::default(),
...
pub async fn load_plugin_mcp_servers(
    ...
    load_plugin_mcp_servers_with_policy(plugin_root, auth_mode, PluginMcpPolicy::default()).await
}

/// The launch definitions a plugin declares, as they would be approved.
///
/// Used at install, where the person has just been shown these command lines
/// and agreed to them, so what they saw becomes the pin.
pub async fn declared_plugin_mcp_launch_hashes(plugin_root: &Path) -> BTreeMap<String, String> {
    load_declared_plugin_mcp_servers(plugin_root, PluginMcpPolicy::default())
        .await
        .iter()
        .map(|(name, config)| (name.clone(), plugin_mcp_launch_hash(config)))
        .collect()
}
...
pub async fn load_configured_plugin_mcp_servers(
    ...
    let configured = configured_plugins.get(&plugin_id);
    let plugin_policy = PluginMcpPolicy {
        plugin_id: Some(&plugin_id),
        servers: configured.map(|plugin| &plugin.mcp_servers),
        launches_pinned: configured.is_some_and(|plugin| plugin.mcp_launch_pinned.unwrap_or(false)),
        package_trust: plugin_package_trust(
            plugin_root,
            configured.and_then(|plugin| plugin.trusted_package_hash.as_deref()),
        )
        .await,
    };
...
async fn load_plugin_mcp_servers_with_policy(
    plugin_root: &Path,
    auth_mode: Option<AuthMode>,
    plugin_policy: PluginMcpPolicy<'_>,
) -> HashMap<String, McpServerConfig> {
    ...
    match &manifest_paths.mcp_servers {
        Some(PluginManifestMcpServers::Object(object_servers)) => {
            ...
            for (name, mut config) in plugin_mcp.mcp_servers {
                if let Some(policy) = plugin_policy.server(&name) {
                    apply_plugin_mcp_server_policy(&mut config, policy);
                }
                enforce_plugin_mcp_launch_pin(plugin_policy, &name, &mut config);
...
pub(crate) fn git_command(args: &[&str], cwd: Option<&Path>) -> Command {
    let mut command = Command::new("git");
    // Refreshing a plugin's source runs third-party-controlled hooks and
    // filters through git's own configuration. None of that needs to be told
    // where this account's data lives.
    codex_whisply::withhold_reserved_environment(|name| {
        command.env_remove(name);
    });
    command
        .args(["-c", whisply_git_utils::SAFE_BARE_REPOSITORY_CONFIG])
        .args(args);
