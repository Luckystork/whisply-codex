...
fn git_command_sanitizes_ambient_repository_environment() {
    ...
    assert_eq!(
        ...
        [
            OsStr::new("-c"),
            OsStr::new(whisply_git_utils::SAFE_BARE_REPOSITORY_CONFIG),
        ]
    );
...
async fn ordinary_clone_rejects_tracked_embedded_bare_repository() {
    ...
    let apply_error = whisply_git_utils::apply_git_patch(&whisply_git_utils::ApplyGitRequest {
        cwd: nested.clone(),
        diff: String::new(),
        revert: false,
        preflight: true,
    ...
    assert!(whisply_git_utils::collect_git_info(&nested).await.is_none());
    assert!(
        whisply_git_utils::git_diff_to_remote(&nested)
            .await
            .is_none()
    );
    assert!(
