...
use crate::loader::PluginMcpPolicy;
use crate::loader::TargetCuratedMarketplace;
use crate::loader::configured_curated_plugin_ids_from_codex_home;
use crate::loader::configured_plugin_from_codex_home;
use crate::loader::curated_plugin_cache_version;
use crate::loader::declared_plugin_mcp_launch_hashes;
...
use crate::plugin_disclosure::PluginMcpLaunchDisclosure;
use crate::plugin_disclosure::PluginUpdateBehavior;
use crate::plugin_disclosure::plugin_mcp_launch_summary;
use crate::plugin_package_pin::PluginPackageTrust;
use crate::plugin_package_pin::plugin_package_hash;
...
use whisply_config::types::ToolSuggestDiscoverableType;
...
use whisply_plugin::prompt_safe_plugin_description;
...
use whisply_tools::DiscoverablePluginInfo;
use whisply_tools::DiscoverableTool;
use whisply_tools::filter_request_plugin_install_discoverable_tools_for_client;
...
/// A plugin MCP server whose current launch definition has just been approved.
#[derive(Debug, Clone, PartialEq)]
pub struct PluginMcpLaunchApproval {
    pub server_name: String,
    pub launch_hash: String,
    pub config: McpServerConfig,
    /// How this definition compares with the pin recorded for it, which is the
    /// part of a review that says whether anything needs deciding.
    pub trust: PluginMcpLaunchTrust,
}

/// What a plugin would start right now: the command lines, and the files those
/// command lines run.
///
/// Both halves travel together because approving one without the other is not
/// an approval of anything. A reviewed command line can start rewritten code,
/// and reviewed code can be started by a command line nobody has seen.
#[derive(Debug, Clone, PartialEq)]
pub struct PluginLaunchReview {
    /// The files as they are now, or `None` when the package cannot be hashed.
    pub package_hash: Option<String>,
    /// How those files compare with the hash recorded for them.
    pub package_trust: PluginPackageTrust,
    /// Whether the recorded files were ever put in front of anyone. Curated and
    /// remote plugins are measured at install without being reviewed, and a
    /// report that calls that an approval is telling the reader something that
    /// did not happen.
    pub package_origin: PluginPackageOrigin,
    pub servers: Vec<PluginMcpLaunchApproval>,
}
...
pub struct PluginDetail {
    ...
    /// What each of those servers would start. The names alone say a plugin
    /// contributes a server; they do not say it runs `sh -c` on something it
    /// downloaded, which is the part a person is actually deciding about.
    pub mcp_launches: Vec<PluginMcpLaunchDisclosure>,
    /// What installing this signs up for after today: how a new version
    /// arrives, and whether it runs when it does.
    pub update_behavior: PluginUpdateBehavior,
    /// Host environment variables the plugin's MCP servers declare they will
    /// read, so a user can see what they are being asked to share before
    /// installing. Refused entries are kept separate rather than dropped: a
    /// plugin reaching for Whisply's own namespace is a fact about that plugin.
    pub host_env_usage: DeclaredHostEnvUsage,
    pub details_unavailable_reason: Option<PluginDetailsUnavailableReason>,
}

/// Merges what every server in a plugin declares into one disclosure.
pub fn declared_host_env_usage<'a>(
    servers: impl IntoIterator<Item = &'a McpServerConfig>,
) -> DeclaredHostEnvUsage {
    let mut merged = DeclaredHostEnvUsage::default();
    for server in servers {
        let usage = server.declared_host_env_usage();
        merged.forwarded.extend(usage.forwarded);
        merged.refused.extend(usage.refused);
    }
    for bucket in [&mut merged.forwarded, &mut merged.refused] {
...
pub struct PluginHookSummary {
    ...
    /// The command this hook runs unprompted once the plugin is installed.
    /// `None` for prompt and agent handlers, which execute nothing locally.
    pub command: Option<String>,
    /// Which occurrences of the event this hook is scoped to; `None` means all
    /// of them.
...
impl PluginsManager {
    ...
    pub fn new_with_options(
        ...
        // Product restrictions are enforced at marketplace admission time for a given WHISPLY_HOME:
        ...
        // contents of that WHISPLY_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single WHISPLY_HOME is only used by one product.
        Self {
    ...
    async fn install_resolved_plugin(
        ...
        // The install detail just showed these command lines and the person
        // agreed to them, so what they saw is what may run. A later edit to the
        // manifest is refused at load until it is reviewed.
        let mut launch_pins = vec![PluginConfigEdit::MarkMcpLaunchesPinned {
            plugin_key: result.plugin_id.as_key(),
        }];
        // The command lines are not the programs they start. Recording the
        // files as installed is what makes an edit to one of them afterwards
        // something other than the plugin the person agreed to.
        if let Some(package_hash) = plugin_package_hash(result.installed_path.as_path()).await {
            launch_pins.push(PluginConfigEdit::SetTrustedPackageHash {
                plugin_key: result.plugin_id.as_key(),
                package_hash,
            });
        }
        launch_pins.extend(
            declared_plugin_mcp_launch_hashes(result.installed_path.as_path())
                .await
                .into_iter()
                .map(
                    |(server_name, launch_hash)| PluginConfigEdit::SetMcpTrustedLaunchHash {
                        plugin_key: result.plugin_id.as_key(),
                        server_name,
                        launch_hash,
                    },
        ...
        apply_user_plugin_config_edits(&self.codex_home, launch_pins)
            .await
            .map_err(anyhow::Error::from)?;
    ...
    /// What an installed plugin would start right now, and how each definition
    /// compares with the pin recorded for it.
    ///
    /// Reading is separate from approving so a caller can show the person what
    /// they are agreeing to before anything is recorded. Approving without
    /// having seen this is approving a blank cheque.
    pub async fn review_plugin_mcp_launch_definitions(
        &self,
        plugin_id: String,
    ) -> anyhow::Result<PluginLaunchReview> {
        let plugin_id = PluginId::parse(&plugin_id)?;
        let plugin_key = plugin_id.as_key();
        let Some(plugin_root) = self.store.active_plugin_root(&plugin_id) else {
            anyhow::bail!("plugin `{plugin_key}` is not installed");
        };
        let configured = configured_plugin_from_codex_home(&self.codex_home, &plugin_key);
        let recorded = |server_name: &str| {
            configured.as_ref().and_then(|plugin| {
                plugin
                    .mcp_servers
                    .get(server_name)
                    .and_then(|server| server.trusted_launch_hash.clone())
            })
        };
        let recorded_package_hash = configured
            .as_ref()
            .and_then(|plugin| plugin.trusted_package_hash.clone());
        let package_hash = plugin_package_hash(plugin_root.as_path()).await;
        let package_trust = match (&package_hash, &recorded_package_hash) {
            (_, None) => PluginPackageTrust::Unpinned,
            (None, Some(_)) => PluginPackageTrust::Unreadable,
            (Some(actual), Some(trusted)) if actual == trusted => PluginPackageTrust::Trusted,
            (Some(_), Some(_)) => PluginPackageTrust::Changed,
        };
        let servers = load_plugin_mcp_servers(plugin_root.as_path(), self.auth_mode()).await;
        let mut approvals = servers
            .into_iter()
            .map(|(server_name, config)| PluginMcpLaunchApproval {
                launch_hash: plugin_mcp_launch_hash(&config),
                trust: plugin_mcp_launch_trust(&config, recorded(&server_name).as_deref()),
                server_name,
                config,
            })
            .collect::<Vec<_>>();
        approvals.sort_by(|left, right| left.server_name.cmp(&right.server_name));
        let package_origin = match configured.as_ref() {
            Some(plugin) if plugin.mcp_launch_pinned.unwrap_or(false) => {
                PluginPackageOrigin::Approved
            }
            _ => PluginPackageOrigin::Installed,
        };
        Ok(PluginLaunchReview {
            package_hash,
            package_trust,
            package_origin,
            servers: approvals,
    ...
    /// Record the reviewed definitions as what this plugin may launch.
    ///
    /// This is the way back from a refused server: the caller shows what the
    /// plugin will start, the person decides, and what they saw becomes the new
    /// pin.
    pub async fn trust_plugin_mcp_launch_definitions(
        &self,
        plugin_id: String,
        review: &PluginLaunchReview,
    ) -> anyhow::Result<()> {
        let plugin_key = PluginId::parse(&plugin_id)?.as_key();
        let mut edits = vec![PluginConfigEdit::MarkMcpLaunchesPinned {
            plugin_key: plugin_key.clone(),
        }];
        // The hash recorded is the one that was read out, not a fresh read: an
        // edit made between the review and the answer must not be approved by
        // an answer given before it happened.
        if let Some(package_hash) = review.package_hash.clone() {
            edits.push(PluginConfigEdit::SetTrustedPackageHash {
                plugin_key: plugin_key.clone(),
                package_hash,
            });
        }
        edits.extend(review.servers.iter().map(|approval| {
        ...
        apply_user_plugin_config_edits(&self.codex_home, edits).await?;
        Ok(())
    }
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        if plugin.source.is_install_materialized() && !plugin.installed {
            ...
            return Ok(PluginDetail {
                ...
                mcp_server_names: Vec::new(),
                mcp_launches: Vec::new(),
                update_behavior: self.plugin_update_behavior(config, marketplace_name),
                host_env_usage: DeclaredHostEnvUsage::default(),
        ...
        let loaded_manifest =
            if whisply_utils_plugins::find_plugin_manifest_path(source_path.as_path()).is_some() {
        ...
        let skill_config_rules = whisply_core_skills::config_rules::skill_config_rules_from_stack(
            &config.config_layer_stack,
        );
        ...
        let mut mcp_servers = load_plugin_mcp_servers_from_manifest_with_format(
            source_path.as_path(),
            &manifest.paths,
            // Pre-install disclosure: nothing is pinned yet, and these are the
            // definitions the person is being asked to approve.
        ...
        // Derived before the configs are reduced to names, which is the only
        // point where what a server intends to read is still visible.
        let host_env_usage = declared_host_env_usage(mcp_servers.values());
        let mut mcp_launches = mcp_servers
            .into_iter()
            .map(|(server_name, config)| PluginMcpLaunchDisclosure {
                server_name,
                launch: plugin_mcp_launch_summary(&config),
            })
            .collect::<Vec<_>>();
        mcp_launches.sort_by(|left, right| left.server_name.cmp(&right.server_name));
        mcp_launches.dedup_by(|left, right| left.server_name == right.server_name);
        let mcp_server_names = mcp_launches
            .iter()
            .map(|launch| launch.server_name.clone())
            .collect::<Vec<_>>();
    ...
    /// Whether Whisply upgrades this marketplace on its own is a property of
    /// how the person configured it, so it is read from their config rather
    /// than guessed from the plugin.
    fn plugin_update_behavior(
        &self,
        config: &PluginsConfigInput,
        marketplace_name: &str,
    ) -> PluginUpdateBehavior {
        let marketplace_is_git = configured_git_marketplace_names(&config.config_layer_stack)
            .iter()
            .any(|configured| configured == marketplace_name);
        PluginUpdateBehavior::for_marketplace(marketplace_name, marketplace_is_git)
    }
