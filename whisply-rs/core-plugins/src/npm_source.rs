...
/// The child that fetches a plugin's package from a registry.
///
/// It keeps the person's own npm configuration -- their registry, their
/// credentials for it, their proxy -- because that is what makes a private
/// package install at all. What it does not keep is anything of Whisply's.
pub(crate) fn npm_pack_command(
    destination: &Path,
    package_spec: &str,
    registry: Option<&str>,
    npm_command: &OsStr,
...
fn pack_npm_package(
    ...
    let mut command = npm_pack_command(destination, &package_spec, registry, npm_command);
