...
use whisply_utils_plugins::AGENT_PLUGIN_SCHEMA_URI;
...
/// A plugin whose manifest declares one stdio MCP server.
async fn plugin_declaring_one_mcp_server(
    temp_dir: &TempDir,
    command: &str,
) -> (std::path::PathBuf, PluginManifestPaths) {
    ...
    write_file(
        ...
        &format!(r#"{{"mcpServers":{{"notes":{{"command":"{command}"}}}}}}"#),
    );
    let manifest = load_plugin_manifest(&plugin_root).expect("load manifest");
    (plugin_root, manifest.paths)
}

async fn load_notes_server(
    plugin_root: &std::path::Path,
    manifest_paths: &PluginManifestPaths,
    policy: Option<PluginMcpServerConfig>,
) -> McpServerConfig {
    load_notes_server_for_plugin(plugin_root, manifest_paths, policy, /*pinned*/ false).await
}

async fn load_notes_server_for_plugin(
    plugin_root: &std::path::Path,
    manifest_paths: &PluginManifestPaths,
    policy: Option<PluginMcpServerConfig>,
    launches_pinned: bool,
) -> McpServerConfig {
    load_notes_server_for_package(
        plugin_root,
        manifest_paths,
        policy,
        launches_pinned,
...
async fn load_notes_server_for_package(
    plugin_root: &std::path::Path,
    manifest_paths: &PluginManifestPaths,
    policy: Option<PluginMcpServerConfig>,
    launches_pinned: bool,
    ...
    let servers = policy.map(|policy| HashMap::from([("notes".to_string(), policy)]));
    load_plugin_mcp_servers_from_manifest_with_format(
        plugin_root,
        manifest_paths,
        PluginMcpPolicy {
            plugin_id: Some("plugin@market"),
...
#[tokio::test]
async fn a_plugin_mcp_server_that_changed_after_approval_is_not_started() {
    let temp_dir = TempDir::new().expect("tempdir");
    let (plugin_root, manifest_paths) =
        plugin_declaring_one_mcp_server(&temp_dir, "replacement").await;

    let server = load_notes_server(
        &plugin_root,
        &manifest_paths,
        Some(PluginMcpServerConfig {
            trusted_launch_hash: Some("sha256:what-was-approved".to_string()),
            ..PluginMcpServerConfig::default()
        }),
    )
    .await;

    assert!(!server.enabled, "a substituted command line was launchable");
    let reason = server
        .disabled_reason
        .as_ref()
        ...
        .to_string();
    assert!(
        reason.contains("plugin@market"),
        "the reason does not name the plugin: {reason}"
    );
    assert!(
        reason.contains("whisply plugin trust"),
        "the reason does not say how to approve the change: {reason}"
    );
}

#[tokio::test]
async fn the_approved_definition_and_an_unpinned_one_both_start() {
    let temp_dir = TempDir::new().expect("tempdir");
    let (plugin_root, manifest_paths) = plugin_declaring_one_mcp_server(&temp_dir, "echo").await;

    let unpinned = load_notes_server(&plugin_root, &manifest_paths, None).await;
    assert!(
        unpinned.enabled,
        "installations that predate pins stopped working"
    );

    let approved = load_notes_server(
        &plugin_root,
        &manifest_paths,
        Some(PluginMcpServerConfig {
            trusted_launch_hash: Some(plugin_mcp_launch_hash(&unpinned)),
            ..PluginMcpServerConfig::default()
        }),
    )
    .await;
    assert!(approved.enabled);
    assert!(approved.disabled_reason.is_none());
}

/// The pin is per server, so the interesting case is the server that has none:
/// on a plugin that was never pinned it predates pinning, and on a plugin that
/// was pinned it is a server nobody agreed to run.
#[tokio::test]
async fn a_server_that_appeared_after_the_plugin_was_reviewed_is_not_started() {
    let temp_dir = TempDir::new().expect("tempdir");
    let (plugin_root, manifest_paths) = plugin_declaring_one_mcp_server(&temp_dir, "echo").await;

    let added = load_notes_server_for_plugin(
        &plugin_root,
        &manifest_paths,
        /*policy*/ None,
        /*launches_pinned*/ true,
    )
    .await;

    assert!(
        !added.enabled,
        "a server added after review launched unreviewed"
    );
    let reason = added
        .disabled_reason
        .as_ref()
        ...
        .to_string();
    assert!(
        reason.contains("added after"),
        "the reason describes a change rather than a new server: {reason}"
    );
    assert!(
        reason.contains("whisply plugin trust"),
        "the reason does not say how to approve it: {reason}"
    );
}

/// The case a launch pin cannot see: `node server.js` is the same sentence
/// after `server.js` has been rewritten.
#[tokio::test]
async fn an_unchanged_command_line_does_not_approve_changed_files() {
    let temp_dir = TempDir::new().expect("tempdir");
    let (plugin_root, manifest_paths) = plugin_declaring_one_mcp_server(&temp_dir, "echo").await;
    let approved = load_notes_server(&plugin_root, &manifest_paths, None).await;

    let server = load_notes_server_for_package(
        &plugin_root,
        &manifest_paths,
        Some(PluginMcpServerConfig {
            trusted_launch_hash: Some(plugin_mcp_launch_hash(&approved)),
            ..PluginMcpServerConfig::default()
        }),
        /*launches_pinned*/ true,
    ...
    assert!(
        !server.enabled,
        "a rewritten program launched behind an approved command line"
    );
    let reason = server
        .disabled_reason
        .as_ref()
    ...
    assert!(
        ...
        "the reason blames the command line rather than the files: {reason}"
    );
    assert!(
        reason.contains("plugin@market") && reason.contains("whisply plugin trust"),
        "the reason does not say whose it is or how to approve it: {reason}"
    );
}

#[tokio::test]
async fn a_package_that_cannot_be_read_is_not_launched_either() {
    let temp_dir = TempDir::new().expect("tempdir");
    let (plugin_root, manifest_paths) = plugin_declaring_one_mcp_server(&temp_dir, "echo").await;
    ...
    let reason = server
        ...
        .to_string();
    assert!(
        reason.contains("can no longer be read"),
        "an unreadable package was reported as something else: {reason}"
    );
}

#[tokio::test]
async fn approved_files_behind_an_approved_command_line_still_start() {
    let temp_dir = TempDir::new().expect("tempdir");
    let (plugin_root, manifest_paths) = plugin_declaring_one_mcp_server(&temp_dir, "echo").await;
    let approved = load_notes_server(&plugin_root, &manifest_paths, None).await;
...
/// The person's own settings must not read as a substituted command line.
#[tokio::test]
async fn changing_tool_policy_does_not_revoke_the_approval() {
    let temp_dir = TempDir::new().expect("tempdir");
    let (plugin_root, manifest_paths) = plugin_declaring_one_mcp_server(&temp_dir, "echo").await;
    let approved = load_notes_server(&plugin_root, &manifest_paths, None).await;

    let restricted = load_notes_server(
        &plugin_root,
        &manifest_paths,
        Some(PluginMcpServerConfig {
            trusted_launch_hash: Some(plugin_mcp_launch_hash(&approved)),
    ...
    assert_eq!(restricted.disabled_tools, Some(vec!["write".to_string()]));
}

/// A server the person turned off stays off for their own reason, not for a
/// pin that never applied to it.
#[tokio::test]
async fn a_server_disabled_by_the_person_is_not_relabelled() {
    let temp_dir = TempDir::new().expect("tempdir");
    let (plugin_root, manifest_paths) = plugin_declaring_one_mcp_server(&temp_dir, "echo").await;
