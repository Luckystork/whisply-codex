...
use whisply_utils_image::data_url_from_bytes;
...
use whisply_utils_image::ImageProcessingError;
...
fn local_media_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!("Whisply could not read the local {media_name} at `{path}`: {error}"),
    }
}
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Whisply cannot attach image at `{}`: unsupported image `{}`.",
            path.display(),
            mime
        ),
    }
...
pub fn local_image_content_items_with_label_number(
    ...
    match load_for_prompt_bytes(path, file_bytes, mode) {
        ...
        Err(err) => match &err {
            ...
            | ImageProcessingError::ImageTooLargeToDecode { .. } => {
...
fn unsupported_audio_error_placeholder(path: &std::path::Path) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Whisply cannot attach audio at `{}`: unsupported audio format; use wav, mp3, m4a, webm, or ogg.",
            path.display()
        ),
    }
}
...
// (Moved event mapping logic into whisply-core to avoid coupling protocol to UI-facing events.)
...
- ["a", "b", "c"]"#
        ...
        for i in 0..200 {
            exec_policy
                .add_prefix_rule(
                    ...
                    whisply_execpolicy::Decision::Allow,
                )
                .expect("add rule");
        }
    ...
    fn replaces_unreadable_local_audio_with_placeholder() {
        ...
        assert!(
            text.starts_with("Whisply could not read the local audio at `missing.wav`: "),
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Whisply cannot attach image at `{}`: unsupported image `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
