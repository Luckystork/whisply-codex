...
pub struct CompactedItem {
    ...
    ///
    /// Keeping it in the checkpoint makes a compaction one durable record: a failed write can
    /// never leave a replayable replacement history without the context that explains it.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub reference_context_item: Option<TurnContextItem>,
    /// Full world-state baseline for this replacement-history window.
    ///
    /// Compaction resets the prior baseline, so this belongs to the same durable checkpoint as
    /// the replacement history rather than a later independent rollout record.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(whisply_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                whisply_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
pub struct SkillMetadata {
    ...
    /// Declared by the package in its `SKILL.md` frontmatter. Opaque: packages
    /// in the wild version themselves with dates and commit shorthands, not
...
mod tests {
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd =
            whisply_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_codex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".codex"))
            .expect("canonical .codex");
        let expected_dot_whisply =
            AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".whisply"))
                .expect("canonical .whisply");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: FileSystemSpecialPath::project_roots(/*subpath*/ None),
            ...
            vec![
                (
                    ...
                    vec![
                        ...
                        expected_dot_whisply.to_path_buf(),
