...
use whisply_http_client::HttpError;
use whisply_utils_string::truncate_middle_chars;
...
pub enum CodexErrorDetails {
    ...
    Stream(String),
    #[error(
        "Whisply ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ContextWindowExceeded,
    #[error(
        "Whisply could not compact this conversation because this Mac is out of storage. Free up space, then try again. The original conversation is unchanged."
    )]
    StorageFull,
...
impl CodexErr {
    ...
    pub fn to_codex_protocol_error(&self) -> CodexErrorInfo {
        match &self.details {
            ...
            CodexErrorDetails::StorageFull => CodexErrorInfo::StorageFull,
            CodexErrorDetails::UsageLimitReached(_)
            | CodexErrorDetails::QuotaExceeded
