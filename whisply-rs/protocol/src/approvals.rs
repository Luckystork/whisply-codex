...
/// Outcome returned by the guardian reviewer.
///
/// `Uncertain` is what the reviewer says when it looked at the action and
/// could not tell. It is offered only under Whisply's `Auto`, where an
/// undecidable review becomes a question for the person rather than a guess
...
pub enum GuardianAssessmentStatus {
    ...
    TimedOut,
    Aborted,
    /// The review reached no trustworthy verdict, so the decision was handed
    /// to the person instead of being made on their behalf. Terminal for the
    /// review; the approval itself continues as a user prompt.
    AskUser,
}
