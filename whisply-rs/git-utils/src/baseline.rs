...
const BASELINE_COMMIT_MESSAGE: &str =
    "Initialize Whisply git baseline\n\nCo-authored-by: Whisply <noreply@whisply.net>";
...
fn commit_current_tree(repo: &gix::Repository, message: &str) -> anyhow::Result<()> {
    ...
    let signature = whisply_signature();
...
fn whisply_signature() -> gix::actor::Signature {
    gix::actor::Signature {
        name: "Whisply".into(),
        email: "noreply@whisply.net".into(),
        time: gix::date::Time {
