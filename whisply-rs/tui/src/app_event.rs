...
pub(crate) enum AppEvent {
    ...
    /// Start a new session in a directory the user just chose.
    ///
    /// A thread's working directory is fixed for its lifetime, so choosing a
    /// different one means starting a thread rather than editing this one.
    NewSessionInDirectory {
    ...
    /// Settled result of a broker-backed Whisply Usage read. This keeps
    ...
    ManagedStatusUsageResult {
        request_id: u64,
        result: Result<Box<ContextualUsageSnapshot>, String>,
    },
