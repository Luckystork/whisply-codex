...
use codex_whisply::PROMPT_COMPOSITION_CONTRACT_VERSION;
...
pub(crate) fn new_debug_config_output(
    ...
    lines.extend(render_prompt_composition_lines(config));
...
/// Reports which prompt profile this session composes under.
///
/// The profile is resolved by calling the composition contract's own activation
/// rule rather than restating it here, so this cannot drift from the profile the
/// runtime actually applies. It follows the user's explicit directory choice:
/// every session has a working directory, including a no-directory one, so the
/// presence of `cwd` is not the signal.
fn render_prompt_composition_lines(config: &Config) -> Vec<Line<'static>> {
    let intent = if config.workspace_directory_selected {
        WorkspaceIntent::SelectedDirectory
    } else {
        WorkspaceIntent::NoDirectory
    };
    let profile = PromptProfile::activate(intent);
    vec![
        "".into(),
        "Prompt composition:".bold().into(),
        format!("  - contract_version = {PROMPT_COMPOSITION_CONTRACT_VERSION}").into(),
        format!("  - profile          = {}", profile.catalog_id()).into(),
        format!(
            "  - workspace_directory_selected = {}",
            config.workspace_directory_selected
        )
        .into(),
    ]
}
...
mod tests {
    ...
    use super::render_prompt_composition_lines;
    ...
    use whisply_config::ConstraintError;
    use whisply_config::FeatureRequirementsToml;
    use whisply_config::FilesystemConstraints;
    ...
    use whisply_config::NetworkUnixSocketPermissionToml;
    use whisply_config::NetworkUnixSocketPermissionsToml;
    use whisply_config::RequirementSource;
    use whisply_config::ResidencyRequirement;
    use whisply_config::SandboxModeRequirement;
    use whisply_config::Sourced;
    use whisply_config::WebSearchModeRequirement;
    use whisply_config::WindowsRequirementsToml;
...
"#,
    ...
    /// `/debug-config` must report the profile the runtime will actually
    /// compose under, and that follows the user's explicit directory choice.
    /// A session always has a working directory, so reporting `workspace`
    /// whenever `cwd` is set would mislabel every no-directory session.
    #[tokio::test]
    async fn debug_config_reports_the_prompt_profile_from_the_explicit_choice() {
        let codex_home = tempfile::tempdir().expect("create temp dir");
        ...
        let general = render_to_text(&render_prompt_composition_lines(&config));
        assert!(general.contains("profile          = general"), "{general}");
        assert!(
        ...
        let workspace = render_to_text(&render_prompt_composition_lines(&config));
        assert!(
            workspace.contains("profile          = workspace"),
            "{workspace}"
        );
    }
