...
pub(crate) fn migration_copy_for_models(
    ...
    let heading_text = Span::from(format!(
        "Whisply just got an upgrade. Introducing {target_display_name}."
    ))
    .bold();
...
impl WidgetRef for &ModelMigrationScreen {
    fn render_ref(&self, area: ratatui::layout::Rect, buf: &mut ratatui::buffer::Buffer) {
        ...
        if let Some(markdown) = self.copy.markdown.as_ref() {
            ...
            // Wrap the heading like the body below it. Pushed as a bare line it
            // is truncated at the pane width, which silently cut the model name
            // in the middle of a word once the product name grew.
...
impl ModelMigrationScreen {
    ...
    fn render_menu(&self, column: &mut ColumnRenderable) {
        column.push(Line::from(""));
        column.push(
            Paragraph::new("Choose how you'd like Whisply to proceed.")
                .wrap(Wrap { trim: false })
                .inset(Insets::tlbr(
                    /*top*/ 0, /*left*/ 2, /*bottom*/ 0, /*right*/ 0,
                )),
