...
use super::rate_limits::compose_managed_usage_data;
...
const WHISPLY_USAGE_COMMAND: &str = "whisply usage";
...
impl StatusHistoryHandle {
    ...
    /// Fills the limits section from the Whisply-managed Usage snapshot.
    pub(crate) fn finish_managed_usage_refresh(
        ...
        let mut state = self
            ...
            .write()
            .expect("status history rate-limit state poisoned");
        // A failed read leaves the section unavailable rather than claiming a
        // refresh is still coming, because no later event will fill it in.
        state.rate_limits = match snapshot {
            Some(snapshot) => compose_managed_usage_data(snapshot, now),
            None => StatusRateLimitData::Unavailable,
        };
        state.refreshing_rate_limits = false;
    }
}
...
impl StatusHistoryCell {
    ...
    fn new(
        ...
        let show_whisply_usage_hint = is_whisply_provider(&config.model_provider);
...
fn status_permissions_label(
    ...
    match active_id {
            ...
            if permission_profile == &PermissionProfile::Disabled =>
        {
            return if approval_policy == AskForApproval::Never {
                crate::permission_mode::PermissionMode::FullAccess
                    .display_name()
                    .to_string()
            } else {
    ...
    {
        return crate::permission_mode::PermissionMode::FullAccess
            .display_name()
            .to_string();
    }
    let sandbox = decorate_workspace_sandbox_label(sandbox, workspace_root_suffix);
...
fn status_approval_label(
    ...
    if let Some(mode) =
        crate::permission_mode::active_permission_mode(approval_policy, approvals_reviewer)
        && approval_policy == AskForApproval::OnRequest
    {
        return mode.display_name().to_string();
    }
    ...
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            StatusAccountDisplay::ChatGpt { email, plan } => match (email, plan) {
                ...
                (None, None) => "Whisply account".to_string(),
            },
            StatusAccountDisplay::ApiKey => {
                "Whisply account session required (run whisply login)".to_string()
            }
        });
        ...
        let note_first_line = Line::from(vec![
            ...
            Span::from("Run ").cyan(),
            WHISPLY_USAGE_COMMAND.cyan().bold(),
            Span::from(" for up-to-date").cyan(),
        ]);
        let note_second_line = Line::from(vec![
            Span::from(FieldFormatter::INDENT),
            Span::from("limits, resets, and rate card").cyan(),
        ]);
        ...
        // Whisply meters only its own provider. Bedrock, a local model, or a
        // bring-your-own key is billed somewhere Whisply cannot report, so
        // naming Whisply's usage there would describe the wrong account.
        if self.show_whisply_usage_hint {
