...
/// Shapes the Whisply-managed Usage snapshot into `/status` rows.
///
/// The managed gateway does not send the upstream rate-limit headers, so a
/// managed session has no `RateLimitSnapshot` to compose and would otherwise
/// report its limits as permanently unavailable. These rows are the same meters
/// `whisply usage` prints, worded the same way, so the two surfaces agree.
pub(crate) fn compose_managed_usage_data(
    snapshot: &ContextualUsageSnapshot,
    now: DateTime<Local>,
) -> StatusRateLimitData {
    let mut rows: Vec<StatusRateLimitRow> = Vec::with_capacity(snapshot.windows.len() + 1);
    ...
    rows.push(StatusRateLimitRow {
        ...
        value: StatusRateLimitValue::Text(format!(
            "{} · {} {}",
            snapshot.metering.rate_card_version,
            snapshot.metering.basis,
            snapshot.metering.currency
        )),
    });
