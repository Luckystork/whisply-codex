...
impl App {
    pub(super) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::NewSessionInDirectory { selection } => {
                self.pending_directory_selection = Some(selection);
                self.start_fresh_session_with_summary_hint(
                    tui, app_server, /*session_start_source*/ None,
            ...
            AppEvent::ManagedStatusUsageResult { request_id, result } => {
                if let Err(err) = &result {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    tokio::task::spawn_blocking(move || {
                            ...
                            )
                        {
                            ...
                            // the legacy spelling out of static binary data. A
                            // plain concat is folded back into one string by
                            // release LTO, so decode only the final word at
                            // runtime through an optimizer barrier.
                            let mut metric_name =
                            ...
                            session_telemetry.counter(&metric_name, /*inc*/ 1, &[]);
                            tracing::warn!(
                                error = %err,
                        ...
                        Ok(_) => {
                                ...
                                self.chat_widget.world_writable_warning_details()
                            {
                                ...
                                self.chat_widget.add_plain_history_lines(vec![
                                    ...
                                    Line::from(vec![
                                        "  ".into(),
                                        "Whisply can now safely edit files and execute commands in your computer"
                                            .dark_gray(),
                                    ]),
                                ]);
                            } else {
            ...
            AppEvent::CyberModelAutoReviewNotice => {
                self.chat_widget.add_warning_message(format!(
                    "Cyber models default to \"{}\" for safety reasons.",
                    crate::permission_mode::PermissionMode::Auto.display_name()
                ));
            }
