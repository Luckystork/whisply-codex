...
async fn stale_rate_limit_reads_preserve_newer_workspace_hard_stop_for_every_origin() -> Result<()>
{
    for origin_name in [
        "startup",
        "status",
        // `/usage` is no longer a rate-limit refresh origin: it reads the
        // Whisply-managed projection through the broker instead of the upstream
        // account, so it has no rate-limit read to leave stale.
        "reset-picker",
        "reset-consume",
    ] {
