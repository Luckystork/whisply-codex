...
impl App {
    ...
    pub(super) async fn start_fresh_session_with_summary_hint(
        ...
        if let Some(selection) = self.pending_directory_selection.take()
            && let Err(err) = apply_directory_selection(&mut config, &selection)
        {
            self.chat_widget
                .add_error_message(format!("Could not open that folder: {err}"));
            return;
        }
...
mod tests {
    ...
    async fn config_with_home(codex_home: &std::path::Path) -> Config {
    ...
    /// WCD-713: the folder chosen in the TUI has to mean the same thing as the
    /// folder chosen at launch. If `/directory none` and `whisply --no-directory`
    /// resolved different roots, the two would be lookalike private folders and
    /// a person moving between them would silently lose their files.
    #[tokio::test]
    async fn choosing_no_directory_lands_where_the_no_directory_flag_lands() {
        let home = tempfile::tempdir().expect("temporary home");
        let whisply_home = home.path().to_path_buf();
        let mut config = config_with_home(&whisply_home).await;

        apply_directory_selection(&mut config, &DirectorySelection::NoDirectory)
            .expect("no-directory root");

        let from_flag = codex_whisply::resolve_cli_working_root(
            &whisply_home,
            /*explicit_cwd*/ None,
            /*no_directory*/ true,
        )
        ...
        assert_eq!(config.cwd.as_path(), from_flag.cwd.expect("cli cwd"));
        assert!(
            !config.workspace_directory_selected,
            "the private root is not a workspace the person opened"
        );
    }

    /// A folder the person did choose is recorded as chosen, which is what
    /// turns project discovery and the workspace prompt profile back on.
    #[tokio::test]
    async fn choosing_a_folder_records_it_as_a_selected_workspace() {
        let home = tempfile::tempdir().expect("temporary home");
        let chosen = tempfile::tempdir().expect("chosen folder");
        let canonical_path = chosen.path().canonicalize().expect("canonical folder");
        let mut config = config_with_home(home.path()).await;
        ...
        assert_eq!(config.cwd.as_path(), canonical_path);
        assert!(config.workspace_directory_selected);
    }
...
/// Points a fresh thread's configuration at the directory the person chose.
///
/// `No directory` still needs a real absolute cwd for execution, and it gets
/// the app-owned root every other entry point uses, so a thread started here
/// and one started with `whisply --no-directory` land in the same place. The
/// accompanying `workspace_directory_selected` is what stops that private root
/// from being mistaken for a workspace the person opened.
fn apply_directory_selection(
    config: &mut Config,
    selection: &DirectorySelection,
) -> anyhow::Result<()> {
    let cwd = match selection {
        DirectorySelection::NoDirectory => {
            codex_whisply::no_directory_working_root(config.codex_home.as_path())?
        }
        DirectorySelection::Selected { canonical_path } => canonical_path.clone(),
    };
    config.cwd = AbsolutePathBuf::from_absolute_path(&cwd)?;
    config.workspace_directory_selected = matches!(selection, DirectorySelection::Selected { .. });
    Ok(())
}
