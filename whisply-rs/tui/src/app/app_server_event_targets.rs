...
pub(super) fn server_request_thread_id(request: &ServerRequest) -> Option<ThreadId> {
    match request {
        ...
        ServerRequest::WhisplyToolAdmittedExecute { params, .. } => {
            ThreadId::from_string(&params.thread_id).ok()
        }
        ServerRequest::WhisplyToolAdmittedCancel { params, .. } => {
            ThreadId::from_string(&params.thread_id).ok()
        }
...
pub(super) fn server_notification_thread_target(
    ...
    let thread_id = match notification {
        // A progress line is user-visible activity, so it carries the thread
        // and turn the app-server bound its call to and is routed on those.
        // The execution id is a correlation, never routing authority: a bare id
        // would let one thread's activity be projected into another.
        ServerNotification::WhisplyToolProgress(notification) => {
            Some(notification.thread_id.as_str())
        }
        // Still bare telemetry with no thread or turn, and produced by nobody.
        ...
        ServerNotification::WhisplyExamExtraUsage(notification) => {
            Some(notification.thread_id.as_str())
        }
...
mod tests {
    ...
    #[test]
    fn native_tool_progress_is_shown_in_the_thread_the_call_was_bound_to() {
        let notification = ServerNotification::WhisplyToolProgress(WhisplyToolProgress {
            thread_id: "0199e1f6-6a6a-7c6a-9c6a-6a6a6a6a6a6a".to_string(),
        ...
        assert_eq!(
            target,
            ServerNotificationThreadTarget::Thread(
                ThreadId::from_string("0199e1f6-6a6a-7c6a-9c6a-6a6a6a6a6a6a")
                    .expect("a thread id the app-server issued")
            )
        );
    }

    #[test]
    fn native_tool_progress_for_an_unusable_thread_is_not_shown_somewhere_else() {
        let notification = ServerNotification::WhisplyToolProgress(WhisplyToolProgress {
            thread_id: "not-a-thread".to_string(),
            turn_id: "turn-1".to_string(),
            execution_id: "call-1".to_string(),
            label: "Opening the first result".to_string(),
            fraction: None,
        ...
        assert_eq!(
            target,
            ServerNotificationThreadTarget::InvalidThreadId("not-a-thread".to_string()),
            "activity that cannot be attributed must not fall back to the visible thread"
        );
    }
