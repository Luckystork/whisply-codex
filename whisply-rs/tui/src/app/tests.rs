...
use tokio::time;
use whisply_app_server_client::AppServerPath;
...
use whisply_protocol::models::NetworkPermissions;
use whisply_protocol::models::PermissionProfile;
use whisply_protocol::protocol::EventMsg;
use whisply_protocol::protocol::MAX_THREAD_GOAL_OBJECTIVE_CHARS;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let whisply_app_server_client::AppServerEvent::ServerNotification(notification) = event
...
async fn history_lookup_response_is_routed_to_requesting_thread() -> Result<()> {
    ...
    let cursor = whisply_message_history::HistoryBatchCursor::new(/*end_offset*/ 10);
...
async fn active_history_batch_is_delivered_without_replay_buffering() -> Result<()> {
    ...
    let cursor = whisply_message_history::HistoryBatchCursor::new(/*end_offset*/ 1);
    let event = HistoryLookupResponse::Batch {
        ...
        next_older_cursor: Some(whisply_message_history::HistoryBatchCursor::new(
            /*end_offset*/ 0,
        )),
    };
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let codex_home = tempdir()?;
        app.config.codex_home = codex_home.path().to_path_buf().abs();
        app.config.sqlite = whisply_state::SqliteConfig::new_for_testing(codex_home.path().abs());
        ...
        let state_db = whisply_state::StateRuntime::init(
...
async fn primary_thread_ignores_child_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        whisply_app_server_client::AppServerEvent::ServerNotification(Box::new(
...
async fn background_side_cleanup_removes_local_state_and_ignores_late_events() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        whisply_app_server_client::AppServerEvent::ServerRequest(Box::new(exec_approval_request(
            side_thread_id,
            "turn-1",
            "item-1",
            Some("approval-1"),
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        whisply_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
async fn remote_exec_resume_current_cwd_is_rejected() -> Result<()> {
    ...
    app.environment_manager = Arc::new(
        EnvironmentManager::create_for_tests(
            ...
            Some(whisply_exec_server::ExecServerRuntimePaths::new(
                std::env::current_exe()?,
...
async fn remembered_current_cwd_stays_at_launch_across_in_app_resumes() -> Result<()> {
    ...
    crate::legacy_core::config::edit::ConfigEditsBuilder::for_config(&config)
        .set_resume_cwd(whisply_config::types::ResumeCwdMode::Current)
        .apply()
        .await
...
async fn prompt_edit_forks_before_selected_prompt_and_preserves_source() -> Result<()> {
    ...
    for (turn_id, message, images, local_images) in [
        ...
        for item in [
            ...
            whisply_rollout::append_rollout_item_to_path(&source_path, &item).await?;
        }
    }
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                whisply_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
            /*windows_sandbox_level*/ None,
...
async fn changing_cyber_model_reasoning_preserves_selected_permissions() {
    Box::pin(async {
        ...
        assert!(
            app.apply_permission_profile_selection(PermissionProfileSelection {
                profile_id: whisply_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
                    .to_string(),
        ...
        for effort in [ReasoningEffortConfig::High, ReasoningEffortConfig::Ultra] {
            ...
            assert_eq!(
                settings
                    ...
                    .expect("active permission profile")
                    .id,
                whisply_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
            );
            assert_eq!(
...
async fn selecting_cyber_model_respects_auto_review_requirements() {
    Box::pin(async {
        ...
        let mut requirements_with_sources =
            whisply_config::ConfigRequirementsWithSources::default();
        requirements_with_sources.merge_unset_fields(
            whisply_config::RequirementSource::Unknown,
            requirements_toml.clone(),
        );
        let requirements = whisply_config::ConfigRequirements::try_from(requirements_with_sources)
