...
use codex_whisply::CliWorkingRoot;
use codex_whisply::ManagedGatewayClient;
use codex_whisply::resolve_cli_working_root;
...
use url::Url;
use uuid::Uuid;
use whisply_app_server_client::AppServerClient;
use whisply_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use whisply_app_server_client::InProcessAppServerClient;
use whisply_app_server_client::InProcessClientStartArgs;
use whisply_app_server_client::RemoteAppServerClient;
use whisply_app_server_client::RemoteAppServerConnectArgs;
pub use whisply_app_server_client::RemoteAppServerEndpoint;
use whisply_config::CloudConfigBundleLoader;
use whisply_config::ConfigLoadError;
use whisply_config::LoaderOverrides;
use whisply_config::format_config_error_with_source;
...
use whisply_login::AuthConfig;
use whisply_login::default_client::originator;
use whisply_login::default_client::set_default_client_residency_requirement;
use whisply_login::enforce_login_restrictions;
...
use whisply_rollout::StateDbHandle;
use whisply_rollout::state_db;
use whisply_state::log_db;
use whisply_utils_absolute_path::AbsolutePathBuf;
use whisply_utils_absolute_path::canonicalize_existing_preserving_symlinks;
use whisply_utils_home_dir::find_codex_home;
use whisply_utils_oss::ensure_oss_provider_ready;
use whisply_utils_oss::get_default_model_for_oss_provider;

pub(crate) use whisply_app_server_client::legacy_core;
...
const TUI_LOG_FILE_NAME: &str = "whisply-tui.log";
/// The upstream filename. Still named here so startup keeps deleting a file an
/// earlier build left in someone's home.
...
async fn start_embedded_app_server(
    ...
    feedback: whisply_feedback::CodexFeedback,
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = whisply_state::runtime_db_path_for_corruption_error(&err)
...
fn remove_legacy_tui_log_file(codex_home: &Path) {
    ...
    let _ = std::fs::remove_file(log_dir.join(LEGACY_TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let codex_home = find_codex_home().wrap_err("failed to resolve WHISPLY_HOME")?;
            whisply_app_server_client::app_server_control_socket_path(&codex_home)
...
async fn maybe_probe_default_daemon_socket(codex_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = whisply_app_server_client::app_server_control_socket_path(codex_home).ok()?;
...
async fn resolve_startup_resume_or_fork_cwd(
    tui: &mut Tui,
    config: &Config,
    state_db: Option<&whisply_state::StateRuntime>,
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = whisply_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    // A remote workspace already supplies its own root, so `No directory` only
    // applies to local sessions.
    let no_directory = cli.no_directory && !app_server_target.uses_remote_workspace();
    let working_root =
        resolve_cli_working_root(&codex_home, cwd_override, no_directory).map_err(|error| {
            std::io::Error::other(format!("failed to prepare the working directory: {error}"))
        })?;
    ...
    // Keyed to the resolved root rather than `cli.cwd` so a remote workspace,
    // which clears the local override above, does not claim a local selection.
    let CliWorkingRoot {
        cwd: cwd_override,
    ...
    let overrides = ConfigOverrides {
        ...
        sandbox_mode,
        cwd: cwd_override,
        workspace_directory_selected: Some(workspace_directory_selected),
        // `Auto` in the TUI keeps the upstream reusable reviewer; only the
        // native app's `Auto` mode asks for a stateless review.
        stateless_auto_review: None,
        // WCD-714: the terminal offers the permission selector, so a new thread
        // starts in `Auto` unless the person has chosen otherwise.
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        whisply_app_server_client::build_otel_provider(
            &config,
    ...
    if let Some(metrics) = otel.as_ref().and_then(whisply_otel::OtelProvider::metrics) {
        let _ = whisply_otel::record_process_start_once(metrics, otel_originator.as_str());
        let telemetry =
            whisply_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
        let _ = whisply_state::install_process_db_telemetry(telemetry);
    }
    ...
    let (tui_file_layer, _tui_file_log_guard) = if config_toml_log_dir_configured {
        ...
        // A configured log directory is not cleared at startup the way the
        // default one is, so without this the file is the one thing in the
        // product that grows for as long as the product is installed.
        let log_path = log_dir.join(TUI_LOG_FILE_NAME);
        codex_whisply::enforce_log_retention(&log_path, codex_whisply::MAX_LOG_FILE_BYTES)
            .map_err(std::io::Error::other)?;

        // Everything any tracing call site in the tree emits lands here
        // verbatim, and some of it carries material the product refuses to put
        // on disk anywhere else — `login` logs the body of an authentication
        // error at debug, and an error body can echo the code that produced it.
        // The writer scrubs credentials and personal identifiers on the way out
        // and keeps the file within its budget while the session runs, not only
        // when it was opened. It also owns the file mode, so the log stays
        // readable by its owner alone.
        let log_file =
            codex_whisply::RedactedLogWriter::open(&log_path, codex_whisply::MAX_LOG_FILE_BYTES)?;
        let (non_blocking, guard) = non_blocking(log_file);
        let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("whisply_core=info,whisply_tui=info,whisply_rmcp_client=info")
        });
    ...
    let feedback = whisply_feedback::CodexFeedback::new();
...
async fn register_managed_skill_roots(
    ...
    if !home.is_absolute() || config.codex_home.as_path() != home.as_path() {
        color_eyre::eyre::bail!(
            "managed Whisply skill roots do not match the launcher runtime home"
        );
    }
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, whisply_config::TomlValue)>,
    loader_overrides: LoaderOverrides,
    strict_config: bool,
    cloud_config_bundle: CloudConfigBundleLoader,
    ...
    match load_config_toml_with_layer_stack(
        ...
        cwd,
        cli_kv_overrides,
        whisply_config::ConfigLoadOptions {
            loader_overrides,
            strict_config,
            cloud_config_bundle,
        },
...
mod tests {
    ...
    fn write_session_rollout(
        ...
        let session_meta = whisply_protocol::protocol::SessionMeta {
            ...
            source: whisply_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(whisply_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    fn startup_removes_legacy_tui_log_file() -> std::io::Result<()> {
        ...
        let current_log = legacy_log_dir.join(TUI_LOG_FILE_NAME);
        let upstream_log = legacy_log_dir.join(LEGACY_TUI_LOG_FILE_NAME);
        std::fs::write(&current_log, "log")?;
        std::fs::write(&upstream_log, "log written by an earlier build")?;
        ...
        assert!(
            !upstream_log.exists(),
            "renaming the log must not orphan the file an earlier build left behind"
        );
        Ok(())
    }
    ...
    async fn startup_services_use_final_cloud_managed_http_policy() -> color_eyre::Result<()> {
            ...
            [(true, false), (false, true)]
        {
            ...
            let cloud_config_bundle =
                whisply_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: whisply_app_server_client::app_server_control_socket_path(
                    &codex_home
                )?,
            }
        );
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let codex_home = TempDir::new()?;
        let socket_path =
            whisply_app_server_client::app_server_control_socket_path(codex_home.path())?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            whisply_app_server_client::AppServerClient::InProcess(
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        let sqlite =
            whisply_state::SqliteConfig::new_for_testing(occupied_sqlite_home.as_path().abs());
    ...
    -> color_eyre::Result<()> {
        ...
        let sqlite = whisply_state::SqliteConfig::new_for_testing(sqlite_home.as_path().abs());
        ...
        assert!(
            whisply_state::sqlite_error_detail_is_corruption(startup_error.detail()),
