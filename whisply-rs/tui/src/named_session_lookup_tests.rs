...
async fn state_runtime(config: &Config) -> std::io::Result<Arc<whisply_state::StateRuntime>> {
    let runtime = whisply_state::StateRuntime::init(
        whisply_state::SqliteConfig::new_for_testing(config.codex_home.as_path().abs()),
...
async fn upsert_thread(
    ...
    metadata: whisply_state::ThreadMetadata,
) -> std::io::Result<()> {
    runtime
...
async fn add_scan_sentinel(config: &Config, search_term: &str) -> color_eyre::Result<ThreadId> {
    ...
    whisply_rollout::append_thread_name(
...
async fn uses_sqlite_title_without_scanning_rollouts() -> color_eyre::Result<()> {
    ...
    whisply_rollout::append_thread_name(config.codex_home.as_path(), duplicate_id, "saved-session")
        .await?;
...
async fn recovers_legacy_index_name_without_requiring_sidecar_rewrite() -> color_eyre::Result<()> {
    ...
    whisply_rollout::append_thread_name(config.codex_home.as_path(), thread_id, "saved-session")
        .await?;
...
async fn skips_stale_legacy_index_candidate() -> color_eyre::Result<()> {
    ...
    .await?;
    whisply_rollout::append_thread_name(config.codex_home.as_path(), stale_id, "saved-session")
        .await?;
    ...
    whisply_rollout::append_thread_name(config.codex_home.as_path(), fallback_id, "saved-session")
        .await?;
...
async fn keeps_newer_paginated_sqlite_name_when_index_is_stale() -> color_eyre::Result<()> {
    ...
    runtime
        ...
        .await
        .map_err(std::io::Error::other)?;
    whisply_rollout::append_thread_name(config.codex_home.as_path(), thread_id, "old-session")
        .await?;
...
async fn prefers_most_recent_duplicate_legacy_name() -> color_eyre::Result<()> {
    ...
    whisply_rollout::append_thread_name(config.codex_home.as_path(), recent_id, "same-name")
        .await?;
    whisply_rollout::append_thread_name(config.codex_home.as_path(), older_id, "same-name").await?;
    for (path, timestamp) in [
