...
pub(crate) async fn run_cwd_selection_prompt(
    ...
    let mut screen = CwdPromptScreen::new(
        tui.frame_requester(),
        action,
        working_directory_label(current_cwd, codex_home),
        working_directory_label(session_cwd, codex_home),
        working_directory_label(remembered_current_cwd, codex_home),
...
/// How a working directory should be named to the person choosing one.
///
/// A `No directory` thread records a real path inside the app's own home.
/// Printing it would show an internal folder the person never chose, and invite
/// them to treat it as a workspace; naming it by the choice they actually made
/// is both accurate and actionable.
fn working_directory_label(path: &Path, codex_home: &Path) -> String {
    if AbsolutePathBuf::from_absolute_path(path)
        .is_ok_and(|path| is_inside_runtime_home(&path, codex_home))
    {
...
mod tests {
    ...
    /// WCD-712: `No directory` records a real path inside the app's own home.
    /// Showing it here would put an internal folder on screen as though the
    /// person had chosen it, and invite them to keep using it as a workspace.
    #[tokio::test]
    async fn the_private_root_is_named_by_the_choice_not_by_its_path() -> Result<()> {
        let home = TempDir::new()?;
        let codex_home = home.path();
        let neutral = codex_home.join("neutral-workspace");
        std::fs::create_dir_all(&neutral)?;

        assert_eq!(
            working_directory_label(&neutral, codex_home),
            "No directory"
        );
        Ok(())
    }
    ...
    #[tokio::test]
    async fn an_ordinary_folder_is_still_shown_by_path() -> Result<()> {
        let home = TempDir::new()?;
        let project = home.path().parent().unwrap_or(home.path()).join("project");

        assert_eq!(
            working_directory_label(&project, home.path()),
            project.display().to_string(),
            "a folder the person chose has to be identifiable"
        );
        Ok(())
    }
}
