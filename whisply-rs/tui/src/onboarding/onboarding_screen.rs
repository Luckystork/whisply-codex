...
impl OnboardingScreen {
    pub(crate) async fn new(tui: &mut Tui, args: OnboardingScreenArgs) -> Self {
        ...
        if show_trust_screen {
            // `--no-directory` runs inside the app's own home, whose nearest
            // enclosing checkout is ordinarily the person's home directory.
            // The trust screen would then offer to trust their whole home for
            // a session that deliberately opened no project.
            let trust_target = if is_inside_runtime_home(&config.cwd, config.codex_home.as_path()) {
                cwd.clone()
            } else {
