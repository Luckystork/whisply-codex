...
//! in-memory `Pet`: built-in catalog ids, custom pet ids under WHISPLY_HOME,
...
//! built-in pet has been materialized from the bundled pack before asking the
//! model layer to load it.
...
impl Pet {
    ...
    /// has already been materialized into WHISPLY_HOME; if callers skip the
...
fn load_builtin_pet(pet: catalog::BuiltinPet, codex_home: Option<&Path>) -> Result<Pet> {
    let codex_home = codex_home.context("WHISPLY_HOME is not available")?;
