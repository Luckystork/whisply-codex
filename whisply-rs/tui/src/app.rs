...
use uuid::Uuid;
use whisply_ansi_escape::ansi_escape_line;
...
use whisply_models_manager::model_presets::HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG;
use whisply_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
#[cfg(target_os = "windows")]
use whisply_protocol::permissions::FileSystemSandboxKind;
...
use whisply_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
...
fn session_summary(
    ...
    // The TUI is a Whisply surface. Quit copy must not invent a Codex token
    // meter the account is not billed in.
    let usage_line = None;
...
fn resume_hint_for_resumable_thread(
    ...
    whisply_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
pub(crate) struct App {
    ...
    launch_cwd: PathBuf,
    /// A directory the person chose that the next fresh thread has to adopt.
    ///
    /// The choice is applied when the thread is built rather than written into
    /// `config` when it is made, so an abandoned or failed start cannot leave
    /// the session pointing somewhere the person never opened.
    pending_directory_selection: Option<DirectorySelection>,
...
fn archived_session_guidance(err: &color_eyre::eyre::Report) -> Option<String> {
    ...
    if !message.contains(" is archived. Run `whisply unarchive ") {
        return None;
    }
    let message = message
        .split_once(" (code ")
...
impl App {
    ...
    pub async fn run(
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            whisply_login::default_client::originator().value,
...
See the Whisply keymap documentation for supported actions and examples."
            )
        })?;
        #[cfg(not(debug_assertions))]
