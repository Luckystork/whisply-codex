...
async fn lookup_legacy_index_target(
    ...
    let candidates = whisply_rollout::find_thread_meta_candidates_by_name_str(
    ...
    for (path, session_meta) in candidates {
        if !path.starts_with(config.codex_home.join(whisply_rollout::SESSIONS_SUBDIR)) {
            continue;
        }
        ...
        loop {
            if let Some(thread) = self.pending.next() {
                ...
                let session_meta = if app_server.uses_remote_workspace() {
                    ...
                    let Some(path) = whisply_rollout::existing_rollout_path(path).await else {
                        continue;
                    };
                    let expected_root = self.codex_home.join(match self.collection {
                        SessionCollection::Active => whisply_rollout::SESSIONS_SUBDIR,
                        SessionCollection::Archived => whisply_rollout::ARCHIVED_SESSIONS_SUBDIR,
                    });
                    if !path.starts_with(expected_root) {
                        continue;
                    }
                    let Ok(session_meta) =
                        whisply_rollout::read_session_meta_line(path.as_path()).await
                    else {
                        continue;
                    };
