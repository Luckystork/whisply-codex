...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<whisply_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<whisply_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(whisply_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`whisply-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
...
pub(crate) fn empty_mcp_output() -> WebHyperlinkHistoryCell {
    let docs_line = HyperlinkLine::new(Line::from("    Add one with `whisply mcp add`."));
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, whisply_protocol::mcp::Tool>,
...
pub(crate) fn new_mcp_tools_output_from_statuses(
    ...
    for status in statuses {
        ...
        let mut names = status
            .tools
            .keys()
            .filter(|name| !status.unavailable_tools.contains(name))
            .cloned()
            .collect::<Vec<_>>();
        names.sort();
        if names.is_empty() {
        ...
        if !status.unavailable_tools.is_empty() {
            let mut unavailable = status.unavailable_tools.clone();
            unavailable.sort();
            lines.push(
                vec![
                    "    • Unavailable: ".into(),
                    unavailable.join(", ").into(),
                    " (a built-in capability owns the name; give this server a tool prefix to use it)"
                        .dim(),
                ]
                .into(),
            );
