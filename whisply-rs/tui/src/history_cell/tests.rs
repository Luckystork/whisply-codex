...
#[test]
fn empty_mcp_output_names_the_command_instead_of_linking_upstream_docs() {
    ...
    🔌  MCP Tools
      ...
      • No MCP servers configured.
        Add one with `whisply mcp add`.
    ");

    // Whisply publishes no MCP documentation, so this line has to carry the
    // remedy itself rather than link to another product's.
    assert!(
        cell.display_hyperlink_lines(/*width*/ 80)[5]
            .hyperlinks
            .is_empty(),
        "the empty-MCP line must not link anywhere"
    );
    assert!(
        cell.transcript_hyperlink_lines(/*width*/ 80)[5]
            .hyperlinks
            .is_empty(),
        "the empty-MCP transcript line must not link anywhere"
    );
    ...
    let linked_text = area
        ...
        .filter_map(|position| {
            ...
            symbol.contains("\x1b]8;;").then(|| symbol.to_string())
        })
        .collect::<String>();
    assert_eq!(linked_text, "");
}
...
/// A server that publishes a name the product owns is listed, but not as
/// something the model can call: the tool never reaches the model, so showing
/// it beside working tools sends the reader looking for a bug that is really a
/// naming conflict.
#[test]
fn mcp_tools_output_separates_a_tool_the_model_will_never_see() {
    ...
    let statuses = vec![McpServerStatus {
        ...
        server_info: None,
        tools: HashMap::from([
            ("lookup".to_string(), tool("lookup")),
            ("browser".to_string(), tool("browser")),
        ]),
        resources: Vec::new(),
        resource_templates: Vec::new(),
        auth_status: codex_app_server_protocol::McpAuthStatus::Unknown,
        unavailable_tools: vec!["browser".to_string()],
    }];
    ...
    assert!(rendered.contains("Tools: lookup"), "{rendered}");
    assert!(!rendered.contains("Tools: browser, lookup"), "{rendered}");
    assert!(rendered.contains("Unavailable: browser"), "{rendered}");
    assert!(
        rendered.contains("built-in capability owns the name"),
        "{rendered}"
    );
}
