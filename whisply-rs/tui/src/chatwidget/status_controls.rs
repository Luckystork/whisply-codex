...
impl ChatWidget {
    /// Shows what a still-running native tool call is doing.
    ///
    /// A native call can take minutes, and until it returns the turn has
    /// nothing else to say, so without this the session looks stalled. The step
    /// is shown only while a task is actually running: once the turn has
    /// finished, a late report would describe work that is already over.
    pub(super) fn on_whisply_tool_progress(&mut self, label: String) {
        let label = label.trim();
        if label.is_empty() || !self.bottom_pane.is_task_running() {
            return;
        }
        self.status_state.terminal_title_status_kind = TerminalTitleStatusKind::Working;
        if !self.set_status_header(label.to_string()) {
            self.request_redraw();
        }
    }
    ...
    /// Fills a pending `/status` card from the Whisply-managed Usage snapshot.
    pub(crate) fn finish_status_managed_usage_refresh(
        &mut self,
        request_id: u64,
        ...
        for (pending_request_id, handle) in self.refreshing_status_outputs.drain(..) {
            if pending_request_id == request_id {
                updated_any = true;
                handle.finish_managed_usage_refresh(snapshot.as_ref(), now);
            } else {
