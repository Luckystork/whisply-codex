...
#[tokio::test]
async fn managed_status_does_not_report_an_upstream_window_as_the_account_limit() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    // A leftover upstream window is not this account's limit: Whisply meters the
    // managed account itself, so the card must wait for that read instead of
    // presenting header data as the answer.
    ...
    assert!(
        ...
        "expected /status to ignore the upstream window on a managed account, got: {refreshed}"
    );
    assert!(
        !std::iter::from_fn(|| rx.try_recv().ok())
            .any(|event| matches!(event, AppEvent::RefreshRateLimits { .. })),
        "managed usage must not trigger a direct rate-limit refresh"
    );
}
