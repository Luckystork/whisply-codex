...
#[tokio::test]
async fn usage_command_never_opens_the_upstream_reset_menu() {
    // Redeeming a usage limit reset is upstream account authority: eligibility
    // came from a ChatGPT account and redemption from the upstream account API,
    // which the app-server now refuses. `/usage` therefore must not offer it,
    // even for a session carrying leftover upstream auth and cached credits.
    ...
    assert_eq!(
        render_bottom_popup(&chat, /*width*/ 80).find("Redeem usage limit reset"),
        None,
        "/usage must not surface upstream reset redemption"
    );
...
#[tokio::test]
async fn usage_command_does_not_request_upstream_rate_limits() {
    // The upstream menu refreshed `account/rateLimits/read` whenever its cached
    // credit count was zero. That read is managed account data the app-server
    // declines, so issuing it would be a request Whisply can never satisfy.
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    set_chatgpt_auth(&mut chat);
    let request_id = chat.start_rate_limit_reset_startup_check();
    assert!(chat.finish_rate_limit_reset_hint_refresh(
    ...
    assert!(
        !requested_rate_limits,
        "/usage must not refresh upstream rate limits"
    );
}
...
#[tokio::test]
async fn no_credit_outcome_clears_cached_reset_credits() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    set_chatgpt_auth(&mut chat);
    ...
    // A NoCredit outcome must clear the cached count rather than leave a stale
    // one behind. `/usage` no longer reads it, so assert the state directly.
...
async fn failed_post_consume_refresh_does_not_keep_stale_reset_count() {
    ...
    // A failed post-consume refresh must drop the pre-consume count rather than
    // keep advertising credits that were already spent.
    assert_eq!(chat.available_rate_limit_reset_credits, None);
}
