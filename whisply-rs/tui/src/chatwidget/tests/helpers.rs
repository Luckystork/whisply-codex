...
pub(super) async fn test_config() -> Config {
    ...
    config.sqlite = whisply_state::SqliteConfig::new_for_testing(codex_home.as_path().abs());
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = whisply_shell_command::parse_command::parse_command(&command)
        .into_iter()
    ...
    let item = AppServerThreadItem::CommandExecution {
        ...
        command: whisply_shell_command::parse_command::shlex_join(&command),
...
/// A fixed absolute path for plugin metadata in tests.
///
/// Deliberately not `std::env::temp_dir()`: nothing here is ever written to
/// disk, and now that the plugin detail renders a local plugin's path, a
/// per-user temp directory would bake this machine's `/var/folders/...` into
/// the snapshots. Fixed literals under `/tmp` are what the other snapshotted
/// paths in these tests already use.
pub(super) fn plugins_test_absolute_path(path: &str) -> AbsolutePathBuf {
    PathBuf::from("/tmp/codex-plugin-menu-tests")
        .join(path)
        .abs()
}
...
pub(super) fn plugins_test_detail(
    ...
    PluginDetail {
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    codex_app_server_protocol::PluginHookSummary {
                        ...
                        command: Some(format!("./hook-{event_index}-{handler_index}.sh")),
                        matcher: None,
                    }
                })
            })
        ...
        mcp_server_launches: mcp_servers
            .iter()
            .map(|name| codex_app_server_protocol::PluginMcpLaunchSummary {
                server_name: (*name).to_string(),
                launch: format!("node {name}-server.js"),
            })
            .collect(),
