...
async fn experimental_mode_plan_is_ignored_on_startup() {
    ...
    let init = ChatWidgetInit {
        ...
        feedback: whisply_feedback::CodexFeedback::new(),
        is_first_run: true,
...
/// Sharing an environment variable with a plugin should be a decision, not a
/// discovery made later. The names are the disclosure; the values are the
/// user's secrets and are never rendered.
#[tokio::test]
async fn plugin_detail_states_what_the_plugin_will_read_from_the_environment() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Plugins, /*enabled*/ true);
    ...
    let response = plugins_test_response(vec![PluginMarketplaceEntry {
        name: "ChatGPT Marketplace".to_string(),
        path: None,
        interface: None,
    ...
    plugin.host_env_vars = vec!["LINEAR_API_KEY".to_string()];
    chat.on_plugin_detail_loaded(cwd.to_path_buf(), Ok(PluginReadResponse { plugin }));

    let popup = render_bottom_popup(&chat, /*width*/ 100);
    // The capability rows already saturate `MAX_POPUP_ROWS`, so this passing
    // also demonstrates the disclosure survives the row cap instead of landing
    // below the fold where nobody scrolls before clicking install.
    assert!(
        popup.contains("Reads LINEAR_API_KEY"),
        "expected the detail to name what the plugin reads, got:\n{popup}"
    );
}

#[tokio::test]
async fn plugin_detail_reports_a_refused_request_rather_than_hiding_it() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Plugins, /*enabled*/ true);
    ...
    let summary = plugins_test_summary(
        ...
        "suspicious",
        Some("Suspicious"),
        Some("Reaches for more than it should."),
        /*installed*/ false,
        /*enabled*/ true,
        PluginInstallPolicy::Available,
    );
    ...
    chat.add_plugins_output();
    let mut plugin =
        plugins_test_detail(summary, Some("Does things."), &[], &[], &[], &["odd-mcp"]);
    plugin.refused_host_env_vars = vec!["WHISPLY_GATEWAY_AUTH_FD".to_string()];
    chat.on_plugin_detail_loaded(cwd.to_path_buf(), Ok(PluginReadResponse { plugin }));

    let popup = render_bottom_popup(&chat, /*width*/ 100);
    // The attempt is the interesting part. Hiding it because it cannot succeed
    // would withhold the clearest signal a user could have about this plugin.
    assert!(
        popup.contains("WHISPLY_GATEWAY_AUTH_FD") && popup.contains("does not share"),
        "expected the detail to report the refused request, got:\n{popup}"
    );
}

#[tokio::test]
async fn plugin_detail_omits_the_environment_line_when_nothing_is_read() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Plugins, /*enabled*/ true);
    ...
    chat.on_plugins_loaded(cwd.to_path_buf(), Ok(response));
    chat.add_plugins_output();
    let plugin = plugins_test_detail(summary, Some("Does things."), &[], &[], &[], &[]);
    chat.on_plugin_detail_loaded(cwd.to_path_buf(), Ok(PluginReadResponse { plugin }));

    let popup = render_bottom_popup(&chat, /*width*/ 100);
    // A line reading "reads nothing" on every plugin teaches people to skip the
    // line, which costs exactly on the plugin where it says something.
    assert!(
        !popup.contains("Reads ") && !popup.contains("does not share"),
        "expected no environment disclosure when there is nothing to disclose, got:\n{popup}"
    );
}

/// Loads a not-yet-installed plugin's detail popup and returns what is on
/// screen, so the hook disclosure tests below differ only in the hooks.
async fn rendered_detail_for_hooks(
    mutate: impl FnOnce(&mut codex_app_server_protocol::PluginDetail),
) -> String {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Plugins, /*enabled*/ true);
    ...
    chat.on_plugins_loaded(cwd.to_path_buf(), Ok(response));
    chat.add_plugins_output();
    let mut plugin = plugins_test_detail(summary, Some("Does things."), &[], &[], &[], &[]);
    mutate(&mut plugin);
    chat.on_plugin_detail_loaded(cwd.to_path_buf(), Ok(PluginReadResponse { plugin }));
...
#[tokio::test]
async fn plugin_detail_names_the_command_a_hook_will_run() {
    let popup = rendered_detail_for_hooks(|plugin| {
        plugin.hooks = vec![command_hook(
    ...
    // Hooks are the only part of a plugin that runs without being asked, and
    // the detail used to report the event and a count. A count is not a
    // description of what will execute.
    assert!(
        popup.contains("./bootstrap.sh --force"),
        "expected the detail to name the command, got:\n{popup}"
    );
    assert!(
        popup.contains("SessionStart"),
        "expected the detail to say when it runs, got:\n{popup}"
    );
}

#[tokio::test]
async fn the_command_disclosure_survives_the_row_cap() {
    // `MAX_POPUP_ROWS` is 8 and the detail has ten rows, with Install second.
    // A user can reach Install without the Hooks row ever being drawn, so this
    // has to be in the header or it is not a pre-install disclosure at all.
    let popup = rendered_detail_for_hooks(|plugin| {
        plugin.hooks = vec![command_hook(
            codex_app_server_protocol::HookEventName::SessionStart,
            "./bootstrap.sh",
    ...
    let install_row = popup
        .find("Install plugin")
        .expect("expected an install action");
    let disclosure = popup
        .find("./bootstrap.sh")
        .expect("expected the command to be disclosed");
    assert!(
        disclosure < install_row,
        "expected the command disclosure above the install action, got:\n{popup}"
    );
}

#[tokio::test]
async fn a_hook_scoped_to_some_tools_says_which() {
    let popup = rendered_detail_for_hooks(|plugin| {
        plugin.hooks = vec![command_hook(
    ...
    // "Runs before tool use" and "runs before every shell command" are
    // different amounts of exposure to agree to.
    assert!(
        popup.contains("PreToolUse: Bash"),
        "expected the scope of the hook, got:\n{popup}"
    );
}

#[tokio::test]
async fn a_hook_that_only_adds_instructions_is_not_reported_as_running_something() {
    let popup = rendered_detail_for_hooks(|plugin| {
        plugin.hooks = vec![codex_app_server_protocol::PluginHookSummary {
            key: "hooked:prompt".to_string(),
            event_name: codex_app_server_protocol::HookEventName::SessionStart,
            handler_type: codex_app_server_protocol::HookHandlerType::Prompt,
            command: None,
            matcher: None,
        }];
    })
    ...
    assert!(
        !popup.contains("Runs on its own"),
        "a prompt handler executes nothing locally, got:\n{popup}"
    );
    assert!(
        popup.contains("none run commands"),
        "expected the hooks row to distinguish it from an executing hook, got:\n{popup}"
    );
}

#[tokio::test]
async fn a_long_list_of_commands_is_capped_rather_than_flooding_the_header() {
    let popup = rendered_detail_for_hooks(|plugin| {
        plugin.hooks = (0..6)
            .map(|index| {
                command_hook(
    ...
    assert!(
        popup.contains("./step-0.sh") && popup.contains("and 3 more"),
        "expected a bounded list that admits what it left out, got:\n{popup}"
    );
}

/// Renders the detail popup for a plugin served from git at `ref_name`/`sha`.
async fn rendered_detail_for_git_source(ref_name: Option<&str>, sha: Option<&str>) -> String {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Plugins, /*enabled*/ true);
    ...
    let mut summary = plugins_test_summary(
        ...
        "fromgit",
        Some("From Git"),
        Some("Served from a repository."),
        /*installed*/ false,
        /*enabled*/ true,
        PluginInstallPolicy::Available,
    );
    summary.source = codex_app_server_protocol::PluginSource::Git {
        url: "https://example.test/acme/plugin".to_string(),
        path: None,
...
#[tokio::test]
async fn a_git_plugin_states_the_commit_it_is_pinned_to() {
    let popup = rendered_detail_for_git_source(
        Some("main"),
        Some("0123456789abcdef0123456789abcdef01234567"),
    )
    .await;

    // The declared commit was carried all the way through the protocol and
    // then dropped at the last step, so the row named a branch and implied a
    // fixed version it did not have.
    assert!(
        popup.contains("pinned to 0123456789ab"),
        "expected the pinned commit on the source row, got:\n{popup}"
    );
}

#[tokio::test]
async fn a_git_plugin_without_a_commit_says_it_is_not_pinned() {
    let popup = rendered_detail_for_git_source(Some("main"), None).await;

    // `main` is a name the other side can repoint, so this is not a version.
    assert!(
        popup.contains("not pinned to a commit"),
        "expected an unpinned source to say so, got:\n{popup}"
    );
}

/// Renders the detail popup for a plugin served from `source`.
async fn rendered_detail_for_source(
    source: codex_app_server_protocol::PluginSource,
    installed: bool,
) -> String {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Plugins, /*enabled*/ true);
...
fn local_plugin_source(path: &str) -> codex_app_server_protocol::PluginSource {
    codex_app_server_protocol::PluginSource::Local {
        path: AbsolutePathBuf::try_from(PathBuf::from(path)).expect("absolute local plugin path"),
    }
}

fn git_plugin_source() -> codex_app_server_protocol::PluginSource {
    codex_app_server_protocol::PluginSource::Git {
        url: "https://example.test/acme/plugin".to_string(),
        path: None,
...
#[tokio::test]
async fn a_local_plugin_names_the_folder_it_will_run() {
    let popup = rendered_detail_for_source(
        local_plugin_source("/tmp/acme-plugin-checkout"),
        /*installed*/ false,
    )
    .await;

    // The path is the whole disclosure for a local source: nothing is copied,
    // so that directory is the code. The row used to say only "Local".
    assert!(
        popup.contains("/tmp/acme-plugin-checkout"),
        "expected the local path on the source row, got:\n{popup}"
    );
}

#[tokio::test]
async fn a_local_plugin_says_its_folder_keeps_taking_effect() {
    let popup = rendered_detail_for_source(
        local_plugin_source("/tmp/acme-plugin-checkout"),
        /*installed*/ false,
    )
    ...
    // A local source is not install-materialized, so every load re-reads the
    // directory. Everything else disclosed here describes only this moment.
    assert!(
        popup.contains("no update step"),
        "expected a local plugin to say its folder keeps taking effect, got:\n{popup}"
    );
}

#[tokio::test]
async fn a_git_plugin_says_installing_takes_a_copy() {
    let popup = rendered_detail_for_source(git_plugin_source(), /*installed*/ false).await;

    // Git and npm sources are copied into the plugin store at install and
    // resolved from `active_plugin_root` afterwards, so the reviewed code is
    // what keeps running. That is what makes the pinning line worth reading.
    assert!(
        popup.contains("installing takes a copy"),
        "expected an installable source to say the copy is what runs, got:\n{popup}"
    );
}

#[tokio::test]
async fn an_installed_plugin_does_not_repeat_the_pre_install_update_note() {
    let popup = rendered_detail_for_source(git_plugin_source(), /*installed*/ true).await;

    // Afterwards it describes a decision already made, and the row costs more
    // than it says.
    assert!(
        !popup.contains("installing takes a copy"),
        "expected the update note to be pre-install only, got:\n{popup}"
    );
}
    ...
    chat.on_plugin_detail_loaded(
        ...
        Ok(PluginReadResponse {
            plugin: PluginDetail {
                ...
                mcp_servers: Vec::new(),
                mcp_server_launches: Vec::new(),
                update_behavior: None,
                host_env_vars: Vec::new(),
                refused_host_env_vars: Vec::new(),
...
async fn experimental_features_popup_snapshot() {
    ...
    let mut config = whisply_config::types::TuiKeymap::default();
    config.list.accept = Some(whisply_config::types::KeybindingsSpec::One(
        whisply_config::types::KeybindingSpec("ctrl-x enter".to_string()),
    ));
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &whisply_feedback::FeedbackDiagnostics::new(vec![whisply_feedback::FeedbackDiagnostic {
