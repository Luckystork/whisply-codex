...
fn windows_sandbox_requirements_stack(
    ...
    let requirements_toml = whisply_config::ConfigRequirementsToml {
        windows: Some(whisply_config::WindowsRequirementsToml {
...
fn requirements_stack(
    requirements_toml: whisply_config::ConfigRequirementsToml,
) -> ConfigLayerStack {
    let mut requirements_with_sources = whisply_config::ConfigRequirementsWithSources::default();
...
async fn profile_permissions_selection_popup_with_disallowed_full_access_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.explicit_permission_profile_mode = true;
    chat.config.config_layer_stack = requirements_stack(whisply_config::ConfigRequirementsToml {
...
async fn startup_windows_sandbox_prompt_blocks_disallowed_unelevated_fallback() {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 120);
    assert!(
        popup.contains("Your organization requires the default Whisply agent sandbox"),
