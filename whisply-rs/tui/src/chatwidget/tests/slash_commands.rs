...
/// Installs the `/usage` card that a Whisply-managed session produces.
///
/// `/usage` resolves through the native broker, which a unit test has no way to
/// reach, so dispatching the command here would only ever exercise the
/// broker-unavailable branch. Installing the card directly keeps this card
/// lifecycle coverage pointed at the surface users actually get.
fn dispatch_usage_and_expect_refresh(
    chat: &mut ChatWidget,
    _rx: &mut tokio::sync::mpsc::UnboundedReceiver<AppEvent>,
) -> u64 {
    chat.add_managed_usage_output()
}

fn drained_events(rx: &mut tokio::sync::mpsc::UnboundedReceiver<AppEvent>) -> Vec<AppEvent> {
...
async fn slash_init_does_not_depend_on_loaded_instruction_sources() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.instruction_source_paths = vec![whisply_utils_path_uri::PathUri::from_abs_path(
...
async fn queued_goal_slash_command_preserves_large_paste() {
    ...
    chat.thread_id = Some(thread_id);
    handle_turn_started(&mut chat, "turn-1");
    let paste = "x".repeat(whisply_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
async fn interrupt_disambiguates_same_sized_goal_pastes() {
    ...
    chat.set_feature_enabled(Feature::Goals, /*enabled*/ true);
    handle_turn_started(&mut chat, "turn-1");
    let first = "a".repeat(whisply_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
#[tokio::test]
async fn signed_out_usage_command_reports_whisply_app_requirement() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    assert_chatwidget_snapshot!(
        "signed_out_usage_command_reports_whisply_app_requirement",
        rendered
    );
...
#[tokio::test]
async fn local_provider_usage_command_reports_unmetered_local_session() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    // Whisply permits local OSS providers alongside the managed gateway, and
    // those sessions route away from managed usage. They have no hosted
    // account, so `/usage` must not send the user looking for a sign-in.
    chat.config.model_provider.name = "Ollama".to_string();
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .join("\n");
    assert!(
        rendered.contains("Local provider sessions aren't metered."),
        "{rendered}"
    );
...
#[tokio::test]
async fn signed_out_usage_command_with_args_is_refused_without_upstream_login_prompt() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    let rendered = cells
        ...
        .join("\n");
    assert!(
        !rendered.contains("ChatGPT"),
        "/usage must not advertise an upstream account, got: {rendered:?}"
    );
...
/// A stale backend-auth flag must not reopen the upstream account read.
///
/// `RefreshTokenActivity` resolves `account/usage/read`, which the app-server
/// refuses as unavailable managed account data. Whisply reads usage through the
/// broker instead, so reaching the upstream request at all would mean a legacy
/// account path had been re-exposed by an auth flag rather than by policy.
#[tokio::test]
async fn backend_auth_flag_does_not_reopen_upstream_account_usage_read() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    assert!(
        !drained_events(&mut rx)
            .iter()
            .any(|event| matches!(event, AppEvent::RefreshTokenActivity { .. })),
        "/usage must not issue the upstream account read"
    );
    assert!(!chat.has_chatgpt_account());
}

/// Same guarantee as above for auth supplied at widget construction, which is
/// the path a resumed session takes.
#[tokio::test]
async fn backend_auth_at_widget_init_does_not_reopen_upstream_account_usage_read() {
...
async fn clearing_pending_token_activity_refreshes_discards_late_result() {
    ...
    assert!(
        !chat.finish_managed_usage_refresh(
            request_id,
...
async fn completed_token_activity_refresh_returns_one_history_cell() {
    ...
    assert!(chat.finish_managed_usage_refresh(
        request_id,
        Err("Whisply Usage is unavailable.".to_string())
    ));
    assert!(chat.pending_token_activity_output().is_some());
    let cell = chat
...
async fn repeated_token_activity_refreshes_keep_only_latest_card() {
    ...
    let second_request_id = dispatch_usage_and_expect_refresh(&mut chat, &mut rx);
...
async fn copy_shortcut_can_be_remapped() {
    ...
    keymap_config.global.copy = Some(whisply_config::types::KeybindingsSpec::One(
        whisply_config::types::KeybindingSpec("ctrl-x".to_string()),
    ));
    let runtime_keymap =
...
/// WCD-711: the TUI needs a way to choose the working directory from inside a
/// session, not only a flag at launch.
#[tokio::test]
async fn slash_directory_none_starts_a_no_directory_thread() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.bottom_pane
        .set_composer_text("/directory none".to_string(), Vec::new(), Vec::new());

    chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));

    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::NewSessionInDirectory {
            selection: codex_whisply::DirectorySelection::NoDirectory
        })
    );
}

#[tokio::test]
async fn slash_directory_takes_an_absolute_folder() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let chosen = tempfile::tempdir().expect("temporary folder");
    let expected = chosen.path().canonicalize().expect("canonical folder");
    chat.bottom_pane.set_composer_text(
        format!("/directory {}", chosen.path().display()),
        Vec::new(),
        Vec::new(),
    );
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::NewSessionInDirectory {
            selection: codex_whisply::DirectorySelection::Selected { canonical_path }
        }) if canonical_path == expected
    );
}

/// A relative path is the most likely typo, and starting a thread in the wrong
/// place is worse than saying so.
#[tokio::test]
async fn slash_directory_rejects_a_relative_path_without_starting_a_thread() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.bottom_pane.set_composer_text(
    ...
    let mut started_a_thread = false;
    while let Ok(event) = rx.try_recv() {
        if matches!(event, AppEvent::NewSessionInDirectory { .. }) {
            started_a_thread = true;
        }
    }
    assert!(
        !started_a_thread,
        "a relative path must be reported, not silently resolved against the launch directory"
    );
}

/// A folder that is not there is reported before a thread is started in it.
#[tokio::test]
async fn slash_directory_rejects_a_missing_folder_without_starting_a_thread() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let missing = tempfile::tempdir().expect("temporary folder");
    let missing = missing.path().join("not-here");
    chat.bottom_pane.set_composer_text(
        format!("/directory {}", missing.display()),
        Vec::new(),
        Vec::new(),
    );
    ...
    assert!(
        !started_a_thread,
        "a missing folder must be reported before a thread is started in it"
    );
}
