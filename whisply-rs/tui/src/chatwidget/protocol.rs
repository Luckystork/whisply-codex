    ...
    pub(crate) fn handle_server_notification(
        ...
        match notification {
            ...
            ServerNotification::WhisplyToolProgress(notification) => {
                // Replayed history is a record of what already happened; a
                // step of a call that is no longer running must not be shown
                // as the current activity.
                if !from_replay {
                    self.on_whisply_tool_progress(notification.label);
                }
            }
            ...
            | ServerNotification::WhisplyExamExtraUsage(_) => {}
