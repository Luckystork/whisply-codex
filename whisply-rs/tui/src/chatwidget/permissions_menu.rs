...
impl ChatWidget {
    pub(super) fn open_permission_profiles_popup(&mut self) {
        ...
        items.push(self.builtin_permission_mode_selection_item(
            ...
            // Naming what Full access does not do is more use than a general
            // caution, and this is the only place the person sees before
            // choosing it.
            format!(
                "{} {}",
                full_access
                    .description
                    .replace(" Exercise caution when using.", ""),
                codex_whisply::FULL_ACCESS_WARNING
            ),
    ...
    fn builtin_permission_mode_selection_item(
        ...
        // The upstream preset labels name profiles (`Default`, `Full Access`);
        // the three public modes have their own names, shared with the app.
        let label = match (preset.id, approvals_reviewer) {
            ...
            ("full-access", _) => crate::permission_mode::PermissionMode::FullAccess.display_name(),
            _ => preset.label,
        };
        let active_profile_id = self
            .config
