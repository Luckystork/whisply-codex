...
// Whisply resolves `/usage` through the managed gateway. The only other
// providers it permits are local OSS runtimes, which have no hosted account to
// meter, so a session that reaches this branch cannot obtain usage by signing
// in anywhere. The upstream text told those users to sign in with ChatGPT,
// which is an account Whisply never establishes.
const USAGE_UNAVAILABLE_FOR_LOCAL_PROVIDER: &str =
    "/usage reports Whisply-managed model usage. Local provider sessions aren't metered.";
...
impl ChatWidget {
        ...
        match cmd {
            ...
            SlashCommand::Archive => {
                self.bottom_pane.show_selection_view(SelectionViewParams {
                    ...
                    subtitle: Some(
                        "Are you sure? This will archive the current session and exit Whisply"
                            .to_string(),
                    ),
            ...
            SlashCommand::Directory => {
                self.open_directory_popup();
            }
            SlashCommand::Vim => {
            ...
            SlashCommand::Status => {
                if self.uses_managed_usage() {
                    ...
                    self.dispatch_managed_status_usage(request_id);
            ...
            SlashCommand::Usage => {
                if self.uses_managed_usage() {
                    self.dispatch_managed_usage();
                } else {
                    self.report_usage_unavailable_for_local_provider();
                }
            }
            SlashCommand::Ide => {
    ...
    fn dispatch_prepared_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Usage => {
                if self.uses_managed_usage() {
                    if trimmed.is_empty() {
                        ...
                        self.add_error_message("Usage: /usage".to_string());
                    }
                } else {
                    self.report_usage_unavailable_for_local_provider();
                }
            ...
            SlashCommand::Directory if !trimmed.is_empty() => {
                self.select_directory_by_path(args);
            }
            SlashCommand::Pets
                if matches!(
    ...
    fn dispatch_host_controls_command(&mut self, args: &str) {
        ...
        let broker = match self.managed_broker_client() {
            Ok(client) => client,
            Err(message) => {
                self.add_error_message(message);
                return;
    ...
    /// Reads the managed Usage projection through the already authenticated
    /// native broker. This deliberately does not touch the legacy ChatGPT
    /// token-activity or reset-credit RPCs.
    fn dispatch_managed_usage(&mut self) {
        let broker = match self.managed_broker_client() {
            Ok(client) => client,
            Err(message) => {
                self.add_error_message(message);
        ...
        // Render through the same `/usage` card the upstream path uses so the
        // command keeps one surface, one lifecycle, and one place in history.
        let request_id = self.add_managed_usage_output();
        let app_event_tx = self.app_event_tx.clone();
        tokio::task::spawn_blocking(move || {
    ...
    /// Reads the managed Usage projection for a `/status` card that is already
    /// on screen waiting for its limits section.
    fn dispatch_managed_status_usage(&mut self, request_id: u64) {
        let broker = match self.managed_broker_client() {
            Ok(client) => client,
            Err(_) => {
        ...
        let app_event_tx = self.app_event_tx.clone();
        tokio::task::spawn_blocking(move || {
            app_event_tx.send(AppEvent::ManagedStatusUsageResult {
                request_id,
                result: usage::read_managed_usage_snapshot(&broker).map(Box::new),
            });
        });
    }

    fn managed_broker_client(&mut self) -> Result<Arc<NativeBrokerClient>, String> {
        match &self.managed_broker {
            Some(broker) => Ok(broker.clone()),
            None => match host_controls_client_from_environment() {
                Ok(client) => {
                    self.managed_broker = Some(client.clone());
                    Ok(client)
                }
    ...
    /// Reports `/usage` on a session that is not on the managed gateway.
    ///
    /// This deliberately ignores `has_codex_backend_auth`. Honouring it would
    /// let a leftover upstream auth flag open the ChatGPT usage menu and its
    /// reset-credit redemption, which are account authority Whisply replaced;
    /// the local session still has no hosted account to report either way.
    fn report_usage_unavailable_for_local_provider(&mut self) {
        self.add_error_message(USAGE_UNAVAILABLE_FOR_LOCAL_PROVIDER.to_string());
    }
