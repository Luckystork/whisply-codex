...
impl ChatWidget {
    ...
    pub(crate) fn open_full_access_confirmation(
        ...
        // The confirmation is reached only from the full-access mode, so it is
        // named the way that mode is named everywhere else rather than by the
        // underlying preset.
        let selected_name = crate::permission_mode::PermissionMode::FullAccess
            .display_name()
            .to_string();
        ...
        let info_lines = if is_cyber_model {
            ...
            vec![
                Line::default(),
                Line::from(
                    "When Whisply runs with full access, it can edit any file on your computer and run commands with network, without your approval.",
                ),
                Line::default(),
                Line::from(vec![
