...
impl ChatWidget {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match whisply_windows_sandbox::apply_world_writable_scan_and_denies_for_permissions(
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(
        ...
        header.push(*Box::new(
            Paragraph::new(if allow_unelevated {
                vec![
                    line!["Set up the Whisply agent sandbox to protect your files and control network access."],
                ]
            } else {
                vec![
                    line!["Your organization requires the default Whisply agent sandbox to continue. Set it up to protect your files and control network access."],
                ]
            })
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        if allow_unelevated {
            lines.push(line![
                "You can still use Whisply in a non-admin sandbox. It carries greater risk if prompt injected."
            ]);
        } else {
            lines.push(line![
                "Your organization requires the default sandbox before Whisply can continue."
            ]);
        }
        let mut header = ColumnRenderable::new();
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                name: "Use Whisply with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
