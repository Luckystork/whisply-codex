...
/// themselves instead of taking the formatted card.
pub(super) fn read_managed_usage_snapshot(
    broker: &NativeBrokerClient,
) -> Result<ContextualUsageSnapshot, String> {
    codex_whisply::read_account_usage(broker)
}

fn format_managed_usage(snapshot: &ContextualUsageSnapshot) -> String {
...
#[cfg(test)]
mod managed_usage_tests {
    use super::*;

    fn usage_snapshot() -> ContextualUsageSnapshot {
        let usage_window = |window| codex_whisply::ContextualUsageWindow {
            ...
            cap: 100.0,
            used_fraction: 0.15,
            starts_at: Some("2026-08-01T00:00:00Z".to_string()),
            resets_at: Some("2026-08-02T00:00:00Z".to_string()),
        ...
        ContextualUsageSnapshot {
            ...
            tier: "pro".to_string(),
            windows: vec![
                usage_window(codex_whisply::UsageWindowKind::FiveHour),
                usage_window(codex_whisply::UsageWindowKind::Weekly),
                codex_whisply::ContextualUsageWindow {
                    category: codex_whisply::UsageWindowCategory::Transcription,
                    window: codex_whisply::UsageWindowKind::Weekly,
                    settled: 3.0,
                    reserved: 1.0,
                    cap: 20.0,
                    used_fraction: 0.2,
            ...
            metering: codex_whisply::ContextualUsageMetering {
                ...
                rate_card_version: "rate-card-test-1".to_string(),
                normalized_units_per_cash_micro: 9,
                tokens: codex_whisply::ContextualUsageTokenWindows {
                    five_hour: codex_whisply::ContextualUsageTokenTotals {
                        input: 1,
                        output: 2,
                        cache_read: 3,
                        cache_write: 4,
                    },
                    weekly: codex_whisply::ContextualUsageTokenTotals {
                        input: 5,
                        output: 6,
                        cache_read: 7,
                        cache_write: 8,
            ...
            generated_at: "2026-08-01T01:00:00Z".to_string(),
            stale: false,
        }
    }

    #[test]
    fn managed_usage_output_shows_all_whisply_windows_and_rate_card() {
        let output = format_managed_usage(&usage_snapshot());

        assert!(output.contains("Whisply Usage (pro)"));
        assert!(output.contains("Usage (five-hour): 15.0% used"));
        assert!(output.contains("Usage (weekly): 15.0% used"));
        assert!(output.contains("Transcription (weekly): 20.0% used"));
        assert!(output.contains("Rate card rate-card-test-1 · model_cost_weighted USD"));
    }
}
