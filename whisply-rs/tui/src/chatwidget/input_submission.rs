...
impl ChatWidget {
    ...
    fn submit_user_message_with_history_and_shell_escape_policy(
        ...
        let mut duplicate_notices: Vec<String> = Vec::new();
        ...
        if let Some(skills) = self.bottom_pane.skills() {
            ...
            // A name is not a unique handle. When several installed packages
            // answer to the mention, say which one ran rather than letting the
            // person find out from the result.
            duplicate_notices = duplicate_skill_mention_notices(&resolution);
        }
