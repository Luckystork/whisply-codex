...
use whisply_core_plugins::remote::REMOTE_WORKSPACE_MARKETPLACE_NAME;
use whisply_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_MARKETPLACE_NAME;
use whisply_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_PRIVATE_MARKETPLACE_NAME;
use whisply_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_UNLISTED_MARKETPLACE_NAME;
...
/// A header line that wraps instead of being cut off at the terminal edge.
///
/// Plain `Line`s in the header are clipped, which is survivable for a label and
/// not for a disclosure: the commands a plugin runs and the variables it reads
/// are both long enough to reach the margin, and a sentence that ends mid-word
/// is worse than one the reader can finish.
struct WrappingDisclosureLine {
    line: Line<'static>,
}

impl Renderable for WrappingDisclosureLine {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        Paragraph::new(self.line.clone())
            .wrap(Wrap { trim: false })
            .render(area, buf);
...
impl ChatWidget {
    ...
    pub(super) fn marketplace_remove_confirmation_popup_params(
        ...
        header.push(Line::from(
            "This removes the configured marketplace from Whisply.".dim(),
        ));
    ...
    pub(super) fn plugin_detail_popup_params(
        ...
        // In the header rather than the item list, and not only because it
        // reads better there: the list is capped at `MAX_POPUP_ROWS` and the
        // detail already fills it, so a row here would sit below the fold. A
        // disclosure a user has to scroll to find is not a disclosure.
        if let Some(summary) = plugin_command_summary(plugin) {
            header.push(WrappingDisclosureLine {
                line: Line::from(summary.dim()),
            });
        }
        if let Some(summary) = plugin_environment_summary(plugin) {
            header.push(WrappingDisclosureLine {
                line: Line::from(summary.dim()),
            });
        }
        // Only before install: afterwards this is a description of a decision
        // already made, and the row it would cost is better spent on the rest.
        if !plugin.summary.installed
            && let Some(summary) = plugin_update_summary(plugin)
        {
            header.push(WrappingDisclosureLine {
                line: Line::from(summary.dim()),
            });
...
fn plugin_source_summary(plugin: &PluginDetail) -> String {
    match &plugin.summary.source {
        // The path is the whole disclosure for a local plugin: it is the code
        // that will run, and unlike a git or npm source it is not copied
        // anywhere, so naming it is what lets a user check it.
        PluginSource::Local { path } => format!("Local · {}", path.display()),
        PluginSource::Git {
            url, ref_name, sha, ..
        } => {
            let location = match ref_name {
                Some(ref_name) => format!("Git · {url}@{ref_name}"),
                None => format!("Git · {url}"),
            };
            format!("{location} {}", git_pin_summary(sha.as_deref()))
        }
        PluginSource::Npm {
            package, version, ..
        } => match version {
...
/// How this plugin gets new code once it is installed.
///
/// This is the last of the things WCD-301 asks be shown before install, and it
/// is the one that decides what the others are worth. Knowing a git source is
/// pinned to a commit only matters if that commit is also what keeps running;
/// knowing which commands its hooks run only matters if those commands cannot
/// be swapped afterwards without asking.
///
/// The answer differs by source and the difference is not cosmetic. Git and npm
/// plugins are install-materialized: `install_resolved_plugin` copies the source
/// into the plugin store and every later load resolves through
/// `active_plugin_root`, so the reviewed code is the code that runs until the
/// user updates it. A local plugin is not copied at all -- it stays a path, and
/// each load re-reads whatever is in that directory now. That is a reasonable
/// thing to want while developing a plugin, and an unreasonable thing to be
/// unaware of, because it means everything else disclosed here describes only
/// this moment.
///
/// `Remote` says nothing: its real source resolves server-side at install, so
/// any claim here about what happens afterwards would be a guess.
fn plugin_update_summary(plugin: &PluginDetail) -> Option<String> {
    match &plugin.summary.source {
        PluginSource::Local { .. } => Some(
            "Updates: runs from that folder in place. Whatever is in it when \
             Whisply loads it is what runs, with no update step and no further \
             review."
                .to_string(),
        ),
        PluginSource::Git { .. } | PluginSource::Npm { .. } => Some(
            "Updates: installing takes a copy. It keeps running that copy until \
             you update it."
                .to_string(),
        ),
        PluginSource::Remote => None,
    }
}

/// Whether a git plugin resolves to one fixed commit.
///
/// A branch or tag is a name the other side can repoint, so `url@main` does not
/// describe the code that will be installed tomorrow. Saying so is the point:
/// an unpinned source is the case a user would want to know about, and the
/// declared commit was already carried through the protocol and dropped here.
fn git_pin_summary(sha: Option<&str>) -> String {
    match sha {
        Some(sha) => format!("(pinned to {})", short_git_sha(sha)),
        None => "(not pinned to a commit)".to_string(),
    }
}

/// Abbreviate a full object id for reading. Anything that is not one is shown
/// as written, since a short selector is already the recognisable form.
fn short_git_sha(sha: &str) -> String {
    const ABBREVIATED_GIT_SHA_LEN: usize = 12;
    let is_object_id = sha.len() >= 40 && sha.chars().all(|c| c.is_ascii_hexdigit());
    if is_object_id {
        sha[..ABBREVIATED_GIT_SHA_LEN].to_string()
    } else {
        sha.to_string()
    }
}
...
fn plugin_hook_summary(plugin: &PluginDetail) -> String {
    if plugin.hooks.is_empty() {
        ...
        let events = event_counts
            ...
            .collect::<Vec<_>>()
            .join(", ");
        // The count alone reads the same whether these hooks run commands or
        // only add instructions, and those are very different things to agree
        // to. The header names the commands; this says how many there are.
        let executing = plugin
            .hooks
            .iter()
            .filter(|hook| hook.handler_type == HookHandlerType::Command)
            .count();
        let executing = match executing {
            0 => "none run commands".to_string(),
            1 => "1 runs a command".to_string(),
            many => format!("{many} run commands"),
        };
        format!("{events} · {executing}.")
    }
}
...
/// What the plugin will read out of the user's environment, if anything.
///
/// Returns `None` when it reads nothing, so the row only appears when it has
/// something to report. The variable names are the disclosure -- their values
/// are the user's secrets and are never shown.
fn plugin_environment_summary(plugin: &PluginDetail) -> Option<String> {
    let mut parts = Vec::new();
    if !plugin.host_env_vars.is_empty() {
        parts.push(format!("Reads {}.", plugin.host_env_vars.join(", ")));
    }
    if !plugin.refused_host_env_vars.is_empty() {
        // Stated even though it cannot succeed. A plugin reaching for Whisply's
        // own namespace is a fact about the plugin worth knowing before install,
        // and hiding a blocked attempt would be the more surprising choice.
        parts.push(format!(
            "Asks for {}, which Whisply does not share.",
            plugin.refused_host_env_vars.join(", ")
        ));
    }
    (!parts.is_empty()).then(|| parts.join(" "))
}

/// How many commands are named before the line starts costing more attention
/// than it buys.
const MAX_DISCLOSED_HOOK_COMMANDS: usize = 3;

/// Long enough to recognise a script by name and argument, short enough that
/// one baroque command cannot push the rest of the disclosure off the line.
const MAX_DISCLOSED_COMMAND_GRAPHEMES: usize = 56;

/// What the plugin will execute on its own once installed, if anything.
///
/// Hooks are the only part of a plugin that runs without the user asking, so
/// this is the fact an install decision most depends on, and until now the
/// detail view named only the event and a count. Prompt and agent handlers are
/// excluded: they add instructions rather than execute anything locally, and
/// listing them here would dilute a line that exists to describe execution.
fn plugin_command_summary(plugin: &PluginDetail) -> Option<String> {
    // Grouped by occasion so a plugin with several hooks on one event reads as
    // one statement about that event rather than as the event name repeated.
    // First-seen order, because sorting would put the disclosure in an order
    // the manifest does not have.
    let mut by_occasion: Vec<(String, Vec<String>)> = Vec::new();
    let mut total = 0usize;
    for hook in &plugin.hooks {
        if hook.handler_type != HookHandlerType::Command {
            continue;
        }
        ...
        let event = &hook.event_name;
        let occasion = match hook.matcher.as_deref().map(str::trim) {
            Some(matcher) if !matcher.is_empty() => format!("{event:?}: {matcher}"),
            _ => format!("{event:?}"),
        };
        let command = truncate_text(command, MAX_DISCLOSED_COMMAND_GRAPHEMES);
        let index = match by_occasion
            .iter()
            .position(|(existing, _)| *existing == occasion)
        {
            Some(index) => index,
            None => {
                by_occasion.push((occasion, Vec::new()));
                by_occasion.len() - 1
            }
        };
        let Some((_, commands)) = by_occasion.get_mut(index) else {
            continue;
        };
    ...
    let mut remaining = MAX_DISCLOSED_HOOK_COMMANDS;
    let mut groups = Vec::new();
    for (occasion, mut commands) in by_occasion {
        if remaining == 0 {
            break;
        }
        commands.truncate(remaining);
        remaining -= commands.len();
        groups.push(format!("{} ({occasion})", commands.join(", ")));
    }

    let listed = groups.join("; ");
    Some(match total.saturating_sub(MAX_DISCLOSED_HOOK_COMMANDS) {
        0 => format!("Runs on its own: {listed}."),
        1 => format!("Runs on its own: {listed}, and 1 more."),
        more => format!("Runs on its own: {listed}, and {more} more."),
    })
}
