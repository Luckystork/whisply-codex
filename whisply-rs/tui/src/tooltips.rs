...
const APP_TOOLTIP: &str = "Try the **Desktop app**. Run 'whisply app' to open it.";
const FAST_TOOLTIP: &str =
    "*New* Use **/fast** to enable our fastest inference with increased plan usage.";
const OTHER_TOOLTIP: &str =
    "*New* Build faster with the **Desktop app**. Run 'whisply app' to open it.";
const OTHER_TOOLTIP_NON_MAC: &str = "*New* Build faster with Whisply.";
const FREE_GO_TOOLTIP: &str =
    "*New* For a limited time, Whisply is included in your plan for free – let’s build together.";
...
lazy_static! {
    static ref TOOLTIPS: Vec<&'static str> = RAW_TOOLTIPS
        ...
        .filter(|line| {
            ...
            if !IS_MACOS && !IS_WINDOWS && line.contains("whisply app") {
                return false;
            }
            true
        })
