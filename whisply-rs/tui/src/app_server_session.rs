...
use whisply_app_server_client::AppServerEvent;
use whisply_app_server_client::AppServerPath;
use whisply_app_server_client::AppServerRequestHandle;
use whisply_app_server_client::TypedRequestError;
...
use whisply_protocol::openai_models::ModelUpgrade;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<whisply_protocol::account::PlanType>,
...
impl AppServerSession {
    ...
    pub(crate) async fn turn_start(
        ...
        model: String,
        effort: Option<whisply_protocol::openai_models::ReasoningEffort>,
        summary: Option<whisply_protocol::config_types::ReasoningSummary>,
        service_tier: Option<Option<String>>,
        collaboration_mode: Option<whisply_protocol::config_types::CollaborationMode>,
        personality: Option<whisply_protocol::config_types::Personality>,
...
pub(crate) fn status_account_display_from_auth_mode(
    ...
    plan_type: Option<whisply_protocol::account::PlanType>,
...
async fn thread_session_state_from_thread_response(
    ...
    let history_config =
        whisply_message_history::HistoryConfig::new(config.codex_home.clone(), &config.history);
    let (log_id, entry_count) = whisply_message_history::history_metadata(&history_config).await;
...
mod tests {
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        whisply_message_history::append_entry("older", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        whisply_message_history::append_entry("newer", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        ...
        let session = thread_session_state_from_thread_response(
            ...
            whisply_protocol::config_types::ApprovalsReviewer::User,
            PermissionProfile::read_only(),
            /*active_permission_profile*/ None,
            test_path_buf("/tmp/project").abs(),
            Vec::new(),
    ...
    fn status_account_display_from_auth_mode_uses_remapped_plan_labels() {
        let business = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(whisply_protocol::account::PlanType::EnterpriseCbpUsageBased),
        );
        assert!(matches!(
            business,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let team = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(whisply_protocol::account::PlanType::SelfServeBusinessUsageBased),
        );
        assert!(matches!(
            team,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let business_prolite = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(whisply_protocol::account::PlanType::SelfServeBusinessProLite),
        );
        assert!(matches!(
            business_prolite,
            Some(StatusAccountDisplay::ChatGpt {
