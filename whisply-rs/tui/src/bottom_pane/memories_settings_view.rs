...
impl MemoriesSettingsView {
    pub(crate) fn new(
        ...
        let mut view = Self {
            ...
            complete: false,
            app_event_tx,
            footer_note: Line::from("Memories are stored on this Mac.".dim()),
            keymap,
        };
    ...
    fn settings_header(&self) -> ColumnRenderable<'_> {
        ...
        header.push(Line::from(
            "Choose how Whisply uses and creates memories. Changes are saved to config.toml".dim(),
        ));
        header
    }
    ...
    fn reset_confirmation_header(&self) -> ColumnRenderable<'_> {
        ...
        header.push(Line::from(
            "This clears local memory files and rollout summaries for the current Whisply home."
                .dim(),
        ));
        header
    }
...
impl Renderable for MemoriesSettingsView {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if self.reset_confirmation.is_none() {
            self.footer_note.clone().render(docs_area, buf);
        }
