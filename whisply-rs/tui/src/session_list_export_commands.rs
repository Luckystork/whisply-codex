...
async fn list_with_app_server(
    ...
    for &archived in scope.archived_values() {
        ...
        loop {
            ...
            let Some(next_cursor) = cursor.clone() else {
                break;
            };
            if entries.len().saturating_sub(collection_start) >= limit as usize {
                break;
            }
...
fn list_entry(thread: Thread, archived: bool) -> SessionListEntry {
    SessionListEntry {
        id: thread.id,
        name: thread.name.map(|name| redact_and_bound_text(&name, 160)),
...
mod tests {
    ...
    fn sample_thread(items: Vec<ThreadItem>) -> Thread {
        Thread {
            ...
            path: None,
            cwd: whisply_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
                std::env::temp_dir(),
            )
    ...
    fn all_scope_includes_both_collections() {
        assert_eq!(
            SessionCollectionScope::All.archived_values(),
            &[false, true]
        );
    }
    ...
    fn export_uses_only_user_and_assistant_text_and_redacts_secrets() {
        ...
        assert_eq!(export.transcript[0].text, "[REDACTED SENSITIVE CONTENT]");
