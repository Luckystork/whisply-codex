...
use whisply_protocol::account::ProviderAccount;
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    fn map_api_error(&self, error: ApiError) -> CodexErr {
        whisply_api::map_api_error(error)
    }
    ...
    fn runtime_base_url(
        &self,
    ) -> ModelProviderFuture<'_, whisply_protocol::error::Result<Option<String>>> {
    ...
    ) -> ModelProviderFuture<'_, whisply_protocol::error::Result<ResolvedProviderAuth>> {
        Box::pin(async move {
    ...
    fn models_manager_without_cache(
        ...
        let model_catalog = config_model_catalog
            .or_else(|| whisply_models_manager::bundled_models_response().ok())
            .unwrap_or_default();
...
mod tests {
    ...
    use whisply_model_provider_info::ModelProviderAwsAuthInfo;
    ...
    async fn configured_bedrock_catalog_only_allows_default_service_tier() {
        let configured_model = whisply_models_manager::bundled_models_response()
