...
use whisply_code_mode::WaitOutcome;
use whisply_code_mode::WaitRequest;
use whisply_code_mode::host::MAX_FRAME_BYTES;
...
async fn session_execution_limits_are_isolated_on_a_shared_process_host() {
    let provider: Arc<dyn CodeModeSessionProvider> =
        Arc::new(ProcessOwnedCodeModeSessionProvider::with_host_program(
            whisply_utils_cargo_bin::cargo_bin("codex-code-mode-host").expect("host binary"),
        ));
