...
use whisply_protocol::protocol::GitInfo as ProtocolGitInfo;
...
impl From<whisply_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: whisply_state::ThreadsPage) -> Self {
...
fn thread_item_from_state_metadata(
    item: whisply_state::ThreadMetadata,
    parent_thread_id: Option<ThreadId>,
) -> ThreadItem {
    ThreadItem {
...
async fn select_resume_path_from_db_page(
    page: &whisply_state::ThreadsPage,
    filter_cwd: Option<&Path>,
    default_provider: &str,
) -> Option<PathBuf> {
    match filter_cwd {
