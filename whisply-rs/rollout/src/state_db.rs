...
pub use whisply_state::LogEntry;
use whisply_state::SqliteConfig;
use whisply_state::ThreadMetadataBuilder;
use whisply_utils_path::normalize_for_path_comparison;

/// Core-facing handle to the SQLite-backed state runtime.
pub type StateDbHandle = Arc<whisply_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        whisply_state::StateRuntime::init(sqlite.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    whisply_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == whisply_state::BackfillStatus::Complete {
            return Ok(());
        }
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    ...
    let runtime = match whisply_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    ...
    originator: &str,
) -> whisply_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == whisply_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<whisply_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&whisply_state::StateRuntime>,
    sqlite: &whisply_state::SqliteConfig,
    page_size: usize,
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => whisply_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => whisply_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => whisply_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
    ...
    relation_filter: Option<whisply_state::ThreadRelationFilter>,
    archived: bool,
    section: Option<Option<&str>>,
    search_term: Option<&str>,
) -> Option<whisply_state::ThreadsPage> {
    let ctx = context?;
    if ctx.sqlite() != sqlite {
        warn!(
            "state db SQLite home mismatch: expected {}, got {}",
    ...
    if let Some(relation_filter) = relation_filter {
        let filters = whisply_state::ThreadFilterOptions {
    ...
    Some(whisply_state::ThreadsPage {
        items,
