...
use whisply_otel::OtelProvider;
use whisply_otel::current_span_w3c_trace_context;
use whisply_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $WHISPLY_HOME/config.toml to get a trace URL!";
...
fn serve(codex_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,whisply_=trace {}",
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = whisply_core::otel_init::build_provider(
            &config,
