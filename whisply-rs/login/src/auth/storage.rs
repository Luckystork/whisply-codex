...
use whisply_agent_identity::AgentIdentityJwtClaims;
...
use whisply_keyring_store::DefaultKeyringStore;
use whisply_keyring_store::KeyringStore;
use whisply_protocol::account::PlanType as AccountPlanType;
use whisply_protocol::auth::AuthMode;
use whisply_secrets::LocalSecretsNamespace;
use whisply_secrets::SecretName;
use whisply_secrets::SecretScope;
use whisply_secrets::SecretsBackendKind;
use whisply_secrets::SecretsManager;

/// Expected structure for $WHISPLY_HOME/auth.json.
...
impl FileAuthStorage {
    ...
    /// Attempt to read and parse the `auth.json` file in the given `WHISPLY_HOME` directory.
