...
use whisply_login::LoginCallbackResult;
use whisply_login::LoginOnboardingEntrypoint;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let opts = ServerOptions {
        ...
        auth_route_config: whisply_login::test_support::transport_default_auth_route_config(),
        client_id: whisply_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to Whisply"),
        "error body should clearly explain the Whisply access denial"
    );
    assert!(
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to Whisply to retry"),
    ...
    assert!(
        !body.contains("get access to Whisply"),
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let mut opts = ServerOptions::new(
        ...
        whisply_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
