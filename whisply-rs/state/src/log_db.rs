...
//!
//! # async fn example(state_db: std::sync::Arc<whisply_state::StateRuntime>) {
...
pub fn default_filter() -> Targets {
    Targets::new()
        ...
        .with_target("whisply_rmcp_client", LevelFilter::INFO)
    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: tracing_subscriber::layer::Context<'_, S>) {
        ...
        // The file log is not the only place the product records what it did.
        // This store persists the same events under the account home, and it is
        // the one the Mac app's runtime writes, so scrubbing only the file sink
        // would leave the app's own log unredacted. Both message and body are
        // free text assembled from call sites across the tree, so both pass
        // through the shared scrubber before anything is persisted.
        let entry = LogEntry {
            ...
            message: visitor
                .message
                .map(|message| redact_log_line(&message).into_owned()),
            feedback_log_body: Some(redact_log_line(&feedback_log_body).into_owned()),
            thread_id,
...
mod tests {
    ...
    /// This store is the local log the Mac app's runtime writes, so a
    /// credential reaching it is on disk under the account home for as long as
    /// the retention window allows.
    #[tokio::test]
    async fn a_credential_does_not_reach_the_persisted_log() {
        let codex_home = temp_codex_home();
        let runtime = StateRuntime::init(
            crate::SqliteConfig::new_for_testing(codex_home.as_path().abs()),
            "test-provider".to_string(),
        ...
        tracing::info_span!("redaction-thread", thread_id = "thread-secret").in_scope(|| {
            tracing::info!("refresh failed for person@example.com: Bearer abc123def456xyz");
        });
        ...
        let logs = String::from_utf8(
            runtime
                .query_feedback_logs("thread-secret")
                .await
                .expect("query feedback logs"),
        )
        .expect("valid utf-8");
        assert!(!logs.contains("abc123def456xyz"), "{logs}");
        assert!(!logs.contains("person@example.com"), "{logs}");
        assert!(logs.contains("refresh failed for"), "{logs}");
