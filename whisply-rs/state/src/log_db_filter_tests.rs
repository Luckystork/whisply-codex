...
async fn sqlite_sink_drops_low_level_opentelemetry_sdk_logs() {
    ...
    tracing::debug!(
        target: "whisply_rmcp_client::oauth",
    ...
    tracing::trace!(target: "whisply_state", "retained-trace");
    tracing::trace!(
        target: "whisply_api::responses_websocket_timing",
    ...
    assert_eq!(
        ...
        vec![
            ...
            (
                "INFO",
                "whisply_rmcp_client::oauth",
                Some("retained-codex-rmcp-client-info")
            ),
            ("TRACE", "whisply_state", Some("retained-trace")),
        ]
    );
