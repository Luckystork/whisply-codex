...
static MCP_AUTHORIZATION_HEADER_REGEX: LazyLock<Regex> =
    LazyLock::new(|| compile_regex(r"(?im)(\b(?:proxy-)?authorization\s*[:=]\s*)[^\r\n]*"));
...
mod tests {
    ...
    fn redact_mcp_stderr_redacts_complete_basic_authorization_header_values() {
        ...
        assert!(!redacted.contains(proxy_authorization), "leaked {redacted}");
        assert_eq!(
            redacted,
