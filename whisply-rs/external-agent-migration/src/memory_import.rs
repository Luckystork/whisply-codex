...
- For each project, always read its source `MEMORY.md` first when it exists. Use it to seed or update that project's scoped entry in Whisply `MEMORY.md`, and add only the smallest broadly useful route to `memory_summary.md`.
...
- Keep source-specific frontmatter in the imported resource. Do not reinterpret fields such as `metadata.originSessionId` as a Whisply `thread_id`, `rollout_path`, or `updated_at`.
- Treat every other source `*.md` file as detailed supporting evidence analogous to a rollout summary. Do not flatten its full contents into Whisply `MEMORY.md` or `memory_summary.md`. Keep the detail in the imported resource, add a concise pointer from the scoped `MEMORY.md` entry when useful, and read the resource progressively when a later task needs that topic.
- Preserve this hierarchy after migration: Whisply `MEMORY.md` is the searchable routing layer, `memory_summary.md` is the compact global index, and non-`MEMORY.md` imported resources are progressive-disclosure detail.
...
pub(super) async fn import(
    ...
    let state_db = state_db.ok_or_else(|| {
        io::Error::new(
            ...
            "memory import requires the Whisply state database",
        )
    })?;
    let memory_root = codex_home.join("memories");
    whisply_memories_write::workspace::prepare_memory_workspace(&memory_root)
        .await
