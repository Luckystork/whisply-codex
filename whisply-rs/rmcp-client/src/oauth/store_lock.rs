...
const LOCK_CONTENTION_EVENT_TARGET: &str = "whisply_rmcp_client::oauth::store_lock::contention";
...
impl OAuthStoreLock {
    pub(super) fn acquire(store: OAuthStore) -> Result<Self, OAuthStoreLockFailure> {
        ...
        // authority. Those stores are WHISPLY_HOME-backed today: if WHISPLY_HOME is unset they use
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve WHISPLY_HOME for MCP OAuth {store} aggregate-store lock")]
    CodexHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
