...
This repository uses Bazel to build the Rust workspace under `whisply-rs`.
...
- `rules_rs` imports third-party crates from `whisply-rs/Cargo.toml` and
  `whisply-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
...
- Each crate in `whisply-rs/*/BUILD.bazel` typically uses `codex_rust_crate` and
