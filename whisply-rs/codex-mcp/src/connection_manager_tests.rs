...
use whisply_config::Constrained;
use whisply_config::McpServerAuth;
use whisply_config::McpServerConfig;
use whisply_config::McpServerEnvVar;
...
use whisply_connectors::ConnectorRuntimeContext;
use whisply_connectors::ConnectorRuntimeContextKey;
use whisply_connectors::ConnectorRuntimeFetchSource;
use whisply_connectors::ConnectorRuntimeManager;
use whisply_exec_server_test_support::environment_manager_without_environments;
use whisply_login::AuthHeaders;
use whisply_login::CodexAuth;
use whisply_protocol::ToolName;
...
use whisply_protocol::protocol::McpStartupFailureReason;
use whisply_rmcp_client::ElicitationResponse;
use whisply_rmcp_client::InProcessTransportFactory;
use whisply_rmcp_client::McpAuthState;
use whisply_rmcp_client::McpLoginRequirement;
use whisply_rmcp_client::RmcpClient;
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        whisply_rmcp_client::Elicitation::Mcp(ElicitRequestParams::FormElicitationParams {
            meta: None,
...
async fn shared_elicitation_router_targets_the_exact_pending_request() {
    ...
    let (
        whisply_protocol::mcp::RequestId::String(request_a_id),
        whisply_protocol::mcp::RequestId::String(request_b_id),
...
async fn cancelled_elicitation_is_removed_without_affecting_other_pending_requests() {
    ...
    let (
        whisply_protocol::mcp::RequestId::String(cancelled_id),
        whisply_protocol::mcp::RequestId::String(pending_id),
...
/// Whether a server's tools land in a namespace the product owns decides
/// whether they reach the model at all, and a surface that lists a server's
/// tools has to answer that without rebuilding the catalog. The two answers
/// have to be the same one.
#[test]
fn the_namespace_a_surface_predicts_is_the_one_the_tools_land_in() {
    let cases = [
        (/*prefix*/ true, /*non_prefixed*/ Vec::new()),
        (true, vec!["whisply".to_string()]),
        (false, Vec::new()),
        (true, vec!["my_notes".to_string()]),
    ];

    for (prefix, non_prefixed) in cases {
        for server in ["whisply", "Whisply", "my-notes", "my_notes", "calendar"] {
            let model_tools = normalize_tools_for_model_with_prefix(
                vec![create_test_tool(server, "search")],
                prefix,
                &non_prefixed,
            );
            let landed = model_tools
                .first()
                .expect("the tool survives normalization")
                .callable_namespace
                .clone();

            assert_eq!(
                crate::model_tool_namespace(server, prefix, &non_prefixed),
                landed,
                "predicted the wrong namespace for {server} \
                 (prefix: {prefix}, non-prefixed: {non_prefixed:?})"
            );
        }
    }
}
...
async fn tool_catalog_cache_sanitizes_tools_and_tracks_environment_generation() {
    ...
    let cache_context = |environment: &Arc<whisply_exec_server::Environment>| {
        cache
            .context(
                "docs",
                &config,
...
fn hosted_actor_credentials_are_only_available_to_host_owned_mcp_servers() {
    ...
    whisply_model_provider::auth_provider_from_auth(&bootstrap_auth)
    ...
    let provider = whisply_model_provider::auth_provider_from_auth(&hosted_auth);
    ...
    for (case, static_header, bearer_env_var, env_header, allows_executor_auth) in cases {
        ...
        assert!(
            chatgpt_auth_provider_for_server(
                remote_server,
                Some(whisply_model_provider::auth_provider_from_auth(
                    &hosted_auth
                )),
            )
            .is_none(),
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Run `whisply mcp login {server_name}`."
    );
...
async fn reconciliation_replaces_connection_when_protocol_mode_changes() {
    ...
    mcp_config.protocol_mode = whisply_rmcp_client::McpProtocolMode::V20260728;
...
async fn connection_identity_distinguishes_accounts_with_the_same_token() -> anyhow::Result<()> {
    ...
    let connection_identity = |auth: &CodexAuth| {
        let provider = whisply_model_provider::auth_provider_from_auth(auth);
        McpServerConnectionIdentity::new(
            "docs",
            &server,
            OAuthCredentialsStoreMode::default(),
...
async fn connection_identity_distinguishes_agent_account_runtime_and_task() -> anyhow::Result<()> {
    ...
    let config = reusable_server_config("http://127.0.0.1:1");
    let server = EffectiveMcpServer::configured(config);
    let record = whisply_login::auth::AgentIdentityAuthRecord {
        ...
        plan_type: whisply_protocol::account::PlanType::Plus,
        chatgpt_account_is_fedramp: false,
        task_id: Some("task-a".to_string()),
    };
    let auth_route_config = whisply_login::test_support::transport_default_auth_route_config();
    let previous_auth = CodexAuth::AgentIdentity(
        whisply_login::auth::AgentIdentityAuth::from_record(
            record.clone(),
    ...
    for changed_record in [
        whisply_login::auth::AgentIdentityAuthRecord {
...
/// A tool call that fails because the server behind it is gone should say so,
/// and say whose server it was. The plugin case is the one that matters: the
/// person can disable or reinstall a plugin, and cannot do anything at all
/// about "failed to get client".
#[test]
fn an_unreachable_server_is_named_and_so_is_the_plugin_that_added_it() {
    let message = unreachable_server_message("docs", Some("acme.docs"));
    assert!(message.contains("'docs'"), "{message}");
    assert!(message.contains("acme.docs"), "{message}");
    assert!(message.contains("not running"), "{message}");
}

#[test]
fn a_server_the_person_configured_themselves_is_still_named() {
    let message = unreachable_server_message("my-server", None);
    assert!(message.contains("'my-server'"), "{message}");
    assert!(message.contains("not running"), "{message}");
