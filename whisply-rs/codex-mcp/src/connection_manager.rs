...
use crate::startup_admission::MAX_CONCURRENT_MCP_STARTUPS;
use crate::startup_admission::MAX_STARTED_MCP_SERVERS;
use crate::startup_admission::StartupAdmission;
use crate::startup_admission::admit_servers;
use crate::startup_admission::refused_startup_message;
...
use whisply_protocol::protocol::McpStartupCompleteEvent;
use whisply_protocol::protocol::McpStartupFailure;
...
impl McpConnectionSet {
    ...
    pub async fn new(
        ...
        let static_chatgpt_auth_provider = auth
            ...
            .map(whisply_model_provider::auth_provider_from_auth);
        let codex_apps_auth_provider = codex_apps_auth_manager.and_then(|auth_manager| {
            auth.filter(|auth| auth.uses_codex_backend()).map(|auth| {
                whisply_model_provider::auth_provider_from_auth_manager(auth_manager, auth)
            })
        });
        let enabled_count = mcp_servers
            ...
            .count();
        let StartupAdmission { started, refused } =
            admit_servers(mcp_servers, MAX_STARTED_MCP_SERVERS);
        let refused_message = refused_startup_message(enabled_count, MAX_STARTED_MCP_SERVERS);
        // One permit per server that may be starting at this moment. Startup is
        // where a stdio server costs the most -- a spawn, an interpreter, and
        // whatever it does before it answers -- and it is the moment every
        // server does it together.
        let startup_permits = Arc::new(tokio::sync::Semaphore::new(MAX_CONCURRENT_MCP_STARTUPS));
        for (server_name, server) in started {
            ...
            let startup_permits = Arc::clone(&startup_permits);
            join_set.spawn(async move {
                if !publication_gate.wait().await {
                    return (server_name, Err(StartupOutcomeError::Cancelled));
                }
                // Held until this server is up or has given up, so the number
                // of servers starting at once stays under the ceiling. Waiting
                // for a turn is not being cancelled: the token is checked below
                // either way.
                let _startup_permit = startup_permits.acquire().await;
        ...
        tokio::spawn(async move {
            ...
            if let Some(tx_event) = tx_event {
                ...
                // Reported the same way a failed startup is, because from the
                // outside that is what it is: the server is not there, and the
                // person needs to know which one and why.
                for server_name in refused {
                    let _ = emit_update(
    ...
    pub async fn call_tool(
        ...
        let client = view.connection.client().await.with_context(|| {
            unreachable_server_message(server, self.plugin_id_for_mcp_server_name(server))
        })?;
...
/// What is said when a server that should be there cannot be reached.
///
/// "failed to get client" named nothing at all: not the server, and certainly
/// not who the server belongs to. A server a plugin brought with it is not
/// Whisply, and a failure that reads as though it were sends the person to fix
/// the wrong thing -- usually Whisply, which cannot fix it.
pub(crate) fn unreachable_server_message(server: &str, plugin_id: Option<&str>) -> String {
    match plugin_id {
        Some(plugin_id) => format!(
            "MCP server '{server}', added by the plugin '{plugin_id}', is not running. \
             Its tools stay unavailable until it starts again."
        ),
        None => format!(
            "MCP server '{server}' is not running. Its tools stay unavailable until it \
             starts again."
        ),
    }
}
