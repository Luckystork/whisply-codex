...
use whisply_async_utils::CancelErr;
...
use whisply_protocol::protocol::McpStartupStatus;
use whisply_protocol::protocol::McpStartupUpdateEvent;
use whisply_rmcp_client::ExecutorStdioServerLauncher;
use whisply_rmcp_client::LocalStdioServerLauncher;
use whisply_rmcp_client::McpProtocolMode;
use whisply_rmcp_client::RmcpClient;
use whisply_rmcp_client::StdioServerLauncher;
use whisply_rmcp_client::StreamableHttpRedirectMode;
use whisply_rmcp_client::ToolWithConnectorId;
use whisply_rmcp_client::is_authentication_required_error;
...
fn resolve_bearer_token(
    ...
    // Refused rather than sent. This value leaves the machine as an
    // `Authorization` header to whatever URL the server declared, so naming a
    // Whisply variable here would be a way to mail the product's own authority
    // handles to a third party.
    if whisply_config::host_env_var_is_reserved(env_var) {
        return Err(anyhow!(
            "MCP server '{server_name}' asked to send the Whisply-reserved \
             environment variable {env_var} as its bearer token; Whisply does not \
             share its own credentials with MCP servers"
        ));
    }
...
fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("codex-mcp-client", env!("CARGO_PKG_VERSION")).with_title("Whisply"),
    )
...
async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(whisply_utils_path_uri::LegacyAppPathString::into_string);
...
mod tests {
    ...
    use whisply_protocol::mcp::MCP_APP_UI_EXTENSION_ID;
    use whisply_protocol::mcp::OPENAI_FORM_EXTENSION_ID;

    /// A bearer token is sent to whatever URL the server declared, so a server
    /// naming a Whisply variable here is asking for the product's own authority
    /// handles to be mailed to a third party.
    #[test]
    fn a_reserved_variable_is_refused_as_a_bearer_token() {
        let error = resolve_bearer_token("suspicious", Some("WHISPLY_GATEWAY_AUTH_FD"))
            .expect_err("a reserved variable must not become a bearer token");

        let message = error.to_string();
        assert!(
            message.contains("WHISPLY_GATEWAY_AUTH_FD"),
            "the refusal must name the variable so the server author can fix it: {message}"
        );
    }

    #[test]
    fn refusal_does_not_depend_on_the_variable_being_set() {
        // Refusing only when the value exists would make the rule depend on
        // machine state, so the same config would leak on one machine and not
        // another.
        unsafe { std::env::remove_var("WHISPLY_NEVER_SET_IN_TESTS") };

        assert!(resolve_bearer_token("suspicious", Some("WHISPLY_NEVER_SET_IN_TESTS")).is_err());
    }

    #[test]
    fn an_ordinary_bearer_variable_is_still_resolved() {
        unsafe { std::env::set_var("ORDINARY_MCP_BEARER_TOKEN", "token-value") };

        let token = resolve_bearer_token("ordinary", Some("ORDINARY_MCP_BEARER_TOKEN"))
            .expect("an ordinary variable should resolve");

        assert_eq!(token, Some("token-value".to_string()));
        unsafe { std::env::remove_var("ORDINARY_MCP_BEARER_TOKEN") };
    }
