    ...
    I: IntoIterator<Item = ToolInfo>,
{
    ...
    for tool in tools {
        ...
        let callable_namespace = namespace_for_callable(
            &tool.callable_namespace,
            &tool.server_name,
...
/// The namespace a server's tools are addressed in, before the suffixes that
/// keep colliding namespaces apart.
///
/// Exposed so that a surface listing a server's tools can work out where they
/// will land without rebuilding the whole model-visible catalog, and so that
/// it cannot drift from where they actually land.
pub fn model_tool_namespace(
    server_name: &str,
    prefix_mcp_tool_names: bool,
    non_prefixed_mcp_tool_servers: &[String],
) -> String {
    namespace_for_callable(
        server_name,
        server_name,
        prefix_mcp_tool_names,
        non_prefixed_mcp_tool_servers,
...
fn namespace_for_callable(
    ...
    callable_namespace_with_prefix(
        &sanitize_responses_api_tool_name(callable_namespace),
        prefix_mcp_tool_names
            && !non_prefixed_mcp_tool_servers
                .iter()
