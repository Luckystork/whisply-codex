...
client_request_definitions! {
    ...
    /// Registers a native-owner capability set for exactly one user message.
    WhisplyToolAdmissionRegister => "whisply/tool/admission/register" {
        params: v2::WhisplyToolAdmissionRegisterParams,
        serialization: thread_id(params.thread_id),
        response: v2::WhisplyToolAdmissionRegisterResponse,
    },

    /// Releases a completed one-turn native capability admission.
    WhisplyToolAdmissionFinish => "whisply/tool/admission/finish" {
        params: v2::WhisplyToolAdmissionFinishParams,
        serialization: thread_id(params.thread_id),
        response: v2::WhisplyToolAdmissionFinishResponse,
    },

    /// Reports a step of an admitted native call that is still running.
    WhisplyToolProgressReport => "whisply/tool/progress/report" {
        params: v2::WhisplyToolProgressReportParams,
        serialization: thread_id(params.thread_id),
        response: v2::WhisplyToolProgressReportResponse,
    },
    ...
    WhisplyModelCatalogRead => "whisply/modelCatalog/read" {
        params: v2::WhisplyModelCatalogReadParams,
    ...
    WhisplyPromptCompositionRead => "whisply/promptComposition/read" {
        ...
        serialization: None,
        response: v2::WhisplyPromptCompositionReadResponse,
    },
    WhisplyCompactionRead => "whisply/compaction/read" {
        params: v2::WhisplyCompactionReadParams,
        serialization: None,
        response: v2::WhisplyCompactionReadResponse,
    },
...
server_request_definitions! {
    ...
    /// Execute a model call that matched an owner-approved one-turn native
    /// capability admission. This deliberately has no caller-supplied
    /// execution envelope.
    WhisplyToolAdmittedExecute => "whisply/tool/admittedExecute" {
        params: v2::WhisplyToolAdmittedExecuteRequest,
        response: v2::WhisplyToolResult,
    },

    /// Best-effort cancellation for an owner-approved native capability call.
    WhisplyToolAdmittedCancel => "whisply/tool/admittedCancel" {
        params: v2::WhisplyToolAdmittedCancelRequest,
        response: v2::WhisplyToolAdmittedCancelAcknowledgement,
    },
...
/// Superset of [`whisply_file_search::FileMatch`]
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
...
mod tests {
    ...
    use whisply_protocol::account::PlanType;
    ...
    use whisply_protocol::protocol::CodexResponseHandoffMode;
    use whisply_protocol::protocol::ConversationTextRole;
    use whisply_protocol::protocol::RealtimeConversationVersion;
    use whisply_protocol::protocol::RealtimeOutputModality;
    ...
    fn jsonrpc_request_conversion_preserves_serde_enum_decoding() {
        let requests = [
            JSONRPCRequest {
                ...
                trace: Some(whisply_protocol::protocol::W3cTraceContext {
        ...
        let response = ClientResponse::ThreadStart {
            ...
            response: v2::ThreadStartResponse {
                ...
                instruction_sources: vec![
                    whisply_utils_path_uri::LegacyAppPathString::from_abs_path(&absolute_path(
                        "/tmp/AGENTS.md",
                    )),
                ],
    ...
    /// Whisply's Mac app is an ordinary non-experimental client, and it hands a
    /// continued conversation to the runtime through `turn/start`'s
    /// `additionalContext`. If an upstream merge restores the gate, every such
    /// send is rejected and old conversations start being answered from
    /// nothing, so the difference is asserted here rather than discovered in
    /// the product.
    #[test]
    fn handing_over_an_earlier_conversation_does_not_require_experimental_api() {
        let request = ClientRequest::TurnStart {
            ...
            params: v2::TurnStartParams {
                ...
                additional_context: Some(std::collections::HashMap::from([(
                    ...
                    v2::AdditionalContextEntry {
                        value: "Person: compare the lease offers".to_string(),
                        kind: v2::AdditionalContextKind::Application,
                    },
                )])),
                ..Default::default()
            },
        ...
        // Steering mid-turn is a different surface and stays as upstream has
        // it; the product does not use it to hand over history.
        let steer = ClientRequest::TurnSteer {
            request_id: RequestId::Integer(2),
            params: v2::TurnSteerParams {
                thread_id: "thr_123".to_string(),
        ...
        assert_eq!(
            crate::experimental_api::ExperimentalApi::experimental_reason(&steer),
            Some("turn/steer.additionalContext")
        );
    }
        ...
        let notification =
            ServerNotification::ThreadSettingsUpdated(v2::ThreadSettingsUpdatedNotification {
                ...
                thread_settings: v2::ThreadSettings {
                    ...
                    effort: None,
                    summary: None,
                    collaboration_mode: whisply_protocol::config_types::CollaborationMode {
