...
use tracing::warn;
use uuid::Uuid;
use whisply_extension_items::image_generation::ImageGenerationItem;
use whisply_protocol::items::parse_hook_prompt_message;
use whisply_protocol::models::MessagePhase;
use whisply_protocol::protocol::AgentReasoningEvent;
use whisply_protocol::protocol::AgentReasoningRawContentEvent;
...
use whisply_protocol::protocol::ContextCompactedEvent;
use whisply_protocol::protocol::DynamicToolCallResponseEvent;
use whisply_protocol::protocol::ErrorEvent;
use whisply_protocol::protocol::EventMsg;
use whisply_protocol::protocol::ExecCommandBeginEvent;
use whisply_protocol::protocol::ExecCommandEndEvent;
use whisply_protocol::protocol::GuardianAssessmentEvent;
use whisply_protocol::protocol::GuardianAssessmentStatus;
use whisply_protocol::protocol::ImageGenerationBeginEvent;
use whisply_protocol::protocol::ImageGenerationEndEvent;
use whisply_protocol::protocol::ItemCompletedEvent;
use whisply_protocol::protocol::ItemStartedEvent;
use whisply_protocol::protocol::McpToolCallBeginEvent;
use whisply_protocol::protocol::McpToolCallEndEvent;
use whisply_protocol::protocol::PatchApplyBeginEvent;
use whisply_protocol::protocol::PatchApplyEndEvent;
...
use whisply_protocol::protocol::ViewImageToolCallEvent;
use whisply_protocol::protocol::WebSearchBeginEvent;
use whisply_protocol::protocol::WebSearchEndEvent;
#[cfg(test)]
use whisply_protocol::review_format::REVIEW_FALLBACK_MESSAGE;
...
#[cfg(test)]
use whisply_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use whisply_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `whisply-rs/core/rollout/policy.rs`.
    ...
    fn handle_response_item(&mut self, item: &whisply_protocol::models::ResponseItem) {
        let whisply_protocol::models::ResponseItem::Message {
            role, content, id, ..
        } = item
        else {
            return;
    ...
    fn handle_materialized_item_lifecycle(
        &mut self,
        turn_id: &str,
        item: &whisply_protocol::items::TurnItem,
    ) {
        let is_review_mode_item = matches!(
            item,
            whisply_protocol::items::TurnItem::EnteredReviewMode(_)
                | whisply_protocol::items::TurnItem::ExitedReviewMode(_)
        );
        let should_upsert = match item {
            whisply_protocol::items::TurnItem::Plan(plan) => !plan.text.is_empty(),
            whisply_protocol::items::TurnItem::HookPrompt(_)
            | whisply_protocol::items::TurnItem::CommandExecution(_)
            | whisply_protocol::items::TurnItem::DynamicToolCall(_)
            | whisply_protocol::items::TurnItem::CollabAgentToolCall(_)
            | whisply_protocol::items::TurnItem::SubAgentActivity(_)
            | whisply_protocol::items::TurnItem::Extension(_)
            | whisply_protocol::items::TurnItem::EnteredReviewMode(_)
            | whisply_protocol::items::TurnItem::ExitedReviewMode(_) => true,
            whisply_protocol::items::TurnItem::UserMessage(_)
            | whisply_protocol::items::TurnItem::AgentMessage(_)
            | whisply_protocol::items::TurnItem::Reasoning(_)
            | whisply_protocol::items::TurnItem::WebSearch(_)
            | whisply_protocol::items::TurnItem::ImageView(_)
            | whisply_protocol::items::TurnItem::ImageGeneration(_)
            | whisply_protocol::items::TurnItem::FileChange(_)
            | whisply_protocol::items::TurnItem::McpToolCall(_)
            | whisply_protocol::items::TurnItem::ContextCompaction(_) => false,
        };
    ...
    fn handle_guardian_assessment(&mut self, payload: &GuardianAssessmentEvent) {
        let status = match payload.status {
            ...
            // Neither of these stopped anything: the underlying item records
            // what actually happened, so a separate review row would only
            // report a failure the user may have gone on to approve.
            GuardianAssessmentStatus::Approved | GuardianAssessmentStatus::AskUser => return,
        };
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &whisply_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &whisply_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &whisply_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &whisply_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &whisply_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_sub_agent_activity(
        &mut self,
        payload: &whisply_protocol::protocol::SubAgentActivityEvent,
    ) {
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &whisply_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &whisply_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &whisply_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(
        &mut self,
        payload: &whisply_protocol::protocol::CollabCloseEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &whisply_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &whisply_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(
        &mut self,
        payload: &whisply_protocol::protocol::EnteredReviewModeEvent,
    ) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &whisply_protocol::protocol::ExitedReviewModeEvent,
    ) {
...
fn convert_dynamic_tool_content_items(
    items: &[whisply_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            whisply_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text,
            } => DynamicToolCallOutputContentItem::InputText { text },
            whisply_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
            } => DynamicToolCallOutputContentItem::InputImage { image_url },
            whisply_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputAudio {
                audio_url,
...
mod tests {
    ...
    use uuid::Uuid;
    use whisply_extension_items::ExtensionItem as CoreExtensionItem;
    use whisply_extension_items::sleep::SleepItem as CoreSleepItem;
    use whisply_protocol::ThreadId;
    use whisply_protocol::dynamic_tools::DynamicToolCallOutputContentItem as CoreDynamicToolCallOutputContentItem;
    use whisply_protocol::items::CommandExecutionItem as CoreCommandExecutionItem;
    use whisply_protocol::items::CommandExecutionStatus as CoreCommandExecutionStatus;
    use whisply_protocol::items::EnteredReviewModeItem as CoreEnteredReviewModeItem;
    use whisply_protocol::items::ExitedReviewModeItem as CoreExitedReviewModeItem;
    use whisply_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use whisply_protocol::items::TurnItem as CoreTurnItem;
    use whisply_protocol::items::UserMessageItem as CoreUserMessageItem;
    use whisply_protocol::items::build_hook_prompt_message;
    use whisply_protocol::mcp::CallToolResult;
    use whisply_protocol::models::ImageDetail;
    use whisply_protocol::models::MessagePhase as CoreMessagePhase;
    use whisply_protocol::models::WebSearchAction as CoreWebSearchAction;
    use whisply_protocol::parse_command::ParsedCommand;
    use whisply_protocol::protocol::AgentMessageEvent;
    use whisply_protocol::protocol::AgentReasoningEvent;
    use whisply_protocol::protocol::AgentReasoningRawContentEvent;
    use whisply_protocol::protocol::ApplyPatchApprovalRequestEvent;
    ...
    use whisply_protocol::protocol::CompactedItem;
    use whisply_protocol::protocol::DynamicToolCallResponseEvent;
    use whisply_protocol::protocol::EnteredReviewModeEvent;
    use whisply_protocol::protocol::ExecCommandBeginEvent;
    use whisply_protocol::protocol::ExecCommandEndEvent;
    use whisply_protocol::protocol::ExecCommandSource;
    use whisply_protocol::protocol::ExitedReviewModeEvent;
    use whisply_protocol::protocol::ItemStartedEvent;
    use whisply_protocol::protocol::McpInvocation;
    use whisply_protocol::protocol::McpToolCallEndEvent;
    use whisply_protocol::protocol::PatchApplyBeginEvent;
    use whisply_protocol::protocol::ReviewTarget;
    use whisply_protocol::protocol::ThreadRolledBackEvent;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![whisply_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                whisply_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    whisply_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
    ...
    fn reconstructs_declined_guardian_command_item() {
        let events = vec![
            ...
            EventMsg::GuardianAssessment(GuardianAssessmentEvent {
                ...
                risk_level: Some(whisply_protocol::protocol::GuardianRiskLevel::High),
                user_authorization: Some(
                    whisply_protocol::protocol::GuardianUserAuthorization::Low,
                ),
                rationale: Some("Would delete user data.".into()),
                decision_source: Some(
                    whisply_protocol::protocol::GuardianAssessmentDecisionSource::Agent,
                ),
                action: serde_json::from_value(serde_json::json!({
                    "type": "command",
                    "source": "shell",
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(whisply_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(whisply_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: whisply_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(whisply_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                whisply_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                whisply_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn canonical_hook_prompt_completion_updates_turn_history() {
        let hook_prompt = CoreTurnItem::HookPrompt(whisply_protocol::items::HookPromptItem {
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(whisply_protocol::models::ResponseItem::Message {
                id: Some(whisply_protocol::ResponseItemId::with_suffix("msg", "1")),
                role: "user".into(),
                content: vec![whisply_protocol::models::ContentItem::InputText {
