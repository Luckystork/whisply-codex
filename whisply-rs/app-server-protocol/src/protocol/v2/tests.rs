...
use whisply_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use whisply_protocol::config_types::MultiAgentMode;
use whisply_protocol::items::AgentMessageContent;
use whisply_protocol::items::AgentMessageItem;
use whisply_protocol::items::CollabAgentTool as CoreCollabAgentTool;
use whisply_protocol::items::CollabAgentToolCallItem;
use whisply_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use whisply_protocol::items::CommandExecutionItem;
...
use whisply_protocol::items::FileChangeItem;
use whisply_protocol::items::ImageViewItem;
use whisply_protocol::items::McpToolCallItem;
use whisply_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use whisply_protocol::items::ReasoningItem;
...
use whisply_protocol::items::WebSearchItem as CoreWebSearchItem;
use whisply_protocol::mcp::CallToolResult;
use whisply_protocol::mcp::McpServerInfo;
use whisply_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use whisply_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use whisply_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use whisply_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use whisply_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use whisply_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use whisply_protocol::protocol::AgentStatus as CoreAgentStatus;
use whisply_protocol::protocol::AskForApproval as CoreAskForApproval;
use whisply_protocol::protocol::ConversationTextRole;
use whisply_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use whisply_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use whisply_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use whisply_protocol::protocol::SubAgentActivityKind as CoreSubAgentActivityKind;
use whisply_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use whisply_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: whisply_protocol::protocol::ThreadSource = source.clone().into();
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![whisply_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let sender_thread_id = whisply_protocol::ThreadId::default();
    let receiver_thread_id = whisply_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: whisply_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            whisply_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(whisply_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
#[test]
fn codex_error_info_maps_and_serializes_storage_full_in_camel_case() {
    let mapped = CodexErrorInfo::from(whisply_protocol::protocol::CodexErrorInfo::StorageFull);
    assert_eq!(mapped, CodexErrorInfo::StorageFull);
    assert_eq!(serde_json::to_value(mapped).unwrap(), json!("storageFull"));
}
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(whisply_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        ...
        collaboration_mode: Some(whisply_protocol::config_types::CollaborationMode {
            mode: whisply_protocol::config_types::ModeKind::Plan,
