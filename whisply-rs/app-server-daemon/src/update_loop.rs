...
pub(crate) fn reexec_managed_updater(managed_codex_bin: &std::path::Path) -> Result<()> {
    ...
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Whisply binary {}",
...
async fn install_latest_standalone(http: &RouteAwareClientPool) -> Result<()> {
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone Whisply updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone Whisply updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone Whisply updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone Whisply updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone Whisply updater exited with status {status}")
    }
}
...
async fn fetch_installer_script(http: &impl InstallerHttp) -> Result<Vec<u8>> {
    match http.get(INSTALL_URL).await? {
        ...
        InstallerResponse::Unsuccessful { status } => {
            anyhow::bail!("standalone Whisply updater request failed with status {status}")
        }
    }
}
...
impl InstallerHttp for RouteAwareClientPool {
    async fn get(&self, url: &str) -> Result<InstallerResponse> {
        let response = RouteAwareClientPool::get(self, url)
            .send()
            .await
            .context("failed to fetch standalone Whisply updater")?;
        ...
        let body = response
            .bytes()
            .await
            .context("failed to read standalone Whisply updater")?
            .to_vec();
