...
use whisply_app_server_transport::app_server_control_socket_path;
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "whisply app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
impl Daemon {
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by whisply app-server daemon"
            ));
        }
