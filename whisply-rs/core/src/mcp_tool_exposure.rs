...
use whisply_mcp::ToolInfo as McpToolInfo;
...
pub(crate) fn append_mcp_tools(
    ...
    config: &Config,
    apps_enabled: bool,
    mcp_server_catalog: &whisply_mcp::ResolvedMcpCatalog,
    ...
    for tool in non_app_tools.chain(app_tools) {
        ...
        match handler {
            Ok(handler) => {
                ...
                if registry.register_external_from(
                    Arc::new(handler),
                    tool_exposure,
                    Some(tool.server_name.as_str()),
                ) && fits_agent_budget
                {
