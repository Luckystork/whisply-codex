...
use whisply_api::AgentIdentityTelemetry;
use whisply_api::ApiError;
use whisply_api::AuthProvider;
use whisply_api::CompactClient as ApiCompactClient;
use whisply_api::CompactionInput as ApiCompactionInput;
use whisply_api::Compression;
use whisply_api::MemoriesClient as ApiMemoriesClient;
use whisply_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use whisply_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use whisply_api::Provider as ApiProvider;
use whisply_api::RawMemory as ApiRawMemory;
use whisply_api::RealtimeCallClient as ApiRealtimeCallClient;
use whisply_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use whisply_api::Reasoning;
use whisply_api::ReasoningContext;
use whisply_api::RequestTelemetry;
use whisply_api::ReqwestTransport;
use whisply_api::ResponseCreateWsRequest;
use whisply_api::ResponsesApiRequest;
use whisply_api::ResponsesClient as ApiResponsesClient;
use whisply_api::ResponsesOptions as ApiResponsesOptions;
use whisply_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use whisply_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use whisply_api::ResponsesWsRequest;
use whisply_api::SharedAuthProvider;
use whisply_api::SseTelemetry;
use whisply_api::StreamOptions;
use whisply_api::TransportError;
use whisply_api::WebsocketTelemetry;
use whisply_api::auth_header_telemetry;
use whisply_api::build_session_headers;
use whisply_api::create_text_param_for_request;
use whisply_api::response_create_client_metadata;
...
use whisply_login::RefreshTokenError;
use whisply_login::UnauthorizedRecovery;
use whisply_login::default_client::add_originator_header;
use whisply_login::default_client::create_client_for_route;
...
use whisply_protocol::config_types::Verbosity as VerbosityConfig;
...
use whisply_protocol::protocol::InternalSessionSource;
use whisply_protocol::protocol::SessionSource;
use whisply_protocol::protocol::W3cTraceContext;
use whisply_rollout_trace::CompactionTraceContext;
use whisply_rollout_trace::InferenceTraceAttempt;
use whisply_rollout_trace::InferenceTraceContext;
use whisply_tools::create_tools_json_for_responses_api;
use whisply_tools::create_tools_json_for_responses_lite;
use whisply_tools::create_tools_raw_json_for_responses_api;
...
use whisply_feedback::FeedbackRequestTags;
use whisply_feedback::emit_feedback_request_tags_with_auth_env;
use whisply_login::auth::AgentIdentityAuthPolicy;
use whisply_login::auth_env_telemetry::AuthEnvTelemetry;
use whisply_login::auth_env_telemetry::collect_auth_env_telemetry;
use whisply_model_provider::AgentIdentitySessionFallback;
use whisply_model_provider::ProviderAuthScope;
use whisply_model_provider::SharedModelProvider;
use whisply_model_provider::WHISPLY_GATEWAY_CONNECT_TIMEOUT;
use whisply_model_provider::create_model_provider;
use whisply_model_provider::is_whisply_provider;
#[cfg(test)]
use whisply_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
...
use whisply_response_debug_context::extract_response_debug_context;
use whisply_response_debug_context::extract_response_debug_context_from_api_error;
use whisply_response_debug_context::telemetry_api_error_message;
use whisply_response_debug_context::telemetry_transport_error_message;
...
impl ModelClient {
    ...
    fn build_responses_request(
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery: whisply_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: whisply_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &CodexResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth).connect(
                &self.http_client_factory,
                headers,
                whisply_login::default_client::default_headers(),
                /*turn_state*/ None,
...
fn map_response_stream(
    api_stream: whisply_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let whisply_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = whisply_api::ResponseStream {
        rx_event,
