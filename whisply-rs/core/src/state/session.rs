...
pub(crate) struct SessionState {
    ...
    pub(crate) pending_session_start_sources: VecDeque<whisply_hooks::SessionStartSource>,
...
impl SessionState {
    ...
    /// Returns a complete checkpoint before a compaction attempts to advance the window.
    ///
    /// A failed durable compaction must leave every part of the window state intact, including
    /// its retry flags and prefill baseline, so that freeing storage makes the same retry safe.
    pub(crate) fn auto_compact_window_checkpoint(&self) -> AutoCompactWindow {
        self.auto_compact_window
    }

    /// Restores a full compaction-window checkpoint after a failed durable compaction.
    pub(crate) fn restore_auto_compact_window_checkpoint(&mut self, checkpoint: AutoCompactWindow) {
        self.auto_compact_window = checkpoint;
    }
    ...
    pub(crate) fn queue_pending_session_start_source(
        &mut self,
        value: whisply_hooks::SessionStartSource,
    ) {
