...
use whisply_exec_server::ExecProcess;
use whisply_exec_server::ExecProcessEventReceiver;
use whisply_exec_server::ExecProcessFuture;
use whisply_exec_server::ProcessId;
use whisply_exec_server::ProcessSignal;
use whisply_exec_server::ReadResponse;
use whisply_exec_server::StartedExecProcess;
use whisply_exec_server::WriteResponse;
use whisply_exec_server::WriteStatus;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, whisply_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, whisply_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), whisply_exec_server::ExecServerError> {
...
async fn blocking_terminate_unified_process(
    ...
    Ok(Arc::new(
        UnifiedExecProcess::from_exec_server_started(StartedExecProcess {
            ...
            sandbox_type: Some(whisply_sandboxing::SandboxType::None),
        })
        .await?,
    ))
}
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = whisply_exec_server::Environment::default_for_tests();
