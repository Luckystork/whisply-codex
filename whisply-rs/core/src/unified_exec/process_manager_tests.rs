...
fn exec_env_policy_excludes_runtime_permission_profile() {
    ...
    assert_eq!(
        ...
        whisply_exec_server::ExecEnvPolicy {
...
fn exec_server_params_use_path_uri_and_env_policy_overlay_contract() {
    let cwd: whisply_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let permission_profile = whisply_protocol::models::PermissionProfile::Disabled;
    ...
    let mut request = ExecRequest {
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: whisply_exec_server::ExecEnvPolicy {
                inherit: whisply_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: whisply_sandboxing::SandboxType::None,
    ...
    let proxy_settings_mode = whisply_sandboxing::WindowsSandboxProxySettingsMode::Preserve;
    ...
    request.exec_server_sandbox = Some(
        whisply_exec_server::FileSystemSandboxContext::from_permission_profile(permission_profile),
    );
    ...
    assert_eq!(
        ...
        Some(whisply_sandboxing::WindowsSandboxProxySettingsMode::Preserve)
    );
...
async fn output_collection_stays_bounded_across_repeated_drains() {
    ...
    let output = OutputHandles {
        ...
        terminal_output_buffer: Arc::new(tokio::sync::Mutex::new(HeadTailBuffer::default())),
...
async fn network_denial_fallback_message_names_sandbox_network_proxy() {
    ...
    assert_eq!(
        message,
        "Network access was denied by the Whisply sandbox network proxy."
    );
}
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let request = ExecCommandRequest {
        ...
        shell_mode: whisply_tools::UnifiedExecShellMode::Direct,
        network: None,
        tty: true,
    ...
    let whisply_protocol::protocol::EventMsg::ItemCompleted(completed_event) = event.msg else {
        panic!("expected ItemCompleted event");
    };
    let whisply_protocol::items::TurnItem::CommandExecution(item) = completed_event.item else {
        panic!("expected CommandExecution item");
    };
    assert_eq!(item.id, "call-unified-denied");
    assert_eq!(
        item.status,
        whisply_protocol::items::CommandExecutionStatus::Failed
    );
...
async fn pruning_does_not_evict_live_process_while_exited_process_is_finalizing() {
    let exited_process = Arc::new(
        crate::unified_exec::process_tests::remote_process(
            whisply_exec_server::WriteStatus::Accepted,
            /*terminate_error*/ None,
            whisply_sandboxing::SandboxType::None,
        )
        .await,
    );
    exited_process
