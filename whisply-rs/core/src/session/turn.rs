...
use crate::collect_explicit_skill_mentions_reporting_unused;
...
use crate::tools::registry::DroppedTool;
use crate::tools::registry::ToolArgumentDiffConsumer;
use crate::tools::registry::ToolDropReason;
...
use tracing::warn;
use whisply_analytics::AppInvocation;
use whisply_analytics::CompactionPhase;
use whisply_analytics::CompactionReason;
use whisply_analytics::InvocationType;
use whisply_analytics::TurnResolvedConfigFact;
use whisply_analytics::build_track_events_context;
use whisply_async_utils::OrCancelExt;
use whisply_config::is_inside_runtime_home;
use whisply_connectors::AppToolPolicyEvaluator;
use whisply_core_plugins::RecommendedPluginCandidatesInput;
use whisply_core_skills::injection::InjectedHostSkillPrompts;
use whisply_extension_api::ExtensionData;
use whisply_extension_api::TurnInputContext;
use whisply_extension_api::TurnInputEnvironment;
use whisply_features::Feature;
use whisply_file_system::FindUpErrorPolicy;
...
use whisply_protocol::items::PlanItem;
...
use whisply_protocol::models::MessagePhase;
use whisply_protocol::models::ResponseInputItem;
use whisply_protocol::models::ResponseItem;
use whisply_protocol::protocol::AgentMessageContentDeltaEvent;
use whisply_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use whisply_protocol::protocol::PlanDeltaEvent;
use whisply_protocol::protocol::RawResponseCompletedEvent;
use whisply_protocol::protocol::ReasoningContentDeltaEvent;
use whisply_protocol::protocol::ReasoningRawContentDeltaEvent;
use whisply_protocol::protocol::SafetyBufferingEvent;
use whisply_protocol::protocol::TurnDiffEvent;
use whisply_protocol::protocol::WarningEvent;
use whisply_protocol::user_input::UserInput;
use whisply_skills::ExplicitSkillMentions;
...
use whisply_utils_stream_parser::AssistantTextChunk;
use whisply_utils_stream_parser::AssistantTextStreamParser;
use whisply_utils_stream_parser::ProposedPlanSegment;
use whisply_utils_stream_parser::extract_proposed_plan_text;
use whisply_utils_stream_parser::strip_citations;

const POST_SAMPLING_TOKEN_ESTIMATE_TARGET: &str = "whisply_core::post_sampling_token_estimate";
...
async fn turn_diff_display_roots(step_context: &StepContext) -> Vec<(String, PathUri)> {
    let codex_home = step_context.turn.config.codex_home.as_path();
    ...
    for turn_environment in step_context.environments.turn_environments() {
        ...
        // A `No directory` turn runs in the app's own home. Searching above it
        // finds whatever checkout contains that home, ordinarily the person's,
        // and would show this turn's diffs as paths inside a repository the
        // person never opened.
        let private_root = cwd
            .to_abs_path()
            .is_ok_and(|cwd| is_inside_runtime_home(&cwd, codex_home));
...
async fn required_mcp_servers_for_input(
    ...
    let connector_slug_counts = if turn_context.apps_enabled() && !mentions.plain_names.is_empty() {
        ...
        build_connector_slug_counts(
            &whisply_connectors::merge::merge_plugin_connectors_with_accessible(
                connector_ids,
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = whisply_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
                .map(|connector_id| connector_id.0.clone()),
    ...
    let ExplicitSkillMentions {
        selected: mentioned_skills,
        unused: unused_skill_mentions,
    } = collect_explicit_skill_mentions_reporting_unused(
        user_input,
        skills_outcome,
        &connector_slug_counts,
    );
    // Said before the turn runs, because after it the person is reading an
    // answer and has no reason to suspect the skill they asked for is missing
    // from it.
    for mention in &unused_skill_mentions {
        sess.send_event(
            turn_context,
            EventMsg::Warning(WarningEvent {
                message: mention.describe(),
            }),
        )
        .await;
    }
...
async fn capture_current_model_fallback_step_context(
    ...
    let uses_codex_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(whisply_login::AuthManager::current_auth_uses_codex_backend);
...
/// How many automatic shortenings in a row may leave the conversation over the
/// limit before the product stops asking.
///
/// One is allowed to be bad luck. The second says the conversation cannot be
/// shortened by asking, and every attempt after it is a paid request that ends
/// where it began.
const MAX_UNPRODUCTIVE_AUTO_COMPACTIONS: u32 = 2;

/// Runs one automatic shortening and records whether it achieved anything.
///
/// The turn loop compacts whenever the window is full and then re-checks the
/// window, so this is a loop whose exit depends entirely on compaction working.
/// Upstream says as much in a comment above the call: "as long as compaction
/// works well in getting us way below the token limit, we shouldn't worry
/// about being in an infinite loop." When it does not work, the loop spends the
/// person's money forever, so the assumption is measured here instead of
/// assumed.
async fn run_auto_compact(
    ...
    if sess.unproductive_auto_compactions() >= MAX_UNPRODUCTIVE_AUTO_COMPACTIONS {
        tracing::warn!(
            attempts = sess.unproductive_auto_compactions(),
            "refusing to shorten again; the previous attempts left the conversation over the limit"
        );
        let error = CodexErr::ContextWindowExceeded;
        // Said out loud, because the alternative is a turn that stops for no
        // stated reason. The pre-turn caller reports compaction failures to
        // extensions only, so the sentence has to come from here. It says what
        // is true and what to do about it: the room ran out, and shortening it
        // in place is not going to make any.
        sess.track_turn_codex_error(&step_context.turn, &error);
        sess.send_event(
            &step_context.turn,
    ...
    let status =
        super::context_window::context_window_token_status(sess.as_ref(), turn_context.as_ref())
            .await;
    sess.note_auto_compaction_outcome(status.token_limit_reached);
    Ok(())
}
...
async fn run_auto_compact_attempt(
    sess: &Arc<Session>,
    step_context: Arc<StepContext>,
    fallback_step_context: Option<Arc<StepContext>>,
    client_session: &mut ModelClientSession,
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = whisply_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<whisply_extension_api::ExtensionData>,
    ...
    loop {
        ...
        {
            whisply_protocol::models::bound_executed_tool_calls_for_prompt(&mut prompt_input);
        }
...
pub(crate) async fn built_tools(
    ...
    report_dropped_tools(sess, turn_context, &router).await;
    Ok(Arc::new(router))
}

/// Say which configured tools the model was not given, and why.
///
/// A dropped tool is still listed everywhere the person looks — it is a real
/// tool on a connected server — so without this the only symptom is a tool that
/// never answers, which reads as a broken server rather than a name that
/// something else already owns. Reported once per session per tool: the drop is
/// decided again on every turn, and repeating it each time would train people
/// to skip the line.
async fn report_dropped_tools(sess: &Session, turn_context: &TurnContext, router: &ToolRouter) {
    let unreported = {
        let mut reported = sess.services.reported_dropped_tools.lock().await;
        router
            .dropped_tools()
            .iter()
            .filter(|dropped| reported.insert(dropped.tool_name.to_string()))
            .cloned()
            .collect::<Vec<_>>()
    };
    for dropped in unreported {
        let message = dropped_tool_message(&dropped);
        warn!("{message}");
        sess.send_event(turn_context, EventMsg::Warning(WarningEvent { message }))
            .await;
    }
...
/// Each reason has its own remedy, and naming the wrong one sends someone to
/// edit a setting that has nothing to do with why their tool went missing.
fn dropped_tool_message(dropped: &DroppedTool) -> String {
    let tool_name = &dropped.tool_name;
    let source = match dropped.server_name.as_deref() {
        Some(server_name) => format!("MCP server `{server_name}`"),
        None => "a configured extension".to_string(),
    };
    let explanation = match dropped.reason {
        ToolDropReason::ReservedName => {
            "a built-in capability already answers to that name. Give the server its own tool \
             namespace -- remove it from `non_prefixed_mcp_tool_servers` -- or rename the tool on \
             the server."
        }
        ToolDropReason::ReservedNamespace => {
            "`whisply` is the namespace Whisply's own capabilities answer in, and a tool offered \
             there would be presented to the model as one of them. Rename the server, or let its \
             tools keep their `mcp__` prefix."
        }
        ToolDropReason::DuplicateName => {
            "another tool already answers to that name. Rename the tool on the server, or give \
             the server a name no other server shares."
        }
    };
    format!("`{tool_name}` from {source} is not available to the model, because {explanation}")
}
...
pub(super) fn agent_message_text(item: &whisply_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            whisply_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: whisply_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(whisply_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &whisply_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn try_run_sampling_request(
    ...
    let outcome: CodexResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(whisply_async_utils::CancelErr::Cancelled) => {
                ...
                if analytics_tool_call_ids.len() < MAX_ANALYTICS_TOOL_CALL_IDS_PER_RESPONSE {
                    let call_id = match &item {
                        ...
                        | ResponseItem::ImageGenerationCall { id, .. } => {
                            id.as_ref().map(whisply_protocol::ResponseItemId::as_str)
                        }
                        _ => None,
                    };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![whisply_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
            ...
            ResponseEvent::Completed {
                ...
                sess.services
                    ...
                    .track_code_mode_tool_call(
                        whisply_analytics::CodeModeToolCallFact::SamplingResponseCompleted {
                            thread_id: sess.thread_id.to_string(),
                            turn_id: turn_context.sub_id.clone(),
