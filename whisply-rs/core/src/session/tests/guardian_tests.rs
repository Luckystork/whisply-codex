...
use whisply_protocol::models::AdditionalPermissionProfile as PermissionProfile;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                whisply_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_command_additional_permissions_requests_past_policy_validation() {
    ...
    config
        .permissions
        .set_permission_profile(whisply_protocol::models::PermissionProfile::Disabled)
    ...
    let handler = crate::tools::handlers::ShellCommandHandler::from(
        whisply_tools::ShellCommandBackendConfig::Classic,
    );
    #[allow(deprecated)]
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: whisply_tools::ToolName::plain("shell_command"),
...
fn strict_auto_review_turn_grant_forces_guardian_for_shell_command_policy_skip() {
    crate::guardian::tests::block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(strict_auto_review_turn_grant_forces_guardian_for_shell_command_policy_skip_body())
    })
}

async fn strict_auto_review_turn_grant_forces_guardian_for_shell_command_policy_skip_body() {
    let server = start_mock_server().await;
    let guardian_request_log = mount_sse_once(
        &server,
        sse(vec![
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    ...
    let thread_store = Arc::new(whisply_thread_store::LocalThreadStore::new(
        whisply_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));
    ...
    let (session, io) = Session::spawn(SessionSpawnArgs {
        ...
        plugins_manager,
        mcp_manager,
        code_mode_session_provider: Arc::new(whisply_code_mode::DisabledCodeModeSessionProvider),
        extensions: whisply_extension_api::empty_extension_registry(),
