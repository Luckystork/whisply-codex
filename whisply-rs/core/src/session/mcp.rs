...
use serde_json::Map;
use whisply_exec_server::ExecutorCapabilityDiscoveryCache;
use whisply_exec_server::ExecutorCapabilityDiscoverySnapshot;
use whisply_exec_server::FileSystemSandboxContext;
use whisply_exec_server::MAX_SELECTED_CAPABILITY_ROOTS;
use whisply_exec_server::ResolvedSelectedCapabilityRoot;
use whisply_mcp::CODEX_APPS_MCP_SERVER_NAME;
use whisply_mcp::ElicitationReviewRequest;
use whisply_mcp::ElicitationReviewer;
use whisply_mcp::ElicitationReviewerHandle;
use whisply_mcp::MCP_TOOL_CODEX_APPS_META_KEY;
use whisply_protocol::capabilities::CapabilityRootLocation;
use whisply_protocol::capabilities::SelectedCapabilityRoot;
use whisply_protocol::config_types::ApprovalsReviewer;
use whisply_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_ELICITATION_APPROVAL_KIND_KEY;
use whisply_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_ELICITATION_APPROVAL_KIND_MCP_TOOL_CALL;
use whisply_protocol::mcp_approval_meta::APPROVAL_KIND_TOOL_SUGGESTION as MCP_ELICITATION_APPROVAL_KIND_TOOL_SUGGESTION;
use whisply_protocol::mcp_approval_meta::APPROVALS_REVIEWER_KEY as MCP_ELICITATION_APPROVALS_REVIEWER_KEY;
use whisply_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_ELICITATION_CONNECTOR_DESCRIPTION_KEY;
use whisply_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_ELICITATION_CONNECTOR_ID_KEY;
use whisply_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_ELICITATION_CONNECTOR_NAME_KEY;
use whisply_protocol::mcp_approval_meta::PERSIST_KEY as MCP_ELICITATION_PERSIST_KEY;
use whisply_protocol::mcp_approval_meta::REQUEST_TYPE_APPROVAL_REQUEST as MCP_ELICITATION_REQUEST_TYPE_APPROVAL_REQUEST;
use whisply_protocol::mcp_approval_meta::REQUEST_TYPE_KEY as MCP_ELICITATION_REQUEST_TYPE_KEY;
use whisply_protocol::mcp_approval_meta::STRICT_AUTO_REVIEW_KEY as MCP_ELICITATION_STRICT_AUTO_REVIEW_KEY;
use whisply_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_ELICITATION_TOOL_DESCRIPTION_KEY;
use whisply_protocol::mcp_approval_meta::TOOL_NAME_KEY as MCP_ELICITATION_TOOL_NAME_KEY;
use whisply_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_ELICITATION_TOOL_PARAMS_KEY;
use whisply_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_ELICITATION_TOOL_TITLE_KEY;
use whisply_rmcp_client::Elicitation;
...
impl Session {
    ...
    pub(crate) async fn runtime_mcp_servers(
        ...
        whisply_mcp::configured_mcp_servers(&self.runtime_mcp_config(config).await)
    }
    ...
    pub(crate) async fn hard_refresh_latest_codex_apps_tools(
        self: &Arc<Self>,
    ) -> anyhow::Result<Vec<whisply_mcp::ToolInfo>> {
        let _ = self;
    ...
    pub(crate) async fn mcp_runtime_for_step(
        ...
        Arc::new(whisply_mcp::McpBinding::empty(config))
    }
    ...
    pub(crate) fn mcp_elicitation_lifecycle(&self) -> whisply_mcp::ElicitationLifecycle {
        self.mcp_elicitation_lifecycle_handle
            .get_or_init(|| {
                let elicitations = self.services.elicitations.clone();
                whisply_mcp::ElicitationLifecycle::new(move || elicitations.register())
            })
            .clone()
    }
    ...
    pub async fn request_mcp_server_elicitation(
        ...
        if self.services.mcp_runtime.elicitations_auto_deny() {
            return McpServerElicitationOutcome {
                response: Some(ElicitationResponse {
                    action: whisply_rmcp_client::ElicitationAction::Accept,
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                whisply_protocol::mcp::RequestId::String(value.to_string())
            }
            rmcp::model::NumberOrString::Number(value) => {
                whisply_protocol::mcp::RequestId::Integer(value)
            }
        };
...
async fn review_guardian_mcp_elicitation(
    ...
    if matches!(
        ...
        let outcome = crate::guardian::review_approval_request_with_cancel(
            &session,
            &turn_context,
            ...
            crate::guardian::GuardianReviewOptions {
                ...
                approval_request_source: whisply_analytics::GuardianApprovalRequestSource::MainTurn,
                external_cancel: Some(cancellation_token),
            },
        )
        .await;

        // `None` hands the elicitation back to the normal flow, which asks the
        // person. A review that reached no verdict takes that route.
        let Some(decision) = mcp_guardian_decision(outcome) else {
            return Ok(None);
        };
    ...
    match approval_policy {
        AskForApproval::Never => {
            if whisply_mcp::mcp_permission_prompt_is_auto_approved(
                approval_policy,
                &mcp_config.permission_profile,
                whisply_mcp::McpPermissionPromptAutoApproveContext::default(),
            ) && matches!(
                &request.elicitation,
                Elicitation::Mcp(
    ...
    let outcome = crate::guardian::review_approval_request(
        &session,
        &turn_context,
        review_id.clone(),
        guardian_request,
...
/// The reviewed decision, or `None` when the review reached no verdict and the
/// elicitation should be put to the person instead.
fn mcp_guardian_decision(
    outcome: crate::guardian::GuardianApprovalOutcome,
) -> Option<ReviewDecision> {
    match outcome {
        crate::guardian::GuardianApprovalOutcome::Decided(decision) => Some(decision),
        crate::guardian::GuardianApprovalOutcome::AskUser { reason } => {
            tracing::info!(
                reason = ?reason,
                "automatic approval review could not decide; asking the user about the elicitation"
            );
            None
        }
    }
...
fn plugin_install_elicitation_telemetry_metadata(
    ...
    let whisply_protocol::approvals::ElicitationRequest::Form {
