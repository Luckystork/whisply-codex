...
use whisply_analytics::CompactionTrigger;
use whisply_analytics::ImagePreparationFact;
use whisply_analytics::ImagePreparationMetadata;
use whisply_analytics::SubAgentThreadStartedInput;
use whisply_analytics::TurnCodexErrorFact;
use whisply_async_utils::OrCancelExt;
use whisply_connectors::connector_runtime_context_key;
use whisply_exec_server::Environment;
use whisply_exec_server::EnvironmentManager;
use whisply_execpolicy::prefix_rule_migration;
use whisply_extension_api::ExtensionDataInit;
use whisply_extension_api::LoadedUserInstructions;
use whisply_extension_api::PromptFragment;
use whisply_extension_api::PromptSlot;
use whisply_extension_api::TurnContextContributionInput;
use whisply_features::FEATURES;
use whisply_features::Feature;
use whisply_features::unstable_features_warning_event;
use whisply_hooks::Hooks;
use whisply_hooks::HooksConfig;
use whisply_login::AuthManager;
use whisply_login::CodexAuth;
use whisply_login::auth_env_telemetry::collect_auth_env_telemetry;
use whisply_mcp::McpResourceClient;
use whisply_mcp::McpRuntime;
use whisply_mcp::McpRuntimeContext;
use whisply_mcp::McpRuntimeInput;
use whisply_models_manager::manager::RefreshStrategy;
use whisply_models_manager::manager::SharedModelsManager;
use whisply_network_proxy::NetworkProxy;
use whisply_network_proxy::NetworkProxyAuditMetadata;
use whisply_network_proxy::normalize_host;
use whisply_otel::current_span_trace_id;
use whisply_otel::current_span_w3c_trace_context;
use whisply_otel::set_parent_from_w3c_trace_context;
use whisply_protocol::SessionId;
use whisply_protocol::ThreadId;
use whisply_protocol::approvals::ElicitationRequest;
use whisply_protocol::approvals::ElicitationRequestEvent;
use whisply_protocol::approvals::ExecPolicyAmendment;
use whisply_protocol::approvals::NetworkPolicyAmendment;
use whisply_protocol::approvals::NetworkPolicyRuleAction;
...
use whisply_protocol::config_types::WebSearchMode;
use whisply_protocol::dynamic_tools::DynamicToolResponse;
use whisply_protocol::dynamic_tools::DynamicToolSpec;
use whisply_protocol::items::EnteredReviewModeItem;
use whisply_protocol::items::TurnItem;
use whisply_protocol::items::UserMessageItem;
use whisply_protocol::models::ActivePermissionProfile;
use whisply_protocol::models::AdditionalPermissionProfile;
...
use whisply_protocol::openai_models::ModelPreset;
...
use whisply_protocol::protocol::FileChange;
use whisply_protocol::protocol::HasLegacyEvent;
use whisply_protocol::protocol::HistoryPosition;
use whisply_protocol::protocol::InterAgentCommunication;
use whisply_protocol::protocol::ItemCompletedEvent;
use whisply_protocol::protocol::ItemStartedEvent;
use whisply_protocol::protocol::MULTI_AGENT_MODE_OPEN_TAG;
use whisply_protocol::protocol::MultiAgentVersion;
use whisply_protocol::protocol::RawResponseItemEvent;
...
use whisply_protocol::protocol::ThreadHistoryMode;
use whisply_protocol::protocol::ThreadSource;
use whisply_protocol::protocol::TurnAbortReason;
use whisply_protocol::protocol::TurnContextItem;
use whisply_protocol::protocol::TurnContextNetworkItem;
use whisply_protocol::protocol::TurnEnvironmentSelection;
use whisply_protocol::protocol::TurnEnvironmentSelections;
use whisply_protocol::protocol::W3cTraceContext;
use whisply_protocol::protocol::WorldStateItem;
...
use whisply_protocol::request_permissions::RequestPermissionsArgs;
use whisply_protocol::request_permissions::RequestPermissionsEvent;
use whisply_protocol::request_permissions::RequestPermissionsResponse;
...
use whisply_rmcp_client::ElicitationResponse;
use whisply_rollout::state_db;
use whisply_rollout_trace::AgentResultTracePayload;
use whisply_rollout_trace::ThreadStartedTraceMetadata;
use whisply_rollout_trace::ThreadTraceContext;
use whisply_sandboxing::policy_transforms::intersect_permission_profiles;
use whisply_shell_command::parse_command::parse_command;
use whisply_terminal_detection::user_agent;
use whisply_thread_store::CreateThreadParams;
use whisply_thread_store::LiveThread;
use whisply_thread_store::LiveThreadInitGuard;
use whisply_thread_store::LocalThreadStore;
use whisply_thread_store::ReadThreadParams;
use whisply_thread_store::ResumeThreadParams;
use whisply_thread_store::ThreadPersistenceMetadata;
use whisply_thread_store::ThreadStore;
use whisply_thread_store::ThreadStoreError;
use whisply_utils_audio::prepare_response_items as prepare_audio_response_items;
...
use whisply_model_provider::create_model_provider_with_managed_gateway;
use whisply_model_provider_info::ModelProviderInfo;
use whisply_protocol::error::CodexErr;
use whisply_protocol::error::CodexErrorDetails;
use whisply_protocol::error::Result as CodexResult;
#[cfg(test)]
use whisply_protocol::exec_output::StreamOutput;
...
use crate::shell;
use crate::state::AutoCompactWindow;
...
use whisply_core_plugins::PluginCommandAttribution;
use whisply_core_plugins::PluginsManager;
use whisply_core_plugins::RecommendedPluginCandidatesInput;
use whisply_git_utils::get_git_repo_root;
use whisply_mcp::McpConfig;
use whisply_mcp::effective_mcp_servers;
use whisply_otel::SessionTelemetry;
use whisply_otel::THREAD_STARTED_METRIC;
...
use whisply_protocol::config_types::WindowsSandboxLevel;
use whisply_protocol::mcp::ClientMcpExtensions;
use whisply_protocol::models::LocalImagePreparation;
use whisply_protocol::models::ResponseInputItem;
use whisply_protocol::models::ResponseItem;
use whisply_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use whisply_protocol::protocol::ApplyPatchApprovalRequestEvent;
...
use whisply_protocol::protocol::DeprecationNoticeEvent;
use whisply_protocol::protocol::ErrorEvent;
use whisply_protocol::protocol::Event;
use whisply_protocol::protocol::EventMsg;
use whisply_protocol::protocol::ExecApprovalRequestEvent;
use whisply_protocol::protocol::InitialHistory;
use whisply_protocol::protocol::ModelRerouteEvent;
use whisply_protocol::protocol::ModelRerouteReason;
use whisply_protocol::protocol::ModelVerification;
use whisply_protocol::protocol::ModelVerificationEvent;
use whisply_protocol::protocol::NetworkApprovalContext;
...
use whisply_protocol::protocol::RequestUserInputEvent;
use whisply_protocol::protocol::ReviewDecision;
use whisply_protocol::protocol::SandboxPolicy;
use whisply_protocol::protocol::SessionConfiguredEvent;
use whisply_protocol::protocol::SessionNetworkProxyRuntime;
use whisply_protocol::protocol::StreamErrorEvent;
use whisply_protocol::protocol::Submission;
use whisply_protocol::protocol::ThreadMemoryMode;
...
#[cfg(test)]
use whisply_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) code_mode_session_provider: Arc<dyn whisply_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<whisply_extension_api::ExtensionRegistry<crate::config::Config>>,
    ...
    pub(crate) windows_sandbox_proxy_settings_mode:
        whisply_sandboxing::WindowsSandboxProxySettingsMode,
}
...
impl Session {
    ...
    pub(crate) async fn spawn(args: SessionSpawnArgs) -> CodexResult<(Arc<Self>, SessionIo)> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if whisply_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> CodexResult<(Arc<Self>, SessionIo)> {
        let SessionSpawnArgs {
            ...
            first_party_tool_dispatcher,
            server_tool_dispatcher,
            environment_manager,
            skills_service,
            plugins_manager,
            mcp_manager,
        ...
        let refresh_strategy = if session_source.is_non_root_agent() {
            whisply_models_manager::manager::RefreshStrategy::Offline
        } else {
            whisply_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        let base_instructions = config
            ...
            .unwrap_or_else(|| {
                crate::prompt_layers::profile_base_instructions(&model_info, &config)
            });
...
impl Session {
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &whisply_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn whisply_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn whisply_network_proxy::BlockedRequestObserver>>,
    ...
    pub(crate) async fn auto_compact_window_checkpoint(&self) -> AutoCompactWindow {
        let state = self.state.lock().await;
        state.auto_compact_window_checkpoint()
    }

    pub(crate) async fn restore_auto_compact_window_checkpoint(
        &self,
        checkpoint: AutoCompactWindow,
    ) {
        let mut state = self.state.lock().await;
        state.restore_auto_compact_window_checkpoint(checkpoint);
    }
    ...
    /// Records the composition of a prompt this session is about to send.
    ///
    /// This observes only: it never edits the prompt and never fails the turn.
    /// A composition the contract refuses is dropped rather than reported, so a
    /// stale record can never be mistaken for the current turn's.
    pub(crate) fn record_prompt_composition(
        &self,
        prompt: &crate::client_common::Prompt,
        turn_context: &TurnContext,
    ) {
        // Follows the user's explicit directory choice. Every turn carries a
        // cwd — a no-directory thread runs in a private neutral workspace — so
        // presence alone would activate the workspace profile for everyone.
        let workspace_selected = turn_context.config.workspace_directory_selected;
        let composed =
            crate::prompt_layers::prompt_composition_diagnostics(prompt, workspace_selected).ok();
        if let Ok(mut slot) = self.last_prompt_composition.lock() {
            *slot = composed;
        }
    }

    /// The composition record for this session's most recent prompt.
    pub(crate) fn prompt_composition(&self) -> Option<codex_whisply::PromptCompositionDiagnostics> {
        self.last_prompt_composition
            .lock()
            .ok()
            .and_then(|slot| slot.clone())
    }

    /// Records how close this conversation is to being shortened.
    pub(crate) fn observe_compaction_budget(&self, budget: codex_whisply::CompactionBudget) {
        if let Ok(mut log) = self.compaction_log.lock() {
            log.observe_budget(budget);
        }
    }

    /// Records one shortening of this conversation as it finishes.
    pub(crate) fn observe_compaction(&self, observation: codex_whisply::CompactionObservation) {
        if let Ok(mut log) = self.compaction_log.lock() {
            log.observe_compaction(observation);
        }
    }

    /// How many automatic shortenings in a row have failed to get this
    /// conversation back under the limit that asked for one.
    pub(crate) fn unproductive_auto_compactions(&self) -> u32 {
        self.unproductive_auto_compactions
            .load(std::sync::atomic::Ordering::Relaxed)
    }

    /// Records what an automatic shortening achieved.
    ///
    /// `still_over_limit` is read after the shortened history is installed and
    /// re-measured, so it is the same question the turn loop is about to ask
    /// again. Answering yes twice means asking a third time will cost a
    /// request and change nothing.
    pub(crate) fn note_auto_compaction_outcome(&self, still_over_limit: bool) {
        if still_over_limit {
            self.unproductive_auto_compactions
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            self.reset_unproductive_auto_compactions();
        }
    }

    /// Forgets the run of unproductive shortenings.
    ///
    /// Called wherever the conversation is cut down by something other than
    /// asking the endpoint again -- a new window, in practice -- because after
    /// that the reason automatic shortening was refused no longer holds.
    pub(crate) fn reset_unproductive_auto_compactions(&self) {
        self.unproductive_auto_compactions
            .store(0, std::sync::atomic::Ordering::Relaxed);
    }

    /// What can be said about this conversation being shortened.
    pub(crate) fn compaction_status(&self) -> codex_whisply::ConversationCompactionStatus {
        self.compaction_log
            .lock()
            .map(|log| log.status())
            .unwrap_or_else(|_| codex_whisply::ConversationCompactionLog::new().status())
    }
    ...
    pub(crate) async fn user_instructions(
        &self,
    ) -> Option<whisply_extension_api::UserInstructions> {
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &whisply_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| whisply_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        let persist = match self.current_rollout_path().await {
            Ok(Some(path)) => whisply_rollout::existing_rollout_path(&path)
                .await
                .is_some(),
            Ok(None) => true,
            Err(err) => {
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        let Ok(native_environment_cwd) = environment.cwd.to_abs_path() else {
            warn!(
                cwd = %environment.cwd,
                "request_permissions requires a cwd native to the Whisply host"
            );
            return Some(RequestPermissionsResponse {
                permissions: RequestPermissionProfile::default(),
                scope: PermissionGrantScope::Turn,
        ...
        if crate::guardian::routes_approval_to_guardian(turn_context.as_ref()) {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                GuardianReviewOptions {
                    ...
                    approval_request_source:
                        whisply_analytics::GuardianApprovalRequestSource::MainTurn,
            ...
            let outcome = tokio::select! {
                ...
                outcome = review_rx => outcome.unwrap_or_else(|_| {
                    GuardianApprovalOutcome::review_worker_lost(
                        turn_context.config.stateless_auto_review,
                    )
                }),
            };
            // A review that reached no verdict has not answered the request,
            // so this falls through to the ordinary permission prompt below
            // instead of refusing permissions the person never saw.
            let decision = match outcome {
                GuardianApprovalOutcome::Decided(decision) => Some(decision),
                GuardianApprovalOutcome::AskUser { reason } => {
                    tracing::info!(
                        reason = ?reason,
                        "automatic approval review could not decide; asking the user for permissions"
                    );
                    None
                }
            };
    ...
    pub(crate) async fn capture_step_context_with_required_mcp_servers(
        ...
        let executor_capability_discovery = self
            ...
            .await;
        let extension_data = whisply_extension_api::ExtensionData::new(turn_context.sub_id.clone());
    ...
    /// Durably records then installs a shortened history.
    ///
    /// The durable append is the commit point. A full local volume must not let
    /// us replace the in-memory history with an unrecoverable compacted view:
    /// callers get a typed error and retain the original window so they can
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        let active_context_tokens_before = self.get_total_token_usage().await;
        let trigger = metadata.trigger;
        let mechanism = metadata.mechanism;
        let summary_text = metadata.message.clone();
        ...
        // the replacement history, reference context, and baseline in one rollout record: JSONL
        // writes individual records, so a multi-record checkpoint could otherwise be only partly
        // persisted when the local volume fills.
        let world_state_snapshot = world_state_baseline.as_ref().map(|world_state| {
        ...
        let rollout_items = vec![RolloutItem::Compacted(CompactedItem {
            ...
            world_state_baseline: world_state_snapshot
                .as_ref()
                .map(|(_, world_state_item)| world_state_item.clone()),
        ...
        let persisted = match self.persist_compacted_history_items(&rollout_items).await {
            Ok(persisted) => persisted,
            Err(error) => {
                self.observe_compaction(codex_whisply::CompactionObservation {
                    trigger: match trigger {
                        CompactionTrigger::Auto => codex_whisply::CompactionTriggerKind::Automatic,
                        CompactionTrigger::Manual => {
                            codex_whisply::CompactionTriggerKind::Requested
                        }
                    },
                    mechanism,
                    outcome: codex_whisply::CompactionOutcome::Failed,
                    active_context_tokens_before,
                    active_context_tokens_after: active_context_tokens_before,
                    summary_tokens: None,
        ...
        }
        {
            let mut state = self.state.lock().await;
            state.replace_history(items, reference_context_item);
            if let Some((snapshot, _)) = world_state_snapshot {
                state.history.set_world_state_baseline(snapshot);
            }
            state.queue_pending_session_start_source(whisply_hooks::SessionStartSource::Compact);
        }

        // Recorded here rather than in each caller: this is the moment the
        // shortened history exists and the moment its persistence is known.
        // A remote compaction summarizes on the server and holds no summary
        // text locally, which is why the digest is optional while the
        // mechanism still says the conversation was summarized.
        self.observe_compaction(codex_whisply::CompactionObservation {
            trigger: match trigger {
                CompactionTrigger::Auto => codex_whisply::CompactionTriggerKind::Automatic,
                CompactionTrigger::Manual => codex_whisply::CompactionTriggerKind::Requested,
            },
            mechanism,
            outcome: codex_whisply::CompactionOutcome::Completed,
            active_context_tokens_before,
            active_context_tokens_after: self.get_total_token_usage().await,
            summary_tokens: None,
            summary,
            persisted,
            recorded_at_unix_ms: SystemTime::now()
    ...
    async fn persist_compacted_history_items(&self, items: &[RolloutItem]) -> CodexResult<bool> {
        let Some(live_thread) = self.live_thread() else {
            return Ok(false);
        };
        live_thread.append_items(items).await.map_err(|error| match error {
            ThreadStoreError::StorageFull => CodexErr::StorageFull,
            error => {
                error!("failed to durably record compacted history: {error:#}");
                CodexErr::Fatal(
                    "Whisply could not save the compacted conversation. The original conversation is unchanged."
                        .to_string(),
                )
            }
        })?;
            ...
            && turn_context.model_context_window().is_some()
        {
            ...
            developer_sections.push(
                crate::context::TokenBudgetContext::new(
                    ...
                    session_source
                        .get_agent_path()
                        .unwrap_or_else(whisply_protocol::AgentPath::root),
                    turn_context
                        .config
                        .token_budget
                        .as_ref()
    ...
    #[tracing::instrument(level = "trace", skip_all, fields(item_count = items.len()))]
    /// Appends rollout items and reports whether they reached the thread.
    ///
    /// A failure is logged rather than propagated, which is the existing
    /// behaviour: losing a rollout append must not abort a turn a person is in
    /// the middle of. Returning the outcome lets a caller that has to be honest
    /// about persistence -- compaction -- say so without changing that.
    ...
    pub(crate) async fn start_new_context_window(
        ...
        let (checkpoint, window) = {
            let mut state = self.state.lock().await;
            let checkpoint = state.auto_compact_window_checkpoint();
            (checkpoint, state.start_new_context_window())
        };
        ...
        if let Err(error) = self
            .replace_compacted_history(
                ...
                CompactedHistoryMetadata {
                    ...
                    window_ids,
                    trigger,
                    mechanism: codex_whisply::CompactionMechanism::WindowReset,
                },
            )
            .await
        {
            self.restore_auto_compact_window_checkpoint(checkpoint)
                .await;
            return Err(error);
        }
        self.recompute_token_usage(turn_context).await;
        // A new window is the one shortening that cannot fail to shorten.
        self.reset_unproductive_auto_compactions();
        Ok(window_number)
    }
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<whisply_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
