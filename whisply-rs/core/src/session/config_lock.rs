...
use whisply_config::config_toml::OrchestratorFeatureToml;
use whisply_config::config_toml::OrchestratorToml;
...
use whisply_features::RolloutBudgetConfigToml;
use whisply_features::TokenBudgetConfigToml;
use whisply_features::ToolRegistryConfigToml;
...
mod tests {
    ...
    async fn lock_contains_prompts_and_materializes_features() {
        ...
        config.token_budget = Some(crate::config::TokenBudgetConfig {
            mode: whisply_features::TokenBudgetMode::Thread,
        ...
        for spec in whisply_features::FEATURES {
            assert_eq!(
        ...
        assert_eq!(
            ...
            Some(FeatureToml::Config(TokenBudgetConfigToml {
                enabled: Some(true),
                mode: Some(whisply_features::TokenBudgetMode::Thread),
                reminder_threshold_tokens: Some(16_000),
        ...
        assert_eq!(
            ...
            Some(FeatureToml::Config(CurrentTimeReminderConfigToml {
                ...
                clock_source: Some(whisply_features::CurrentTimeSource::System),
                delivery_mode: Some(
                    whisply_features::CurrentTimeReminderDeliveryMode::AnyInference
                ),
...
"#,
    ...
    async fn lock_validation_rejects_codex_version_mismatch_by_default() {
        ...
        let message = error.to_string();
        assert!(
            message.contains("config lock Whisply version mismatch"),
            "{message}"
        );
        assert!(
