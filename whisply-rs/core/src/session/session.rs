...
use crate::first_party_tools::PendingFirstPartyToolAdmissions;
...
pub(crate) struct Session {
    ...
    pub(super) mcp_elicitation_reviewer_handle: OnceLock<whisply_mcp::ElicitationReviewerHandle>,
    pub(super) mcp_elicitation_lifecycle_handle: OnceLock<whisply_mcp::ElicitationLifecycle>,
    ...
    /// The composition record for the most recent prompt this session sent.
    ///
    /// Recorded as an observation after the prompt is assembled, so a client
    /// can ask which prompt revision actually ran. It holds layer digests
    /// rather than prompt text, and is never read back into a turn.
    pub(crate) last_prompt_composition:
        std::sync::Mutex<Option<codex_whisply::PromptCompositionDiagnostics>>,
    /// What can be said about this conversation being shortened.
    ///
    /// Observed as compaction happens rather than reconstructed later, because
    /// the facts that matter -- what the window held before, whether the
    /// shortened history was written down -- are only true at that moment. It
    /// holds a digest of each summary and never the summary.
    pub(crate) compaction_log: std::sync::Mutex<codex_whisply::ConversationCompactionLog>,
    /// How many automatic shortenings in a row have left this conversation
    /// still over the limit that asked for one.
    ///
    /// Shortening is the answer to a full context window, and the turn loop
    /// re-checks the window and asks again. That is only a loop with an end if
    /// shortening actually shortens; a compaction that returns a history no
    /// smaller than the one it was given -- an endpoint having a bad day, a
    /// model that restates instead of summarizing -- puts the loop back
    ...
    /// One-turn native capability admissions waiting for a queued user
    /// submission. A turn consumes its admission exactly once.
    pub(crate) pending_first_party_tool_admissions: PendingFirstPartyToolAdmissions,
...
impl SessionConfiguration {
    ...
    pub(super) fn sandbox_policy(&self) -> SandboxPolicy {
        ...
        whisply_sandboxing::compatibility_sandbox_policy_for_permission_profile(
            &permission_profile,
            self.cwd(),
        )
    }
            ...
            if let Some(active_permission_profile) = next_configuration.active_permission_profile()
            {
                ...
                config.permissions.network = config
                    ...
                    .map_err(|err| ConstraintError::InvalidValue {
                        ...
                        requirement_source: whisply_config::RequirementSource::Unknown,
                    })?;
                config
                    .permissions
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        config: Arc<Config>,
        user_instructions: Option<whisply_extension_api::UserInstructions>,
        ...
        code_mode_session_provider: Arc<dyn whisply_code_mode::CodeModeSessionProvider>,
        ...
        extensions: Arc<whisply_extension_api::ExtensionRegistry<crate::config::Config>>,
        ...
        windows_sandbox_proxy_settings_mode: whisply_sandboxing::WindowsSandboxProxySettingsMode,
        ...
        thread_extension_init.insert(whisply_extension_api::ThreadOriginator(
            session_configuration.originator.clone(),
        ));
        let mcp_thread_init = thread_extension_init.clone();
        let thread_extension_data = whisply_extension_api::ExtensionData::new_with_init(
            thread_id.to_string(),
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let mcp_server_names =
                whisply_mcp::effective_mcp_servers(
            ...
            let session_extension_data =
                whisply_extension_api::ExtensionData::new(session_id.to_string());
            ...
            for contributor in extensions.thread_lifecycle_contributors() {
                contributor.on_thread_start(whisply_extension_api::ThreadStartInput {
            ...
            let sess = Arc::new(Session {
                ...
                compaction_log: std::sync::Mutex::new(
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Resumed(_) => whisply_hooks::SessionStartSource::Resume,
                InitialHistory::New | InitialHistory::Forked(_) => {
                    whisply_hooks::SessionStartSource::Startup
                }
                InitialHistory::Cleared => whisply_hooks::SessionStartSource::Clear,
            };
