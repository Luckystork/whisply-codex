...
use whisply_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
...
use whisply_models_manager::bundled_models_response;
use whisply_models_manager::model_info;
...
use whisply_protocol::AgentPath;
use whisply_protocol::ResponseItemId;
use whisply_protocol::SessionId;
use whisply_protocol::ThreadId;
use whisply_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use whisply_protocol::config_types::ServiceTier;
use whisply_protocol::config_types::TrustLevel;
use whisply_protocol::exec_output::ExecToolCallOutput;
use whisply_protocol::models::AgentMessageInputContent;
use whisply_protocol::models::FileSystemPermissions;
use whisply_protocol::models::FunctionCallOutputBody;
use whisply_protocol::models::FunctionCallOutputContentItem;
use whisply_protocol::models::FunctionCallOutputPayload;
use whisply_protocol::models::ImageDetail;
use whisply_protocol::models::PermissionProfile;
use whisply_protocol::models::SandboxEnforcement;
use whisply_protocol::openai_models::ModelServiceTier;
...
use whisply_protocol::protocol::NonSteerableTurnKind;
use whisply_protocol::protocol::SandboxPolicy;
use whisply_protocol::protocol::TurnEnvironmentSelections;
use whisply_protocol::request_permissions::PermissionGrantScope;
use whisply_protocol::request_permissions::RequestPermissionProfile;
...
use whisply_config::config_toml::ProjectConfig;
use whisply_config::permissions_toml::FilesystemPermissionToml;
use whisply_config::permissions_toml::FilesystemPermissionsToml;
use whisply_config::permissions_toml::NetworkToml;
use whisply_config::permissions_toml::PermissionProfileToml;
use whisply_config::permissions_toml::PermissionsToml;
use whisply_execpolicy::Decision;
use whisply_execpolicy::NetworkRuleProtocol;
use whisply_execpolicy::Policy;
use whisply_network_proxy::NetworkProxyConfig;
use whisply_otel::MetricsClient;
use whisply_otel::MetricsConfig;
use whisply_otel::TelemetryAuthMode;
...
use whisply_protocol::items::HookPromptFragment;
use whisply_protocol::items::build_hook_prompt_message;
use whisply_protocol::models::BaseInstructions;
use whisply_protocol::models::ContentItem;
use whisply_protocol::models::InternalChatMessageMetadataPassthrough;
use whisply_protocol::models::ResponseItem;
use whisply_protocol::protocol::AskForApproval;
use whisply_protocol::protocol::CodexErrorInfo;
use whisply_protocol::protocol::CompactedItem;
use whisply_protocol::protocol::ConversationAudioParams;
use whisply_protocol::protocol::CreditsSnapshot;
use whisply_protocol::protocol::GranularApprovalConfig;
use whisply_protocol::protocol::InitialHistory;
use whisply_protocol::protocol::InterAgentCommunication;
use whisply_protocol::protocol::MultiAgentVersion;
use whisply_protocol::protocol::NetworkApprovalProtocol;
use whisply_protocol::protocol::RateLimitSnapshot;
use whisply_protocol::protocol::RateLimitWindow;
use whisply_protocol::protocol::RealtimeAudioFrame;
use whisply_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use whisply_protocol::protocol::RealtimeVoice;
use whisply_protocol::protocol::RealtimeVoicesList;
use whisply_protocol::protocol::ResumedHistory;
use whisply_protocol::protocol::RolloutItem;
use whisply_protocol::protocol::SessionMeta;
use whisply_protocol::protocol::SessionMetaLine;
use whisply_protocol::protocol::Submission;
use whisply_protocol::protocol::ThreadRolledBackEvent;
use whisply_protocol::protocol::ThreadSettingsOverrides;
use whisply_protocol::protocol::TokenCountEvent;
use whisply_protocol::protocol::TokenUsage;
use whisply_protocol::protocol::TokenUsageInfo;
use whisply_protocol::protocol::TurnAbortedEvent;
use whisply_protocol::protocol::TurnCompleteEvent;
use whisply_protocol::protocol::TurnStartedEvent;
use whisply_protocol::protocol::UserMessageEvent;
use whisply_protocol::protocol::W3cTraceContext;
use whisply_rmcp_client::ElicitationAction;
...
use whisply_protocol::mcp::CallToolResult as McpCallToolResult;

pub(crate) fn mcp_config_for_test(config: &crate::config::Config) -> Arc<whisply_mcp::McpConfig> {
    Arc::new(config.to_mcp_config_with_loaded_plugins(
        &whisply_core_plugins::PluginLoadOutcome::default(),
        std::iter::empty(),
    ))
}
...
impl StepContext {
    pub(crate) fn for_test(turn: Arc<TurnContext>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            mcp: Arc::new(whisply_mcp::McpBinding::empty(mcp_config_for_test(
                &turn.config,
            ))),
...
async fn world_state_extension_metrics_follow_turn_model_switch() {
    ...
    impl whisply_extension_api::ContextContributor for WorldStateMetricsRecorder {
        fn contribute_world_state<'a>(
            &'a self,
            input: whisply_extension_api::WorldStateContributionInput<'a>,
        ) -> whisply_extension_api::ExtensionFuture<
            'a,
            Vec<whisply_extension_api::WorldStateSectionContribution>,
        > {
            Box::pin(async move {
                input
                    .extension_metrics
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        whisply_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        codex_home.join(whisply_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<whisply_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&whisply_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: whisply_hooks::StartHookTarget::SessionStart {
                source: whisply_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn whisply_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { whisply_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<TurnEnvironment> for ProbeToolRuntime {
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::Shell {
                ...
                environment_id: whisply_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> whisply_sandboxing::SandboxablePreference {
            whisply_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(whisply_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: whisply_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = whisply_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn session_has_skill(session: &Session, skill_name: &str) -> bool {
    ...
    session
        ...
        .snapshot_for_cwd(
            ...
            /*force_reload*/ true,
            Some(Arc::clone(&whisply_exec_server::LOCAL_FS)),
        )
        .await
        .outcome()
        .skills
...
async fn reload_user_config_layer_revokes_project_skill_roots() -> anyhow::Result<()> {
    ...
    assert!(
        reloaded_config
            ...
            .all(|layer| !matches!(
                layer.name,
                whisply_config::ConfigLayerSource::Project { .. }
            ))
    );
    assert!(!session_has_skill(&session, "project-skill").await);
    assert!(!session_has_skill(&session, "compatibility-skill").await);
...
-> anyhow::Result<()> {
    ...
    assert_eq!(
        config
            .config_layer_stack
            .trusted_project_root()
            .map(whisply_utils_absolute_path::AbsolutePathBuf::as_path),
        Some(workspace.path())
    );
    assert!(
        config
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&codex_home)?;
    let config_toml_path = codex_home.join(CONFIG_TOML_FILE);
    let user_config: whisply_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = whisply_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = whisply_hooks::list_hooks(whisply_hooks::HooksConfig {
        feature_enabled: true,
        ...
        ..whisply_hooks::HooksConfig::default()
    });
    assert_eq!(hook_list.hooks.len(), 1);
    assert_eq!(
        hook_list.hooks[0].trust_status,
        whisply_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: whisply_config::TomlValue =
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: whisply_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: whisply_config::MatcherGroup {
                matcher: None,
                hooks: vec![whisply_config::HookHandlerConfig::Command {
        ...
        let identity = whisply_config::TomlValue::try_from(identity)?;
        whisply_config::version_for_toml(&identity)
    };
...
async fn refresh_mcp_config_replaces_managed_server_and_plugin_requirements() {
    ...
    next_config.mcp_servers = whisply_config::Constrained::normalized(
...
async fn subagent_activity_emits_matching_start_and_completion() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let item = whisply_protocol::items::SubAgentActivityItem {
        id: "activity-1".to_string(),
        kind: whisply_protocol::protocol::SubAgentActivityKind::Started,
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl whisply_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a whisply_extension_api::ExtensionData,
            thread_store: &'a whisply_extension_api::ExtensionData,
            turn_store: &'a whisply_extension_api::ExtensionData,
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl whisply_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: whisply_extension_api::TurnStartInput<'a>,
        ) -> whisply_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl whisply_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: whisply_extension_api::TurnErrorInput<'a>,
        ) -> whisply_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl whisply_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &whisply_extension_api::ExtensionData,
            thread_store: &whisply_extension_api::ExtensionData,
...
async fn start_new_context_window_assigns_and_persists_item_ids() {
    ...
    session
        .start_new_context_window(
            &step_context,
            world_state,
            whisply_analytics::CompactionTrigger::Auto,
        )
        .await
        .expect("context window should persist before installation");
...
#[tokio::test]
async fn storage_full_during_context_window_reset_preserves_history_and_allows_retry() {
    let (mut session, turn_context, _rx) = make_session_and_context_with_auth_and_config_and_rx(
        CodexAuth::from_api_key("Test API Key"),
        Vec::new(),
        |_| {},
    ...
    let store = attach_in_memory_thread_store(
        Arc::get_mut(&mut session).expect("unique session while attaching persistence"),
    )
    .await;

    let original_history = vec![
        user_message("keep this user message when storage is full"),
        assistant_message("keep this assistant message when storage is full"),
    ];
    session
        .replace_history(original_history.clone(), None)
        .await;
    session
        .persist_rollout_response_items(&original_history)
        .await;
    ...
    let original_window_id = session.current_window_id().await;

    store.fail_next_append_with_storage_full_for_testing().await;
    let error = session
        .start_new_context_window(
            &step_context,
        ...
        .await
        .expect_err("a full volume must reject the compaction before live history changes");
    assert!(matches!(error.details(), CodexErrorDetails::StorageFull));
    assert_eq!(
        session.clone_history().await.raw_items(),
        original_history.as_slice()
    );
    assert_eq!(session.current_window_id().await, original_window_id);

    let persisted_after_failure = session
        .live_thread()
        .expect("in-memory live thread should be attached")
        .load_history(/*include_archived*/ true)
        .await
        .expect("read the original durable history");
    let persisted_response_items: Vec<_> = persisted_after_failure
        .items
        .iter()
        .filter_map(|item| match item {
        ...
        .collect();
    assert_eq!(persisted_response_items, original_history);
    assert!(
    ...
    session
        ...
        .await
        .expect("freeing storage should allow the same compaction to retry");
    assert_ne!(session.current_window_id().await, original_window_id);
    let persisted_after_retry = session
        .live_thread()
        .expect("in-memory live thread should remain attached")
        .load_history(/*include_archived*/ true)
        .await
        .expect("read the compacted durable history");
    let persisted_checkpoint = persisted_after_retry
        .items
        .iter()
        ...
        .expect("retry should write a compacted checkpoint");
    assert!(persisted_checkpoint.reference_context_item.is_some());
    assert!(
        persisted_checkpoint
            .world_state_baseline
            .as_ref()
            .is_some_and(|world_state| world_state.full)
    );
    let round_tripped_checkpoint: CompactedItem = serde_json::from_value(
        serde_json::to_value(&persisted_checkpoint).expect("serialize compacted checkpoint"),
    )
    .expect("deserialize compacted checkpoint");
    assert_eq!(round_tripped_checkpoint, persisted_checkpoint);
}
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: whisply_protocol::protocol::NetworkAccess::Restricted,
    };
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        config.permissions.approval_policy =
            whisply_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        whisply_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "whisply_core__codex_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            whisply_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            whisply_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            whisply_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn set_rate_limits_retains_previous_credits() {
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(whisply_protocol::account::PlanType::Plus),
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(whisply_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = whisply_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        whisply_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    ...
    std::fs::write(
        codex_home.path().join(whisply_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&whisply_exec_server::LOCAL_FS));
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(whisply_code_mode::DisabledCodeModeSessionProvider),
        /*first_party_tool_dispatcher*/ None,
        /*server_tool_dispatcher*/ None,
        Arc::new(whisply_extension_api::ExtensionRegistryBuilder::new().build()),
        whisply_extension_api::ExtensionDataInit::default(),
        ClientMcpExtensions::default(),
        AgentControl::default(),
        environment_manager,
        /*inherited_environments*/ None,
        /*analytics_events_client*/ None,
        Arc::new(whisply_thread_store::LocalThreadStore::new(
            whisply_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        whisply_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
        /*external_time_provider*/ None,
        Some(config.multi_agent_version_from_features()),
        GitEnrichmentPolicy::Fresh,
        whisply_sandboxing::WindowsSandboxProxySettingsMode::Reconcile,
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(whisply_mcp::McpRuntime::empty(
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: whisply_rollout_trace::ThreadTraceContext::disabled(),
        ...
        reported_dropped_tools: Mutex::new(HashSet::new()),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(whisply_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: whisply_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: whisply_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: whisply_extension_api::ExtensionDataInit::default(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(whisply_thread_store::LocalThreadStore::new(
            whisply_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
    ...
    let turn_context = Session::make_turn_context(
        ...
        "turn_id".to_string(),
        skills_snapshot,
        /*first_party_tool_admission*/ None,
    );
    let session = Session {
        thread_id,
        ...
        pending_first_party_tool_admissions: Default::default(),
...
async fn make_session_with_history_source_and_agent_control_and_rx(
    ...
    let session = Session::new(
        ...
        Arc::new(whisply_thread_store::LocalThreadStore::new(
            whisply_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            Some(
                whisply_state::StateRuntime::init(
                    config.sqlite.clone(),
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            whisply_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(whisply_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(whisply_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            whisply_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(whisply_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = whisply_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(whisply_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    &turn_context,
                    call_id,
                    whisply_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(whisply_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: whisply_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(whisply_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserInput {
            ...
            thread_settings: whisply_protocol::protocol::ThreadSettingsOverrides {
                ...
                approvals_reviewer: Some(whisply_config::types::ApprovalsReviewer::AutoReview),
    ...
    assert_eq!(
        ...
        whisply_config::types::ApprovalsReviewer::AutoReview
    );
    assert!(
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = whisply_utils_path_uri::PathUri::from_abs_path(&second_cwd);
    turn_context
        .environments
        .environments
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        whisply_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context =
        whisply_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(whisply_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn whisply_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        whisply_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl whisply_extension_api::ThreadLifecycleContributor<crate::config::Config>
        for ThreadStopRecorder
    {
        fn on_thread_stop<'a>(
            &'a self,
            input: whisply_extension_api::ThreadStopInput<'a>,
        ) -> whisply_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl whisply_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: whisply_extension_api::TurnAbortInput<'a>,
        ) -> whisply_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn refresh_mcp_servers_uses_latest_state_for_existing_turns() {
    ...
    .expect("parse refreshed MCP servers");
    {
        ...
        config.mcp_oauth_credentials_store_mode =
            whisply_config::types::OAuthCredentialsStoreMode::Auto;
        config
            .features
    ...
    let configured_servers = whisply_mcp::configured_mcp_servers(new_step.mcp.config());
    assert_eq!(
        configured_servers.get("refreshed"),
        refreshed_mcp_servers.get("refreshed")
    );
    assert!(
        !whisply_mcp::configured_mcp_servers(old_step.mcp.config()).contains_key("refreshed"),
        "an already-bound step must keep its captured config"
    );
    assert!(
        whisply_mcp::configured_mcp_servers(rematerialized_old.config()).contains_key("refreshed"),
    ...
    assert!(
        whisply_mcp::configured_mcp_servers(current.config()).contains_key("refreshed"),
...
async fn mcp_elicitation_reviewer_uses_latest_runtime_authority() {
    ...
    let request = whisply_mcp::ElicitationReviewRequest {
        server_name: "browser-use".to_string(),
        request_id: rmcp::model::NumberOrString::Number(7),
        elicitation: whisply_rmcp_client::Elicitation::Mcp(
...
async fn mcp_refresh_updates_plugin_auth_mode_before_checking_pending_state() {
    ...
    whisply_login::login_with_api_key(
        codex_home.path(),
        "new-api-key",
        whisply_login::AuthCredentialsStoreMode::File,
        whisply_login::AuthKeyringBackendKind::default(),
    )
...
impl whisply_exec_server::NoiseRendezvousConnectProvider for PendingNoiseConnectProvider {
    fn connect_bundle(
        &self,
        _: whisply_exec_server::NoiseChannelPublicKey,
    ) -> futures::future::BoxFuture<
        '_,
        Result<
            whisply_exec_server::NoiseRendezvousConnectBundle,
            whisply_exec_server::ExecServerError,
        >,
    > {
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    ...
    let selected_root =
        |environment_id: &str, path: &str| whisply_protocol::capabilities::SelectedCapabilityRoot {
            id: "shared-root".to_string(),
            location: whisply_protocol::capabilities::CapabilityRootLocation::Environment {
    ...
    for selected_root in &selected_roots {
        let whisply_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
        ...
        environment_manager
            .report_environment_provisioning_status(
                ...
                Ok(whisply_exec_server::EnvironmentReadyInfo {
...
async fn step_context_keeps_its_mcp_runtime_for_tools() -> anyhow::Result<()> {
    ...
    let next_step = session
        ...
        .await
        .expect("a fresh cancellation token cannot be cancelled");
    assert!(whisply_mcp::configured_mcp_servers(next_step.mcp.config()).contains_key("newer"));
    ...
    let current = session
        ...
        .await
        .expect("refreshed runtime should be available");
    assert!(whisply_mcp::configured_mcp_servers(current.config()).contains_key("newer"));
...
impl whisply_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        ...
        Box<
            dyn std::future::Future<Output = Vec<whisply_extension_api::PromptFragment>>
                + Send
                + 'a,
        >,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    whisply_extension_api::PromptFragment::developer_policy(
...
-> Arc<whisply_extension_api::ExtensionRegistry<crate::config::Config>> {
...
impl whisply_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: whisply_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<Output = Vec<whisply_extension_api::PromptFragment>>
                + Send
...
async fn build_initial_context_adds_multi_agent_v2_subagent_usage_hint_as_developer_message() {
    ...
    config.token_budget = Some(crate::config::TokenBudgetConfig {
        mode: whisply_features::TokenBudgetMode::Name,
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<whisply_thread_store::InMemoryThreadStore> {
    let store = Arc::new(whisply_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn whisply_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &whisply_thread_store::InMemoryThreadStore,
...
async fn guardian_auto_review_emits_thread_idle_after_interrupt() {
        ...
        for ThreadIdleRecorder
    {
        fn on_thread_idle<'a>(
            &'a self,
            _input: whisply_extension_api::ThreadIdleInput<'a>,
        ) -> whisply_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = whisply_protocol::user_input::TextElement::new(
        whisply_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
        ...
        for ThreadIdleRecorder
    {
        fn on_thread_idle<'a>(
            &'a self,
            input: whisply_extension_api::ThreadIdleInput<'a>,
        ) -> whisply_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(whisply_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use whisply_protocol::protocol::AskForApproval;
    use whisply_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: whisply_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: whisply_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let expected_source_path = whisply_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
