...
use uuid::Uuid;
use whisply_protocol::protocol::SessionContextWindow;
...
#[derive(Debug)]
enum WorldStateReplayItem<'a> {
    Compaction,
    WorldState(&'a WorldStateItem),
}
...
struct ActiveReplaySegment<'a> {
    ...
    world_state_replay: Vec<WorldStateReplayItem<'a>>,
...
fn finalize_active_segment<'a>(
    ...
    world_state_replay: &mut Vec<WorldStateReplayItem<'a>>,
...
impl Session {
    pub(super) async fn reconstruct_history_from_rollout(
        ...
        for (index, item) in rollout_items.iter().enumerate().rev() {
            match item {
                RolloutItem::Compacted(compacted) => {
                    ...
                    // An atomic compaction checkpoint carries its full baseline. Push the
                    // baseline before the reset while scanning backward so chronological replay
                    // still observes the reset first and then installs the new baseline.
                    if let Some(world_state_baseline) = compacted.world_state_baseline.as_ref() {
                        active_segment
                            .world_state_replay
                            .push(WorldStateReplayItem::WorldState(world_state_baseline));
                    }
                    active_segment
                        .world_state_replay
                        .push(WorldStateReplayItem::Compaction);
                    ...
                    if let Some(context) = compacted.reference_context_item.as_ref() {
                        ...
                        // Legacy checkpoints have no embedded reference context. Looking backward,
                        // they clear any older baseline unless a newer `TurnContextItem` in this
                        // same segment has already re-established it.
                ...
                RolloutItem::WorldState(_) => {
                    let active_segment =
                        active_segment.get_or_insert_with(ActiveReplaySegment::default);
                    let RolloutItem::WorldState(world_state) = item else {
                        unreachable!("world-state match arm received a different rollout item")
                    };
                    active_segment
                        .world_state_replay
                        .push(WorldStateReplayItem::WorldState(world_state));
                }
        ...
        for item in world_state_replay {
            match item {
                WorldStateReplayItem::Compaction => world_state_baseline = None,
                WorldStateReplayItem::WorldState(world_state) if world_state.full => {
                ...
                WorldStateReplayItem::WorldState(world_state) => {
