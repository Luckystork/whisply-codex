...
use whisply_analytics::TurnProfileFact;
use whisply_analytics::TurnTokenUsageFact;
use whisply_otel::SessionTelemetry;
use whisply_otel::TURN_E2E_DURATION_METRIC;
use whisply_otel::TURN_MEMORY_METRIC;
use whisply_otel::TURN_NETWORK_PROXY_METRIC;
use whisply_otel::TURN_TOKEN_USAGE_METRIC;
use whisply_otel::TURN_TOOL_CALL_METRIC;
...
impl Session {
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; Whisply will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) fn has_outstanding_durable_sleep(&self) -> bool {
        self.services
            ...
            .get::<whisply_extension_items::sleep::SleepItem>()
            .is_some()
    }
