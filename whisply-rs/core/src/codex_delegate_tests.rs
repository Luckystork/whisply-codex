...
use whisply_protocol::protocol::GuardianAssessmentAction;
...
use whisply_protocol::request_user_input::RequestUserInputEvent;
...
async fn dropped_approval_review_fails_closed() {
    ...
    assert_eq!(
        receive_approval_review(rx, /*stateless_auto_review*/ false)
            .await
            .expect_decided(),
...
#[tokio::test]
async fn dropped_approval_review_asks_the_user_under_whisply_auto() {
    let (tx, rx) = oneshot::channel();
    drop(tx);

    assert!(
        matches!(
            receive_approval_review(rx, /*stateless_auto_review*/ true).await,
            GuardianApprovalOutcome::AskUser { .. }
        ),
        "a review worker that never answered must not be reported as a refusal"
    );
}
