...
use whisply_protocol::items::is_safe_plugin_relative_path;
...
pub(crate) async fn run_codex_thread_interactive(
    ...
    let (session, io) = Box::pin(Session::spawn(SessionSpawnArgs {
        ...
        first_party_tool_dispatcher: parent_session.services.first_party_tool_dispatcher.clone(),
        server_tool_dispatcher: parent_session.services.server_tool_dispatcher.clone(),
        ...
        parent_rollout_thread_trace: whisply_rollout_trace::ThreadTraceContext::disabled(),
        parent_trace: None,
        environment_selections: parent_ctx.environments.to_selections(),
        thread_extension_init: whisply_extension_api::ExtensionDataInit::default(),
...
async fn handle_exec_approval(
    ...
    let reviewed = if routes_approval_to_guardian(parent_ctx) {
        ...
        delegated_guardian_decision(
            await_approval_with_cancel(
                receive_approval_review(review_rx, parent_ctx.config.stateless_auto_review),
                parent_session,
                &approval_id_for_op,
                cancel_token,
                Some(&review_cancel),
    ...
    // Either no review was owed, or the review reached no verdict; in both
    // cases the person is asked rather than answered for.
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                whisply_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                whisply_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                whisply_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    // Returning `None` is how this path says "guardian did not answer this
    // one", and the caller then asks the person.
    let decision = delegated_guardian_decision(
        await_approval_with_cancel(
            receive_approval_review(review_rx, parent_ctx.config.stateless_auto_review),
            parent_session,
            &event.call_id,
...
async fn receive_approval_review(
    review_rx: oneshot::Receiver<GuardianApprovalOutcome>,
    stateless_auto_review: bool,
) -> GuardianApprovalOutcome {
    review_rx
        .await
        .unwrap_or_else(|_| GuardianApprovalOutcome::review_worker_lost(stateless_auto_review))
}

/// The guardian decision to act on, or `None` when the review declined to
/// decide and the person should be asked instead.
fn delegated_guardian_decision(outcome: GuardianApprovalOutcome) -> Option<ReviewDecision> {
    match outcome {
        GuardianApprovalOutcome::Decided(decision) => Some(decision),
        GuardianApprovalOutcome::AskUser { reason } => {
            tracing::info!(
                reason = ?reason,
                "automatic approval review could not decide; asking the user"
            );
            None
        }
    }
...
async fn await_approval_with_cancel<F, T>(
    fut: F,
    parent_session: &Session,
    approval_id: &str,
    cancel_token: &CancellationToken,
...
where
    ...
    T: From<whisply_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(
                    approval_id,
                    whisply_protocol::protocol::ReviewDecision::Abort,
                )
                .await;
            T::from(whisply_protocol::protocol::ReviewDecision::Abort)
        }
        decision = fut => {
            decision
        }
