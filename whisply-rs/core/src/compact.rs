...
use futures::prelude::*;
use tracing::error;
use whisply_analytics::CodexCompactionEvent;
use whisply_analytics::CompactionImplementation;
use whisply_analytics::CompactionPhase;
use whisply_analytics::CompactionReason;
use whisply_analytics::CompactionStatus;
use whisply_analytics::CompactionStrategy;
use whisply_analytics::CompactionTrigger;
use whisply_analytics::now_unix_seconds;
...
use whisply_protocol::items::ContextCompactionItem;
...
pub use whisply_prompts::SUMMARIZATION_PROMPT;
pub use whisply_prompts::SUMMARY_PREFIX;
...
pub(crate) struct CompactedHistoryMetadata {
    ...
    ///
    /// Carried here so the single place that installs a shortened history can
    /// record an inspectable account of it, rather than each caller
    /// remembering to.
    pub(crate) trigger: CompactionTrigger,
    /// Whether the thread was summarized or simply given a fresh window.
    pub(crate) mechanism: codex_whisply::CompactionMechanism,
}
...
async fn run_compact_task_inner_impl(
    ...
    let window_checkpoint = sess.auto_compact_window_checkpoint().await;
    ...
    if let Err(error) = sess
        .replace_compacted_history(
            ...
            CompactedHistoryMetadata {
                ...
                window_ids,
                trigger,
                mechanism: codex_whisply::CompactionMechanism::Summarized,
            },
        )
        .await
    {
        sess.restore_auto_compact_window_checkpoint(window_checkpoint)
            .await;
        return Err(error);
    }
