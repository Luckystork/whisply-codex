...
use whisply_git_utils::ApplyGitRequest;
use whisply_git_utils::apply_git_patch;
...
async fn apply_verified_patch(root: &Path, patch: &str) -> AppliedPatchDelta {
    ...
    match whisply_apply_patch::maybe_parse_apply_patch_verified(
        &argv,
        &cwd,
        LOCAL_FS.as_ref(),
        /*sandbox*/ None,
    ...
    whisply_apply_patch::apply_patch(
        patch,
        &cwd,
        &mut stdout,
        &mut stderr,
