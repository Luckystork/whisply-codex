...
use whisply_git_utils::GitSha;

/// A runtime home that contains none of the directories these tests build, so
/// the runtime-home bound never fires and each case still exercises ordinary
/// project discovery.
const UNRELATED_RUNTIME_HOME: &str = "/nonexistent/whisply-runtime-home";
...
use whisply_protocol::protocol::GitInfo;
...
fn startup_context_blob_is_wrapped_in_tags_without_final_truncation() {
    let body = "Startup context from Whisply.\n## Current Thread\nhello";
    let wrapped = format_startup_context_blob(body);

    assert_eq!(
        wrapped,
        "<startup_context>\nStartup context from Whisply.\n## Current Thread\nhello\n</startup_context>"
    );
}
...
async fn workspace_section_includes_tree_when_entries_exist() {
    ...
    let section = build_workspace_section_with_user_root(
        &cwd.path().abs(),
        /*user_root*/ None,
        UNRELATED_RUNTIME_HOME.as_ref(),
    )
...
async fn recent_work_section_groups_threads_by_cwd() {
    ...
    let section = build_recent_work_section(
        &current_cwd.abs(),
        &recent_threads,
        UNRELATED_RUNTIME_HOME.as_ref(),
    )
...
/// WCD-712: the startup context is written into the model's prompt. A
/// `No directory` thread runs inside the app's own home, and the nearest
/// checkout above that home is ordinarily the person's own. Describing it as
/// this thread's workspace would put a listing of their home directory in
/// front of the model for a chat that deliberately opened no folder.
#[tokio::test]
async fn no_directory_does_not_describe_the_home_checkout_as_the_workspace() {
    let home = TempDir::new().expect("tempdir");
    fs::create_dir_all(home.path().join(".git")).expect("create home git dir");
    fs::write(home.path().join("taxes.md"), "private").expect("write home file");
    fs::create_dir_all(home.path().join("Secret Project")).expect("create home child");

    let codex_home = home.path().join(".whisply");
    let neutral = codex_home.join("neutral-workspace");
    fs::create_dir_all(&neutral).expect("create neutral workspace");
    ...
    assert!(
        !section.contains("taxes.md") && !section.contains("Secret Project"),
        "the home checkout leaked into a no-directory thread's context: {section}"
    );
}

/// The same bound must still describe a real project the person opened.
#[tokio::test]
async fn an_ordinary_project_is_still_described_as_the_workspace() {
    let root = TempDir::new().expect("tempdir");
    ...
    fs::create_dir_all(&codex_home).expect("create runtime home");
    ...
    .await
    .expect("a real project still has a workspace section");

    assert!(
        section.contains("README.md"),
        "bounding no-directory discovery must not hide a real project: {section}"
    );
}
