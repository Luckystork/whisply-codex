...
use whisply_exec_server::CopyOptions;
use whisply_exec_server::CreateDirectoryOptions;
use whisply_exec_server::Environment;
use whisply_exec_server::ExecutorFileSystemFuture;
use whisply_exec_server::FileMetadata;
use whisply_exec_server::FileSystemReadStream;
use whisply_exec_server::FileSystemSandboxContext;
use whisply_exec_server::LOCAL_FS;
use whisply_exec_server::ReadDirectoryEntry;
...
/// WCD-712: a `No directory` thread runs in a private root inside the Whisply
/// home so that no project is inferred for it. Home directories are commonly
/// git checkouts -- a dotfiles repository is the ordinary case -- and an
/// unbounded search for a project root finds that checkout, adopts the person's
/// entire home as this thread's project, and loads the `AGENTS.md` sitting in
/// it. The thread would then be following project instructions the person never
/// opened a project to get.
#[tokio::test]
async fn no_directory_does_not_load_agents_md_from_the_home_checkout() {
    let home = TempDir::new().unwrap();
    fs::create_dir_all(home.path().join(".git")).unwrap();
    fs::write(
        home.path().join(DEFAULT_AGENTS_MD_FILENAME),
        "instructions from the person's home checkout",
    )
    .unwrap();

    let whisply_home = home.path().join(".whisply");
    let neutral_workspace = whisply_home.join("neutral-workspace");
    fs::create_dir_all(&neutral_workspace).unwrap();

    let mut config = ConfigBuilder::default()
        .codex_home(whisply_home)
        .build()
        .await
        .expect("defaults for test should always succeed");
    config.cwd = neutral_workspace.abs();
    ...
    assert_eq!(
        agents_md_paths(&config).await.unwrap(),
        Vec::new(),
        "a no-directory thread must not inherit instructions from the home checkout above it"
    );
    assert_eq!(get_user_instructions(&config).await, None);
}

/// The same bound must leave an ordinary project alone: a real checkout outside
/// the Whisply home still contributes its `AGENTS.md` from a subdirectory.
#[tokio::test]
async fn an_ordinary_project_still_contributes_its_agents_md() {
    let root = TempDir::new().unwrap();
    let project = root.path().join("project");
    fs::create_dir_all(project.join(".git")).unwrap();
    fs::write(
        project.join(DEFAULT_AGENTS_MD_FILENAME),
        "instructions from the project the person opened",
    )
    .unwrap();
    let nested = project.join("crates").join("thing");
    fs::create_dir_all(&nested).unwrap();

    let whisply_home = root.path().join(".whisply");
    fs::create_dir_all(&whisply_home).unwrap();
    ...
    assert_eq!(
        get_user_instructions(&config).await.as_deref(),
        Some("instructions from the project the person opened"),
        "bounding no-directory discovery must not stop an opened project from being read"
    );
}
