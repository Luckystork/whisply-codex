...
use whisply_connectors::AppToolPolicy;
use whisply_connectors::AppToolPolicyEvaluator;
use whisply_connectors::AppToolPolicyInput;
use whisply_features::Feature;
use whisply_hooks::PermissionRequestDecision;
use whisply_mcp::CODEX_APPS_MCP_SERVER_NAME;
use whisply_mcp::MCP_TOOL_CODEX_APPS_META_KEY;
use whisply_mcp::McpPermissionPromptAutoApproveContext;
use whisply_mcp::PreparedMcpCall;
use whisply_mcp::SandboxState;
use whisply_mcp::auth_elicitation_completed_result;
use whisply_mcp::build_auth_elicitation_plan;
use whisply_mcp::mcp_permission_prompt_is_auto_approved;
use whisply_protocol::approvals::ElicitationRequest;
use whisply_protocol::items::McpToolCallError;
use whisply_protocol::items::McpToolCallItem;
use whisply_protocol::items::McpToolCallStatus;
use whisply_protocol::items::TurnItem;
use whisply_protocol::mcp::CallToolResult;
use whisply_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use whisply_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use whisply_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use whisply_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use whisply_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use whisply_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use whisply_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use whisply_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use whisply_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use whisply_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use whisply_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use whisply_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use whisply_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use whisply_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
use whisply_protocol::request_user_input::RequestUserInputAnswer;
...
use whisply_utils_output_truncation::TruncationPolicy;
use whisply_utils_output_truncation::truncate_text;
...
async fn maybe_request_codex_apps_auth_elicitation(
    ...
    let install_url = connector_id.map(|connector_id| {
        whisply_connectors::metadata::connector_install_url(
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                whisply_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(step_context: &StepContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == whisply_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, whisply_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
async fn maybe_request_mcp_tool_approval(
    ...
    ) && approvals_reviewer == ApprovalsReviewer::AutoReview
    {
        ...
        .await;
        // A review that reached no verdict falls through to the ordinary tool
        // approval prompt below rather than declining the call unasked.
        match outcome {
            GuardianApprovalOutcome::Decided(decision) => {
                return Some(mcp_tool_approval_decision_from_guardian(decision));
            }
            GuardianApprovalOutcome::AskUser { reason } => {
                tracing::info!(
                    reason = ?reason,
                    "automatic approval review could not decide; asking the user about the tool call"
                );
            }
        }
    }
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, whisply_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
    ...
    config
        ...
        .find_map(|layer| {
            ...
            let servers = layer
                ...
                .cloned()
                .and_then(|value| {
                    HashMap::<String, whisply_config::types::McpServerConfig>::deserialize(value)
                        .ok()
                })?;
