...
//!
//! The model-facing local-path schema is owned by `whisply-mcp` alongside MCP tool inventory, so this
...
use whisply_api::OPENAI_FILE_UPLOAD_LIMIT_BYTES;
use whisply_api::upload_openai_file;
...
async fn build_uploaded_argument_value(
    ...
    let upload_auth = whisply_model_provider::auth_provider_from_auth(auth);
