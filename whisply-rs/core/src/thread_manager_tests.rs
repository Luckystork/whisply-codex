...
use whisply_protocol::models::ReasoningItemReasoningSummary;
...
impl whisply_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: whisply_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> whisply_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<whisply_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> whisply_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<whisply_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> whisply_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
async fn mcp_invalidation_refreshes_threads_that_are_still_starting() {
    ...
    impl whisply_extension_api::ThreadLifecycleContributor<Config> for BlockingThreadStartup {
        fn on_thread_start<'a>(
            &'a self,
            _input: whisply_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl whisply_extension_api::McpServerContributor<Config> for BlockingThreadStartup {
        ...
        fn contribute<'a>(
            &'a self,
            _context: whisply_extension_api::McpServerContributionContext<'a, Config>,
        ) -> whisply_extension_api::ExtensionFuture<
            'a,
            Vec<whisply_extension_api::McpServerContribution>,
        > {
            Box::pin(async move {
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl whisply_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
    ...
    impl whisply_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        fn id(&self) -> &'static str {
            "selected_root_test"
        }
        ...
        fn contribute<'a>(
            &'a self,
            context: whisply_extension_api::McpServerContributionContext<'a, Config>,
        ) -> whisply_extension_api::ExtensionFuture<
            'a,
            Vec<whisply_extension_api::McpServerContribution>,
        > {
            Box::pin(async move {
                ...
                let server = whisply_config::McpServerConfig {
                    transport: whisply_config::McpServerTransportConfig::Stdio {
                ...
                vec![
                    whisply_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = whisply_extension_api::ExtensionDataInit::new();
    ...
    let selected_servers = |config: &whisply_mcp::McpConfig| {
        whisply_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    assert!(
        !whisply_mcp::configured_mcp_servers(&first_resolved.config)
            .contains_key(whisply_mcp::CODEX_APPS_MCP_SERVER_NAME),
