...
use whisply_otel::ToolDecisionSource;
...
fn guardian_cwd(environment_id: &str, cwd: PathUri) -> std::io::Result<AbsolutePathBuf> {
    match cwd.to_abs_path() {
        Ok(cwd) => Ok(cwd),
        Err(err) if environment_id != whisply_exec_server::LOCAL_ENVIRONMENT_ID => Err(err),
        Err(_) => {
        ...
        match reviewer {
            ...
            ApprovalReviewer::Guardian => {
                match self.request_guardian_approval(action.clone(), ctx).await {
                    GuardianApprovalOutcome::Decided(decision) => ApprovalResolution {
                        decision,
                        source: ApprovalResolutionSource::Guardian,
                    },
                    // The review reached no verdict, so nothing has been
                    // decided yet and the person is asked. The resolution is
                    // theirs, not the reviewer's, and is attributed that way.
    ...
    async fn request_guardian_approval(
        ...
        let action = match action.into_guardian_request() {
            ...
            Err(err) => {
                ...
                return GuardianApprovalOutcome::unreviewable_action(
                    ctx.turn.config.stateless_auto_review,
                );
            }
        };
