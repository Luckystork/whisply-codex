...
use whisply_model_provider_info::AMAZON_BEDROCK_GPT_5_5_MODEL_ID;
use whisply_model_provider_info::AMAZON_BEDROCK_GPT_5_6_LUNA_MODEL_ID;
use whisply_model_provider_info::AMAZON_BEDROCK_GPT_5_6_SOL_MODEL_ID;
...
use whisply_protocol::openai_models::ApplyPatchToolType;
...
fn zsh_fork_config_for_spec_plan_tests() -> whisply_tools::ZshForkConfig {
    let placeholder_exe = whisply_utils_absolute_path::AbsolutePathBuf::try_from(
...
impl ToolExecutor<ExtensionToolCall> for TestNamespaceExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(whisply_tools::ResponsesApiNamespace {
    ...
    fn handle(&self, _call: ExtensionToolCall) -> whisply_tools::ToolExecutorFuture<'_> {
        Box::pin(async {
            Ok(Box::new(whisply_tools::JsonToolOutput::new(json!({}))) as Box<dyn ToolOutput>)
        })
    }
}
...
impl ToolExecutor<ExtensionToolCall> for DeferredExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Function(ResponsesApiTool {
            ...
            strict: true,
            defer_loading: None,
            parameters: whisply_tools::JsonSchema::object(
                BTreeMap::from([(
                    "message".to_string(),
                    whisply_tools::JsonSchema::string(/*description*/ None),
                )]),
...
fn dynamic_tool(namespace: Option<&str>, name: &str, defer_loading: bool) -> DynamicToolSpec {
    let function = whisply_protocol::dynamic_tools::DynamicToolFunctionSpec {
        name: name.to_string(),
    ...
    match namespace {
        Some(namespace) => {
            DynamicToolSpec::Namespace(whisply_protocol::dynamic_tools::DynamicToolNamespaceSpec {
                name: namespace.to_string(),
                description: format!("{namespace} dynamic tools"),
                tools: vec![
                    whisply_protocol::dynamic_tools::DynamicToolNamespaceTool::Function(function),
                ],
            })
        }
...
async fn request_user_input_stays_direct_in_code_mode_only() {
    ...
    plan.assert_visible_contains(&[
        "request_user_input",
        whisply_code_mode::PUBLIC_TOOL_NAME,
        whisply_code_mode::WAIT_TOOL_NAME,
    ]);
    ...
    let ToolSpec::Freeform(exec) = plan.visible_spec(whisply_code_mode::PUBLIC_TOOL_NAME) else {
    ...
    for (tool_name, guardian) in [
        ...
        for allow_login_shell in [false, true] {
            let plan = probe(|turn| {
                ...
                if guardian {
                    turn.session_source = whisply_protocol::protocol::SessionSource::SubAgent(
                        whisply_protocol::protocol::SubAgentSource::Other(
...
async fn shell_command_is_not_registered_without_a_single_local_environment() {
    let remote_environment = probe(|turn| {
        ...
        environment.environment = Arc::new(
            whisply_exec_server::Environment::create_for_tests(Some(
...
async fn shell_zsh_fork_stays_standalone_until_unified_exec_composition_is_enabled() {
    ...
    if whisply_utils_pty::conpty_supported() {
...
async fn zsh_fork_unified_exec_hides_shell_parameter() {
    ...
    let plan = probe(|turn| {
        ...
        turn.unified_exec_shell_mode =
            whisply_tools::UnifiedExecShellMode::ZshFork(zsh_fork_config_for_spec_plan_tests());
    })
    .await;
...
async fn environment_tools_follow_the_step_context() {
    ...
    turn.environments.environments.clear();
    let turn = Arc::new(turn);
    let mcp = Arc::new(whisply_mcp::McpBinding::empty(mcp_config_for_test(
        &turn.config,
    )));
...
async fn sleep_tool_stays_direct_and_outside_code_mode() {
    for code_mode_only in [false, true] {
        ...
        let ToolSpec::Freeform(exec) = plan.visible_spec(whisply_code_mode::PUBLIC_TOOL_NAME)
        else {
            panic!("expected code mode exec tool");
        };
        if code_mode_only {
    ...
    let cases = [
        ...
        (
            ...
            ToolPlanInputs {
                ...
                dynamic_tools: vec![dynamic_tool(
                    /*namespace*/ None,
                    whisply_tools::TOOL_SEARCH_TOOL_NAME,
                    /*defer_loading*/ false,
                )],
                ..ToolPlanInputs::default()
            },
...
async fn code_mode_uses_the_first_normalized_tool_identity() {
    for (code_mode_only, winner_exposure, shadow_is_deferred) in [
        ...
        assert!(!code_mode_tool_names.contains_key(whisply_tools::TOOL_SEARCH_TOOL_NAME));
