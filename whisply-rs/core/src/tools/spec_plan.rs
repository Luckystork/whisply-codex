...
use crate::tools::handlers::ServerToolHandler;
...
use whisply_protocol::dynamic_tools::DynamicToolNamespaceTool;
...
use whisply_tools::TOOL_SEARCH_TOOL_NAME;
use whisply_tools::ToolCall as ExtensionToolCall;
use whisply_tools::ToolEnvironmentMode;
use whisply_tools::ToolExecutor;
use whisply_tools::ToolExposures;
use whisply_tools::ToolName;
use whisply_tools::ToolSearchInfo;
use whisply_tools::ToolSpec;
use whisply_tools::UnifiedExecShellMode;
use whisply_tools::can_request_original_image_detail;
use whisply_tools::collect_code_mode_exec_prompt_tool_definitions;
use whisply_tools::collect_request_plugin_install_entries;
use whisply_tools::default_namespace_description;
use whisply_tools::request_user_input_available_modes;
use whisply_tools::shell_command_backend_for_features;
use whisply_tools::shell_type_for_model_and_features;
...
pub(crate) fn build_tool_router(
    ...
    add_admitted_first_party_tools(session, turn_context, &mut registry);
    add_server_tools(session, &mut registry);
...
pub(crate) fn finalize_tool_router(
    ...
    if code_mode_enabled {
        for tool_name in [
            ToolName::plain(whisply_code_mode::PUBLIC_TOOL_NAME),
            ToolName::plain(whisply_code_mode::WAIT_TOOL_NAME),
        ] {
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && whisply_code_mode::is_code_mode_nested_tool(spec.name())
        && code_mode_tool_names
            .get(&whisply_code_mode::normalize_code_mode_identifier(
                &whisply_tools::code_mode_name_for_tool_name(tool_name),
            ))
            .is_some_and(|winner| winner == tool_name)
    {
        whisply_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure.is_available_in_code_mode()
        && whisply_code_mode::is_code_mode_nested_tool(
            &whisply_tools::code_mode_name_for_tool_name(tool_name),
        )
}
...
fn register_code_mode_executors(
    ...
    for tool in registry.entries() {
        ...
        if !whisply_code_mode::is_code_mode_nested_tool(&code_mode_name) {
            continue;
        }
        match code_mode_tool_names.entry(whisply_code_mode::normalize_code_mode_identifier(
            &code_mode_name,
        )) {
            Entry::Vacant(entry) => {
    ...
    let default_exec_yield_time_ms =
        default_exec_yield_time_override_ms(&turn_context.config.features)
            .unwrap_or(whisply_code_mode::DEFAULT_EXEC_YIELD_TIME_MS);
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, whisply_code_mode::ToolNamespaceDescription> {
    let mut namespace_descriptions = BTreeMap::new();
    for spec in specs {
        let ToolSpec::Namespace(namespace) = spec else {
            continue;
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| whisply_code_mode::ToolNamespaceDescription {
...
/// Registers only the native descriptors explicitly admitted for this turn.
/// A static registry descriptor or dispatcher alone is never sufficient: both
/// owner-issued admission and a live owner-bound dispatcher must be present.
fn add_admitted_first_party_tools(
    session: &Session,
    turn_context: &TurnContext,
    registry: &mut ToolRegistry,
) {
    let (Some(admission), Some(dispatcher)) = (
        turn_context.first_party_tool_admission.clone(),
        session.services.first_party_tool_dispatcher.clone(),
    ) else {
        return;
    };
    for handler in FirstPartyToolHandler::for_admission(admission, dispatcher) {
        registry.add_with_exposure(handler, ToolExposure::DirectModelOnly);
    }
}

fn add_server_tools(session: &Session, registry: &mut ToolRegistry) {
    let Some(dispatcher) = session.services.server_tool_dispatcher.clone() else {
        return;
    };
    for handler in ServerToolHandler::all(dispatcher) {
        registry.add_with_exposure(handler, ToolExposure::DirectModelOnly);
    }
}
...
fn compare_code_mode_tools(
    left: &whisply_code_mode::ToolDefinition,
    right: &whisply_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, whisply_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &whisply_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, whisply_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
