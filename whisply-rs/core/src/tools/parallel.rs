...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call_with_source(
        ...
        if self
            ...
            .config
            .features
            .enabled(whisply_features::Feature::ExecutedToolCallMetadata)
    ...
    fn failure_response(call: ToolCall, err: FunctionCallError) -> ResponseInputItem {
        ...
        match call.payload {
            ...
            ToolPayload::Custom { .. } => ResponseInputItem::CustomToolCallOutput {
                call_id: call.call_id,
                name: None,
                output: whisply_protocol::models::FunctionCallOutputPayload {
                    body: whisply_protocol::models::FunctionCallOutputBody::Text(message),
                    success: Some(false),
                },
            },
        ...
        macro_rules! log_tool_call {
            ($dispatch_duration_ms:expr, $handler_duration_ms:expr, $total_duration_ms:expr) => {
                info!(
                    ...
                    trace_id = %whisply_otel::current_span_trace_id().unwrap_or_default(),
...
mod tests {
    ...
    async fn cancellation_before_dispatch_admission_logs_dispatch_only_timing() -> anyhow::Result<()>
    {
        ...
        let tool_name = whisply_tools::ToolName::plain("test_tool");
    ...
    impl whisply_extension_api::ToolLifecycleContributor for FinishRecorder {
        fn on_tool_finish<'a>(
            &'a self,
    ...
    impl whisply_extension_api::ToolLifecycleContributor for BlockingFinishContributor {
        fn on_tool_finish<'a>(
            &'a self,
            input: whisply_extension_api::ToolFinishInput<'a>,
        ) -> whisply_extension_api::ToolLifecycleFuture<'a> {
    ...
    -> anyhow::Result<()> {
        ...
        let session = Arc::new(session);
        let turn_context = Arc::new(turn_context);
        let tool_name = whisply_tools::ToolName::plain("cleanup_tool");
