...
use whisply_network_proxy::BlockedRequest;
...
use whisply_network_proxy::NetworkPolicyRequest;
use whisply_network_proxy::NetworkProtocol;
use whisply_network_proxy::NetworkProxy;
use whisply_protocol::approvals::NetworkApprovalContext;
...
use whisply_sandboxing::record_network_sandbox_violation;
...
impl NetworkApprovalService {
    ...
    pub(crate) async fn handle_inline_policy_request(
        ...
        let reviewed_decision = if let Some(hook_approval_decision) = hook_approval_decision {
            Some(hook_approval_decision)
            ...
            let outcome = review.await.unwrap_or_else(|err| {
            ...
            match outcome {
                ...
                GuardianApprovalOutcome::AskUser { reason } => {
                    warn!(
                        reason = ?reason,
                        "automatic approval review could not decide; asking the user about network access"
                    );
                    None
                }
            }
        ...
        // Nothing has answered the request yet, so it goes to the person.
        let approval_decision = if let Some(reviewed_decision) = reviewed_decision {
            reviewed_decision
        } else {
...
pub(crate) async fn begin_network_approval(
    ...
    let execution_proxy = network
        ...
        .map_err(|err| {
            ToolError::Codex(whisply_protocol::error::CodexErr::Io(io::Error::other(
