...
use whisply_shell_command::parse_command::parse_shell_script;
use whisply_tools::ToolName;
use whisply_tools::ToolSpec;

pub(crate) type ToolTelemetryTags = Vec<(&'static str, String)>;

pub use whisply_tools::ToolExecutor;
pub use whisply_tools::ToolExposure;
...
/// Why a tool the person configured was not offered to the model.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum ToolDropReason {
    /// A built-in capability owns this name in the default namespace.
    ReservedName,
    /// The tool was offered inside the namespace the product answers in.
    ReservedNamespace,
    /// Something already registered answers to this name.
    DuplicateName,
}

/// A configured tool that will not be callable this turn.
///
/// Dropping one is the right call — two tools cannot share a name the model
/// addresses — but doing it silently leaves the tool listed as available and
/// never answering, which reads as a broken server rather than a name clash.
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct DroppedTool {
    pub(crate) tool_name: ToolName,
    pub(crate) reason: ToolDropReason,
    /// The MCP server that declared it, where the drop came from one.
...
impl ToolRegistry {
    ...
    pub(crate) fn register_external_with_exposure(
        ...
        self.register_external_from(runtime, exposure, /*server_name*/ None)
    }

    /// Register a tool that came from outside the product, remembering where it
    /// came from so a drop can be explained in the person's own terms.
    pub(crate) fn register_external_from(
        &mut self,
        runtime: Arc<dyn CoreToolRuntime>,
        exposure: ToolExposure,
        ...
        let tool_name = runtime.tool_name().with_default_namespace();
        // Same-named namespaces are coalesced into one group before the model
        // sees them, so a tool admitted here would be listed among the
        // capabilities the person approved, under the product's own
        // description of them. Refusing the whole namespace rather than the
        // five capability names keeps that true on turns where the real
        // capability was never admitted and the namespace is otherwise empty.
        if tool_name.is_whisply_namespace() {
            tracing::warn!(
                tool_name = %tool_name,
                "skipping external tool offered inside the Whisply namespace"
            );
            self.record_drop(
                tool_name.clone(),
        ...
        let reserved = crate::first_party_tools::is_reserved_model_tool_name(&tool_name.name);
        if tool_name.is_default_namespace() && reserved {
            tracing::warn!(tool_name = %tool_name, "skipping external tool with reserved name");
            self.record_drop(tool_name.clone(), ToolDropReason::ReservedName, server_name);
            if self.tools.contains_key(&tool_name) {
                self.record_collision(tool_name);
            }
            return false;
        ...
        match self.tools.entry(tool_name) {
            ...
            Entry::Occupied(entry) => {
                ...
                self.first_collision
                    .get_or_insert_with(|| tool_name.clone());
                self.record_drop(tool_name, ToolDropReason::DuplicateName, server_name);
                false
            }
        }
    }
    ...
    fn record_drop(
        &mut self,
        tool_name: ToolName,
        reason: ToolDropReason,
        server_name: Option<&str>,
    ) {
        let dropped = DroppedTool {
            tool_name,
            reason,
            server_name: server_name.map(str::to_string),
        };
        if !self.dropped.contains(&dropped) {
            self.dropped.push(dropped);
        }
    }

    pub(crate) fn dropped_tools(&self) -> &[DroppedTool] {
        &self.dropped
    }
