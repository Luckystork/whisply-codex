...
use whisply_tools::ConversationHistory;
use whisply_tools::ExtensionTurnItem;
use whisply_tools::ResponsesApiNamespaceTool;
use whisply_tools::ToolCall as ExtensionToolCall;
use whisply_tools::ToolEnvironment;
use whisply_tools::ToolName;
use whisply_tools::ToolSearchInfo;
use whisply_tools::ToolSpec;
use whisply_tools::TurnItemEmissionFuture;
use whisply_tools::TurnItemEmitter;
use whisply_utils_string::to_ascii_json_string;
...
pub(crate) struct ExtensionToolAdapter(Arc<dyn whisply_tools::ToolExecutor<ExtensionToolCall>>);

impl ExtensionToolAdapter {
    pub(crate) fn new(executor: Arc<dyn whisply_tools::ToolExecutor<ExtensionToolCall>>) -> Self {
        Self(executor)
    }
}
...
mod tests {
    ...
    use whisply_extension_items::web_search::WebSearchItem;
    use whisply_protocol::items::TurnItem;
    ...
    impl whisply_extension_api::ToolExecutor<whisply_tools::ToolCall> for StubExtensionExecutor {
        fn tool_name(&self) -> whisply_tools::ToolName {
            whisply_tools::ToolName::plain("extension_echo")
        }
        ...
        fn spec(&self) -> whisply_tools::ToolSpec {
            whisply_tools::ToolSpec::Function(whisply_tools::ResponsesApiTool {
                ...
                strict: true,
                parameters: whisply_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle(&self, _call: whisply_tools::ToolCall) -> whisply_tools::ToolExecutorFuture<'_> {
            Box::pin(async {
                Ok(
                    Box::new(whisply_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn whisply_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<whisply_tools::ToolCall>>>,
    }

    impl whisply_extension_api::ToolExecutor<whisply_tools::ToolCall> for CapturingExtensionExecutor {
        fn tool_name(&self) -> whisply_tools::ToolName {
            whisply_tools::ToolName::plain("extension_echo")
        }
        ...
        fn handle(&self, call: whisply_tools::ToolCall) -> whisply_tools::ToolExecutorFuture<'_> {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: whisply_tools::ToolCall,
        ) -> Result<Box<dyn whisply_tools::ToolOutput>, whisply_tools::FunctionCallError> {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: whisply_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = whisply_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        whisply_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        whisply_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {
