...
use whisply_protocol::protocol::FileSystemAccessMode;
use whisply_protocol::protocol::FileSystemPath;
use whisply_protocol::protocol::FileSystemSandboxEntry;
use whisply_protocol::protocol::FileSystemSandboxPolicy;
use whisply_protocol::protocol::InitialHistory;
use whisply_protocol::protocol::InterAgentCommunication;
use whisply_protocol::protocol::NetworkSandboxPolicy;
...
use whisply_state::DirectionalThreadSpawnEdgeStatus;
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    whisply_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn multi_agent_v2_spawn_fork_turns_all_rejects_agent_type_override() {
    ...
    let turn = TurnContext {
        ...
        multi_agent_version: whisply_protocol::protocol::MultiAgentVersion::V2,
        ..turn
    };
...
#[tokio::test]
async fn spawn_agent_rejects_explicit_model_without_managed_catalog() {
    let (session, turn) = make_session_and_context().await;
    let err = SpawnAgentHandler::default()
        .handle(invocation(
            Arc::new(session),
        ...
        .await
        .err()
        .expect("an explicit model must require a managed catalog");

    assert_eq!(
        err,
        FunctionCallError::RespondToModel(
            "Unknown model `gpt-5.4` for spawn_agent. Available models: ".to_string()
        )
    );
}

#[tokio::test]
async fn spawn_agent_service_tier_inheritance_clears_unavailable_tiers() {
    #[derive(Debug, Deserialize)]
    struct SpawnAgentResult {
        agent_id: String,
    }

    {
        ...
        let output = SpawnAgentHandler::default()
            ...
            .await
            .expect("spawn_agent should discard an unavailable parent service tier");
        let (content, _) = expect_text_output(output);
        let result: SpawnAgentResult =
            serde_json::from_str(&content).expect("spawn_agent result should be json");
        let snapshot = manager
...
"#,
        ...
        let output = SpawnAgentHandler::default()
            ...
            .await
            .expect("spawn_agent should discard an unavailable child role service tier");
        let (content, _) = expect_text_output(output);
        let result: SpawnAgentResult =
            serde_json::from_str(&content).expect("spawn_agent result should be json");
        let snapshot = manager
...
#[tokio::test]
async fn spawn_agent_role_service_tier_clears_unavailable_parent_tier() {
    #[derive(Debug, Deserialize)]
    struct SpawnAgentResult {
        agent_id: String,
    }
...
"#,
    ...
    let output = SpawnAgentHandler::default()
        ...
        .await
        .expect("spawn_agent should discard unavailable role and parent tiers");
    let (content, _) = expect_text_output(output);
    let result: SpawnAgentResult =
        serde_json::from_str(&content).expect("spawn_agent result should be json");
    let snapshot = manager
...
"#,
    ...
    assert_eq!(
        ...
        Some(FunctionCallError::RespondToModel(
            "Service tier `turbo` is not supported for model `gpt-5.4`. Supported service tiers: none"
                .to_string()
        ))
    );
}
...
#[tokio::test]
async fn spawn_agent_full_history_fork_rejects_unavailable_explicit_service_tier() {
    let (mut session, turn) = make_session_and_context().await;
    let turn = turn
        .with_model("gpt-5.4".to_string(), &session.services.models_manager)
        .await;
    ...
    let err = SpawnAgentHandler::default()
        ...
        .await
        .err()
        .expect("full-history fork must reject an unavailable explicit service tier");
    assert_eq!(
        err,
        FunctionCallError::RespondToModel(
            "Service tier `priority` is not supported for model `gpt-5.4`. Supported service tiers: none"
                .to_string()
        )
    );
}
...
#[tokio::test]
async fn multi_agent_v2_full_history_fork_rejects_unavailable_explicit_service_tier() {
    let (mut session, turn) = make_session_and_context().await;
    let mut turn = turn
        .with_model("gpt-5.4".to_string(), &session.services.models_manager)
        .await;
