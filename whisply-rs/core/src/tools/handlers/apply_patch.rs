...
use whisply_apply_patch::ApplyPatchAction;
use whisply_apply_patch::ApplyPatchFileChange;
use whisply_apply_patch::Hunk;
use whisply_apply_patch::StreamingPatchParser;
...
use whisply_protocol::protocol::PatchApplyUpdatedEvent;
use whisply_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use whisply_sandboxing::policy_transforms::merge_permission_profiles;
use whisply_sandboxing::policy_transforms::normalize_additional_permissions;
...
fn format_update_chunks_for_progress(chunks: &[whisply_apply_patch::UpdateFileChunk]) -> String {
...
fn write_permissions_for_paths(
    ...
    file_system_sandbox_policy: &whisply_protocol::permissions::FileSystemSandboxPolicy,
    cwd: &AbsolutePathBuf,
...
async fn effective_patch_permissions(
    ...
    whisply_protocol::permissions::FileSystemSandboxPolicy,
)> {
...
fn patch_permissions_without_path_matching(
    ...
    (
        ...
        whisply_protocol::permissions::FileSystemSandboxPolicy::unrestricted(),
    )
}
...
impl ApplyPatchHandler {
    async fn handle_call(
        ...
        let args = match whisply_apply_patch::parse_patch(&patch_input) {
            Ok(args) => args,
            Err(parse_error) => {
        ...
        match whisply_apply_patch::verify_apply_patch_args(
            args,
        ...
        .await
        {
            whisply_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            whisply_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            whisply_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            whisply_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match whisply_apply_patch::maybe_parse_apply_patch_verified(command, cwd, fs, Some(&sandbox))
        .await
    {
        whisply_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            let tool_ctx = ToolCtx {
        ...
        whisply_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
