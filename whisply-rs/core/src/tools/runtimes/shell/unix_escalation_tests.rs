...
use whisply_execpolicy::PolicyParser;
use whisply_execpolicy::RuleMatch;
use whisply_hooks::Hooks;
use whisply_hooks::HooksConfig;
use whisply_network_proxy::PROXY_ACTIVE_ENV_KEY;
use whisply_network_proxy::PROXY_ENV_KEYS;
...
use whisply_sandboxing::policy_transforms::effective_permission_profile;
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(whisply_execpolicy::Policy::empty())),
        ...
        stopwatch: whisply_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = whisply_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = whisply_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    let config_toml_path = turn_context
        .config
        .codex_home
        .join(whisply_config::CONFIG_TOML_FILE);
    let hook_list = whisply_hooks::list_hooks(HooksConfig {
    ...
    let expected_hook_command = whisply_shell_command::parse_command::shlex_join(&[
        "/usr/bin/touch".to_string(),
        target_str,
    ]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(whisply_execpolicy::Policy::empty())),
    ...
    let action = tokio::time::timeout(
        ...
        whisply_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        whisply_shell_escalation::EscalationDecision::Escalate(
            whisply_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, whisply_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        whisply_shell_escalation::EscalationDecision::Deny {
