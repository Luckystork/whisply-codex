...
use whisply_execpolicy::MatchOptions;
...
use whisply_protocol::error::SandboxErr;
...
use whisply_protocol::protocol::GuardianCommandSource;
use whisply_protocol::protocol::NetworkPolicyRuleAction;
use whisply_protocol::protocol::ReviewDecision;
use whisply_sandboxing::SandboxCommand;
use whisply_sandboxing::SandboxManager;
use whisply_sandboxing::SandboxTransformRequest;
use whisply_sandboxing::SandboxType;
use whisply_sandboxing::SandboxablePreference;
use whisply_sandboxing::record_filesystem_sandbox_violation;
use whisply_shell_command::bash::parse_shell_lc_plain_commands;
use whisply_shell_command::bash::parse_shell_lc_single_command_prefix;
use whisply_shell_escalation::EscalateServer;
use whisply_shell_escalation::EscalationDecision;
use whisply_shell_escalation::EscalationExecution;
use whisply_shell_escalation::EscalationPermissions;
use whisply_shell_escalation::EscalationPolicy;
use whisply_shell_escalation::EscalationPolicyFuture;
use whisply_shell_escalation::EscalationSession;
use whisply_shell_escalation::ExecParams;
use whisply_shell_escalation::ExecResult;
use whisply_shell_escalation::PreparedExec;
use whisply_shell_escalation::ResolvedPermissionProfile;
use whisply_shell_escalation::ShellCommandExecutor;
use whisply_shell_escalation::ShellCommandExecutorFuture;
use whisply_shell_escalation::Stopwatch;
        ...
        Ok(stopwatch
            .pause_for(async move {
                ...
                let permission_request = PermissionRequestPayload::bash(
                    whisply_shell_command::parse_command::shlex_join(&command),
                    /*description*/ None,
                );
                ...
                if let Some(review_id) = guardian_review_id {
                    ...
                    .await;
                    match outcome {
                        GuardianApprovalOutcome::Decided(decision) => return decision,
                        // Leave the decision to step 3 rather than refusing an
                        // escalation the reviewer never managed to judge.
                        GuardianApprovalOutcome::AskUser { reason } => {
                            tracing::info!(
                                reason = ?reason,
                                "automatic approval review could not decide; asking the user to escalate"
                            );
                        }
                    }
                }
...
struct CoreShellCommandExecutor {
    ...
    network: Option<whisply_network_proxy::NetworkProxy>,
