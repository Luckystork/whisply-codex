...
use whisply_thread_store::ListThreadsParams;
use whisply_thread_store::SortDirection;
use whisply_thread_store::StoredThread;
use whisply_thread_store::ThreadSortKey;
...
pub(crate) async fn build_realtime_startup_context(
    ...
    let recent_work_section =
        build_recent_work_section(&cwd, &recent_threads, codex_home.as_path()).await;
    let workspace_section =
        build_workspace_section_with_user_root(&cwd, home_dir(), codex_home.as_path()).await;
...
/// The repository a working directory belongs to, or nothing when it belongs
/// to the app instead.
///
/// A `No directory` thread runs in the app's own home, and the nearest checkout
/// above that root is ordinarily the person's home directory. Treating it as
/// this thread's repository would describe the thread to the model as work
/// inside a project the person never opened.
async fn owning_git_root(cwd: &AbsolutePathBuf, codex_home: &Path) -> Option<AbsolutePathBuf> {
    if is_inside_runtime_home(cwd, codex_home) {
        return None;
    }
    resolve_root_git_project_for_trust(LOCAL_FS.as_ref(), cwd).await
}
...
async fn build_recent_work_section(
    ...
    for entry in recent_threads {
        let group = match AbsolutePathBuf::from_absolute_path(entry.cwd.as_path()) {
            Ok(entry_cwd) => owning_git_root(&entry_cwd, codex_home)
                .await
    ...
    let current_group = owning_git_root(cwd, codex_home)
        .await
        .map(AbsolutePathBuf::into_path_buf)
    ...
    for (group, mut entries) in groups.into_iter().take(MAX_RECENT_WORK_GROUPS) {
        ...
        if let Some(section) =
            format_thread_group(&current_group, &group, entries, codex_home).await
        {
...
async fn build_workspace_section_with_user_root(
    ...
    let git_root = owning_git_root(cwd, codex_home).await;
...
async fn format_thread_group(
    ...
    let group_label =
        if let Ok(latest_cwd) = AbsolutePathBuf::from_absolute_path(latest.cwd.as_path()) {
            if owning_git_root(&latest_cwd, codex_home).await.is_some() {
