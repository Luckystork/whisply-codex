...
use whisply_config::default_project_root_markers;
use whisply_config::is_inside_runtime_home;
use whisply_config::merge_toml_values;
use whisply_config::project_root_markers_from_config;
...
async fn agents_md_paths(
    ...
    // A thread running inside the app's own home has no project above it, so
    // the search for one must not climb out into the person's home directory
    // and return their `AGENTS.md` as this thread's project instructions.
    let bounded_to_cwd = dir
        .to_abs_path()
        .is_ok_and(|dir| is_inside_runtime_home(&dir, config.codex_home.as_path()));
    let project_root = if bounded_to_cwd {
        Some(dir.clone())
    } else {
...
impl LoadedAgentsMd {
    ...
    /// compile `whisply-core` as a normal dependency without that configuration.
