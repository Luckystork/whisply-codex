...
use whisply_otel::sanitize_metric_tag_value;
...
pub fn sandbox_setup_is_complete(codex_home: &Path) -> bool {
    whisply_windows_sandbox::sandbox_setup_is_complete(codex_home)
}
...
pub fn run_elevated_setup(
    ...
    let permissions =
        whisply_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
            permission_profile,
            workspace_roots,
        )?;
    whisply_windows_sandbox::run_elevated_setup(
        whisply_windows_sandbox::SandboxSetupRequest {
        ...
        whisply_windows_sandbox::SetupRootOverrides::default(),
    )
}
...
) -> std::io::Result<whisply_windows_sandbox::WindowsSandboxProvisioningSettings> {
    let Some(network) = network.filter(|network| network.enabled()) else {
        return Ok(whisply_windows_sandbox::WindowsSandboxProvisioningSettings::default());
    };
    Ok(
        whisply_windows_sandbox::WindowsSandboxProvisioningSettings {
...
pub fn run_elevated_provisioning_setup(
    ...
    whisply_windows_sandbox::run_elevated_provisioning_setup(
        codex_home,
        real_user,
...
pub fn run_legacy_setup_preflight(
    ...
    whisply_windows_sandbox::run_windows_sandbox_legacy_preflight(
        permission_profile,
        workspace_roots,
        codex_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    whisply_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        permission_profile,
        workspace_roots,
        command_cwd,
        env_map,
...
fn emit_windows_sandbox_setup_success_metrics(
    ...
    let Some(metrics) = whisply_otel::global() else {
        return;
    };
...
fn emit_windows_sandbox_setup_failure_metrics(
    ...
    if matches!(mode, WindowsSandboxSetupMode::Elevated) {
        #[cfg(target_os = "windows")]
        {
            ...
            if let Some(failure) = whisply_windows_sandbox::extract_setup_failure(_err) {
                code_tag = Some(failure.code.as_str().to_string());
                message_tag = Some(whisply_windows_sandbox::sanitize_setup_metric_tag_value(
                    &failure.message,
                ));
            }
            ...
            let metric_name =
                if whisply_windows_sandbox::extract_setup_failure(_err).is_some_and(|failure| {
                    matches!(
                        failure.code,
                        whisply_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
                    )
                }) {
        ...
        let mut metric_name = String::from("codex.windows_sandbox.legacy_setup_preflight_");
