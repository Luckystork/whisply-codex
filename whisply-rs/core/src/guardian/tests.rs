...
use whisply_analytics::GuardianReviewFailureReason;
use whisply_config::ConfigLayerStack;
use whisply_config::FeatureRequirementsToml;
use whisply_config::NetworkConstraints;
use whisply_config::NetworkDomainPermissionToml;
use whisply_config::NetworkDomainPermissionsToml;
use whisply_config::RequirementSource;
use whisply_config::Sourced;
...
use whisply_model_provider::create_model_provider;
use whisply_model_provider_info::AMAZON_BEDROCK_GPT_5_4_MODEL_ID;
use whisply_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
use whisply_model_provider_info::ModelProviderInfo;
use whisply_model_provider_info::OPENAI_PROVIDER_ID;
use whisply_models_manager::manager::StaticModelsManager;
use whisply_network_proxy::NetworkProxyConfig;
use whisply_protocol::ThreadId;
use whisply_protocol::approvals::NetworkApprovalProtocol;
...
use whisply_protocol::permissions::FileSystemSandboxPolicy;
...
use whisply_protocol::protocol::GranularApprovalConfig;
use whisply_protocol::protocol::GuardianAssessmentStatus;
use whisply_protocol::protocol::GuardianRiskLevel;
use whisply_protocol::protocol::GuardianUserAuthorization;
use whisply_protocol::protocol::ReviewDecision;
use whisply_protocol::protocol::RolloutItem;
use whisply_protocol::protocol::TurnCompleteEvent;
...
// The memories extension depends on whisply-core, so this probe verifies the nested Guardian config
...
impl whisply_extension_api::ThreadLifecycleContributor<Config> for GuardianMemoryContextProbe {
    fn on_thread_start<'a>(
        &'a self,
        input: whisply_extension_api::ThreadStartInput<'a, Config>,
    ) -> whisply_extension_api::ExtensionFuture<'a, ()> {
        Box::pin(async move {
...
impl whisply_extension_api::ContextContributor for GuardianMemoryContextProbe {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a whisply_extension_api::ExtensionData,
        thread_store: &'a whisply_extension_api::ExtensionData,
    ) -> whisply_extension_api::ExtensionFuture<'a, Vec<whisply_extension_api::PromptFragment>>
    {
        Box::pin(async move {
            if thread_store
            ...
            {
                vec![whisply_extension_api::PromptFragment::developer_policy(
...
async fn guardian_test_session_turn_and_rx(
    ...
    let (mut session, mut turn, rx) =
        Box::pin(crate::session::tests::make_session_and_context_with_rx()).await;
...
/// Runs a guardian test body with the stack budget the product gives its own
/// review threads. Spawning a review session is deep enough to exhaust the
/// default one in debug builds, and the future is built inside the thread so
/// it is never materialized on the caller's stack.
pub(crate) fn block_on_guardian_review_test<T: Send + 'static>(
    paused_clock: bool,
    body: impl FnOnce() -> std::pin::Pin<Box<dyn Future<Output = T> + Send>> + Send + 'static,
) -> T {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            .expect("build guardian test runtime");
            runtime.block_on(body())
        })
        .expect("spawn guardian test thread")
        .join()
        .expect("guardian test thread panicked")
}

#[test]
fn a_review_that_never_happened_is_not_reported_as_a_refusal() {
    // Under Whisply's `Auto` these are questions for the person. Outside it,
    // upstream's fail-closed denial is preserved exactly.
    assert!(matches!(
        GuardianApprovalOutcome::unreviewable_action(/*stateless_auto_review*/ true),
        GuardianApprovalOutcome::AskUser {
            reason: whisply_analytics::GuardianReviewFailureReason::UnsupportedAction
        }
    ));
    assert!(matches!(
        GuardianApprovalOutcome::review_worker_lost(/*stateless_auto_review*/ true),
        GuardianApprovalOutcome::AskUser {
            reason: whisply_analytics::GuardianReviewFailureReason::SessionError
        }
    ));
    assert!(matches!(
        GuardianApprovalOutcome::unreviewable_action(/*stateless_auto_review*/ false)
            .expect_decided(),
        ReviewDecision::Denied { .. }
    ));
    assert!(matches!(
        GuardianApprovalOutcome::review_worker_lost(/*stateless_auto_review*/ false)
            .expect_decided(),
        ReviewDecision::Denied { .. }
    ));
}

/// A guardian session running under Whisply's `Auto` permission mode.
async fn guardian_session_for_stateless_auto_review(
    server: &wiremock::MockServer,
) -> (
    Arc<Session>,
    Arc<TurnContext>,
    ...
    config.stateless_auto_review = true;
    turn_mut.config = Arc::new(config);
    (session, turn, rx)
}

fn guardian_assessment_statuses(
    rx: async_channel::Receiver<Event>,
) -> Vec<GuardianAssessmentStatus> {
...
async fn guardian_test_session_and_turn_with_base_url(
    ...
    let (mut session, mut turn) = Box::pin(crate::session::tests::make_session_and_context()).await;
...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    let mut text = text;
    for canonical_path in ["/repo/whisply-rs/core", "/repo"] {
...
fn guardian_prompt_text(items: &[whisply_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            whisply_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
...
async fn build_guardian_prompt_full_mode_preserves_initial_review_format() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Whisply agent has requested the following action:\n"));
...
async fn build_guardian_prompt_truncates_oversized_approval_reason() -> anyhow::Result<()> {
    ...
    let expected_reason = whisply_utils_output_truncation::truncate_text(
        &approval_reason,
        whisply_utils_output_truncation::TruncationPolicy::Tokens(/*tokens*/ 512),
    );
    ...
    let reason_item = prompt
        ...
        .find_map(|item| match item {
            whisply_protocol::user_input::UserInput::Text { text, .. }
        ...
        &FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: whisply_protocol::permissions::FileSystemSpecialPath::Root,
                },
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: whisply_protocol::permissions::FileSystemSpecialPath::project_roots(
...
async fn build_guardian_prompt_delta_mode_preserves_original_numbering() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Whisply agent has requested the following next action:\n"));
...
async fn build_guardian_prompt_items_explains_network_access_review_scope() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "whisply_core__guardian__tests__network_access_guardian_prompt_layout",
...
async fn cancelled_guardian_review_emits_terminal_abort_without_warning() {
    ...
    assert_eq!(decision.expect_decided(), ReviewDecision::Abort);
...
async fn routes_approval_to_guardian_requires_guardian_reviewer() {
    let (_session, mut turn) = Box::pin(crate::session::tests::make_session_and_context()).await;
    let mut config = (*turn.config).clone();
...
async fn routes_approval_to_guardian_can_use_app_reviewer_override() {
    let (_session, turn) = Box::pin(crate::session::tests::make_session_and_context()).await;
...
fn guardian_output_schema_requires_only_outcome_and_allows_optional_details() {
    let schema = guardian_output_schema(/*stateless_auto_review*/ false);
...
#[test]
fn only_whisply_auto_offers_the_reviewer_an_uncertain_answer() {
    let upstream = guardian_output_schema(/*stateless_auto_review*/ false);
    let whisply_auto = guardian_output_schema(/*stateless_auto_review*/ true);

    assert_eq!(
        upstream["properties"]["outcome"]["enum"],
        serde_json::json!(["allow", "deny"]),
        "upstream has no one to defer to, so its reviewer must still choose"
    );
    assert_eq!(
        whisply_auto["properties"]["outcome"]["enum"],
        serde_json::json!(["allow", "deny", "uncertain"]),
    );

    let upstream_prompt = guardian_policy_prompt_with_config_and_template(
        BUNDLED_GUARDIAN_POLICY,
        BUNDLED_GUARDIAN_POLICY_TEMPLATE,
        /*stateless_auto_review*/ false,
    );
    let whisply_auto_prompt = guardian_policy_prompt_with_config_and_template(
        BUNDLED_GUARDIAN_POLICY,
        BUNDLED_GUARDIAN_POLICY_TEMPLATE,
        /*stateless_auto_review*/ true,
    );
    assert!(
        upstream_prompt.contains(r#""outcome": "allow" | "deny""#)
            && !upstream_prompt.contains(r#""uncertain""#),
        "the upstream contract must keep offering exactly two answers"
    );
    assert!(
        whisply_auto_prompt.contains(r#""outcome": "allow" | "deny" | "uncertain""#)
    ...
    assert!(
        whisply_auto_prompt.contains("lean conservative")
            && whisply_auto_prompt
                .contains(r#"This replaces any earlier instruction to lean conservative"#),
        "the uncertain contract must resolve the conflicting template guidance"
    );
}

#[test]
fn parse_guardian_assessment_claims_no_verdict_for_an_uncertain_reviewer() {
    let parsed =
        parse_guardian_assessment(Some(r#"{"outcome":"uncertain"}"#)).expect("guardian assessment");
    ...
    assert_eq!(
        ...
        GuardianAssessment {
            risk_level: GuardianRiskLevel::Medium,
            user_authorization: GuardianUserAuthorization::Unknown,
            outcome: GuardianAssessmentOutcome::Uncertain,
            rationale: "Auto-review could not tell whether this is safe.".to_string(),
        }
    );
}

#[test]
fn every_review_records_which_policy_decided_it() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(every_review_records_which_policy_decided_it_body())
    })
}

async fn every_review_records_which_policy_decided_it_body() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));

    let workspace_policy = "Deny anything that touches the production deploy keys.";
    let bundled = guardian_review_policy_revision_for(None).await?;
    let bundled_again = guardian_review_policy_revision_for(None).await?;
    let managed = guardian_review_policy_revision_for(Some(workspace_policy)).await?;

    let (Some(bundled), Some(bundled_again), Some(managed)) =
        (bundled, bundled_again, managed.clone())
    else {
        panic!("every review should record the policy it was decided under");
    };
    assert_eq!(
        bundled, bundled_again,
        "the same policy must keep the same revision, or the revision means nothing"
    );
    assert_ne!(
        bundled, managed,
        "a verdict reached under a workspace-managed policy must be distinguishable from one reached under the default"
    );
    assert!(
        !managed.contains(workspace_policy) && managed.starts_with("sha256:"),
        "the revision must identify the policy without carrying it: {managed}"
    );

    Ok(())
}

async fn guardian_review_policy_revision_for(
    ...
    mount_sse_once(
        &server,
        sse(vec![
            ev_response_created("resp-policy-revision"),
            ev_assistant_message("msg-policy-revision", r#"{"outcome":"allow"}"#),
            ev_completed("resp-policy-revision"),
        ]),
    )
    .await;
    ...
    if let Some(policy) = workspace_policy {
        ...
        Arc::make_mut(&mut turn_mut.config).guardian_policy_config = Some(policy.to_string());
    }
    seed_guardian_parent_history(&session, &turn).await;
    ...
    let (outcome, analytics_result) = run_guardian_review_session_for_test(
        Arc::clone(&session),
        turn,
        guardian_shell_request("shell-policy-revision"),
        ApprovalRequestReasons::default(),
        guardian_output_schema(/*stateless_auto_review*/ false),
    ...
    Ok(analytics_result.guardian_policy_revision)
}
...
async fn guardian_request_model_for_auto_review(
    ...
    String,
    String,
    whisply_analytics::GuardianReviewAnalyticsResult,
)> {
    let server = start_mock_server().await;
...
#[test]
fn guardian_review_uses_model_catalog_override_when_preferred_review_model_exists()
-> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(
            guardian_review_uses_model_catalog_override_when_preferred_review_model_exists_body(),
        )
    })
}

async fn guardian_review_uses_model_catalog_override_when_preferred_review_model_exists_body()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
...
#[test]
fn guardian_review_uses_preferred_review_model_without_model_catalog_override() -> anyhow::Result<()>
{
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(guardian_review_uses_preferred_review_model_without_model_catalog_override_body())
    })
}

async fn guardian_review_uses_preferred_review_model_without_model_catalog_override_body()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
...
#[test]
fn guardian_review_records_missing_auto_review_model_in_analytics_metadata() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(guardian_review_records_missing_auto_review_model_in_analytics_metadata_body())
    })
}

async fn guardian_review_records_missing_auto_review_model_in_analytics_metadata_body()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
...
fn guardian_review_request_layout_matches_model_visible_request_snapshot() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(guardian_review_request_layout_matches_model_visible_request_snapshot_body())
    })
}

async fn guardian_review_request_layout_matches_model_visible_request_snapshot_body()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let mut extensions = whisply_extension_api::ExtensionRegistryBuilder::<Config>::new();
    ...
    assert!(matches!(
        ...
        Some(whisply_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    let request = request_log.single_request();
    let request_body = request.body_json();
    ...
    settings.bind(|| {
        assert_snapshot!(
            "whisply_core__guardian__tests__guardian_review_request_layout",
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    let (session, _) = Box::pin(crate::session::tests::make_session_and_context()).await;
    let prompt = build_guardian_prompt_items(
        &session,
        /*retry_reason*/ None,
        GuardianApprovalRequest::Shell {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            whisply_protocol::user_input::UserInput::Text { text, .. } => text,
            whisply_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
...
#[test]
fn stateless_auto_review_shares_nothing_between_consecutive_reviews() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(stateless_auto_review_shares_nothing_between_consecutive_reviews_body())
    })
}

async fn stateless_auto_review_shares_nothing_between_consecutive_reviews_body()
-> anyhow::Result<()> {
    // The inverse of `guardian_reuses_prompt_cache_key_and_appends_prior_reviews`
    // above: same two reviews on the same thread, but under Whisply's public
    // `Auto` mode, where each approval must be decided on its own facts.
    let server = start_mock_server().await;
    let first_rationale = "first stateless rationale from the prior review";
    let request_log = mount_sse_sequence(
        &server,
        vec![
            sse(vec![
                ev_response_created("resp-stateless-1"),
                ...
                ev_completed("resp-stateless-1"),
            ]),
            sse(vec![
                ev_response_created("resp-stateless-2"),
                ev_assistant_message(
                    "msg-stateless-2",
                    "{\"risk_level\":\"low\",\"user_authorization\":\"high\",\"outcome\":\"allow\",\"rationale\":\"second stateless rationale\"}",
                ),
                ev_completed("resp-stateless-2"),
            ]),
        ],
    )
    .await;
    ...
    Arc::make_mut(&mut turn_mut.config).stateless_auto_review = true;
    seed_guardian_parent_history(&session, &turn).await;
    ...
    let second_outcome = run_guardian_review_session_for_test(
        Arc::clone(&session),
        Arc::clone(&turn),
        guardian_shell_request("shell-2"),
        ApprovalRequestReasons {
            approval: None,
    ...
    assert!(
        matches!(
            ...
            Some(whisply_analytics::GuardianReviewSessionKind::StatelessAuto)
        ),
        "stateless Auto must not open a reusable trunk, got {:?}",
        first_metadata.guardian_session_kind
    );
    assert!(
        matches!(
        ...
        "the second stateless review must not reuse the first, got {:?}",
        second_metadata.guardian_session_kind
    );
    ...
    assert_eq!(
        ...
        Some(false),
        "a stateless review must not carry the previous verdict"
    );
    assert_ne!(
        first_metadata.guardian_thread_id, second_metadata.guardian_thread_id,
        "each stateless review must be its own thread"
    );
    assert!(
        session
            ...
            .await
            .is_none(),
        "stateless Auto must leave no committed fork snapshot behind"
    );
    ...
    assert_ne!(
        ...
        "stateless reviews must not share a prompt cache key"
    );
    let second_body = second_body.to_string();
    assert!(
        !second_body.contains(first_rationale),
        "a stateless review must not be told what the previous review decided"
    );
    assert!(
        !second_body.contains("Use prior reviews as context, not binding precedent."),
        "a stateless review has no prior reviews to be reminded about"
    );
    let second_user_message = requests[1]
        .message_input_text_groups("user")
        .last()
        .expect("second guardian user message")
        .join("");
    assert!(
...
fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(guardian_reuses_prompt_cache_key_and_appends_prior_reviews_body())
    })
}

async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews_body() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    assert!(matches!(
        second_metadata.guardian_session_kind,
        Some(whisply_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "whisply_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
...
#[test]
fn guardian_reused_trunk_ignores_stale_prior_turn_completion() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(guardian_reused_trunk_ignores_stale_prior_turn_completion_body())
    })
}

async fn guardian_reused_trunk_ignores_stale_prior_turn_completion_body() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
...
fn guardian_review_surfaces_responses_api_errors_in_rejection_reason() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(guardian_review_surfaces_responses_api_errors_in_rejection_reason_body())
    })
}

async fn guardian_review_surfaces_responses_api_errors_in_rejection_reason_body()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let ReviewDecision::Denied { rejection } = decision.expect_decided() else {
...
#[test]
fn whisply_auto_asks_the_user_when_the_review_cannot_run() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(whisply_auto_asks_the_user_when_the_review_cannot_run_body())
    })
}

async fn whisply_auto_asks_the_user_when_the_review_cannot_run_body() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let request_log = mount_response_sequence(
        ...
        vec![
            wiremock::ResponseTemplate::new(400).set_body_json(serde_json::json!({
                "error": {
                    "message": "Item 'rs_test' of type 'reasoning' was provided without its required following item.",
                    "type": "invalid_request_error",
                    "param": "input"
                }
            })),
    ...
    let outcome = review_approval_request(
        &session,
        &turn,
        "review-shell-guardian-unreachable".to_string(),
        GuardianApprovalRequest::Shell {
            id: "shell-guardian-unreachable".to_string(),
            command: vec!["git".to_string(), "push".to_string()],
            cwd: test_path_buf("/repo/whisply-rs/core").abs(),
            sandbox_permissions: crate::sandboxing::SandboxPermissions::UseDefault,
            additional_permissions: None,
    ...
    assert!(
        ...
        "a review that could not run must not answer for the user: {outcome:?}"
    );
    assert_eq!(request_log.requests().len(), 1);
    ...
    assert!(
        ...
        "the review should report that it handed the decision over: {statuses:?}"
    );
    assert!(
        !statuses.contains(&GuardianAssessmentStatus::Denied),
        "a review that never judged the action must not report a denial: {statuses:?}"
    );
...
#[test]
fn whisply_auto_asks_the_user_when_the_reviewer_says_it_cannot_tell() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(whisply_auto_asks_the_user_when_the_reviewer_says_it_cannot_tell_body())
    })
}

async fn whisply_auto_asks_the_user_when_the_reviewer_says_it_cannot_tell_body()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));

    let server = start_mock_server().await;
    let rationale = "The command's effect depends on which remote is configured.";
    let request_log = mount_sse_sequence(
        &server,
        vec![sse(vec![
            ev_response_created("resp-uncertain"),
            ev_assistant_message(
                "msg-uncertain",
                &format!(
                    "{{\"risk_level\":\"medium\",\"user_authorization\":\"unknown\",\"outcome\":\"uncertain\",\"rationale\":\"{rationale}\"}}"
                ),
            ),
            ev_completed("resp-uncertain"),
        ])],
    )
    .await;
    ...
    let outcome = review_approval_request(
        &session,
        &turn,
        "review-shell-guardian-uncertain".to_string(),
        GuardianApprovalRequest::Shell {
            id: "shell-guardian-uncertain".to_string(),
            command: vec!["git".to_string(), "push".to_string()],
            cwd: test_path_buf("/repo/whisply-rs/core").abs(),
            sandbox_permissions: crate::sandboxing::SandboxPermissions::UseDefault,
            additional_permissions: None,
    ...
    assert!(
        matches!(
            outcome,
            GuardianApprovalOutcome::AskUser {
                reason: GuardianReviewFailureReason::ReviewerUncertain
            }
        ),
        "a reviewer that cannot tell must hand the decision over, not guess: {outcome:?}"
    );
    assert_eq!(request_log.requests().len(), 1);
    ...
    let assessments: Vec<_> = events
        ...
        .filter_map(|event| match &event.msg {
            EventMsg::GuardianAssessment(assessment) => Some(assessment),
            _ => None,
        })
        .collect();
    let statuses: Vec<_> = assessments
        .iter()
    ...
    assert!(
        ...
        "an unsure reviewer must not be recorded as having decided: {statuses:?}"
    );
    assert!(
        assessments.iter().any(|assessment| assessment
            .rationale
            .as_deref()
            .is_some_and(|text| text.contains(rationale))),
        "the reviewer's own reason for being unsure should reach the person"
    );
    assert!(
        !events.iter().any(|event| matches!(
            &event.msg,
            EventMsg::GuardianWarning(warning) if warning.message.contains("denied")
        )),
        "nothing was denied, so nothing should warn that it was"
    );

    Ok(())
}

/// WCD-718: the reviewer can answer `uncertain` whether or not it was offered.
///
/// Outside Whisply's `Auto` there is nobody the review can hand the question
/// to -- the point of that configuration is that the automatic review is the
/// answer. A model that says "I cannot tell" anyway has still not said yes,
/// and the schema it was given is a request, not a guarantee: these requests
/// go out with `strict: false`, and a reviewer that answers off-contract is
/// exactly the case where reading its answer generously is worst.
#[test]
fn an_unsure_reviewer_is_not_a_yes_where_there_is_nobody_to_ask() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(an_unsure_reviewer_is_not_a_yes_where_there_is_nobody_to_ask_body())
    })
}

async fn an_unsure_reviewer_is_not_a_yes_where_there_is_nobody_to_ask_body() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let request_log = mount_sse_sequence(
        ...
        vec![sse(vec![
            ...
            ev_assistant_message(
                "msg-uncertain",
                r#"{"risk_level":"high","user_authorization":"unknown","outcome":"uncertain","rationale":"I cannot tell what this touches."}"#,
            ),
            ev_completed("resp-uncertain"),
        ])],
    )
    ...
    assert!(
        !turn.config.stateless_auto_review,
        "this test is about the configuration that has nobody to defer to"
    );

    let outcome = review_approval_request(
        &session,
        &turn,
        "review-shell-guardian-unsure".to_string(),
        GuardianApprovalRequest::Shell {
            id: "shell-guardian-unsure".to_string(),
            command: vec!["git".to_string(), "push".to_string()],
            cwd: test_path_buf("/repo/whisply-rs/core").abs(),
            sandbox_permissions: crate::sandboxing::SandboxPermissions::UseDefault,
            additional_permissions: None,
    ...
    let GuardianApprovalOutcome::Decided(decision) = outcome else {
        panic!("this configuration has nobody to ask, so it must decide: {outcome:?}");
    };
    assert!(
        !matches!(
            decision,
        ...
        "a reviewer that could not tell must not be read as having approved: {decision:?}"
    );
    assert_eq!(request_log.requests().len(), 1);
    ...
    assert!(
        !statuses.contains(&GuardianAssessmentStatus::Approved),
        "an unsure reviewer must not be recorded as having approved: {statuses:?}"
    );
...
#[test]
fn whisply_auto_asks_the_user_when_the_review_times_out() -> anyhow::Result<()> {
    // Paused: the review only ends on its own 90s deadline, which is virtual
    // time here rather than ninety real seconds.
    block_on_guardian_review_test(/*paused_clock*/ true, || {
        Box::pin(whisply_auto_asks_the_user_when_the_review_times_out_body())
    })
}

async fn whisply_auto_asks_the_user_when_the_review_times_out_body() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));

    let server = start_mock_server().await;
    // Never answers, so the review can only end on its own deadline.
    let request_log = mount_response_sequence(
        &server,
        vec![
            wiremock::ResponseTemplate::new(200)
                .set_delay(GUARDIAN_REVIEW_TIMEOUT + Duration::from_secs(30)),
        ],
    )
    .await;
    ...
    assert!(
        !statuses.contains(&GuardianAssessmentStatus::TimedOut),
        "a timeout that becomes a question must not also be recorded as a verdict: {statuses:?}"
    );
...
#[test]
fn guardian_review_retries_transient_session_failure_then_approves() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(guardian_review_retries_transient_session_failure_then_approves_body())
    })
}

async fn guardian_review_retries_transient_session_failure_then_approves_body() -> anyhow::Result<()>
{
    skip_if_no_network!(Ok(()));
...
#[test]
fn guardian_review_does_not_retry_missing_assessment_payload() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(guardian_review_does_not_retry_missing_assessment_payload_body())
    })
}

async fn guardian_review_does_not_retry_missing_assessment_payload_body() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
...
fn guardian_review_retries_two_parse_failures_then_approves() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(guardian_review_retries_two_parse_failures_then_approves_body())
    })
}

async fn guardian_review_retries_two_parse_failures_then_approves_body() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
...
#[test]
fn guardian_review_exhausts_three_failures_with_one_terminal_event() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(guardian_review_exhausts_three_failures_with_one_terminal_event_body())
    })
}

async fn guardian_review_exhausts_three_failures_with_one_terminal_event_body() -> anyhow::Result<()>
{
    skip_if_no_network!(Ok(()));
...
#[test]
fn guardian_review_does_not_retry_valid_denial() -> anyhow::Result<()> {
    block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(guardian_review_does_not_retry_valid_denial_body())
    })
}

async fn guardian_review_does_not_retry_valid_denial_body() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
...
async fn guardian_ephemeral_retry_preserves_parallel_trunk_and_fork_history() -> anyhow::Result<()>
{
    ...
    let handle =
        ...
        .await;
        assert_eq!(third_decision.expect_decided(), ReviewDecision::Approved);
        ...
        assert_eq!(second_review.await?.expect_decided(), ReviewDecision::Approved);
...
async fn guardian_review_session_config_clears_parent_developer_instructions() {
    ...
    assert_eq!(
        ...
        Some(guardian_policy_prompt_with_config_and_template(
            BUNDLED_GUARDIAN_POLICY,
            BUNDLED_GUARDIAN_POLICY_TEMPLATE,
            parent_config.stateless_auto_review,
        ))
    );
}
...
async fn guardian_review_session_config_uses_requirements_guardian_policy_config() {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        whisply_config::ConfigRequirementsToml {
