...
use whisply_otel::GUARDIAN_REVIEW_COUNT_METRIC;
use whisply_otel::GUARDIAN_REVIEW_DURATION_METRIC;
use whisply_otel::GUARDIAN_REVIEW_TOKEN_USAGE_METRIC;
use whisply_otel::GUARDIAN_REVIEW_TTFT_DURATION_METRIC;
...
fn decision_tag(decision: GuardianReviewDecision) -> &'static str {
    match decision {
        ...
        GuardianReviewDecision::AskedUser => "asked_user",
    }
}
...
fn terminal_status_tag(status: GuardianReviewTerminalStatus) -> &'static str {
    match status {
        ...
        GuardianReviewTerminalStatus::AskedUser => "asked_user",
    }
}

fn failure_reason_tag(reason: Option<GuardianReviewFailureReason>) -> &'static str {
    match reason {
        Some(GuardianReviewFailureReason::UnsupportedAction) => "unsupported_action",
        Some(GuardianReviewFailureReason::ReviewerUncertain) => "reviewer_uncertain",
...
fn reviewed_action_tag(action: &GuardianReviewedAction) -> &'static str {
    match action {
        ...
        GuardianReviewedAction::FirstPartyTool { .. } => "whisply_first_party",
    }
}
...
fn session_kind_tag(kind: Option<GuardianReviewSessionKind>) -> &'static str {
    match kind {
        ...
        Some(GuardianReviewSessionKind::StatelessAuto) => "stateless_auto",
        None => "none",
    }
}
...
fn outcome_tag(outcome: Option<GuardianAssessmentOutcome>) -> &'static str {
    match outcome {
        ...
        Some(GuardianAssessmentOutcome::Uncertain) => "uncertain",
        None => "none",
    }
}
    ...
    fn guardian_review_metrics_record_counts_durations_and_token_usage() {
        ...
        emit_guardian_review_metrics(
            ...
            &GuardianReviewedAction::NetworkAccess {
                protocol: whisply_protocol::approvals::NetworkApprovalProtocol::Https,
                port: 443,
            },
