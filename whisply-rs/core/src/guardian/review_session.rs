...
use whisply_analytics::GuardianReviewSessionAnalyticsParams;
use whisply_analytics::GuardianReviewSessionKind;
use whisply_extension_api::UserInstructions;
...
use super::prompt::guardian_policy_revision;
...
pub(crate) fn prompt_cache_key_override_for_review_session(
    ...
    if stateless_auto_review {
        // Not `None`: without an override the client falls back to the parent
        // session id, which every review of the thread would share. A cache
        // key is state carried between verdicts, so key each review to itself.
        return Some(format!("guardian-stateless:{review_thread_id}"));
    }
...
impl GuardianReviewSessionManager {
    ...
    pub(super) async fn run_review(
        ...
        if params.spawn_config.stateless_auto_review {
            // Deliberately before the trunk is consulted or spawned. Reaching
            // the trunk at all is what would let one review see another's
            // session, the parent transcript, or its own prior verdict count.
            return Box::pin(self.run_ephemeral_review(
                params,
                next_reuse_key,
                deadline,
                /*fork_snapshot*/ None,
                GuardianReviewSessionKind::StatelessAuto,
            ))
            .await;
        }
...
async fn spawn_guardian_review_session(
    ...
    let (session, io) = Box::pin(run_codex_thread_interactive(
        ...
        whisply_sandboxing::WindowsSandboxProxySettingsMode::Preserve,
    ))
    .await?;
...
async fn run_review_on_session(
    ...
    let mut analytics_result =
        GuardianReviewAnalyticsResult::from_session(GuardianReviewSessionAnalyticsParams {
            ...
            guardian_policy_revision: guardian_policy_revision(
                params
                    .spawn_config
                    .base_instructions
                    .as_deref()
...
pub(crate) fn build_guardian_review_session_config(
    ...
    live_network_config: Option<whisply_network_proxy::NetworkProxyConfig>,
    active_model: &str,
...
mod tests {
    use super::*;
    use whisply_protocol::openai_models::AutoReviewMessages;
    ...
    async fn guardian_prompt_cache_key_is_scoped_to_parent_thread() {
        ...
        let first_review = ThreadId::new();
        let stateless_key = prompt_cache_key_override_for_review_session(
            &session_source,
            Some(parent_thread_id),
            first_review,
            /*stateless_auto_review*/ true,
        )
        .expect("a stateless review needs its own key, not the parent session fallback");
        assert_ne!(
            stateless_key, key,
            "a stateless review must not share the thread's reusable review key"
        );
        assert_ne!(
            stateless_key,
            prompt_cache_key_override_for_review_session(
            ...
            .expect("second stateless review key"),
            "two stateless reviews of one thread must not share a cache key"
        );
        assert!(
            stateless_key.len() <= 64,
            "stateless prompt cache key should fit the Responses API limit"
        );
    }
    ...
    #[test]
    fn stateless_auto_review_never_reaches_the_shared_trunk() {
        // Spawning a whole review session is deep enough to exhaust a default
        // test stack, the same reason the product gives its review worker an
        // explicit budget.
        ...
        runtime.block_on(stateless_auto_review_never_reaches_the_shared_trunk_body());
    }

    async fn stateless_auto_review_never_reaches_the_shared_trunk_body() {
        // Deliberately the same setup as the reuse test above, including a
        // trunk whose reuse key matches, so the only thing under test is
        // whether Whisply's `Auto` mode still declines to use it.
        let (mut review_session, tx_event, _rx_sub) = test_review_session().await;
        let mut params = test_review_params().await;
        params.spawn_config.stateless_auto_review = true;
        params.deadline = tokio::time::Instant::now() + Duration::from_millis(750);
        review_session.reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            &params.spawn_config,
            params.parent_session.user_instructions().await,
        );
        let seeded_trunk = Arc::new(review_session);
        let manager = GuardianReviewSessionManager {
            state: Arc::new(Mutex::new(GuardianReviewSessionState {
                trunk: Some(Arc::clone(&seeded_trunk)),
                ephemeral_reviews: Vec::new(),
            })),
            ..Default::default()
        };
        // The trunk's event stream is broken, so a review that reuses it fails
        // and takes the trunk down with it. A review that never touches it
        // leaves it in place.
        drop(tx_event);

        let (_outcome, _) = manager.run_review(params).await;

        let trunk = manager.state.lock().await.trunk.clone();
        let trunk = trunk.expect("a stateless review must not consume the shared trunk");
        assert!(
            Arc::ptr_eq(&trunk, &seeded_trunk),
            "a stateless review must not replace the shared trunk either"
        );
        assert_eq!(
            0,
            trunk.state.lock().await.prior_review_count,
            "a stateless review must not advance the trunk's verdict history"
        );
    }
