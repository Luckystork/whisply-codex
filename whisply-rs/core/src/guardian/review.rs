...
use whisply_analytics::GuardianReviewAnalyticsResult;
use whisply_analytics::GuardianReviewDecision;
use whisply_analytics::GuardianReviewFailureReason;
use whisply_analytics::GuardianReviewTerminalStatus;
use whisply_analytics::GuardianReviewTrackContext;
use whisply_analytics::GuardianReviewedAction;
...
use whisply_protocol::protocol::GuardianAssessmentDecisionSource;
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &whisply_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        whisply_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(label))
...
async fn run_guardian_review(
        ...
        .is_some_and(CancellationToken::is_cancelled)
    {
        ...
        return GuardianApprovalOutcome::Decided(ReviewDecision::Abort);
    }

    let schema = guardian_output_schema(turn.config.stateless_auto_review);
    ...
    let (assessment, count_denial_for_circuit_breaker) = match outcome {
        GuardianReviewOutcome::Completed(assessment)
            if matches!(assessment.outcome, GuardianAssessmentOutcome::Uncertain)
                && turn.config.stateless_auto_review =>
        {
            // The reviewer looked and could not tell. That is a real answer,
            // and the only honest thing to do with it is ask.
            track_guardian_review(
                session.as_ref(),
                &review_tracking,
                approval_request_source,
                &reviewed_action,
                GuardianReviewAnalyticsResult {
                    decision: GuardianReviewDecision::AskedUser,
                    terminal_status: GuardianReviewTerminalStatus::AskedUser,
                    failure_reason: Some(GuardianReviewFailureReason::ReviewerUncertain),
            ...
            session
                .send_event(
                    ...
                    EventMsg::GuardianAssessment(GuardianAssessmentEvent {
                        ...
                        started_at_ms,
                        completed_at_ms: Some(completed_at_ms),
                        status: GuardianAssessmentStatus::AskUser,
                        risk_level: Some(assessment.risk_level),
                        user_authorization: Some(assessment.user_authorization),
                ...
                .await;
            record_guardian_non_denial(&session, &assessment_turn_id).await;
            return GuardianApprovalOutcome::AskUser {
        ...
        GuardianReviewOutcome::Error(error) => match error {
            GuardianReviewError::Timeout => {
                // Whisply's `Auto` promises a review of every approval, not a
                // refusal whenever the reviewer is slow. A timeout says
                // nothing about whether the action is safe, so the person is
                // asked instead of being answered for.
                let ask_user = turn.config.stateless_auto_review;
                let rationale = if ask_user {
                    "Automatic approval review timed out, so this approval is going to you."
                } else {
                ...
                track_guardian_review(
                    ...
                    &reviewed_action,
                    GuardianReviewAnalyticsResult {
                        decision: if ask_user {
                            GuardianReviewDecision::AskedUser
                        } else {
                            GuardianReviewDecision::Denied
                        },
                        terminal_status: if ask_user {
                            GuardianReviewTerminalStatus::AskedUser
                        } else {
                ...
                return GuardianApprovalOutcome::Decided(ReviewDecision::TimedOut);
            }
            ...
            | GuardianReviewError::Parse { .. } => {
                ...
                // A malformed, unusable, or unreachable reviewer has not
                // judged the action; it has failed to look at it. Under
                // Whisply's `Auto` that becomes a question for the person
                // rather than a refusal attributed to a review that never
                // happened.
                let ask_user = turn.config.stateless_auto_review;
                if ask_user {
                    track_guardian_review(
                    ...
                    session
                        .send_event(
                            ...
                            EventMsg::GuardianAssessment(GuardianAssessmentEvent {
                                ...
                                user_authorization: None,
                                rationale: Some(
                                    "Automatic approval review could not complete, so this approval is going to you."
                                        .to_string(),
                                ),
                                decision_source: Some(GuardianAssessmentDecisionSource::Agent),
                                action: terminal_action,
    ...
    let approved = match assessment.outcome {
        ...
        // Only reachable outside Whisply's `Auto`, which returns before this
        // point. Upstream has no one to defer to, so an unsure reviewer keeps
        // upstream's fail-closed answer.
        GuardianAssessmentOutcome::Deny | GuardianAssessmentOutcome::Uncertain => false,
    };
    ...
    GuardianApprovalOutcome::Decided(if approved {
...
/// The typed result of one automatic approval review.
///
/// `AskUser` is not a decision: the review reached no verdict it could stand
/// behind, so the approval continues as an ordinary user prompt. It exists
/// because refusing an action because the reviewer was unreachable is a
/// different, worse outcome than refusing it because the reviewer judged it
/// unsafe, and the two must not look alike to the caller.
#[derive(Debug)]
pub(crate) enum GuardianApprovalOutcome {
    Decided(ReviewDecision),
    AskUser { reason: GuardianReviewFailureReason },
}

impl From<ReviewDecision> for GuardianApprovalOutcome {
...
#[cfg(test)]
impl GuardianApprovalOutcome {
    /// The decision this review reached, for tests that are asserting on the
    /// verdict rather than on whether one was reached.
    #[track_caller]
    pub(crate) fn expect_decided(self) -> ReviewDecision {
        match self {
            Self::Decided(decision) => decision,
            Self::AskUser { reason } => {
                panic!("expected the review to decide, but it asked the user ({reason:?})")
            }
        }
    }
}
...
impl GuardianApprovalOutcome {
    /// Outcome for an action that cannot be put to a reviewer at all.
    ///
    /// Nothing has judged it unsafe; nothing has judged it. Under Whisply's
    /// `Auto` that is the person's call.
    pub(crate) fn unreviewable_action(stateless_auto_review: bool) -> Self {
        if stateless_auto_review {
            Self::AskUser {
                reason: GuardianReviewFailureReason::UnsupportedAction,
            }
        } else {
            Self::Decided(ReviewDecision::denied(
                "automatic approval review could not prepare the action",
            ))
        }
    }

    /// Outcome for a review that never reported back at all, because its
    /// worker could not be started or died before answering.
    ///
    /// That is the same kind of non-answer as a timeout, so it is treated the
    /// same way rather than being charged to the action as a refusal.
    pub(crate) fn review_worker_lost(stateless_auto_review: bool) -> Self {
        if stateless_auto_review {
            Self::AskUser {
                reason: GuardianReviewFailureReason::SessionError,
            }
        } else {
            Self::Decided(ReviewDecision::denied(
...
const REVIEW_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;
...
pub(crate) async fn review_approval_request_with_cancel(
    ...
    // Boxed for the same reason as `review_approval_request`: the guardian
    // session state machine is large enough that inlining it into a caller's
    // async stack overflows it.
    Box::pin(run_guardian_review(
        Arc::clone(session),
        Arc::clone(turn),
        review_id,
...
pub(crate) fn spawn_approval_request_review(
    ...
    let spawn_result = std::thread::Builder::new()
        ...
        // `block_on` runs the review on this thread's own stack, and the
        // review's future is far too deep for the OS default. Matches the
        // budget the process gives its other runtime threads.
        .stack_size(REVIEW_WORKER_STACK_SIZE_BYTES)
        .spawn(move || {
            let Ok(runtime) = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
            else {
                let _ = tx.send(GuardianApprovalOutcome::review_worker_lost(
                    turn.config.stateless_auto_review,
                ));
                return;
            };
...
pub(super) struct GuardianReviewSessionConfig {
    ...
    model: String,
    reasoning_effort: Option<whisply_protocol::openai_models::ReasoningEffort>,
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = review_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset.supported_reasoning_efforts.iter().any(|effort| {
                effort.effort == whisply_protocol::openai_models::ReasoningEffort::Low
            }),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| {
                    preset.effort == whisply_protocol::openai_models::ReasoningEffort::Low
                }),
