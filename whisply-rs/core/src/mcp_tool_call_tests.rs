...
use whisply_config::types::AppConfig;
use whisply_config::types::AppToolConfig;
use whisply_config::types::AppToolsConfig;
use whisply_config::types::ApprovalsReviewer;
use whisply_config::types::AppsConfigToml;
...
fn approval_config(turn_context: &TurnContext) -> whisply_mcp::McpConfig {
...
async fn codex_apps_auth_elicitation_enabled_by_default_requests_elicitation() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        request.id,
        whisply_protocol::mcp::RequestId::String("codex_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        whisply_protocol::approvals::ElicitationRequest::Url { .. }
    ));
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    ConfigEditsBuilder::new(&codex_home)
        .set_project_trust_level(
            project_dir.path(),
            whisply_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
...
fn guardian_mode_mcp_denial_returns_rationale_message() {
    crate::guardian::tests::block_on_guardian_review_test(/*paused_clock*/ false, || {
        Box::pin(guardian_mode_mcp_denial_returns_rationale_message_body())
    })
}

async fn guardian_mode_mcp_denial_returns_rationale_message_body() {
    let server = start_mock_server().await;
