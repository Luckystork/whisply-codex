...
use whisply_exec_server::SelectedCapabilityRootsStatus;
...
use whisply_protocol::protocol::ThreadSettingsSnapshot;
...
use whisply_thread_store::ThreadStoreResult;
...
impl CodexThread {
    ...
    /// The layered composition of the most recent prompt this thread sent.
    ///
    /// `None` until the thread has sent one, or when the composition did not
    /// satisfy the contract — an unreported composition is preferable to a
    /// stale one being read as the current turn's.
    pub fn prompt_composition(&self) -> Option<codex_whisply::PromptCompositionDiagnostics> {
        self.session.prompt_composition()
    }

    /// What can be said about this conversation being shortened: how close it
    /// is to its budget, and what each shortening did.
    ///
    /// Always answers, because "this thread has never been shortened and has
    /// most of its room left" is the answer someone is often looking for.
    pub fn compaction_status(&self) -> codex_whisply::ConversationCompactionStatus {
        self.session.compaction_status()
    }
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(whisply_extension_api::ThreadResumeInput {
    ...
    pub async fn submit_user_input_and_wait_for_admission(
        ...
        self.submit_user_input_and_wait_for_admission_inner(op, trace, client_user_message_id, None)
            .await
    }

    /// Waits for Core to accept a user message with an explicitly registered,
    /// one-turn native first-party tool admission.
    ///
    /// The caller must retain the admission's owner state separately. Core
    /// verifies that the client message ID matches exactly, consumes the
    /// admission only if it starts a new turn, and discards it when the input
    /// is steered into an existing turn.
    pub async fn submit_user_input_and_wait_for_admission_with_first_party_tool_admission(
        &self,
        op: Op,
        trace: Option<W3cTraceContext>,
        client_user_message_id: Option<String>,
        first_party_tool_admission: FirstPartyToolAdmission,
    ) -> CodexResult<UserMessageAdmission> {
        if client_user_message_id.as_deref()
            != Some(first_party_tool_admission.client_user_message_id())
        {
            return Err(CodexErr::InvalidRequest(
                "first-party tool admission must match the client user message id".to_string(),
            ));
        }
        self.submit_user_input_and_wait_for_admission_inner(
            ...
            Some(first_party_tool_admission),
        )
        .await
    }
    ...
    async fn submit_user_input_and_wait_for_admission_inner(
        ...
        trace: Option<W3cTraceContext>,
        client_user_message_id: Option<String>,
        first_party_tool_admission: Option<FirstPartyToolAdmission>,
    ) -> CodexResult<UserMessageAdmission> {
        ...
        if let Some(first_party_tool_admission) = first_party_tool_admission {
            self.session
                .pending_first_party_tool_admissions
                .insert(submission_id.clone(), first_party_tool_admission);
        }
        ...
        if matches!(admission, Ok(UserMessageAdmission::Steered { .. })) {
            self.session
                .pending_first_party_tool_admissions
                .remove(&submission_id);
        }
    ...
    ) -> (whisply_mcp::McpConfig, whisply_mcp::McpRuntimeContext) {
    ...
    pub async fn current_mcp_config_and_runtime_context(
        &self,
    ) -> (Arc<whisply_mcp::McpConfig>, whisply_mcp::McpRuntimeContext) {
