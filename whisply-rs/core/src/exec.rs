...
use whisply_protocol::protocol::ExecCommandOutputDeltaEvent;
use whisply_protocol::protocol::ExecOutputStream;
...
use whisply_sandboxing::WindowsSandboxFilesystemOverrides;
pub(crate) use whisply_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use whisply_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use whisply_sandboxing::record_filesystem_sandbox_violation;
use whisply_sandboxing::resolve_windows_elevated_filesystem_overrides;
use whisply_sandboxing::resolve_windows_restricted_token_filesystem_overrides;
#[cfg(test)]
use whisply_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
use whisply_sandboxing::windows_sandbox_uses_elevated_backend;
...
use whisply_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: whisply_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: whisply_protocol::config_types::WindowsSandboxLevel,
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        whisply_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = whisply_otel::global() {
...
async fn exec_windows_sandbox(
    ...
    use whisply_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use whisply_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            whisply_windows_sandbox::WindowsSandboxCancellationToken::new(move || {
                token.is_cancelled()
            })
        });
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                whisply_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        whisply_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                whisply_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }
