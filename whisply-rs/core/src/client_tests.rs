...
use whisply_rollout_trace::RolloutTrace;
    ...
    let output = client
        .compact_conversation_history(
            ...
            CompactConversationRequestSettings {
                effort: None,
                summary: whisply_protocol::config_types::ReasoningSummary::None,
                service_tier: None,
            },
...
fn output_message(id: &str, text: &str) -> ResponseItem {
    ResponseItem::Message {
        id: Some(whisply_protocol::ResponseItemId::with_suffix("msg", id)),
...
async fn bedrock_unauthorized_error_uses_provider_mapping() {
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "Amazon Bedrock rejected the request because its AWS signature has expired. Refresh your AWS credentials and retry. If `AWS_BEARER_TOKEN_BEDROCK` is set, update or unset it, then restart Whisply, url: {url}"
        )
    );
}
