...
use tracing::warn;
pub use whisply_connectors::AppBranding;
pub use whisply_connectors::AppInfo;
pub use whisply_connectors::AppMetadata;
use whisply_connectors::ConnectorDirectoryCacheContext;
use whisply_connectors::ConnectorDirectoryCacheKey;
use whisply_connectors::apps_config_from_layer_stack;
...
use whisply_mcp::ToolPluginProvenance;
use whisply_mcp::effective_mcp_servers;
use whisply_mcp::tool_plugin_provenance;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = whisply_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        whisply_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_mcp_manager(
    ...
    let codex_apps_auth_manager =
        whisply_mcp::host_owned_codex_apps_enabled(&mcp_config, auth.as_ref())
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + whisply_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    whisply_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
fn collect_accessible_connectors_from_mcp_tools<'a>(
    ...
    let tools = mcp_tools.filter_map(|tool| {
        ...
        Some(whisply_connectors::accessible::AccessibleConnectorTool {
    ...
    whisply_connectors::accessible::collect_accessible_connectors(tools)
}
