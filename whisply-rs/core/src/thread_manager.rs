...
use tracing::warn;
use whisply_agent_graph_store::AgentGraphStore;
use whisply_agent_graph_store::LocalAgentGraphStore;
use whisply_analytics::AnalyticsEventsClient;
use whisply_code_mode::CodeModeSessionProvider;
use whisply_code_mode::DisabledCodeModeSessionProvider;
use whisply_code_mode::ProcessOwnedCodeModeSessionProvider;
...
use whisply_extension_api::empty_extension_registry;
use whisply_features::Feature;
use whisply_login::AuthManager;
use whisply_login::CodexAuth;
use whisply_login::default_client::CODEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
use whisply_protocol::config_types::CollaborationModeMask;
...
use whisply_thread_store::InMemoryThreadStore;
use whisply_thread_store::LoadThreadHistoryParams;
use whisply_thread_store::LocalThreadStore;
use whisply_thread_store::LocalThreadStoreConfig;
use whisply_thread_store::MoveThreadToSectionParams;
use whisply_thread_store::PreparedFork;
use whisply_thread_store::ReadThreadByRolloutPathParams;
use whisply_thread_store::ReadThreadParams;
use whisply_thread_store::StoredModelContext;
use whisply_thread_store::StoredThread;
use whisply_thread_store::ThreadMetadataPatch;
use whisply_thread_store::ThreadStore;
use whisply_thread_store::ThreadStoreError;
use whisply_thread_store::UpdateThreadMetadataParams;
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<whisply_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    ...
    first_party_tool_dispatcher: Option<Arc<dyn FirstPartyToolDispatcher>>,
    server_tool_dispatcher: Option<Arc<dyn ServerToolDispatcher>>,
...
pub fn thread_store_from_config(
    ...
    match &config.experimental_thread_store {
        ThreadStoreConfig::Local => {
            ...
            {
                whisply_rollout::spawn_rollout_compression_worker(config.codex_home.to_path_buf());
            }
...
impl ThreadManager {
    ...
    /// Installs the owner-authenticated dispatcher before this manager is
    /// shared with threads. It remains inert unless a turn also carries an
    /// explicit one-turn native tool admission.
    pub fn with_first_party_tool_dispatcher(
        mut self,
        dispatcher: Arc<dyn FirstPartyToolDispatcher>,
    ) -> Self {
        let Some(state) = Arc::get_mut(&mut self.state) else {
            unreachable!("first-party tool dispatcher must be set before thread manager is shared");
        };
        state.first_party_tool_dispatcher = Some(dispatcher);
        self
    }

    /// Installs the server-owned tool dispatcher before this manager is shared
    /// with threads.
    pub fn with_server_tool_dispatcher(
        mut self,
        dispatcher: Arc<dyn ServerToolDispatcher>,
    ) -> Self {
        let Some(state) = Arc::get_mut(&mut self.state) else {
            unreachable!("server tool dispatcher must be set before thread manager is shared");
        };
        state.server_tool_dispatcher = Some(dispatcher);
        self
    }
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite: whisply_state::SqliteConfig::new_for_testing(absolute_codex_home),
    ...
    /// Returns the typed broker client installed by an embedded Whisply host.
    /// This is intentionally separate from configuration so catalog reads can
    /// never reconstruct a user-configured provider endpoint or credential.
    pub fn managed_gateway_client(&self) -> Option<Arc<ManagedGatewayClient>> {
        self.state.managed_gateway_client.clone()
    }
...
impl ThreadManagerState {
    ...
    async fn spawn_thread(&self, request: ThreadSpawnRequest) -> CodexResult<NewThread> {
        ...
        let (session, io) = Box::pin(Session::spawn(SessionSpawnArgs {
            ...
            first_party_tool_dispatcher: self.first_party_tool_dispatcher.clone(),
            server_tool_dispatcher: self.server_tool_dispatcher.clone(),
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> whisply_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return whisply_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(whisply_rollout_trace::ThreadTraceContext::disabled)
    }
}
