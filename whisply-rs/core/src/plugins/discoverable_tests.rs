...
use whisply_core_plugins::remote::fetch_and_cache_global_remote_plugin_catalog;
...
async fn list_discoverable_plugins_with_auth(
    config: &crate::config::Config,
    auth: Option<&whisply_login::CodexAuth>,
    loaded_plugin_app_connector_ids: &[String],
) -> anyhow::Result<Vec<DiscoverablePluginInfo>> {
    let plugins_manager = PluginsManager::new_with_options(
        config.codex_home.to_path_buf(),
        Some(Product::Codex),
        auth.map(whisply_login::CodexAuth::api_auth_mode),
    );
