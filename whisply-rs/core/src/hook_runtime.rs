...
use whisply_analytics::HookRunFact;
use whisply_analytics::build_track_events_context;
use whisply_hooks::PermissionRequestDecision;
use whisply_hooks::PermissionRequestOutcome;
use whisply_hooks::PermissionRequestRequest;
use whisply_hooks::PostToolUseOutcome;
use whisply_hooks::PostToolUseRequest;
use whisply_hooks::PreToolUseOutcome;
use whisply_hooks::PreToolUseRequest;
use whisply_hooks::SessionStartOutcome;
use whisply_hooks::StartHookTarget;
use whisply_hooks::StopHookTarget;
use whisply_hooks::StopOutcome;
use whisply_hooks::SubagentHookContext;
use whisply_hooks::UserPromptSubmitOutcome;
use whisply_hooks::UserPromptSubmitRequest;
use whisply_otel::HOOK_RUN_DURATION_METRIC;
use whisply_otel::HOOK_RUN_METRIC;
...
use whisply_protocol::protocol::HookCompletedEvent;
use whisply_protocol::protocol::HookEventName;
use whisply_protocol::protocol::HookRunStatus;
use whisply_protocol::protocol::HookRunSummary;
use whisply_protocol::protocol::HookSource;
use whisply_protocol::protocol::HookStartedEvent;
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = whisply_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = whisply_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = whisply_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = whisply_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(whisply_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: whisply_hooks::HookEvent::AfterAgent {
                event: whisply_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            whisply_hooks::HookResult::Success => continue,
            whisply_hooks::HookResult::FailedContinue(error) => (error, false),
            whisply_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(whisply_protocol::protocol::ErrorEvent {
        message,
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (whisply_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use whisply_protocol::protocol::HookEventName;
    use whisply_protocol::protocol::HookExecutionMode;
    use whisply_protocol::protocol::HookHandlerType;
    use whisply_protocol::protocol::HookRunStatus;
    use whisply_protocol::protocol::HookScope;
    use whisply_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    whisply_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
