...
fn preparation_errors_use_bounded_actionable_placeholders() {
    let cases = [
        ...
        (
            // An image refused for needing too much memory to decode is the
            // same problem to the person holding it as one refused for its
            // byte count, and gets the same sentence.
            ImagePreparationError::Processing(ImageProcessingError::ImageTooLargeToDecode {
