...
use whisply_network_proxy::BlockedRequestObserver;
use whisply_network_proxy::ConfigReloader;
use whisply_network_proxy::ConfigReloaderFuture;
use whisply_network_proxy::ConfigState;
use whisply_network_proxy::NetworkDecision;
...
use whisply_network_proxy::NetworkProxyConstraints;
use whisply_network_proxy::NetworkProxyHandle;
use whisply_network_proxy::NetworkProxyState;
use whisply_network_proxy::build_config_state;
use whisply_network_proxy::host_and_port_from_network_addr;
#[cfg(any(target_os = "windows", test))]
use whisply_network_proxy::managed_proxy_ports;
use whisply_network_proxy::normalize_host;
use whisply_network_proxy::validate_policy_against_constraints;
    ...
    for host in hosts {
        if incoming.insert(host.clone()) {
            config.upsert_domain_permission(
                host,
                if allow {
                    whisply_network_proxy::NetworkDomainPermission::Allow
                } else {
                    whisply_network_proxy::NetworkDomainPermission::Deny
                },
                normalize_host,
            );
        }
