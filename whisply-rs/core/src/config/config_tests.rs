...
use whisply_config::ConfigLayerEntry;
use whisply_config::ConfigLayerSource;
use whisply_config::ConfigLayerStack;
use whisply_config::McpServerCommandMatcher;
use whisply_config::McpServerIdentity;
use whisply_config::McpServerRequirement;
use whisply_config::McpServerValueMatcher;
...
use whisply_config::config_toml::AgentRoleToml;
use whisply_config::config_toml::AgentsToml;
use whisply_config::config_toml::AutoReviewToml;
use whisply_config::config_toml::ConfigToml;
use whisply_config::config_toml::ExperimentalRequestUserInput;
use whisply_config::config_toml::ProjectConfig;
use whisply_config::config_toml::RealtimeConfig;
use whisply_config::config_toml::RealtimeToml;
use whisply_config::config_toml::RealtimeTransport;
use whisply_config::config_toml::RealtimeWsMode;
use whisply_config::config_toml::RealtimeWsVersion;
use whisply_config::config_toml::ToolsToml;
...
use whisply_config::permissions_toml::NetworkDomainPermissionToml;
use whisply_config::permissions_toml::NetworkDomainPermissionsToml;
use whisply_config::permissions_toml::NetworkMitmActionToml;
use whisply_config::permissions_toml::NetworkMitmHookToml;
use whisply_config::permissions_toml::NetworkMitmToml;
...
use whisply_config::permissions_toml::WorkspaceRootsToml;
use whisply_config::types::AppToolApproval;
use whisply_config::types::ApprovalsReviewer;
use whisply_config::types::BundledSkillsConfig;
use whisply_config::types::FeedbackConfigToml;
use whisply_config::types::HistoryPersistence;
use whisply_config::types::McpServerEnvVar;
use whisply_config::types::McpServerOAuthConfig;
use whisply_config::types::McpServerToolConfig;
use whisply_config::types::McpServerTransportConfig;
use whisply_config::types::MemoriesConfig;
use whisply_config::types::MemoriesToml;
use whisply_config::types::ModelAvailabilityNuxConfig;
use whisply_config::types::Notice;
use whisply_config::types::NotificationCondition;
use whisply_config::types::NotificationMethod;
use whisply_config::types::Notifications;
use whisply_config::types::OtelConfigToml;
use whisply_config::types::OtelExporterKind;
use whisply_config::types::ResumeCwdMode;
use whisply_config::types::SandboxWorkspaceWrite;
use whisply_config::types::SessionPickerViewMode;
use whisply_config::types::SkillsConfig;
use whisply_config::types::ToolSuggestDisabledTool;
use whisply_config::types::ToolSuggestDiscoverableType;
use whisply_config::types::Tui;
...
use whisply_config::types::WindowsToml;
...
use whisply_network_proxy::NetworkMode;
...
use whisply_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
...
use whisply_protocol::protocol::NetworkAccess;
use whisply_protocol::protocol::RealtimeVoice;
use whisply_protocol::protocol::SandboxPolicy;
use whisply_utils_path_uri::LegacyAppPathString;
...
use tempfile::TempDir;
use whisply_config::test_support::CloudConfigBundleFixture;
...
"#,
    ...
    assert_eq!(
        ...
        whisply_http_client::OutboundProxyPolicy::RespectSystemProxy
    );
    assert_eq!(
        config
            .auth_route_config()
    ...
    assert_eq!(
        ...
        whisply_core_plugins::remote::RemotePluginServiceConfig::new(
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        whisply_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".whisply"), cwd.path(),),
            "expected :workspace metadata carveouts, policy: {policy:?}"
        );
    }
    Ok(())
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".codex", ".whisply"] {
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Whisply and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Whisply and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of Whisply."
        )),
        "{:?}",
        config.startup_warnings
    );
                        ...
                        file_system_policy
                            ...
                            .contains(&FileSystemSandboxEntry {
                                ...
                                missing_path_behavior: Some(
                                    whisply_protocol::permissions::FileSystemSandboxEntryMissingPathBehavior::Skip,
                                ),
                            }),
...
fn filter_plugin_mcp_servers_without_allowlists_does_not_filter_any_plugin() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            whisply_config::PluginRequirementsToml { mcp_servers: None },
        )]),
        RequirementSource::LegacyManagedConfigTomlFromMdm,
    );
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    ...
    let requirements_toml = whisply_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = whisply_config::ConfigRequirements {
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            whisply_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn to_mcp_config_flows_mcp_2026_feature_from_config() -> std::io::Result<()> {
    ...
    assert_eq!(
        ...
        whisply_mcp::McpProtocolMode::Legacy
    );
    ...
    assert_eq!(
        mcp_config.protocol_mode,
        whisply_mcp::McpProtocolMode::V20260728
    );
...
"#,
    ...
    let config_layer_stack = whisply_config::ConfigLayerStack::new(
        vec![whisply_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
        ...
        Default::default(),
        whisply_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
    ...
    let cfg = ConfigToml {
        ...
        otel: Some(OtelConfigToml {
            exporter: Some(OtelExporterKind::OtlpHttp {
                ...
                headers: HashMap::new(),
                protocol: whisply_config::types::OtelHttpProtocol::Binary,
                tls: None,
            }),
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = whisply_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![whisply_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = whisply_config::RequirementSource::Unknown;
    ...
    let requirements = whisply_config::ConfigRequirements {
        web_search_mode: whisply_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        whisply_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
"#,
    ...
    assert_eq!(
        ...
        Some(whisply_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
/// An entry point that does not offer the permission selector keeps answering
/// approvals itself. `Auto` waits on a reviewer, and where there is nobody to
/// fall back to when the reviewer cannot answer, that wait has no end worth
/// having; the surfaces with a selector opt in instead.
#[tokio::test]
async fn a_surface_without_a_selector_still_asks_the_person() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
...
/// WCD-714: a surface that offers the selector starts new threads in `Auto`.
#[tokio::test]
async fn a_surface_with_a_selector_starts_in_auto() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        ...
        .fallback_cwd(Some(codex_home.path().to_path_buf()))
        .harness_overrides(ConfigOverrides {
            default_approvals_reviewer: Some(ApprovalsReviewer::AutoReview),
            ..Default::default()
        })
        .build()
        .await?;
...
/// A person who set the mode keeps it: a surface default is a starting point,
/// not an override.
#[tokio::test]
async fn a_chosen_mode_outranks_the_surface_default() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    std::fs::write(
        codex_home.path().join(CONFIG_TOML_FILE),
...
/// Without the reviewer there is nothing for `Auto` to delegate to, so even a
/// selector surface falls back to the person.
#[tokio::test]
async fn without_the_reviewer_even_a_selector_surface_asks_the_person() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    std::fs::write(
        codex_home.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
#[tokio::test]
async fn enabling_the_reviewer_is_not_by_itself_a_choice_of_auto() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    std::fs::write(
        codex_home.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
fn sqlite_home_env_conflict_reports_an_override() -> std::io::Result<()> {
    ...
    assert_eq!(
        warnings,
        vec![format!(
            "Environment value for `${}` is overridden by the required `sqlite_home` value {required:?} from {}.",
            whisply_state::SQLITE_HOME_ENV,
            RequirementSource::Unknown
        )]
    );
...
/// WCD-712: `No directory` runs in an app-owned root inside the Whisply home,
/// so it must not inherit the trust the person granted to whatever checkout
/// happens to contain that home. A trusted home checkout would otherwise hand
/// every `No directory` thread the authority of the person's own dotfiles
/// repository, silently and on first run.
#[tokio::test]
async fn no_directory_does_not_inherit_the_trust_of_the_home_checkout() -> std::io::Result<()> {
    let home = TempDir::new()?;
    std::fs::create_dir_all(home.path().join(".git"))?;
    let codex_home = home.path().join(".whisply");
    let neutral_workspace = codex_home.join("neutral-workspace");
    std::fs::create_dir_all(&neutral_workspace)?;

    let home_key = home.path().to_string_lossy().replace('\\', "\\\\");
    tokio::fs::write(
...
"#
    ...
    assert_eq!(
        config.active_project.trust_level, None,
        "a no-directory thread must not borrow the home checkout's trust level"
    );
    Ok(())
}

/// The same bound must leave an ordinary project alone: a trusted checkout
/// outside the Whisply home is still trusted from a subdirectory of it.
#[tokio::test]
async fn an_ordinary_trusted_checkout_is_still_trusted_from_a_subdirectory() -> std::io::Result<()>
{
    let root = TempDir::new()?;
    let repo_root = root.path().join("project");
    std::fs::create_dir_all(repo_root.join(".git"))?;
    let nested_cwd = repo_root.join("packages").join("app");
    std::fs::create_dir_all(&nested_cwd)?;
    let codex_home = root.path().join(".whisply");
    std::fs::create_dir_all(&codex_home)?;

    let workspace_key = repo_root.to_string_lossy().replace('\\', "\\\\");
    tokio::fs::write(
        codex_home.join(CONFIG_TOML_FILE),
        format!(
            r#"[projects."{workspace_key}"]
...
"#
    ...
    assert!(
        ...
        "bounding no-directory discovery must not withdraw trust from a real project"
    );
    Ok(())
}
