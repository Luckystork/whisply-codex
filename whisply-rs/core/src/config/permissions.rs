...
use whisply_config::permissions_toml::NetworkUnixSocketPermissionToml;
use whisply_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use whisply_features::NetworkProxyDomainPermissionToml;
use whisply_features::NetworkProxyModeToml;
use whisply_features::NetworkProxyUnixSocketPermissionToml;
use whisply_network_proxy::NetworkMode;
use whisply_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use whisply_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
use whisply_protocol::permissions::project_roots_glob_pattern;
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Whisply. Filesystem access will remain restricted. Upgrade Whisply if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{subpath}` is not recognized by this version of Whisply and will be ignored. Upgrade Whisply if this path is required."
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Whisply and will be ignored. Upgrade Whisply if this path is required."
            ),
        },
    );
}
