...
use whisply_config::ConfigRequirements;
use whisply_config::ConfigRequirementsToml;
use whisply_config::ConstrainedWithSource;
use whisply_config::FeatureRequirementsToml;
use whisply_config::ManagedAuthPolicy;
use whisply_config::McpServerRequirement;
use whisply_config::PluginRequirementsToml;
use whisply_config::ProfileV2Name;
use whisply_config::ResidencyRequirement;
use whisply_config::SandboxModeRequirement;
use whisply_config::Sourced;
use whisply_config::ThreadConfigLoader;
use whisply_config::config_toml::ConfigLockfileToml;
use whisply_config::config_toml::ConfigToml;
use whisply_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use whisply_config::config_toml::ProjectConfig;
use whisply_config::config_toml::RealtimeAudioConfig;
use whisply_config::config_toml::RealtimeConfig;
use whisply_config::config_toml::ThreadStoreToml;
use whisply_config::config_toml::validate_model_providers;
use whisply_config::is_inside_runtime_home;
use whisply_config::loader::load_config_layers_state;
use whisply_config::loader::project_trust_key;
use whisply_config::permissions_toml::PermissionsToml;
use whisply_config::sandbox_mode_requirement_for_permission_profile;
use whisply_config::types::ApprovalsReviewer;
use whisply_config::types::AuthCredentialsStoreMode;
use whisply_config::types::AuthKeyringBackendKind;
use whisply_config::types::History;
use whisply_config::types::McpServerConfig;
use whisply_config::types::McpServerDisabledReason;
use whisply_config::types::MemoriesConfig;
use whisply_config::types::ModelAvailabilityNuxConfig;
use whisply_config::types::Notice;
use whisply_config::types::OAuthCredentialsStoreMode;
use whisply_config::types::OtelExporterKind;
use whisply_config::types::ResumeCwdMode;
use whisply_config::types::SessionPickerViewMode;
use whisply_config::types::ToolSuggestConfig;
use whisply_config::types::ToolSuggestDisabledTool;
use whisply_config::types::ToolSuggestDiscoverable;
use whisply_config::types::TuiKeymap;
use whisply_config::types::TuiNotificationSettings;
use whisply_config::types::TuiPetAnchor;
use whisply_config::types::UriBasedFileOpener;
use whisply_config::types::WindowsSandboxModeToml;
use whisply_core_plugins::PluginLoadOutcome;
use whisply_core_plugins::PluginsConfigInput;
use whisply_exec_server::ExecutorFileSystem;
use whisply_exec_server::LOCAL_FS;
use whisply_features::CodeModeConfigToml;
use whisply_features::CurrentTimeReminderConfigToml;
use whisply_features::CurrentTimeReminderDeliveryMode;
use whisply_features::CurrentTimeSource;
use whisply_features::Feature;
use whisply_features::FeatureConfigSource;
use whisply_features::FeatureOverrides;
use whisply_features::FeatureToml;
use whisply_features::Features;
use whisply_features::FeaturesToml;
use whisply_features::MultiAgentV2ConfigToml;
use whisply_features::NetworkProxyConfigToml;
use whisply_features::TokenBudgetConfigToml;
use whisply_features::TokenBudgetMode;
use whisply_git_utils::resolve_root_git_project_for_trust;
use whisply_http_client::HttpClientFactory;
use whisply_http_client::OutboundProxyPolicy;
use whisply_install_context::InstallContext;
use whisply_login::AuthManagerConfig;
use whisply_login::AuthRouteConfig;
use whisply_login::AuthSourceMode;
use whisply_mcp::McpConfig;
use whisply_mcp::McpPluginAttribution;
use whisply_mcp::McpProtocolMode;
use whisply_mcp::McpServerRegistration;
use whisply_mcp::ResolvedMcpCatalog;
use whisply_memories_read::memory_root;
use whisply_model_provider::ProviderCapabilities;
use whisply_model_provider::whisply_provider_info;
use whisply_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use whisply_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use whisply_model_provider_info::ModelProviderInfo;
use whisply_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use whisply_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use whisply_model_provider_info::built_in_model_providers;
use whisply_model_provider_info::merge_configured_model_providers;
use whisply_models_manager::ModelsManagerConfig;
use whisply_protocol::config_types::AltScreenMode;
use whisply_protocol::config_types::AutoCompactTokenLimitScope;
use whisply_protocol::config_types::ForcedLoginMethod;
...
use whisply_protocol::config_types::SandboxMode;
...
use whisply_protocol::config_types::Verbosity;
use whisply_protocol::config_types::WebSearchConfig;
...
pub use whisply_thread_store::ExtraConfig;
use whisply_utils_absolute_path::AbsolutePathBuf;
use whisply_utils_absolute_path::AbsolutePathBufGuard;
...
pub use whisply_config::ConfigLoadOptions;
pub use whisply_config::Constrained;
pub use whisply_config::ConstraintError;
pub use whisply_config::ConstraintResult;
pub use whisply_config::LoaderOverrides;
pub use whisply_network_proxy::NetworkProxyAuditMetadata;
use whisply_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use whisply_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<AbsolutePathBuf> {
    let raw = std::env::var(whisply_state::SQLITE_HOME_ENV).ok()?;
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<whisply_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Whether the user explicitly chose the working directory.
    ///
    /// `cwd` is always populated — a no-directory session is given a private
    /// neutral workspace so path resolution still works — so its presence says
    /// nothing about intent. Behavior that must follow the user's choice, such
    /// as activating the workspace prompt profile, reads this instead and never
    /// infers a selection from `cwd`.
    pub workspace_directory_selected: bool,

    /// Whether Whisply's public `Auto` permission mode is in force.
    ///
    /// `Auto` promises one fresh review per approval. Upstream's reviewer
    /// keeps a reusable session that forks the parent transcript and carries
    /// its prior verdict count forward, which is the right default for a
    /// terminal user who opted into `Approve for me` and the wrong one here:
    /// a conversation that has already been steered must not be able to steer
    /// the decision about what it is allowed to do next. Set, every review is
    /// spawned fresh with no history and destroyed after its verdict.
    pub stateless_auto_review: bool,
    ...
    ///          https://github.com/openai/codex/blob/main/whisply-rs/rmcp-client/src/oauth.rs#L2
    /// file: WHISPLY_HOME/.credentials.json
    ...
    /// overridden by the `WHISPLY_HOME` environment variable).
    pub codex_home: AbsolutePathBuf,

    /// Resolved configuration shared by all Codex SQLite databases.
    pub sqlite: whisply_state::SqliteConfig,

    /// Directory where Codex writes log files (defaults to `$WHISPLY_HOME/log`).
    pub log_dir: PathBuf,
    ...
    /// [`whisply_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    ...
    pub otel: whisply_config::types::OtelConfig,
}
...
impl ConfigBuilder {
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&whisply_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = whisply_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    whisply_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(whisply_config::io_error_from_config_error(
...
impl Config {
    pub fn sqlite_config(&self) -> &whisply_state::SqliteConfig {
        &self.sqlite
    }
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &whisply_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            connector_snapshot:
                whisply_connectors::ConnectorSnapshot::from_plugin_capability_summaries(
    ...
    pub async fn load_default_with_cli_overrides_for_codex_home(
        ...
        let cli_layer = whisply_config::build_cli_overrides_layer(&cli_overrides);
        whisply_config::merge_toml_values(&mut merged, &cli_layer);
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let config_layer_stack = load_config_layers_state(
        ...
        &cli_overrides,
        options,
        &whisply_config::NoopThreadConfigLoader,
    )
    .await?;
...
/// Patch `WHISPLY_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(codex_home: &Path, provider: &str) -> std::io::Result<()> {
    whisply_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &whisply_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            whisply_protocol::permissions::FileSystemSandboxEntry {
                path: whisply_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: whisply_protocol::permissions::FileSystemAccessMode::Deny,
            ...
            whisply_protocol::permissions::FileSystemSandboxEntry {
                path: whisply_protocol::permissions::FileSystemPath::Path { path },
                access: whisply_protocol::permissions::FileSystemAccessMode::Deny,
                missing_path_behavior: None,
            }
        };
...
pub struct ConfigOverrides {
    ...
    /// Set when the caller knows the working directory was explicitly chosen.
    ///
    /// Left `None` by callers that cannot distinguish a chosen directory from
    /// the one they happened to start in, which resolves to not-selected.
    pub workspace_directory_selected: Option<bool>,
    /// Set when the caller is Whisply's public `Auto` permission mode.
    pub stateless_auto_review: Option<bool>,
    pub approval_policy: Option<AskForApproval>,
    pub approvals_reviewer: Option<ApprovalsReviewer>,
    /// The mode a surface starts a new thread in when the person has not set
    /// one, as opposed to `approvals_reviewer`, which overrides what they did
    /// set. Only surfaces that offer the permission selector supply this:
    /// entry points with nobody to fall back to must keep answering approvals
    /// themselves rather than waiting on a reviewer.
    pub default_approvals_reviewer: Option<ApprovalsReviewer>,
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&whisply_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let ConfigOverrides {
            ...
            cwd,
            workspace_directory_selected,
            stateless_auto_review,
        ...
        let requested_additional_writable_roots: Vec<AbsolutePathBuf> = additional_writable_roots
            ...
            .collect();
        // A thread running inside the app's own home has no repository above
        // it, so it must not pick up the trust level recorded for whatever
        // checkout happens to contain that home. See
        // `whisply_config::is_inside_runtime_home`.
        let repo_root = if is_inside_runtime_home(&resolved_cwd, codex_home.as_path()) {
            None
        } else {
        ...
        // WCD-714: a surface that offers the permission selector starts new
        // threads in `Auto`. `Auto` is not "approve everything": the reviewer
        // answers the approvals it is sure about and hands every other one back
        // to the person. Without the reviewer there is nothing to delegate to,
        // so the person answers.
        let surface_default_approvals_reviewer = default_approvals_reviewer
            .filter(|_| features.enabled(Feature::GuardianApproval))
            .unwrap_or(ApprovalsReviewer::User);
        let mut approvals_reviewer = approvals_reviewer_override
            .or(cfg.approvals_reviewer)
            .unwrap_or(surface_default_approvals_reviewer);
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(whisply_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
            ...
            && constrained_approval_policy.value() == AskForApproval::Never
        {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; Whisply would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode.",
            ));
        }
        if permission_profile_was_constrained {
        ...
        let config = Self {
            ...
            model_provider,
            cwd: resolved_cwd,
            workspace_directory_selected: workspace_directory_selected.unwrap_or(false),
            stateless_auto_review: stateless_auto_review.unwrap_or(false),
            ...
            codex_home,
            sqlite: whisply_state::SqliteConfig::from_sqlite_home(sqlite_home),
            log_dir,
