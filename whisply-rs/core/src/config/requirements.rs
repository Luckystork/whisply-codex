...
pub(super) fn push_sqlite_home_env_override_warning(
    ...
    let env = whisply_state::SQLITE_HOME_ENV;
    tracing::warn!(
        ?source,
        ?value,
        "`{env}` is overridden by an exact requirement for sqlite_home"
    );
    startup_warnings.push(format!(
        "Environment value for `${env}` is overridden by the required `sqlite_home` value {value:?} from {source}."
    ));
}
