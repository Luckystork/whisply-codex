...
use whisply_config::ConfigError;
...
use whisply_config::ConfigRequirementsWithSources;
use whisply_config::FilesystemDenyReadPattern;
...
use whisply_config::config_error_from_ignored_toml_fields;
use whisply_config::config_error_from_toml;
use whisply_config::config_error_from_typed_toml;
...
use whisply_config::loader::load_requirements_toml;
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when WHISPLY_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let config_error = config_error_from_io(&err);
    let _guard = whisply_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        whisply_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = whisply_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = whisply_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when WHISPLY_HOME/config.toml does not exist");
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![whisply_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(whisply_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(whisply_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            whisply_config::SandboxModeRequirement::ReadOnly,
            whisply_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    whisply_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(PROJECT_CONFIG_DIRECTORY)).await?;
    tokio::fs::create_dir_all(project_root.join(PROJECT_CONFIG_DIRECTORY)).await?;
...
"#,
    ...
    .await?;
    tokio::fs::write(
        nested.join(PROJECT_CONFIG_DIRECTORY).join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    assert_eq!(
        ...
        nested.join(PROJECT_CONFIG_DIRECTORY).as_path()
    );
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(PROJECT_CONFIG_DIRECTORY).as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(PROJECT_CONFIG_DIRECTORY)).await?;
    tokio::fs::create_dir_all(worktree_child.join(PROJECT_CONFIG_DIRECTORY)).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(PROJECT_CONFIG_DIRECTORY),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(PROJECT_CONFIG_DIRECTORY),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(PROJECT_CONFIG_DIRECTORY),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(PROJECT_CONFIG_DIRECTORY),
        Some("worktree-child"),
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    .await?;
    write_project_hook_config(
        &project_root.join(PROJECT_CONFIG_DIRECTORY),
        /*foo*/ None,
        "echo project root hook",
    )
    .await?;
    write_project_hook_config(
        &nested.join(PROJECT_CONFIG_DIRECTORY),
        /*foo*/ None,
        "echo nested hook",
    )
    .await?;
...
"#;
    ...
    .await?;
    tokio::fs::write(
        project_root.join(PROJECT_CONFIG_DIRECTORY).join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(PROJECT_CONFIG_DIRECTORY).join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn codex_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    ...
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(PROJECT_CONFIG_DIRECTORY);
...
mod requirements_exec_policy_tests {
    ...
    use whisply_config::ConfigRequirementsWithSources;
    use whisply_config::RequirementSource;
    use whisply_config::RequirementsExecPolicyDecisionToml;
    use whisply_config::RequirementsExecPolicyParseError;
    use whisply_config::RequirementsExecPolicyPatternTokenToml;
    use whisply_config::RequirementsExecPolicyPrefixRuleToml;
    use whisply_config::RequirementsExecPolicyToml;
    use whisply_execpolicy::Decision;
    use whisply_execpolicy::Evaluation;
    use whisply_execpolicy::RuleMatch;
    use whisply_utils_absolute_path::AbsolutePathBuf;
