...
use whisply_arg0::Arg0PathEntryGuard;
use whisply_utils_cargo_bin::CargoBinError;
...
pub use whisply_core::test_support::TestCodexResponsesRequestKind;
pub use whisply_core::test_support::responses_metadata;
use whisply_utils_absolute_path::AbsolutePathBuf;
pub use whisply_utils_absolute_path::test_support::PathBufExt;
pub use whisply_utils_absolute_path::test_support::PathExt;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    whisply_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    whisply_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(whisply_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = whisply_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("whisply-rs"));
...
pub fn find_codex_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    whisply_utils_cargo_bin::cargo_bin("codex-linux-sandbox")
}

pub async fn wait_for_event<F>(
    codex: &CodexThread,
    predicate: F,
) -> whisply_protocol::protocol::EventMsg
where
    F: FnMut(&whisply_protocol::protocol::EventMsg) -> bool,
{
...
where
    F: Fn(&whisply_protocol::protocol::EventMsg) -> Option<T>,
{
...
pub fn sandbox_env_var() -> &'static str {
    whisply_core::spawn::CODEX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    whisply_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    whisply_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    whisply_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
