...
use whisply_core::TimeProvider;
use whisply_core::config::Config;
use whisply_core::resolve_installation_id;
use whisply_core::shell::Shell;
use whisply_core::shell::get_shell_by_model_provided_path;
use whisply_core::thread_store_from_config;
use whisply_exec_server::CreateDirectoryOptions;
use whisply_exec_server::ExecutorFileSystem;
use whisply_exec_server::RemoveOptions;
use whisply_extension_api::ExtensionRegistry;
use whisply_extension_api::LoadUserInstructionsFuture;
...
use whisply_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
#[derive(Debug)]
pub struct TestEnv {
    environment: whisply_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let selection = match exec_server_url {
            Some(_) => TurnEnvironmentSelection {
                environment_id: whisply_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: PathUri::from_abs_path(&cwd),
                workspace_roots: vec![PathUri::from_abs_path(&cwd)],
            },
        ...
        let environment =
            whisply_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &whisply_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                whisply_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
impl TestCodexBuilder {
    ...
    async fn build_with_home_and_base_url(
        ...
        let local_runtime_paths = whisply_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            codex_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            whisply_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            whisply_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        resume_from: Option<PathBuf>,
        test_env: TestEnv,
        environment_manager: Arc<whisply_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestCodex> {
        ...
        let auth_manager = whisply_core::test_support::auth_manager_from_auth(auth.clone());
        let managed_gateway_client = self.managed_gateway_client.clone();
        let models_manager = self.models_manager.clone().unwrap_or_else(|| {
            whisply_core::build_models_manager_with_managed_gateway(
                &config,
                auth_manager.clone(),
        ...
        let mut thread_manager = ThreadManager::new(
            ...
            auth_manager.clone(),
            models_manager,
            whisply_core::CodexAppsToolsCache::default(),
            SessionSource::Exec,
            ...
            thread_store,
            whisply_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                let auth_manager = whisply_core::test_support::auth_manager_from_auth(auth);
                Box::pin(
                    whisply_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    whisply_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = whisply_utils_cargo_bin::cargo_bin("whisply") {
            config.codex_self_exe = Some(path);
        } else if let Ok(path) = whisply_utils_cargo_bin::cargo_bin("codex-exec") {
