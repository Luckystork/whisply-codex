...
async fn build_harness_inner(
    ...
    TestCodexHarness::with_builder(builder.with_config(move |config| {
        config.cwd = whisply_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
async fn capture_from_requests(
    mode: Mode,
    codex: &whisply_core::CodexThread,
    rollout_path: &Path,
...
async fn submit_user_input(codex: &whisply_core::CodexThread, items: Vec<UserInput>) -> Result<()> {
    codex
        .submit(Op::UserInput {
            items,
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<WHISPLY_HOME>");
            search_start = start_index + "<WHISPLY_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <WHISPLY_HOME>/skills/.system/imagegen/SKILL.md and \
         <WHISPLY_HOME>/skills/custom/SKILL.md"
    );
}
