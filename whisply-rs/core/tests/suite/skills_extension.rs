...
use whisply_skills_extension::OrchestratorSkillProvider;
use whisply_skills_extension::SkillProvider;
use whisply_skills_extension::SkillProviderSource;
...
fn catalog_extensions(
    ...
    Arc<whisply_extension_api::ExtensionRegistry<Config>>,
...
async fn production_turn_fairly_shortens_extension_catalog_descriptions() -> Result<()> {
    ...
    assert_eq!(
        warning.message,
        "Skill descriptions were shortened to fit the skills context budget. Whisply can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest."
    );
