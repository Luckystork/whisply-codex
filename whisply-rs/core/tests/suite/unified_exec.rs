...
) -> (whisply_protocol::protocol::ExecCommandEndEvent, bool) {
...
async fn unified_exec_formats_large_output_summary() -> Result<()> {
    ...
    // Keep the producer below the yield deadline so this checks completed
    // large-output handling instead of timing-sensitive partial output.
    let script = format!(
        "printf '%s\\n' 'HEAD'; yes 'token token ' | head -n {output_repetitions}; printf '%s\\n' 'TAIL'"
    );
