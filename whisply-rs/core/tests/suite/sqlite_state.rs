...
use whisply_web_search_extension::install as install_web_search_extension;
use wiremock::Mock;
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let log_db_layer = whisply_state::log_db::start(db.clone());
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = whisply_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };
