...
use whisply_extension_items::sleep::SleepItem;
...
impl whisply_extension_api::ThreadLifecycleContributor<whisply_core::config::Config>
    for SleepingRootExtension
{
    fn on_thread_start<'a>(
        &'a self,
        input: whisply_extension_api::ThreadStartInput<'a, whisply_core::config::Config>,
...
async fn queue_only_agent_mail_wakes_sleeping_root_and_persists_message() {
    ...
    let mut extensions =
        whisply_extension_api::ExtensionRegistryBuilder::<whisply_core::config::Config>::new();
    ...
    assert!(history.items.iter().any(|item| {
        matches!(
            item,
            RolloutItem::ResponseItem(whisply_protocol::models::ResponseItem::AgentMessage {
                content,
                ..
            }) if content.iter().any(|content| matches!(
                content,
                whisply_protocol::models::AgentMessageInputContent::InputText { text }
