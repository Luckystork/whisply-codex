...
use whisply_config::types::McpServerAuth;
...
use whisply_exec_server::HttpRedirectPolicy;
use whisply_exec_server::HttpRequestParams;
use whisply_features::Feature;
use whisply_http_client::HttpClientBuilder;
use whisply_login::CodexAuth;
use whisply_mcp::MCP_SANDBOX_STATE_META_CAPABILITY;
...
use whisply_utils_cargo_bin::cargo_bin;
...
pub(super) fn remote_aware_environment_id() -> String {
    if is_remote_test_environment() {
        ...
        whisply_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
async fn stdio_image_responses_are_sanitized_for_text_only_model() -> anyhow::Result<()> {
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: whisply_protocol::openai_models::ReasoningEffort::Medium,
    ...
    fixture
        ...
        .list_models(
            ...
            whisply_core::test_support::default_http_client_factory(),
        )
        .await;
...
/// This test writes to a fallback credentials file in WHISPLY_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire WHISPLY_HOME through the code.
#[test]
#[serial(codex_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 3: seed an isolated WHISPLY_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _codex_home_guard = EnvVarGuard::set("WHISPLY_HOME", temp_home.path().as_os_str());
...
fn write_fallback_oauth_tokens(
    ...
    whisply_rmcp_client::save_oauth_tokens(
        server_name,
        &tokens,
        OAuthCredentialsStoreMode::File,
        whisply_config::types::AuthKeyringBackendKind::default(),
    )
}
