...
use whisply_core::test_support::auth_manager_from_auth;
use whisply_login::CodexAuth;
use whisply_login::OPENAI_API_KEY_ENV_VAR;
...
use whisply_protocol::protocol::RealtimeNoopRequested;
...
const REALTIME_BACKEND_PROMPT: &str = whisply_prompts::BACKEND_PROMPT;
...
fn run_realtime_conversation_test_in_subprocess(
    ...
    for &key in whisply_network_proxy::PROXY_ENV_KEYS {
...
async fn seed_recent_thread(
    ...
    let mut metadata_builder = whisply_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("whisply-rs/core"))?;
    ...
    assert!(startup_context.contains("whisply-rs/"));
...
async fn conversation_flushes_assistant_deltas_every_200ms_for_v3_handoff() -> Result<()> {
    ...
    test.codex
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            codex_response_item_prefix: None,
            codex_response_handoff_mode:
                whisply_protocol::protocol::CodexResponseHandoffMode::BemTags,
