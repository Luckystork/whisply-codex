...
use whisply_core::ModelClient;
use whisply_core::NewThread;
use whisply_core::Prompt;
use whisply_core::ResponseEvent;
...
use whisply_protocol::config_types::ModelProviderAuthInfo;
...
use whisply_protocol::models::DEFAULT_IMAGE_DETAIL;
...
use whisply_protocol::models::LocalShellAction;
use whisply_protocol::models::LocalShellExecAction;
use whisply_protocol::models::LocalShellStatus;
use whisply_protocol::models::MessagePhase;
use whisply_protocol::models::ReasoningItemContent;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> whisply_core::CodexResponsesMetadata {
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: whisply_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = whisply_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = whisply_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![whisply_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = whisply_protocol::models::ResponseItem::Message {
        id: None,
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    ...
    let model = whisply_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        whisply_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &whisply_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        whisply_core::build_models_manager(&config, auth_manager),
        whisply_core::CodexAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(whisply_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(whisply_core::test_support::EmptyUserInstructionsProvider),
...
async fn azure_responses_request_includes_store_and_prefixed_item_ids() {
    ...
    let thread_id = ThreadId::new();
    let auth_manager =
        whisply_core::test_support::auth_manager_from_auth(CodexAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        thread_id,
        model.as_str(),
        model_info.slug.as_str(),
