...
use whisply_config::types::Personality;
...
use whisply_protocol::openai_models::ModelInstructionsVariables;
...
async fn disabled_personality_sends_remote_default_instructions() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex()
        .with_auth(whisply_login::CodexAuth::create_dummy_chatgpt_auth_for_testing())
