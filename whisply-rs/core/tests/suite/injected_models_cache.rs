...
use whisply_models_manager::cache::ModelsCacheEntry;
use whisply_models_manager::cache::ModelsCacheError;
use whisply_models_manager::cache::ModelsCacheFuture;
...
async fn injected_cache_hit_drives_agent_model_selection() -> Result<()> {
    ...
    let cache = Arc::new(TestModelsCache {
        entry: Some(ModelsCacheEntry {
            fetched_at: Utc::now(),
            etag: None,
            client_version: Some(whisply_models_manager::client_version_to_whole()),
