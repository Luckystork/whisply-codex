...
//! `just test -p whisply-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let mut cmd = Command::new(whisply_utils_cargo_bin::cargo_bin("whisply-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn whisply-rs");
