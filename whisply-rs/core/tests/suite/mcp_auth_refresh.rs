...
use whisply_login::ExternalAuth;
use whisply_login::ExternalAuthFuture;
use whisply_login::ExternalAuthRefreshContext;
use whisply_mcp::CODEX_APPS_MCP_SERVER_NAME;
use whisply_mcp::CodexAppsToolsCache;
...
async fn hosted_plugin_runtime_ps_mcp_tool_calls_use_current_auth_manager_token() -> Result<()> {
    ...
    let mut hosted_plugin_runtime_config = whisply_mcp::hosted_plugin_runtime_mcp_server_config(
    ...
    *bearer_token_env_var = None;
    ...
    let plugins_manager = whisply_core_plugins::PluginsManager::new(home.path().to_path_buf());
    ...
    let runtime = McpRuntime::new(McpRuntimeInput {
        ...
        codex_apps_tools_cache_key: whisply_mcp::codex_apps_tools_cache_key(Some(&expected_auth)),
