...
use whisply_backend_client::ConfigBundleResponse;
use whisply_backend_client::DeliveredTomlFragment;
use whisply_config::AbsolutePathBuf;
use whisply_config::CloudConfigFragment;
use whisply_config::CloudConfigTomlBundle;
use whisply_config::CloudRequirementsFragment;
use whisply_config::CloudRequirementsTomlBundle;
...
use whisply_login::auth::ExternalAuth;
use whisply_login::auth::ExternalAuthRefreshContext;
...
async fn auth_manager_with_agent_identity_business_plan() -> Arc<AuthManager> {
    let key_material =
        whisply_agent_identity::generate_agent_key_material().expect("generate agent key material");
...
impl ExternalAuth for TestExternalChatgptAuth {
    fn resolve(&self) -> whisply_login::ExternalAuthFuture<'_, CodexAuth> {
        Box::pin(async {
            self.current
                .read()
                .map(|auth| auth.clone())
    ...
    ) -> whisply_login::ExternalAuthFuture<'_, CodexAuth> {
        Box::pin(async {
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(
            whisply_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            whisply_backend_client::DeliveredRequirementsToml {
