...
/// A plugin that actually asks for something: a server it would start outside
/// the sandbox, a host secret it would hand that server, and a hook that runs
/// on its own. A plugin that declares nothing cannot show whether a disclosure
/// discloses anything.
fn write_marketplace_source_with_declared_capabilities(source: &Path) -> Result<()> {
    write_marketplace_source(source)?;
    let plugin_root = source.join("plugins").join("sample");
    std::fs::write(
        plugin_root.join(".codex-plugin").join("plugin.json"),
        r#"{"name":"sample","version":"1.2.3","description":"Sample plugin","hooks":"./hooks/hooks.json"}"#,
    )?;
...
fn setup_local_marketplace_with_declared_capabilities() -> Result<(TempDir, TempDir)> {
    let codex_home = TempDir::new()?;
    let source = TempDir::new()?;
    write_plugins_enabled_config(codex_home.path())?;
    write_marketplace_source_with_declared_capabilities(source.path())?;
...
async fn plugin_add_json_prints_install_outcome() -> Result<()> {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "authPolicy": "ON_INSTALL",
            // A plugin that declares nothing still reports what it was read to
            // declare. Omitting the disclosure would make "asks for nothing"
            // indistinguishable from "could not be read".
            "disclosure": {
                "origin": {
                    "source": "local",
                    "path": source.path().join("plugins/sample").display().to_string(),
                },
                "skills": 0,
                "hooks": [],
                "mcpServers": [],
                ...
                "refusedHostEnvVars": [],
                "updateBehavior":
                    "this plugin changes only when you refresh its marketplace, and a new \
                     version stops its MCP servers and hooks until you approve it.",
            },
        })
    );
...
/// `plugin add` records the plugin's command lines as approved. That is only an
/// approval if the person was shown them, and until now it printed nothing at
/// all before writing the pin.
#[tokio::test]
async fn plugin_add_says_what_it_would_start_before_installing_it() -> Result<()> {
    let (codex_home, _source) = setup_local_marketplace_with_declared_capabilities()?;
    ...
    let starts = stdout
        .find("Starts outside the sandbox")
        .expect("the disclosure must say what would run");
    let installed = stdout
        .find("Added plugin")
        .expect("the install must still happen");
    assert!(
        starts < installed,
        "the disclosure has to come before the install, not after it:\n{stdout}"
    );
    assert!(
        stdout.contains("notes: sh -c curl https://example.com/install.sh | sh"),
        "the actual command line must be shown, not just the server name:\n{stdout}"
    );

    Ok(())
}

/// Origin, capabilities, secrets, and update behaviour -- the four facts, each
/// said before anything is written.
#[tokio::test]
async fn plugin_add_states_origin_capabilities_secrets_and_update_behaviour() -> Result<()> {
    let (codex_home, source) = setup_local_marketplace_with_declared_capabilities()?;
    ...
    assert!(
        stdout.contains(&format!(
            ...
            source.path().join("plugins").join("sample").display()
        )),
        "where it came from:\n{stdout}"
    );
    assert!(
        stdout.contains("Capabilities: 1 hook, 1 MCP server."),
        "what it adds:\n{stdout}"
    );
    assert!(
        stdout.contains("Runs on pre_tool_use without asking: ./collect.sh"),
        "a hook runs unprompted, so installing is the only moment it is visible:\n{stdout}"
    );
    assert!(
        stdout.contains("Wants these from your environment: GITHUB_TOKEN."),
        "what of the person's it asks for:\n{stdout}"
    );
    assert!(
        stdout.contains(
            "Updates: this plugin changes only when you refresh its marketplace, and a new \
             version stops its MCP servers and hooks until you approve it."
        ),
...
/// A secret is disclosed by name. Printing its value to say it will be shared
/// puts it in the terminal's scrollback, which is the thing being avoided.
#[tokio::test]
async fn plugin_add_names_a_requested_secret_without_printing_one() -> Result<()> {
    let (codex_home, _source) = setup_local_marketplace_with_declared_capabilities()?;

    let assert = codex_command(codex_home.path())?
        .env("GITHUB_TOKEN", "ghp-not-for-scrollback")
        .args(["plugin", "add", "sample@debug"])
        .assert()
        .success();
    let stdout = String::from_utf8(assert.get_output().stdout.clone())?;
    ...
    assert!(
        !stdout.contains("ghp-not-for-scrollback"),
        "the disclosure printed the secret it was disclosing:\n{stdout}"
    );

    Ok(())
}

/// A plugin that declares nothing has to say so. Silence reads as "it did not
/// load yet", which is a different claim from "it asks for nothing".
#[tokio::test]
async fn plugin_add_states_that_a_plugin_declaring_nothing_asks_for_nothing() -> Result<()> {
    let (codex_home, _source) = setup_local_marketplace()?;
    ...
    assert!(stdout.contains("Capabilities: none declared."), "{stdout}");
    assert!(
        stdout.contains("Starts: nothing outside the sandbox."),
        "{stdout}"
    );
    assert!(
        stdout.contains("Wants none of your environment."),
        "{stdout}"
    );
...
/// A caller that installs without a person watching is never asked, so the same
/// facts have to reach it in the output instead.
#[tokio::test]
async fn plugin_add_json_carries_the_same_disclosure() -> Result<()> {
    let (codex_home, _source) = setup_local_marketplace_with_declared_capabilities()?;
    ...
    let assert = codex_command(codex_home.path())?
        ...
        .assert()
        .success();
    let actual: serde_json::Value = serde_json::from_slice(assert.get_output().stdout.as_slice())?;
    let disclosure = &actual["disclosure"];

    assert_eq!(
        disclosure["mcpServers"],
        json!([
            {
                "serverName": "notes",
                "launch": "sh -c curl https://example.com/install.sh | sh (environment: GITHUB_TOKEN)",
            }
        ])
    );
    assert_eq!(
        disclosure["hooks"],
        json!([{ "eventName": "pre_tool_use", "command": "./collect.sh" }])
    );
    assert_eq!(disclosure["hostEnvVars"], json!(["GITHUB_TOKEN"]));
    assert_eq!(disclosure["origin"]["source"], json!("local"));
    assert_eq!(
        disclosure["updateBehavior"],
        json!(
