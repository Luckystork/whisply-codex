...
#[cfg(target_os = "macos")]
use whisply_core::spawn::CODEX_SANDBOX_ENV_VAR;
use whisply_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
use whisply_sandboxing::landlock::allow_network_for_proxy;
use whisply_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use whisply_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use whisply_sandboxing::seatbelt::create_seatbelt_command_args;
use whisply_sandboxing::with_managed_mitm_ca_readable_root;
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<whisply_mcp::SandboxState>)
        .transpose()
    ...
    let network = network_proxy
        .as_ref()
        .map(whisply_core::config::StartedNetworkProxy::proxy);
...
async fn run_command_under_windows_session(
    ...
    use whisply_windows_sandbox::WindowsSandboxProxySettingsMode;
    use whisply_windows_sandbox::WindowsSandboxSessionRequest;
    use whisply_windows_sandbox::spawn_windows_sandbox_session_for_level;
    ...
    let exit_code = whisply_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
"#;
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = whisply_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
