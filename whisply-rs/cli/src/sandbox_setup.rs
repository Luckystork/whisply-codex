...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`whisply sandbox setup` currently requires --elevated");
        }
    }
}
...
async fn run_elevated(
    ...
    let config = ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load target user's Whisply config for sandbox provisioning")?;

    whisply_core::windows_sandbox::run_elevated_provisioning_setup(
