...
use whisply_install_context::StandalonePlatform;
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update Whisply so the JS shim provides CODEX_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("WHISPLY_HOME: {}", config.codex_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.codex_home.join(whisply_config::CONFIG_TOML_FILE);
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "WHISPLY_HOME", &config.codex_home);
...
async fn sqlite_integrity_detail(
    ...
    match whisply_state::sqlite_integrity_check(sqlite, path).await {
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match codex_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "WHISPLY_HOME was resolved without config",
        )
        .detail(format!("WHISPLY_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "WHISPLY_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
