...
//! into a one-shot CLI command while still producing a durable `whisply-login.log` artifact that
...
use whisply_login::CLIENT_ID;
use whisply_login::ServerOptions;
use whisply_login::login_with_access_token;
use whisply_login::login_with_api_key;
use whisply_login::logout_with_revoke;
use whisply_login::run_device_code_login;
...
const LOGIN_LOG_FILE_NAME: &str = "whisply-login.log";
/// Removed at startup so renaming the file does not orphan an earlier build's,
/// which would otherwise sit unbounded and unredacted forever.
const LEGACY_LOGIN_LOG_FILE_NAME: &str = "codex-login.log";
...
/// command produce a durable `whisply-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match whisply_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join(LOGIN_LOG_FILE_NAME);
    let _ = std::fs::remove_file(log_dir.join(LEGACY_LOGIN_LOG_FILE_NAME));

    // This file is the one that most needs both halves of the treatment. It
    // captures `whisply_login` at info, which is the subsystem that handles
    // authorization codes and tokens — `login::auth::util` logs the body of a
    // server error response, and an error body can echo the code that produced
    // it. It also appended for as long as the product stayed installed, with no
    // retention at all. The shared writer redacts what it is given, keeps the
    // file within its budget while the command runs, and owns the file mode so
    // the log stays readable by its owner alone.
    let log_file = match codex_whisply::RedactedLogWriter::open(
        &log_path,
        codex_whisply::MAX_LOG_FILE_BYTES,
    ) {
    ...
    let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::new("whisply_cli=info,whisply_core=info,whisply_login=info")
    });
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `whisply login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | whisply login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv CODEX_ACCESS_TOKEN | whisply login --with-access-token`.",
...
mod tests {
    ...
    use whisply_config::types::AuthCredentialsStoreMode;
    use whisply_login::AuthKeyringBackendKind;
    use whisply_login::load_auth_dot_json;
    use whisply_login::login_with_api_key;
