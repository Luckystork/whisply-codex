...
use whisply_state::LogQuery;
use whisply_state::LogRow;
...
#[command(about = "Tail Whisply logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to the Whisply home directory. Defaults to $WHISPLY_HOME or ~/.whisply.
    #[arg(long, env = "WHISPLY_HOME")]
    whisply_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --whisply-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn resolve_sqlite_config(args: &Args) -> anyhow::Result<SqliteConfig> {
    ...
    if let Some(whisply_home) = args.whisply_home.as_ref() {
        config_builder = config_builder.codex_home(whisply_home.clone());
    }
...
mod tests {
    ...
    /// Explicit database selection must not parse an overridden Whisply home.
    #[tokio::test]
    async fn direct_db_skips_whisply_home_config() {
        let whisply_home = tempfile::tempdir().expect("create Whisply home");
        std::fs::write(whisply_home.path().join("config.toml"), "model = [")
        ...
        let args = Args::try_parse_from([
            ...
            OsString::from("--whisply-home"),
            whisply_home.path().as_os_str().to_owned(),
            OsString::from("--db"),
