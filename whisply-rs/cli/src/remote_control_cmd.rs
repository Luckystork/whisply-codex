...
use anyhow::Context;
use clap::Args;
use whisply_app_server::AppServerRuntimeOptions;
use whisply_app_server::AppServerTransport;
use whisply_app_server::AppServerWebsocketAuthSettings;
use whisply_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use whisply_app_server_daemon::LifecycleOutput as AppServerLifecycleOutput;
use whisply_app_server_daemon::LifecycleStatus as AppServerLifecycleStatus;
use whisply_app_server_daemon::RemoteControlReadyOutput as AppServerRemoteControlReadyOutput;
use whisply_app_server_daemon::RemoteControlReadyStatus as AppServerRemoteControlReadyStatus;
use whisply_app_server_daemon::RemoteControlStartOutput as AppServerRemoteControlStartOutput;
...
pub(crate) async fn run(
    ...
    match command.subcommand {
        ...
        Some(RemoteControlSubcommand::Start) => {
            ...
            let output = whisply_app_server_daemon::ensure_remote_control_ready().await?;
        ...
        Some(RemoteControlSubcommand::Stop) => {
            ...
            let output = whisply_app_server_daemon::run(AppServerLifecycleCommand::Stop).await?;
            print_remote_control_stop_output(&output, command.json)?;
        }
        Some(RemoteControlSubcommand::Pair) => {
            let output = whisply_app_server_daemon::start_remote_control_pairing().await?;
...
async fn run_foreground_remote_control(
    ...
    let runtime_options = AppServerRuntimeOptions {
        remote_control_startup_mode: whisply_app_server::RemoteControlStartupMode::EnabledEphemeral,
        install_shutdown_signal_handler: false,
        psp,
        ..Default::default()
    };
    let (stop_rx, stop_signal_task) = foreground_stop_signal();
    let mut app_server_task = tokio::spawn(whisply_app_server::run_main_with_transport_options(
        arg0_paths,
...
async fn wait_for_foreground_remote_control_ready(
    ...
    whisply_app_server_daemon::enable_remote_control_on_socket(
        socket_path.as_path(),
