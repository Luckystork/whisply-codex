...
use whisply_state::ThreadStateAuditRow;
...
async fn thread_inventory_check_for_roots(
    ...
    let rows = match whisply_state::read_thread_state_audit_rows(sqlite).await {
        Ok(rows) => rows,
        Err(err) => {
...
fn missing_state_db_check(scan: RolloutScan, details: Vec<String>) -> DoctorCheck {
    ...
    if !scan.files.is_empty() {
        check = check
            .issue(
                ...
                .remedy("Start Whisply with no state DB present so startup backfill can create it from rollout files."),
        )
            .remediation(
                "Start Whisply with no state DB present so startup backfill can create it from rollout files.",
            );
    }
    if !scan.scan_errors.is_empty() || !scan.malformed_names.is_empty() || scan.reached_scan_cap {
        check = check.issue(
            ...
            .remedy("Check file permissions and unexpected files under WHISPLY_HOME sessions."),
        );
    }
    check
}
...
async fn scan_rollout_root(root: &Path, archived: bool, scan: &mut RolloutScan) {
    ...
    while let Some(dir) = dirs.pop() {
        ...
        for entry in entries {
            ...
            let logical_path = whisply_rollout::plain_rollout_path(&path);
...
async fn thread_id_from_rollout(path: &Path) -> RolloutThreadId {
    let mut lines = match whisply_rollout::open_rollout_line_reader(path).await {
        Ok(lines) => lines,
    ...
    let logical_path = whisply_rollout::plain_rollout_path(path);
    whisply_rollout::builder_from_items(&[], &logical_path)
...
fn rollout_path_key(path: &Path) -> PathBuf {
    path_key(&whisply_rollout::plain_rollout_path(path))
}
...
mod tests {
    ...
    impl Fixture {
        async fn new() -> Self {
            ...
            let _runtime = whisply_state::StateRuntime::init(
                whisply_state::SqliteConfig::new_for_testing(sqlite_home.path().abs()),
        ...
        fn sqlite(&self) -> whisply_state::SqliteConfig {
            whisply_state::SqliteConfig::new_for_testing(self.sqlite_home.path().abs())
        }
        ...
        fn write_rollout(&self, archived: bool, timestamp: &str, thread_id: &str) -> PathBuf {
            ...
            let rollout_line = RolloutLine {
                ...
                ordinal: None,
                item: RolloutItem::SessionMeta(whisply_protocol::protocol::SessionMetaLine {
                    meta: whisply_protocol::protocol::SessionMeta {
