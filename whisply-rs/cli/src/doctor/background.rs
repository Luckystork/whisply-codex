...
pub(super) async fn background_server_check(config: &Config) -> DoctorCheck {
    ...
    let socket_path = match whisply_app_server::app_server_control_socket_path(&config.codex_home) {
    ...
    if status.check_status() == CheckStatus::Warning {
        check = check.remediation("Run whisply app-server daemon version for more details.");
    }
    check
}
...
async fn socket_status(socket_path: &Path) -> SocketStatus {
    ...
    match whisply_app_server_daemon::probe_app_server_version(socket_path).await {
...
mod tests {
    ...
    fn create_socket_placeholder(config: &Config) {
        let socket_path = whisply_app_server::app_server_control_socket_path(&config.codex_home)
            .expect("socket path");
