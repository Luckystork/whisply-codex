...
use std::path::Path;
use whisply_state::RuntimeDbBackup;
use whisply_tui::LocalStateDbStartupError;
...
pub(crate) fn is_locked(detail: &str) -> bool {
    whisply_state::sqlite_error_detail_is_lock(detail)
}

pub(crate) fn is_corruption(detail: &str) -> bool {
    whisply_state::sqlite_error_detail_is_corruption(detail)
}
...
pub(crate) fn print_auto_backup_start(startup_error: &LocalStateDbStartupError) {
    eprintln!("Whisply couldn't start because its local database appears to be damaged.");
    eprintln!("Moving the damaged local database aside so Whisply can rebuild it from saved data.");
...
pub(crate) async fn backup_files_for_fresh_start(
    ...
    whisply_state::backup_runtime_db_for_fresh_start(startup_error.database_path()).await
}
...
pub(crate) fn confirm_fresh_start_rebuild(
    ...
    eprintln!("Whisply rebuilt its local database.");
    eprintln!(
        "Whisply detected a damaged local database, moved it into a backup folder, and will continue startup with a fresh database."
    );
...
pub(crate) fn print_diagnostic_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("Whisply couldn't start because its local database appears to be damaged.");
    eprintln!("Run `whisply doctor` to check your setup and get next-step guidance.");
...
pub(crate) fn print_locked_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("Whisply couldn't start because another Whisply process is using its local data.");
    eprintln!("Quit any other copies of Whisply that may still be running, then try again.");
    print_technical_details(startup_error);
}
...
mod tests {
    ...
    async fn backup_backs_up_only_failed_database_file() -> std::io::Result<()> {
        ...
        let sqlite = whisply_state::SqliteConfig::new_for_testing(temp_dir.path().abs());
