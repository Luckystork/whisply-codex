...
fn config_toml_from(source: &str) -> ConfigToml {
    toml::from_str(source).expect("parse config")
}

/// A person who turns memory off has said something about what happens next,
/// not only about the next session.
#[test]
fn turning_memory_off_stops_a_run_that_is_already_going() {
    assert!(!memories_wanted_by(&config_toml_from(
        "[features]\nmemories = false\n"
    )));
    assert!(!memories_wanted_by(&config_toml_from(
        "[features]\nmemory_tool = false\n"
...
/// Only an explicit no stops the run. Silence means the session's own answer
/// still stands, so a config that cannot be read does not quietly disable a
/// feature the person turned on.
#[test]
fn a_config_that_says_nothing_does_not_stop_anything() {
    assert!(memories_wanted_by(&config_toml_from("")));
    assert!(memories_wanted_by(&config_toml_from(
        "[features]\nmemories = true\n"
    )));
