...
use whisply_rollout::INTERACTIVE_SESSION_SOURCES;
use whisply_rollout::should_persist_response_item_for_memories;
...
async fn claim_startup_jobs(
...
) -> Option<Vec<whisply_state::Stage1JobClaim>> {
    ...
    match state_db
        ...
        .claim_stage1_jobs_for_startup(
            context.thread_id(),
            whisply_state::Stage1StartupClaimParams {
...
async fn run_jobs(
    ...
    config: Arc<Config>,
    claimed_candidates: Vec<whisply_state::Stage1JobClaim>,
...
mod job {
    ...
    pub(crate) async fn run(
        context: &MemoryStartupContext,
        config: &Config,
        claim: whisply_state::Stage1JobClaim,
        stage_one_context: &StageOneRequestContext,
    ) -> JobResult {
    ...
    pub(super) fn serialize_filtered_rollout_response_items(
        items: &[RolloutItem],
    ) -> whisply_protocol::error::Result<String> {
        let filtered = items
            .iter()
            ...
            job::serialize_filtered_rollout_response_items(&[RolloutItem::ResponseItem(
                ResponseItem::FunctionCallOutput {
                    ...
                    output: whisply_protocol::models::FunctionCallOutputPayload {
                        body: whisply_protocol::models::FunctionCallOutputBody::Text(
