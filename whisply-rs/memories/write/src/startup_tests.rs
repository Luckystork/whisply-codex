...
use whisply_git_utils::diff_since_latest_init;
use whisply_git_utils::reset_git_repository;
use whisply_login::AuthManager;
use whisply_login::CodexAuth;
use whisply_model_provider::ModelProvider;
use whisply_model_provider::ModelProviderFuture;
use whisply_model_provider::ProviderAccountResult;
...
use whisply_state::Phase2JobClaimOutcome;
...
async fn memories_startup_phase1_retains_the_supplied_provider_for_transport() -> anyhow::Result<()>
{
    ...
    context
        .stream_stage_one_prompt(&config, &whisply_core::Prompt::default(), &request_context)
        .await?;
    let request = wait_for_single_request(&stage_one).await;
    assert_eq!(
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<whisply_state::StateRuntime>> {
    let db = whisply_state::StateRuntime::init(
...
) -> (Arc<MemoryStartupContext>, Arc<whisply_core::config::Config>) {
    let config_snapshot = test.codex.config_snapshot().await;
    let mut config = test.config.clone();
    config
        .features
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    ) -> whisply_models_manager::manager::SharedModelsManager {
        self.delegate
...
) -> anyhow::Result<whisply_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.codex.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        whisply_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
