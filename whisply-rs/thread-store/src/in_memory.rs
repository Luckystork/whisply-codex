...
use whisply_rollout::persisted_rollout_items;
...
struct InMemoryThreadStoreState {
    ...
    fail_next_append_with_storage_full: bool,
...
impl InMemoryThreadStore {
    ...
    /// Causes the next non-empty durable append to fail as though the local volume is full.
    pub async fn fail_next_append_with_storage_full_for_testing(&self) {
        self.state.lock().await.fail_next_append_with_storage_full = true;
    }
    ...
    async fn append_items(&self, params: AppendThreadItemsParams) -> ThreadStoreResult<()> {
        ...
        if std::mem::take(&mut state.fail_next_append_with_storage_full) {
...
fn stored_thread_from_state(
    ...
    Ok(StoredThread {
        ...
        section: state
            ...
            .get(&thread_id)
            .cloned()
            .map(|id| whisply_state::ThreadSection {
                name: if id == whisply_state::PINNED_THREAD_SECTION_ID {
                    whisply_state::PINNED_THREAD_SECTION_NAME.to_string()
                } else {
                    id.clone()
                },
                id,
...
fn git_info_from_patch(patch: &ThreadMetadataPatch) -> Option<whisply_protocol::protocol::GitInfo> {
    ...
    Some(whisply_protocol::protocol::GitInfo {
