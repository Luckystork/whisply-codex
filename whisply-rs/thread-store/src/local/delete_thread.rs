...
use whisply_rollout::RolloutReferenceIndex;
use whisply_rollout::SESSIONS_SUBDIR;
use whisply_rollout::find_archived_thread_path_by_id_str;
use whisply_rollout::find_thread_path_by_id_str;
use whisply_rollout::remove_thread_name_entries;
...
fn referenced_thread_error(thread_id: whisply_protocol::ThreadId) -> ThreadStoreError {
...
fn delete_rollout_file(
    ...
    let plain_path = whisply_rollout::plain_rollout_path(rollout_path);
...
mod tests {
    ...
    async fn delete_thread_without_state_db_preserves_materialized_thread_history() {
        ...
        .expect("session file");
        let pool = whisply_state::open_thread_history_db(&config.sqlite)
            .await
