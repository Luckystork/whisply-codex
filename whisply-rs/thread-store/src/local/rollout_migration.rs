...
impl LocalThreadStore {
    ...
    pub async fn migrate_rollouts(
        ...
        let _maintenance_guard = match options.mode {
            ...
            RolloutMigrationMode::Apply => Some(
                whisply_rollout::try_acquire_rollout_maintenance_lock(&self.config.codex_home)
        ...
        let mut paths = find_rollout_paths(
            &self
                .config
                .codex_home
                .join(whisply_rollout::SESSIONS_SUBDIR),
        )
        .await?;
        ...
        for path in paths {
            let metadata = match whisply_rollout::read_session_meta_line(&path).await {
    ...
    async fn migrate_one_rollout(
            ...
            || current_source_metadata.modified().ok() != source_modified
        {
            return Err(ThreadStoreError::Conflict {
                message: "rollout changed while migration was staging it; close older Whisply processes and retry".to_string(),
            });
        }
    ...
    async fn write_canonical_rollout(
        ...
        while let Some(record) = read_rollout_record(&mut source, &mut bytes).await? {
            ...
            if matches!(
                &line.item,
                RolloutItem::EventMsg(whisply_protocol::protocol::EventMsg::ThreadRolledBack(_))
            ) {
    ...
    async fn cleanup_failed_unpublished_migration(
        ...
        let Ok(metadata) = whisply_rollout::read_session_meta_line(rollout_path).await else {
            return Ok(());
        };
        if metadata.meta.history_mode == ThreadHistoryMode::Paginated {
            return Ok(());
