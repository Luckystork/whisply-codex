...
use chrono::DateTime;
use chrono::Utc;
use whisply_rollout::RolloutConfig;
use whisply_rollout::RolloutRecorder;
use whisply_rollout::parse_cursor;
use whisply_state::ThreadFilterOptions;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => whisply_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => whisply_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => whisply_rollout::ThreadSortKey::RecencyAt,
    ...
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => whisply_rollout::SortDirection::Asc,
        SortDirection::Desc => whisply_rollout::SortDirection::Desc,
    };
...
async fn list_section_threads(
    ...
    let anchor = params
        .cursor
        .as_deref()
        .map(|cursor| -> ThreadStoreResult<whisply_state::Anchor> {
            ...
            let thread_id = whisply_protocol::ThreadId::from_string(thread_id).map_err(|_| {
                ThreadStoreError::InvalidRequest {
                    message: format!("invalid cursor: {cursor}"),
                }
            })?;
            Ok(whisply_state::Anchor {
                ts: timestamp,
                id: Some(thread_id),
            })
        })
    ...
    let normalized_cwd_filters = params.cwd_filters.as_ref().map(|filters| {
        filters
            .iter()
            .map(|cwd| whisply_rollout::state_db::normalize_cwd_for_state_db(cwd))
            .collect::<Vec<_>>()
    });
    let filters = ThreadFilterOptions {
        ...
        sort_key: whisply_state::SortKey::SectionPosition,
        sort_direction: match params.sort_direction {
            SortDirection::Asc => whisply_state::SortDirection::Asc,
            SortDirection::Desc => whisply_state::SortDirection::Desc,
        },
    ...
    let page = match params.relation_filter {
        Some(ThreadRelationFilter::DirectChildrenOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    whisply_state::ThreadRelationFilter::DirectChildrenOf(thread_id),
                    filters,
                )
                .await
        }
        Some(ThreadRelationFilter::DescendantsOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    whisply_state::ThreadRelationFilter::DescendantsOf(thread_id),
                    filters,
                )
                .await
        }
...
pub(super) async fn list_rollout_threads(
    state_db: Option<whisply_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&whisply_rollout::Cursor>,
    sort_key: whisply_rollout::ThreadSortKey,
    sort_direction: whisply_rollout::SortDirection,
) -> ThreadStoreResult<whisply_rollout::ThreadsPage> {
        ...
        let relation_filter = params
            ...
            .map(|relation_filter| match relation_filter {
                ThreadRelationFilter::DirectChildrenOf(parent_thread_id) => {
                    whisply_state::ThreadRelationFilter::DirectChildrenOf(parent_thread_id)
                }
                ThreadRelationFilter::DescendantsOf(ancestor_thread_id) => {
                    whisply_state::ThreadRelationFilter::DescendantsOf(ancestor_thread_id)
                }
            });
        let page = whisply_rollout::state_db::list_threads_db(
            state_db.as_deref(),
            &config.sqlite,
            params.page_size,
            cursor,
...
mod tests {
    ...
    use whisply_state::PINNED_THREAD_SECTION_ID;
    ...
    async fn list_threads_preserves_sqlite_title_search_results() {
        ...
        let runtime = whisply_state::StateRuntime::init(
            whisply_state::SqliteConfig::new_for_testing(home.path().abs()),
    ...
    async fn list_paginated_threads_uses_sqlite_name_over_legacy_compatibility() {
        ...
        runtime
            ...
            .await
            .expect("state db upsert should succeed");
        whisply_rollout::append_thread_name(home.path(), thread_id, "stale index name")
            .await
    ...
    async fn section_listing_uses_sqlite_metadata_without_reading_rollouts() {
        let home = TempDir::new().expect("temp dir");
        let config = test_config(home.path());
        let state = whisply_state::StateRuntime::init(
            config.sqlite.clone(),
            config.default_model_provider_id.clone(),
        )
        .await
        ...
        for index in 0..3 {
            ...
            whisply_rollout::state_db::reconcile_rollout(
                Some(state.as_ref()),
