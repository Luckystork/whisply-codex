...
impl LocalThreadStoreConfig {
    pub fn from_config(config: &impl whisply_rollout::RolloutConfigView) -> Self {
        Self {
    ...
    async fn thread_history_db(&self) -> ThreadStoreResult<&sqlx::SqlitePool> {
        ...
        self.thread_history_db
            .get_or_try_init(|| async {
                whisply_state::open_thread_history_db(&self.config.sqlite).await
            })
            .await
...
mod tests {
        ...
        live_thread
            .append_items(&[
                ...
                RolloutItem::EventMsg(EventMsg::TokenCount(
                    whisply_protocol::protocol::TokenCountEvent {
                        info: None,
                        rate_limits: None,
                    },
                )),
    ...
    async fn read_thread_uses_live_writer_rollout_path_for_external_resume() {
        ...
        assert!(error.to_string().contains("must be in Whisply home"));
    }
