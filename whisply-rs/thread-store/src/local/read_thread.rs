...
use whisply_rollout::find_archived_thread_path_by_id_str;
use whisply_rollout::find_thread_name_by_id;
use whisply_rollout::find_thread_path_by_id_str;
use whisply_rollout::read_session_meta_line;
use whisply_rollout::read_thread_item_from_rollout;
...
async fn sqlite_rollout_path_can_load_history_for_thread(
    store: &LocalThreadStore,
    path: &std::path::Path,
    thread_id: whisply_protocol::ThreadId,
) -> bool {
    if whisply_rollout::existing_rollout_path(path).await.is_none() {
        return false;
    }
...
async fn resolve_requested_rollout_path(
    ...
    let Some(path) = whisply_rollout::existing_rollout_path(path.as_path()).await else {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
pub(super) async fn resolve_rollout_path(
    ...
    if let Ok(path) = live_writer::rollout_path(store, thread_id).await
        && whisply_rollout::existing_rollout_path(path.as_path())
            .await
            .is_some()
...
async fn read_thread_from_rollout_path(
    ...
    thread.rollout_path = Some(whisply_rollout::plain_rollout_path(path.as_path()));
...
pub(super) async fn load_history_items(
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<whisply_protocol::protocol::RolloutItem>> {
...
pub(super) async fn stored_thread_from_sqlite_metadata(
    ...
    let session_meta = match read_required_session_meta_line(metadata.rollout_path.as_path()).await
    {
        ...
        Err(_)
            if whisply_rollout::existing_rollout_path(metadata.rollout_path.as_path())
                .await
                .is_none() =>
        {
            None
...
pub(super) fn stored_thread_from_state_metadata(
    store: &LocalThreadStore,
    metadata: ThreadMetadata,
    parent_thread_id: Option<whisply_protocol::ThreadId>,
) -> StoredThread {
    ...
    let rollout_path = whisply_rollout::plain_rollout_path(metadata.rollout_path.as_path());
...
fn stored_thread_from_meta_line(
    ...
    let rollout_path = whisply_rollout::plain_rollout_path(path.as_path());
    StoredThread {
...
mod tests {
    ...
    async fn read_thread_by_rollout_path_preserves_legacy_name_and_sqlite_git_info() {
        ...
        metadata.section = Some(whisply_state::ThreadSection {
            id: whisply_state::PINNED_THREAD_SECTION_ID.to_string(),
            name: whisply_state::PINNED_THREAD_SECTION_NAME.to_string(),
        });
        ...
        whisply_rollout::append_thread_name(home.path(), thread_id, "Latest index name")
            .await
        ...
        assert_eq!(
            ...
            Some(whisply_state::PINNED_THREAD_SECTION_ID)
        );
    ...
    async fn read_paginated_thread_prefers_sqlite_display_metadata() {
        ...
        .expect("session file");
        whisply_rollout::append_rollout_item_to_path(
            &rollout_path,
    ...
    async fn read_thread_uses_legacy_thread_name_when_sqlite_title_is_missing() {
        ...
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        whisply_rollout::append_thread_name(home.path(), thread_id, "Legacy title")
            .await
            .expect("append legacy thread name");
