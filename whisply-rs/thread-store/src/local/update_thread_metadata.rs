...
use whisply_rollout::ARCHIVED_SESSIONS_SUBDIR;
use whisply_rollout::append_rollout_item_to_path;
use whisply_rollout::append_thread_name;
...
async fn refresh_resolved_rollout_path(resolved: &mut ResolvedRolloutPath) {
    if let Some(path) = whisply_rollout::existing_rollout_path(resolved.path.as_path()).await {
        resolved.path = path;
    }
}
...
async fn metadata_for_missing_sqlite_row(
    ...
    patch: &ThreadMetadataPatch,
) -> ThreadStoreResult<whisply_state::ThreadMetadata> {
...
async fn canonical_history_mode(
    ...
    let session_meta = match read_session_meta_line(rollout_path).await {
        ...
        Err(err) => {
            if whisply_rollout::existing_rollout_path(rollout_path)
                .await
                .is_none()
...
fn normalize_cwd(cwd: PathBuf) -> PathBuf {
    whisply_utils_path::normalize_for_path_comparison(cwd.as_path()).unwrap_or(cwd)
}
...
async fn apply_thread_git_info_to_rollout(
    ...
    session_meta.git = Some(GitInfo {
        commit_hash: sha.as_deref().map(whisply_git_utils::GitSha::new),
        branch: branch.clone(),
...
mod tests {
    ...
    async fn update_thread_metadata_sets_name_on_active_rollout_and_indexes_name() {
        ...
        let latest_name = whisply_rollout::find_thread_name_by_id(home.path(), &thread_id)
            .await
    ...
    async fn section_moves_persist_in_sqlite_without_changing_the_rollout() {
        ...
        store
            .move_thread_to_section(MoveThreadToSectionParams {
                thread_id,
                section: Some(whisply_state::PINNED_THREAD_SECTION_ID.to_string()),
        ...
        assert_eq!(
            pinned.section,
            Some(whisply_state::ThreadSection {
        ...
        let pinned_page = store
            .list_threads(ListThreadsParams {
                ...
                cwd_filters: None,
                section: Some(Some(whisply_state::PINNED_THREAD_SECTION_ID.to_string())),
                archived: false,
                search_term: None,
    ...
    async fn paginated_name_updates_use_sqlite_without_rollout_writes() {
        ...
        assert_eq!(
            whisply_rollout::find_thread_name_by_id(home.path(), &thread_id)
                .await
                .expect("find thread name")
                .as_deref(),
    ...
    async fn observed_metadata_normalizes_cwd_for_list_filters() {
        ...
        let normalized_cwd = whisply_utils_path::normalize_for_path_comparison(workspace.as_path())
