...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = whisply_login::default_client::USER_AGENT_SUFFIX.lock() {
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    let ua = whisply_login::default_client::get_codex_user_agent();
    ...
    {
        headers.extend(whisply_model_provider::auth_provider_from_auth(&auth).to_auth_headers());
    }
    headers
}
