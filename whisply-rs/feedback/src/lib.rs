...
use whisply_login::AuthEnvTelemetry;
...
impl CodexFeedback {
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                Targets::new()
                    ...
                    .with_target("whisply_api::responses_websocket_timing", LevelFilter::OFF)
                    .with_target(
                        "whisply_core::post_sampling_token_estimate",
                        LevelFilter::OFF,
                    ),
            )
    }
    ...
    pub fn snapshot(&self, session_id: Option<ThreadId>) -> FeedbackSnapshot {
        ...
        // This buffer holds whatever any tracing call site emitted, and taking
        // the snapshot is the moment it stops being process-local. Scrubbing
        // here rather than at the writer means whole lines are available, so a
        // credential split across two writes cannot slip through in halves.
        let bytes = redact_snapshot_bytes(bytes);
        let tags = {
            #[allow(clippy::expect_used)]
...
/// Scrubs a whole snapshot a line at a time, using the same rules as the local
/// log sinks so the answer does not depend on which surface asked.
fn redact_snapshot_bytes(bytes: Vec<u8>) -> Vec<u8> {
    // The ring truncates at its head, so the first line can be a fragment of an
    // arbitrary byte sequence. Anything that is not text is left as it was
    // rather than scanned as if it were.
    let Ok(text) = std::str::from_utf8(&bytes) else {
        return bytes;
    };
    let mut out = String::with_capacity(text.len());
    for line in text.split_inclusive('\n') {
        out.push_str(&whisply_utils_string::redact_log_line(line));
    }
    out.into_bytes()
}
...
mod tests {
    ...
    /// Taking the snapshot is the moment this buffer stops being process-local,
    /// so it is the last place a credential can be stopped.
    #[test]
    fn a_credential_does_not_leave_the_process_in_a_snapshot() {
        let fb = CodexFeedback::new();
        {
            let mut writer = fb.make_writer().make_writer();
            writer
                .write_all(b"refresh failed: Bearer abc123def456xyz\n")
                .unwrap();
            writer
                .write_all(b"resolved account for person@example.com\n")
                .unwrap();
        }

        let snapshot = fb.snapshot(/*session_id*/ None);
        let text = String::from_utf8(snapshot.bytes).expect("valid utf-8");
        assert!(!text.contains("abc123def456xyz"), "{text}");
    ...
    fn logger_layer_excludes_high_volume_payloads() {
        ...
        tracing::trace!(target: "whisply_api::responses_websocket_timing", payload = "secret");
        tracing::trace!(
            target: "whisply_core::post_sampling_token_estimate",
