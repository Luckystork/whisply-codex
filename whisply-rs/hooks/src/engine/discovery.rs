...
use whisply_config::HookStateToml;
...
use whisply_config::TomlValue;
use whisply_config::version_for_toml;
...
fn append_matcher_groups(
    ...
    display_order: &mut i64,
    source: &HookHandlerSource<'_>,
    event_name: whisply_protocol::protocol::HookEventName,
        ...
        for (handler_index, handler) in group.hooks.iter().cloned().enumerate() {
            match handler {
                HookHandlerConfig::Command {
                    ...
                    if r#async
                        && event_name != whisply_protocol::protocol::HookEventName::SessionEnd
                    {
                        warnings.push(format!(
                    ...
                    let additional_context_limit = if matches!(
                        event_name,
                        whisply_protocol::protocol::HookEventName::PreToolUse
                            | whisply_protocol::protocol::HookEventName::PostToolUse
                            | whisply_protocol::protocol::HookEventName::SessionStart
                            | whisply_protocol::protocol::HookEventName::UserPromptSubmit
                            | whisply_protocol::protocol::HookEventName::SubagentStart
                    ) {
                        additional_context_limit
                    } else {
...
fn normalize_command_hook(
    ...
    if event_name != whisply_protocol::protocol::HookEventName::SessionEnd {
