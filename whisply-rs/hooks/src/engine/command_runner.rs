...
pub(crate) fn build_command(shell: &CommandShell, handler: &ConfiguredHandler) -> Command {
    ...
    // Before the handler's own `env`, so a hook that deliberately sets one of
    // these names still gets the value it asked for: this withholds what the
    // parent happens to be carrying, it does not overrule configuration.
