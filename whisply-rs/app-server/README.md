...
- `whisply/tool/admission/register` — Whisply-native clients may register a bounded, opaque native capability set for one exact `threadId` + `clientUserMessageId` before `turn/start`. The server binds it to the registering connection and exposes only the admitted static native tools for that new turn. It never accepts an account, broker session, file grant, target, browser tab, confirmation, or execution envelope on this route; those remain with the native owner. The corresponding server requests are `whisply/tool/admittedExecute` and `whisply/tool/admittedCancel`, and are sent only to that registering connection. This does not replace or restrict user-configured MCP servers, plugins, skills, hooks, goals, Browser/Chrome, or Computer Use owners.
