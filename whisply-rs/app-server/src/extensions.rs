...
use whisply_extension_api::AgentSpawnFuture;
use whisply_extension_api::AgentSpawner;
...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn whisply_skills_extension::SkillProvider>,
    ...
    S: AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = CodexErr> + 'static,
{
    ...
    if let Some(state_db) = state_db {
        whisply_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            whisply_otel::global(),
            thread_manager,
            goal_service,
            |config: &Config| config.features.enabled(whisply_features::Feature::Goals),
        );
    }
    ...
    whisply_guardian::install(&mut builder, guardian_agent_spawner);
    whisply_memories_extension::install(&mut builder, whisply_otel::global());
    whisply_mcp_extension::install(&mut builder);
    whisply_mcp_extension::install_executor_plugins(&mut builder, environment_manager);
    whisply_web_search_extension::install(&mut builder, auth_manager.clone());
    whisply_image_generation_extension::install(&mut builder, auth_manager, |config: &Config| {
        Some(config.codex_home.clone())
    });
    let skill_providers = whisply_skills_extension::SkillProviders::new()
        .with_executor_provider(executor_skill_provider)
        .with_orchestrator_provider(Arc::new(
            whisply_skills_extension::OrchestratorSkillProvider::new(),
        ))
        .with_host_provider(Arc::new(whisply_skills_extension::HostSkillProvider::new()));
    whisply_skills_extension::install_with_providers_and_metrics(
        &mut builder,
        skill_providers,
        whisply_otel::global(),
        |config: &Config| whisply_skills_extension::SkillsExtensionConfig {
...
mod tests {
    ...
    use whisply_protocol::protocol::ThreadGoal as CoreThreadGoal;
    use whisply_protocol::protocol::ThreadGoalStatus;
    use whisply_protocol::protocol::ThreadGoalUpdatedEvent;
