...
use whisply_protocol::items::CollabAgentTool as CoreCollabAgentTool;
use whisply_protocol::items::TurnItem as CoreTurnItem;
use whisply_protocol::models::AdditionalPermissionProfile as CoreAdditionalPermissionProfile;
use whisply_protocol::plan_tool::UpdatePlanArgs;
use whisply_protocol::protocol::CodexErrorInfo as CoreCodexErrorInfo;
...
use whisply_protocol::protocol::RealtimeEvent;
use whisply_protocol::protocol::ReviewDecision;
use whisply_protocol::protocol::SubAgentActivityKind;
...
use whisply_protocol::request_permissions::PermissionGrantScope as CorePermissionGrantScope;
use whisply_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use whisply_protocol::request_permissions::RequestPermissionsResponse as CoreRequestPermissionsResponse;
use whisply_protocol::request_user_input::RequestUserInputAnswer as CoreRequestUserInputAnswer;
use whisply_protocol::request_user_input::RequestUserInputResponse as CoreRequestUserInputResponse;
use whisply_sandboxing::policy_transforms::intersect_permission_profiles;
use whisply_shell_command::parse_command::shlex_join;
    ...
    match msg {
        ...
        EventMsg::McpStartupUpdate(update) => {
            let (status, error, failure_reason) = match update.status {
                whisply_protocol::protocol::McpStartupStatus::Starting => {
                    (McpServerStartupState::Starting, None, None)
                }
                whisply_protocol::protocol::McpStartupStatus::Ready => {
                    (McpServerStartupState::Ready, None, None)
                }
                whisply_protocol::protocol::McpStartupStatus::Failed { error, reason } => (
                    McpServerStartupState::Failed,
                    Some(error),
                    reason.map(Into::into),
                ),
                whisply_protocol::protocol::McpStartupStatus::Cancelled => {
        ...
        EventMsg::GuardianAssessment(assessment) => {
            ...
            if assessment.status == whisply_protocol::protocol::GuardianAssessmentStatus::InProgress
            ...
            let completion_status = match assessment.status {
                whisply_protocol::protocol::GuardianAssessmentStatus::Denied
                | whisply_protocol::protocol::GuardianAssessmentStatus::Aborted => {
                    Some(CommandExecutionStatus::Declined)
                }
                whisply_protocol::protocol::GuardianAssessmentStatus::TimedOut => {
                    Some(CommandExecutionStatus::Failed)
                }
                // Neither ends the command: `AskUser` means the person is
                // being asked, and their answer is what completes the item.
                whisply_protocol::protocol::GuardianAssessmentStatus::InProgress
                | whisply_protocol::protocol::GuardianAssessmentStatus::Approved
                | whisply_protocol::protocol::GuardianAssessmentStatus::AskUser => None,
            };
                ...
                Err(err) => {
                    ...
                    if let Err(err) = conversation
                        .submit(Op::ResolveElicitation {
                            ...
                            request_id: request.id,
                            decision: whisply_protocol::approvals::ElicitationAction::Cancel,
                            content: None,
                            meta: None,
                        })
                        .await
...
async fn maybe_emit_raw_response_item_completed(
    conversation_id: ThreadId,
    turn_id: &str,
    item: whisply_protocol::models::ResponseItem,
    outgoing: &ThreadScopedOutgoingMessageSender,
) {
...
fn thread_rollback_response_from_stored_thread(
    stored_thread: whisply_thread_store::StoredThread,
    session_id: String,
...
async fn on_mcp_server_elicitation_response(
    server_name: String,
    request_id: whisply_protocol::mcp::RequestId,
    pending_request_id: RequestId,
    receiver: oneshot::Receiver<ClientRequestResult>,
    conversation: Arc<CodexThread>,
    thread_state: Arc<Mutex<ThreadState>>,
...
mod tests {
    ...
    use whisply_protocol::items::AgentMessageContent as CoreAgentMessageContent;
    use whisply_protocol::items::AgentMessageItem as CoreAgentMessageItem;
    use whisply_protocol::items::DynamicToolCallItem;
    use whisply_protocol::items::DynamicToolCallStatus as CoreDynamicToolCallStatus;
    use whisply_protocol::items::SubAgentActivityItem;
    use whisply_protocol::items::TurnItem as CoreTurnItem;
    use whisply_protocol::models::FileSystemPermissions as CoreFileSystemPermissions;
    use whisply_protocol::models::NetworkPermissions as CoreNetworkPermissions;
    ...
    use whisply_protocol::plan_tool::PlanItemArg;
    use whisply_protocol::plan_tool::StepStatus;
    ...
    use whisply_thread_store::StoredThreadHistory;
    use whisply_utils_absolute_path::AbsolutePathBuf;
    ...
    fn turn_aborted_event(turn_id: &str) -> TurnAbortedEvent {
        TurnAbortedEvent {
            ...
            started_at: None,
            reason: whisply_protocol::protocol::TurnAbortReason::Interrupted,
            completed_at: Some(TEST_TURN_COMPLETED_AT),
            duration_ms: Some(TEST_TURN_DURATION_MS),
        }
    }
    ...
    fn guardian_command_assessment(
        ...
        let (risk_level, user_authorization, rationale) = match status {
            ...
            GuardianAssessmentStatus::Approved => (
                Some(whisply_protocol::protocol::GuardianRiskLevel::Low),
                Some(whisply_protocol::protocol::GuardianUserAuthorization::High),
                Some("looks safe".to_string()),
            ),
            GuardianAssessmentStatus::Denied => (
                Some(whisply_protocol::protocol::GuardianRiskLevel::High),
                Some(whisply_protocol::protocol::GuardianUserAuthorization::Low),
            ...
            GuardianAssessmentStatus::AskUser => {
                (None, None, Some("asking the user instead".to_string()))
            }
        };
        GuardianAssessmentEvent {
            id: format!("review-{id}"),
            ...
            decision_source: if matches!(status, GuardianAssessmentStatus::InProgress) {
                ...
                Some(whisply_protocol::protocol::GuardianAssessmentDecisionSource::Agent)
            },
    ...
    fn guardian_assessment_started_uses_event_turn_id_fallback() {
        ...
        let action = whisply_protocol::protocol::GuardianAssessmentAction::Command {
            source: whisply_protocol::protocol::GuardianCommandSource::Shell,
        ...
        let notification = guardian_auto_approval_review_notification(
            ...
            &GuardianAssessmentEvent {
                ...
                status: whisply_protocol::protocol::GuardianAssessmentStatus::InProgress,
                risk_level: None,
    ...
    fn guardian_assessment_completed_emits_review_payload() {
        ...
        let notification = guardian_auto_approval_review_notification(
            ...
            &GuardianAssessmentEvent {
                ...
                status: whisply_protocol::protocol::GuardianAssessmentStatus::Denied,
    ...
    fn guardian_assessment_aborted_emits_completed_review_payload() {
        let conversation_id = ThreadId::new();
        let action = whisply_protocol::protocol::GuardianAssessmentAction::NetworkAccess {
            target: "api.openai.com:443".to_string(),
            host: "api.openai.com".to_string(),
            protocol: whisply_protocol::protocol::NetworkApprovalProtocol::Https,
            port: 443,
        };
        let notification = guardian_auto_approval_review_notification(
            &conversation_id,
            ...
            &GuardianAssessmentEvent {
                ...
                started_at_ms: 1_000,
                completed_at_ms: Some(1_042),
                status: whisply_protocol::protocol::GuardianAssessmentStatus::Aborted,
                risk_level: None,
                user_authorization: None,
                rationale: None,
                decision_source: Some(
    ...
    async fn guardian_command_execution_notifications_wrap_review_lifecycle() -> Result<()> {
        ...
        let thread_manager = Arc::new(
            whisply_core::test_support::thread_manager_with_models_provider_and_home(
                CodexAuth::create_dummy_chatgpt_auth_for_testing(),
                config.model_provider.clone(),
                config.codex_home.to_path_buf(),
                Arc::new(whisply_exec_server::EnvironmentManager::default_for_tests()),
            ),
        );
        let whisply_core::NewThread {
            thread_id: conversation_id,
            thread: conversation,
            ..
        } = thread_manager
            .start_thread(whisply_core::StartThreadOptions::new(config.clone()))
            .await?;
        let thread_state = new_thread_state();
    ...
    async fn turn_started_omits_active_snapshot_items() -> Result<()> {
        ...
        let thread_state = new_thread_state();
        {
            ...
            state.track_current_turn_event(
                "turn-1",
                &EventMsg::UserMessage(whisply_protocol::protocol::UserMessageEvent {
                    client_id: None,
        ...
        apply_bespoke_event_handling(
            Event {
                id: "turn-1".to_string(),
                msg: EventMsg::TurnStarted(whisply_protocol::protocol::TurnStartedEvent {
                    turn_id: "turn-1".to_string(),
                    trace_id: None,
                    started_at: Some(42),
                    model_context_window: None,
    ...
    async fn interrupted_subagent_activity_removes_missing_thread_watch() -> Result<()> {
        ...
        let whisply_core::NewThread {
            ...
            .start_thread(whisply_core::StartThreadOptions::new(config))
            .await?;
