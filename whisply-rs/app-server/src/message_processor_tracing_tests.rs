...
async fn build_test_processor(
    ...
    let config_manager = ConfigManager::new(
        ...
        Arc::new(whisply_config::NoopThreadConfigLoader),
    );
...
/// Runs a tracing test on a thread with the stack budget the product gives its
/// runtime threads. The future is built inside that thread, because merely
/// constructing one of these on the caller's stack is enough to exhaust it.
fn run_current_thread_test_with_stack(
    name: &str,
    body: impl FnOnce() -> std::pin::Pin<Box<dyn Future<Output = Result<()>> + Send>> + Send + 'static,
) -> Result<()> {
    // Matches the budget the product gives its runtime threads; these
    // futures no longer fit in half of it.
...
fn turn_start_jsonrpc_span_parents_core_turn_spans() -> Result<()> {
    run_current_thread_test_with_stack("turn_start_jsonrpc_span_parents_core_turn_spans", || {
        Box::pin(turn_start_jsonrpc_span_parents_core_turn_spans_body())
    })
}

async fn turn_start_jsonrpc_span_parents_core_turn_spans_body() -> Result<()> {
    let mut harness = TracingHarness::new().await?;
