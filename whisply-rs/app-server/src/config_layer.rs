...
use whisply_config::ConfigLayer;
use whisply_config::ConfigLayerMetadata;
use whisply_config::ConfigLayerSource;

/// Converts a config-layer source owned by `whisply-config` into the app-server wire type owned by
...
/// Converts config-layer metadata owned by `whisply-config` into the app-server wire type owned by
/// `codex-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `whisply-config` into the app-server wire type owned by
/// `codex-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
