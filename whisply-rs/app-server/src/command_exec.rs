...
use whisply_core::config::StartedNetworkProxy;
use whisply_core::exec::ExecExpiration;
use whisply_core::exec::ExecExpirationOutcome;
use whisply_core::exec::IO_DRAIN_TIMEOUT_MS;
use whisply_core::sandboxing::ExecRequest;
use whisply_protocol::exec_output::bytes_to_string_smart;
use whisply_sandboxing::SandboxType;
use whisply_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use whisply_utils_pty::ProcessHandle;
use whisply_utils_pty::SpawnedProcess;
use whisply_utils_pty::TerminalSize;
    ...
    pub(crate) async fn start(
        ...
        if matches!(exec_request.sandbox, SandboxType::WindowsRestrictedToken) {
            ...
            tokio::spawn(async move {
                ...
                match whisply_core::sandboxing::execute_env(
                    exec_request,
                    /*stdout_stream*/ None,
                )
                .await
        ...
        let spawned = if tty {
            whisply_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            whisply_utils_pty::spawn_pipe_process(program, args, cwd.as_path(), &env, &arg0, &[])
                .await
        } else {
            whisply_utils_pty::spawn_pipe_process_no_stdin(
                program,
                args,
                cwd.as_path(),
                &env,
...
mod tests {
    ...
    fn windows_sandbox_exec_request() -> ExecRequest {
        ...
        ExecRequest::new(
            ...
            whisply_core::exec::ExecCapturePolicy::ShellTool,
