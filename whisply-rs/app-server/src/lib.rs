...
use whisply_config::NoopThreadConfigLoader;
...
use whisply_config::TextRange as CoreTextRange;
use whisply_core::ExecPolicyError;
...
use whisply_rollout::state_db as rollout_state_db;
use whisply_state::log_db;

const SQLITE_RECOVERY_CONFIG_WARNING_SUMMARY: &str = "Whisply rebuilt its local database.";
...
mod external_auth;
mod filters;
mod first_party_tool_admission;
mod fs_watch;
...
pub async fn run_main_with_transport_options(
    ...
    let otel = whisply_core::otel_init::build_provider(
        &config,
    ...
    whisply_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    whisply_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    ...
    // The brokered provider and catalog projection must share one launch-bound
    // client. A missing or malformed descriptor remains unavailable rather
    // than reconstructing provider authority from configuration.
    let managed_gateway_client = managed_gateway_client_from_environment().ok().flatten();
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        if !whisply_state::is_sqlite_corruption_error(&err)
        ...
        emit_state_db_backup_warning(&format!(
            "Whisply local database at {} appears damaged. Moving it into a backup folder so the app server can rebuild it from saved data.",
            database_path.display()
        ));
        let backups = whisply_state::backup_runtime_db_for_fresh_start(database_path.as_path())
            .await
            .map_err(|backup_err| {
                anyhow::anyhow!(
        ...
        for backup in &backups {
            emit_state_db_backup_warning(&format!(
                "Moved damaged Whisply local database file {} to {}",
...
mod tests {
    ...
    #[cfg(debug_assertions)]
    use whisply_config::LoaderOverrides;
    use whisply_config::ThreadConfigContext;
    use whisply_config::ThreadConfigLoader;
