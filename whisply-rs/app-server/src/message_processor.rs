...
use crate::first_party_tool_admission::app_server_first_party_tool_dispatcher;
...
use crate::server_tool_dispatch::app_server_server_tool_dispatcher;
...
use whisply_analytics::AppServerRpcTransport;
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_store =
            whisply_core::thread_store_from_config(config.as_ref(), state_db.clone());
        let environment_manager_for_requests = Arc::clone(&environment_manager);
        let environment_manager_for_extensions = Arc::clone(&environment_manager);
        let restriction_product = session_source.restriction_product();
        let executor_skill_provider: Arc<dyn whisply_skills_extension::SkillProvider> = Arc::new(
            whisply_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
                Arc::clone(&environment_manager_for_extensions),
                restriction_product,
            ),
        );
        let goal_service = Arc::new(GoalService::new());
        let first_party_tool_admissions = Arc::new(FirstPartyToolAdmissionStore::default());
        let server_tool_executions = Arc::new(ServerToolExecutionStore::default());
        let first_party_tool_dispatcher = app_server_first_party_tool_dispatcher(
            outgoing.clone(),
            Arc::clone(&first_party_tool_admissions),
        );
        let server_tool_dispatcher = app_server_server_tool_dispatcher(
            outgoing.clone(),
            thread_state_manager.clone(),
            installation_id.clone(),
            Arc::clone(&server_tool_executions),
        );
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ...
            let manager =
                manager.with_first_party_tool_dispatcher(first_party_tool_dispatcher.clone());
            let manager = manager.with_server_tool_dispatcher(server_tool_dispatcher.clone());
    ...
    pub(crate) async fn connection_closed(
        ...
        self.process_exec_processor
            .connection_closed(connection_id)
            .await;
        self.turn_processor.connection_closed(connection_id).await;
    ...
    async fn handle_initialized_client_request(
        ...
        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match codex_request {
            ...
            ClientRequest::ModelProviderCapabilitiesRead { params: _, .. } => self
                ...
                .await
                .map(|response| Some(response.into())),
            ClientRequest::WhisplyToolAdmissionRegister { params, .. } => {
                self.turn_processor
                    .whisply_tool_admission_register(connection_id, params)
                    .await
            }
            ClientRequest::WhisplyToolAdmissionFinish { params, .. } => {
                self.turn_processor
                    .whisply_tool_admission_finish(connection_id, params)
                    .await
            }
            ClientRequest::WhisplyToolProgressReport { params, .. } => {
                self.turn_processor
                    .whisply_tool_progress_report(connection_id, params)
                    .await
            }
            ...
            ClientRequest::WhisplyPromptCompositionRead { params, .. } => {
                self.thread_processor
                    .whisply_prompt_composition_read(params)
                    .await
            }
            ClientRequest::WhisplyCompactionRead { params, .. } => {
                self.thread_processor.whisply_compaction_read(params).await
            }
