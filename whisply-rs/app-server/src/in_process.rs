...
//!
//! # Relationship to `whisply-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `whisply-app-server-client`,
...
pub use whisply_rollout::StateDbHandle;
pub use whisply_state::log_db::LogDbLayer;
...
/// through `whisply-app-server-client`, which adds worker-task buffering,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let state_db = whisply_rollout::state_db::try_init(config.as_ref())
            .await
    ...
    /// Starting an app server in process is deep enough to exhaust a default
    /// test stack in debug builds, so these run with the budget the product
    /// gives its own runtime threads. The future is built inside that thread
    /// so it is never materialized on the caller's stack.
    fn block_on_in_process_test(
        body: impl FnOnce() -> std::pin::Pin<Box<dyn Future<Output = ()> + Send>> + Send + 'static,
    ) {
    ...
    fn in_process_start_initializes_and_handles_typed_v2_request() {
        block_on_in_process_test(|| {
            Box::pin(in_process_start_initializes_and_handles_typed_v2_request_body())
        })
    }

    async fn in_process_start_initializes_and_handles_typed_v2_request_body() {
    ...
    fn in_process_start_uses_requested_session_source_for_thread_start() {
        block_on_in_process_test(|| {
            Box::pin(in_process_start_uses_requested_session_source_for_thread_start_body())
        })
    }

    async fn in_process_start_uses_requested_session_source_for_thread_start_body() {
    ...
    fn in_process_start_clamps_zero_channel_capacity() {
        block_on_in_process_test(|| Box::pin(in_process_start_clamps_zero_channel_capacity_body()))
    }

    async fn in_process_start_clamps_zero_channel_capacity_body() {
        let client =
