...
async fn load_default_config_preserves_managed_requirements_and_selected_user_config_path() {
    ...
    assert_eq!(
        config
            ...
            .requirements()
            .managed_auth_policy(),
        whisply_config::ManagedAuthPolicy {
            allowed_login_methods: Some(vec![
                whisply_protocol::config_types::ForcedLoginMethod::Api
            ]),
...
async fn managed_auth_policy_survives_unusable_requirements_file_changes() -> Result<()> {
    ...
    let auth_manager = whisply_login::AuthManager::shared_from_config(
    ...
    assert!(
        !auth_manager
            .is_login_method_allowed(whisply_protocol::config_types::ForcedLoginMethod::Api)
    );
    assert!(
        !auth_manager
            .is_login_method_allowed(whisply_protocol::config_types::ForcedLoginMethod::Chatgpt)
    );
    assert_eq!(
