...
impl McpRequestProcessor {
    ...
    async fn mcp_server_oauth_login_response(
        ...
        let effective_servers = whisply_mcp::effective_mcp_servers(&mcp_config, auth.as_ref());
        ...
        let discovered_scopes = if scopes.is_none() && server.scopes.is_none() {
            discover_supported_scopes(
                ...
                whisply_rmcp_client::OAuthDiscoveryTimeout::Requested,
                redirect_mode,
            )
            .await
        } else {
    ...
    async fn list_mcp_server_status_task(
        ...
        request_id: ConnectionRequestId,
        params: ListMcpServerStatusParams,
        mcp_config: whisply_mcp::McpConfig,
    ...
    async fn list_mcp_server_status_response(
        ...
        let data: Vec<McpServerStatus> = server_names[start..end]
            .iter()
            .map(|name| {
                let tools = tools_by_server.get(name).cloned().unwrap_or_default();
                let unavailable_tools = shadowed_tool_names(
                    name,
                    tools.keys(),
...
/// The tool names this server publishes that the model will not receive.
///
/// A server's tools are addressed in a namespace named after it, so what is
/// refused depends on where that namespace lands. `whisply` is the one the
/// product's own capabilities answer in, and every tool offered there is
/// refused. `functions` is where an unqualified name lives, and there only a
/// name the product owns is refused. Anywhere else the server's own name
/// qualifies its tools and nothing is shadowed -- a tool listed as unavailable
/// when it works sends someone to fix a name that was never the problem.
fn shadowed_tool_names<'a>(
    server: &str,
    tool_names: impl Iterator<Item = &'a String>,
    prefix_mcp_tool_names: bool,
    non_prefixed_mcp_tool_servers: &[String],
) -> Vec<String> {
    let namespace = whisply_mcp::model_tool_namespace(
        server,
        prefix_mcp_tool_names,
        non_prefixed_mcp_tool_servers,
    );
    let mut shadowed =
        if whisply_core::first_party_tools::is_reserved_model_tool_namespace(&namespace) {
            tool_names.cloned().collect::<Vec<_>>()
        } else if namespace == whisply_protocol::DEFAULT_FUNCTION_NAMESPACE {
            tool_names
                .filter(|name| whisply_core::first_party_tools::is_reserved_model_tool_name(name))
                .cloned()
                .collect::<Vec<_>>()
        } else {
            Vec::new()
...
#[cfg(test)]
mod tests {
    use super::shadowed_tool_names;

    fn names() -> [String; 2] {
        ["browser".to_string(), "lookup".to_string()]
    }

    /// A prefixed server's tools are addressed as `mcp__notes.browser`, which
    /// nothing else answers to.
    #[test]
    fn a_prefixed_server_shadows_nothing() {
        let names = names();
    ...
    /// Dropping the prefix does not put a tool in the product's way: it is
    /// still addressed as `notes.browser`, under the server's own name.
    /// Listing it as unavailable would send someone to rename a tool that
    /// works.
    #[test]
    fn a_server_that_is_not_prefixed_still_keeps_its_own_name() {
        let names = names();
    ...
    /// A server named after the product reaches the namespace its capabilities
    /// answer in, where the model would read every one of these as something
    /// the person approved. The registry refuses all of them, so all of them
    /// are reported.
    #[test]
    fn a_server_named_after_the_product_shadows_everything_it_publishes() {
        let names = names();
        ...
        assert_eq!(
            ...
            vec!["browser".to_string(), "lookup".to_string()]
        );
        assert_eq!(
            shadowed_tool_names(
                "Whisply",
            ...
            vec!["browser".to_string(), "lookup".to_string()],
            "a namespace that only looks like the product's still reads as it"
        );
        assert!(
            shadowed_tool_names(
                "whisply",
            ...
            .is_empty(),
            "the prefix keeps it out of the product's namespace, so it is fine"
        );
    }

    /// The unqualified namespace is the one place a bare product name is
    /// reachable, and a server has to be named `functions` to land there.
    #[test]
    fn only_the_unqualified_namespace_shadows_a_product_name() {
        let names = names();
