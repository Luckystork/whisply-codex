...
use codex_app_server_protocol::WhisplyToolProgress;
use codex_app_server_protocol::WhisplyToolProgressReportParams;
use codex_app_server_protocol::WhisplyToolProgressReportResponse;
use whisply_agent_extension::AgentInvocation;
use whisply_agent_extension::AgentRun;
use whisply_agent_extension::AgentRunner;
...
use whisply_protocol::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use whisply_protocol::protocol::AdditionalContextKind as CoreAdditionalContextKind;
...
use whisply_skills::system_cache_root_dir;

use crate::first_party_tool_admission::FirstPartyToolAdmissionStore;
use crate::image_url::REMOTE_IMAGE_URL_ERROR;
use crate::image_url::is_remote_image_url;
use crate::server_tool_dispatch::ServerToolExecutionStore;
...
pub(crate) struct TurnRequestProcessor {
    ...
    first_party_tool_admissions: Arc<FirstPartyToolAdmissionStore>,
    server_tool_executions: Arc<ServerToolExecutionStore>,
}
...
impl TurnRequestProcessor {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            skills_watcher,
            first_party_tool_admissions,
            server_tool_executions,
        }
    }

    /// Accepts one owner-approved native capability set before its matching
    /// user message starts. Registration alone does not expose any tool.
    pub(crate) async fn whisply_tool_admission_register(
        &self,
        connection_id: ConnectionId,
        params: WhisplyToolAdmissionRegisterParams,
    ) -> Result<Option<ClientResponsePayload>, JSONRPCErrorError> {
        self.first_party_tool_admissions
            .register(connection_id, params)
            .await
            .map(|response| Some(response.into()))
            .map_err(|error| invalid_request(error.to_string()))
    }

    /// Releases a completed native admission without allowing a client to
    /// affect another connection, turn, or user message.
    pub(crate) async fn whisply_tool_admission_finish(
        &self,
        connection_id: ConnectionId,
        ...
        Ok(Some(
            self.first_party_tool_admissions
                .finish_turn(connection_id, params)
                .await
                .into(),
        ))
    }

    /// Publishes one step of an admitted native call that is still running.
    ///
    /// The report is only echoed onward once the store has confirmed it belongs
    /// to a call in flight on this connection, thread, and turn, so the thread a
    /// progress line lands in is the one the app-server already bound the call
    /// to rather than one the report chose for itself.
    pub(crate) async fn whisply_tool_progress_report(
        &self,
        connection_id: ConnectionId,
        ...
        let published = self
            .first_party_tool_admissions
            .report_progress(connection_id, &params)
            .await
            || self
                .server_tool_executions
                .report_progress(connection_id, &params)
        ...
        if published {
            self.outgoing
                .send_server_notification(ServerNotification::WhisplyToolProgress(
                    WhisplyToolProgress {
        ...
        Ok(Some(WhisplyToolProgressReportResponse { published }.into()))
    }

    /// Revokes native grants whose registering app-server connection closed.
    ...
    fn review_request_from_target(
        ...
        let hint = whisply_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        let first_party_tool_admission = self
            .first_party_tool_admissions
            .take_for_turn(
            ...
            .await;
        if !turn_has_input && let Some(admission) = first_party_tool_admission.as_ref() {
            self.first_party_tool_admissions
                .discard(admission.admission_id())
                .await;
            return Err(invalid_request(
                "native first-party tool admission requires user input",
            ));
        }
        let request_trace = self.request_trace_context(&request_id).await;
        let turn_id = match first_party_tool_admission {
            Some(admission) => {
                let admission_id = admission.admission_id().to_string();
                match thread
                    .submit_user_input_and_wait_for_admission_with_first_party_tool_admission(
                        turn_op,
                        request_trace,
                    ...
                    .await
                {
                    Ok(whisply_core::UserMessageAdmission::Started { turn_id }) => turn_id,
                    Ok(whisply_core::UserMessageAdmission::Steered { turn_id }) => {
                        self.first_party_tool_admissions
                            .discard(&admission_id)
                            .await;
                        turn_id
                    }
                    Err(err) => {
        ...
        if turn_has_input {
            ...
            whisply_memories_write::start_memories_startup_task(
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<whisply_protocol::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        Ok(whisply_protocol::protocol::ThreadSettingsOverrides {
            environments,
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != whisply_protocol::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
                ...
                let (message, data, error_type) = match err {
                    ...
                    SteerInputError::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            whisply_protocol::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            whisply_protocol::protocol::NonSteerableTurnKind::Compact => (
    ...
    async fn start_detached_review(
        ...
        if matches!(
            ...
            whisply_protocol::protocol::ThreadHistoryMode::Paginated
        ) {
            return Err(invalid_request(
