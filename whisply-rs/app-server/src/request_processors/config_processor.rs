...
use whisply_config::HookHandlerConfig as CoreHookHandlerConfig;
use whisply_config::ManagedHooksRequirementsToml;
use whisply_config::MatcherGroup as CoreMatcherGroup;
use whisply_config::ResidencyRequirement as CoreResidencyRequirement;
use whisply_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use whisply_core::ThreadManager;
use whisply_features::canonical_feature_for_key;
use whisply_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<whisply_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = whisply_core_plugins::toggles::collect_plugin_enabled_candidates(
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            whisply_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            whisply_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: whisply_config::ComputerUseRequirementsToml,
...
fn map_browser_use_requirements_to_api(
    browser_use: whisply_config::BrowserUseRequirementsToml,
...
fn map_network_requirements_to_api(
    network: whisply_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(whisply_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(whisply_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(whisply_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: whisply_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        whisply_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        whisply_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: whisply_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        whisply_config::NetworkUnixSocketPermissionToml::Allow => {
            NetworkUnixSocketPermission::Allow
        }
        whisply_config::NetworkUnixSocketPermissionToml::Deny => NetworkUnixSocketPermission::Deny,
    }
}
...
mod tests {
    ...
    use whisply_config::ComputerUseRequirementsToml;
    use whisply_config::ConfigRequirementsToml;
    use whisply_config::ModelsRequirementsToml;
    use whisply_config::NewThreadModelDefaultsToml;
