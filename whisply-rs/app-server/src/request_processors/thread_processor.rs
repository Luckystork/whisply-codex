...
use codex_whisply::no_directory_working_root;
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    /// Names the model reads as a native capability, plus the shell the
    /// registry reserves alongside them. Sourced from the registry's own list
    /// so the two cannot drift into disagreeing about what is protected.
    fn is_protected_capability_name(name: &str) -> bool {
        whisply_core::first_party_tools::is_reserved_model_tool_name(name)
    }
    ...
    fn validate_dynamic_tool<'a>(
        ...
        // A protected capability name outside a namespace is refused here rather
        // than dropped later. The registry already refuses it, but silently: the
        // caller declared a tool, got no error, and the model never saw it. The
        // same name inside a namespace is fine, because the model reads the
        // qualified name and cannot mistake it for the native capability -- so
        // the answer is "namespace it", and this says so.
        if namespace.is_none() && is_protected_capability_name(name) {
            return Err(format!(
                "dynamic tool name would shadow a protected Whisply capability: {name}. \
                 Declare it inside a namespace so the model can tell them apart."
            ));
        }
        if !seen.insert(name) {
        ...
        if let Err(err) = whisply_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
    ...
    for spec in tools {
        match spec {
            ...
            DynamicToolSpec::Namespace(namespace) => {
                ...
                // Namespacing is the answer given above for a protected
                // capability name, and it stops being an answer if the
                // namespace is the product's own: same-named namespaces are
                // coalesced into one group, so the tool arrives among the
                // capabilities the person approved. The registry refuses it,
                // and refusing it here is what tells the caller.
                if whisply_core::first_party_tools::is_reserved_model_tool_namespace(name) {
                    return Err(format!(
                        "dynamic tool namespace is reserved for Whisply's own capabilities: \
                         {name}. Declare these tools in a namespace of your own so the model can \
                         tell them apart."
                    ));
                }
...
impl ThreadRequestProcessor {
    ...
    /// Reports how the thread's most recent prompt was composed.
    pub(crate) async fn whisply_prompt_composition_read(
        &self,
        params: WhisplyPromptCompositionReadParams,
    ) -> Result<Option<ClientResponsePayload>, JSONRPCErrorError> {
        let (thread_id, thread) = self.load_thread(&params.thread_id).await?;
        let composition = thread.prompt_composition().ok_or_else(|| {
            invalid_request(format!("thread {thread_id} has not composed a prompt yet"))
        })?;
        Ok(Some(
            whisply_prompt_composition_response(composition).into(),
        ))
    }

    /// Reports what can be said about this conversation being shortened.
    ///
    /// Answers for a loaded thread that has never been compacted too: "nothing
    /// has been dropped and there is this much room left" is the answer
    /// someone asking about a long chat usually needs.
    pub(crate) async fn whisply_compaction_read(
        &self,
        params: WhisplyCompactionReadParams,
    ) -> Result<Option<ClientResponsePayload>, JSONRPCErrorError> {
        let (_thread_id, thread) = self.load_thread(&params.thread_id).await?;
        Ok(Some(
            whisply_compaction_response(thread.compaction_status()).into(),
        ))
    }
    ...
    async fn thread_start_inner(
        ...
        // The directory contract is the only authority on whether the user
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<whisply_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        thread_source: Option<whisply_protocol::protocol::ThreadSource>,
        ...
        if should_record_project_trust(
            ...
            let cli_overrides_for_reload = if let Err(err) =
                whisply_core::config::set_project_trust_level(
        ...
        if let Ok(Some(err)) =
            whisply_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = whisply_core::util::normalize_thread_name(&name) else {
    ...
    async fn read_stored_thread_for_resume(
        ...
        if stored_thread.archived_at.is_some() {
            ...
            return Err(invalid_request(format!(
                "session {thread_id} is archived. Run `whisply unarchive {thread_id}` to unarchive it first."
            )));
        }
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(whisply_core::util::normalize_thread_name);
        let prepared_fork = if paginated_source {
            let boundary = match (last_turn_id.as_deref(), before_turn_id.as_deref()) {
                (Some(turn_id), None) => {
                    whisply_thread_store::ForkBoundary::ThroughTurn(turn_id.to_string())
                }
                (None, Some(turn_id)) => {
                    whisply_thread_store::ForkBoundary::BeforeTurn(turn_id.to_string())
                }
                (None, None) => whisply_thread_store::ForkBoundary::Latest,
                (Some(_), Some(_)) => unreachable!("fork boundaries are mutually exclusive"),
            };
            Some(
                self.thread_store
                    .prepare_fork(whisply_thread_store::PrepareForkParams {
...
fn deserialize_stored_thread_item(
    item: whisply_thread_store::StoredThreadItem,
...
) -> (Thread, Option<whisply_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<whisply_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(whisply_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match whisply_core::parse_turn_item(item) {
                Some(whisply_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
/// Whether starting this thread may durably record its directory as a trusted
/// project.
///
/// The grant is written to the user's configuration and outlives the thread, so
/// the one case worth naming separately is `No directory`. Such a thread is
/// still given a real working directory — inside the app's own home — so that
/// execution has somewhere to run, which makes it look here exactly like a
/// thread that asked for a workspace. Trusting it would resolve the nearest
/// checkout above that private root, ordinarily the person's home directory,
/// and permanently mark their entire home as a trusted project on behalf of a
/// chat that deliberately opened no folder.
fn should_record_project_trust(
    directory_requested: bool,
    cwd: &AbsolutePathBuf,
    codex_home: &Path,
    already_trusted: bool,
    permissions_trust_project: bool,
) -> bool {
    directory_requested
        && permissions_trust_project
        && !already_trusted
        && !is_inside_runtime_home(cwd, codex_home)
}
...
fn requested_permissions_trust_project(overrides: &ConfigOverrides, cwd: &Path) -> bool {
    if matches!(
        ...
        Some(
            whisply_protocol::config_types::SandboxMode::WorkspaceWrite
                | whisply_protocol::config_types::SandboxMode::DangerFullAccess
        )
    ) {
        return true;
    }
...
fn permission_profile_trusts_project(
    profile: &whisply_protocol::models::PermissionProfile,
    cwd: &Path,
) -> bool {
    match profile {
        whisply_protocol::models::PermissionProfile::Disabled
        | whisply_protocol::models::PermissionProfile::External { .. } => true,
        whisply_protocol::models::PermissionProfile::Managed { .. } => profile
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<whisply_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {
...
/// Projects the runtime composition record onto the wire type.
///
/// Only layer identity and digests cross this boundary; the contributed text
/// stays in the runtime, so a client can prove which prompt revision ran
/// without receiving the instructions.
/// Projects the runtime's compaction record onto the wire.
///
/// The summary itself never crosses: what travels is its digest, which is
/// enough to show that a resumed thread carries the summary compaction wrote
/// and useless for reading what someone said.
fn whisply_compaction_response(
    status: codex_whisply::ConversationCompactionStatus,
) -> WhisplyCompactionReadResponse {
    WhisplyCompactionReadResponse {
        contract_version: status.contract_version,
        budget: status.budget.map(|budget| WhisplyCompactionBudget {
            used_basis_points: budget.used_basis_points(),
            context_window_tokens: budget.context_window_tokens,
            auto_compact_limit_tokens: budget.auto_compact_limit_tokens,
            limit_scope: match budget.limit_scope {
                codex_whisply::CompactionLimitScope::Total => WhisplyCompactionLimitScope::Total,
                codex_whisply::CompactionLimitScope::BodyAfterPrefix => {
                    WhisplyCompactionLimitScope::BodyAfterPrefix
                }
            },
            active_context_tokens: budget.active_context_tokens,
            scope_tokens: budget.scope_tokens,
            tokens_remaining: budget.tokens_remaining,
            threshold_reached: budget.threshold_reached,
        }),
        compaction_count: status.compaction_count,
        records: status
            .records
            .into_iter()
            .map(|record| WhisplyCompactionRecord {
                sequence: record.sequence,
                trigger: match record.trigger {
                    codex_whisply::CompactionTriggerKind::Automatic => {
                        WhisplyCompactionTrigger::Automatic
                    }
                    codex_whisply::CompactionTriggerKind::Requested => {
                        WhisplyCompactionTrigger::Requested
                    }
                },
                mechanism: match record.mechanism {
                    codex_whisply::CompactionMechanism::Summarized => {
                        WhisplyCompactionMechanism::Summarized
                    }
                    codex_whisply::CompactionMechanism::WindowReset => {
                        WhisplyCompactionMechanism::WindowReset
                    }
                },
                outcome: match record.outcome {
                    codex_whisply::CompactionOutcome::Completed => {
                        WhisplyCompactionOutcome::Completed
                    }
                    codex_whisply::CompactionOutcome::Interrupted => {
                        WhisplyCompactionOutcome::Interrupted
                    }
                    codex_whisply::CompactionOutcome::Failed => WhisplyCompactionOutcome::Failed,
                },
                active_context_tokens_before: record.active_context_tokens_before,
                active_context_tokens_after: record.active_context_tokens_after,
                summary_tokens: record.summary_tokens,
                summary_sha256: record.summary_digest,
...
fn whisply_prompt_composition_response(
    composition: codex_whisply::PromptCompositionDiagnostics,
) -> WhisplyPromptCompositionReadResponse {
    WhisplyPromptCompositionReadResponse {
        contract_version: composition.contract_version,
        profile: match composition.profile {
            codex_whisply::PromptProfile::General => WhisplyPromptProfile::General,
            codex_whisply::PromptProfile::Workspace => WhisplyPromptProfile::Workspace,
        },
        mode_id: composition.mode_id,
        layers: composition
            .layers
            .into_iter()
            .map(|layer| WhisplyPromptLayerDigest {
                layer_id: layer.layer_id,
                precedence: layer.precedence,
                source: match layer.source {
                    codex_whisply::LayerSource::NativeRuntime => {
                        WhisplyPromptLayerSource::NativeRuntime
                    }
                    codex_whisply::LayerSource::WhisplyProduct => {
                        WhisplyPromptLayerSource::WhisplyProduct
                    }
                },
                origin_id: layer.origin_id,
                byte_length: layer.byte_length as u64,
...
/// The complete upstream projection of one Whisply permission mode.
///
/// Kept as a single table because the reviewer choice and the stateless flag
/// have to agree: `AutoReview` with the flag unset silently falls back to the
/// reusable reviewer that carries conversation history between verdicts.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct WhisplyApprovalProjection {
    approval_policy: codex_app_server_protocol::AskForApproval,
    approvals_reviewer: codex_app_server_protocol::ApprovalsReviewer,
    sandbox: codex_app_server_protocol::SandboxMode,
    stateless_auto_review: bool,
}

impl WhisplyApprovalProjection {
    fn of(mode: ApprovalContractMode) -> Self {
        match mode {
            ApprovalContractMode::Ask => Self {
                approval_policy: codex_app_server_protocol::AskForApproval::OnRequest,
                approvals_reviewer: codex_app_server_protocol::ApprovalsReviewer::User,
                sandbox: codex_app_server_protocol::SandboxMode::WorkspaceWrite,
                stateless_auto_review: false,
            },
            ApprovalContractMode::Auto => Self {
                approval_policy: codex_app_server_protocol::AskForApproval::OnRequest,
                approvals_reviewer: codex_app_server_protocol::ApprovalsReviewer::AutoReview,
                sandbox: codex_app_server_protocol::SandboxMode::WorkspaceWrite,
                stateless_auto_review: true,
            },
            ApprovalContractMode::FullAccess => Self {
                approval_policy: codex_app_server_protocol::AskForApproval::Never,
                approvals_reviewer: codex_app_server_protocol::ApprovalsReviewer::User,
                sandbox: codex_app_server_protocol::SandboxMode::DangerFullAccess,
                stateless_auto_review: false,
            },
        }
    }
