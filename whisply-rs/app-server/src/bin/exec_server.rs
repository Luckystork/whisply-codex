...
//!
//! This mirrors `//whisply-rs/exec-server/testing:exec-server` so Cargo-backed
...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let argv1 = args.next();
    #[cfg(unix)]
    if argv1.as_deref() == Some(OsStr::new(whisply_exec_server::CODEX_ARG0_EXEC_HELPER_ARG1)) {
        whisply_exec_server::run_arg0_exec_helper_main();
    }
    if argv1.as_deref() == Some(OsStr::new(whisply_exec_server::CODEX_FS_HELPER_ARG1)) {
    ...
    tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()?
        .block_on(whisply_exec_server::run_main(
            "ws://127.0.0.1:0",
            runtime_paths,
