...
use whisply_exec_server::CODEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use whisply_exec_server::CODEX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
use whisply_exec_server::CODEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR;
use whisply_exec_server::CODEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR;
...
impl TestAppServer {
    ...
    /// Send a `whisply/compaction/read` JSON-RPC request.
    pub async fn send_whisply_compaction_read_request(
        &mut self,
        params: WhisplyCompactionReadParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
        self.send_request("whisply/compaction/read", params).await
    }
...
impl TestAppServerBuilder {
    ...
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
            ...
            TestAppServerEnvironment::Auto => {
                ...
                let (auto_env, delayed_exec_server) = match exec_server_delay {
                    Some(added_delay) => {
                        ...
                        let exec_server_program = whisply_utils_cargo_bin::cargo_bin("exec-server")
        ...
        let mut program = match program {
            ...
            None => whisply_utils_cargo_bin::cargo_bin("codex-app-server")
                .context("should find binary for codex-app-server")?,
        };
        let mut owned_install_dir = None;
        if !custom_program
            && whisply_utils_cargo_bin::runfiles_available()
            && let Ok(code_mode_host_program) =
                whisply_utils_cargo_bin::cargo_bin("codex-code-mode-host")
        {
