...
use whisply_config::types::OtelHttpProtocol;
...
fn direct_otlp_exporter() -> OtelExporterKind {
...
fn set_metrics_exporter(config: &mut whisply_core::config::Config) {
    config.otel.metrics_exporter = direct_otlp_exporter();
}
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = whisply_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
...
/// The metrics assertion above covers one of the three exporters the entrypoint
/// forces to `None`. Traces and logs matter just as much: the built-in Statsig
/// exporter resolves, in a release build, to an OTLP endpoint at
/// `ab.chatgpt.com` carrying a hard-coded upstream API key, so a restored trace
/// or log exporter would send a Whisply user's telemetry to a third party.
#[tokio::test]
async fn app_server_otel_entrypoint_exports_no_traces_or_logs_either() -> Result<()> {
    let codex_home = TempDir::new()?;
    let mut config = ConfigBuilder::default()
        .codex_home(codex_home.path().to_path_buf())
        .build()
        .await?;
    config.otel.exporter = direct_otlp_exporter();
    config.otel.trace_exporter = direct_otlp_exporter();
    config.otel.metrics_exporter = direct_otlp_exporter();
    ...
    assert!(
        ...
        "a configured exporter restored metrics export authority"
    );
    assert!(
        provider
            .tracing_layer::<tracing_subscriber::Registry>()
            .is_none(),
        "a configured exporter restored span export authority"
    );
    assert!(
        provider
            .logger_layer::<tracing_subscriber::Registry>()
            .is_none(),
        "a configured exporter restored log export authority"
    );
    Ok(())
}
