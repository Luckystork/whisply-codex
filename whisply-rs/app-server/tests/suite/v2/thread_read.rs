...
use uuid::Uuid;
use whisply_app_server::in_process;
use whisply_app_server::in_process::InProcessStartArgs;
...
use whisply_protocol::protocol::SessionSource as ProtocolSessionSource;
...
async fn thread_search_occurrences_reads_paginated_projection() -> Result<()> {
    ...
    let thread_id = whisply_protocol::ThreadId::default();
    ...
    store
        .create_thread(CreateThreadParams {
            ...
            history_mode: whisply_protocol::protocol::ThreadHistoryMode::Paginated,
            history_base: None,
            ...
            items: vec![
                ...
                paginated_completed_item(
                    ...
                    CoreTurnItem::UserMessage(UserMessageItem {
                        ...
                        client_id: None,
                        content: vec![
                            whisply_protocol::user_input::UserInput::Text {
                                text: "Nee".to_string(),
...
async fn thread_turns_list_reads_store_history_without_rollout_path() -> Result<()> {
    let codex_home = TempDir::new()?;
    let thread_id =
        whisply_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000123")?;
    ...
    let client = in_process::start(InProcessStartArgs {
        ...
        thread_config_loader: Arc::new(whisply_config::NoopThreadConfigLoader),
...
async fn thread_read_loaded_include_turns_reads_store_history_without_rollout_path() -> Result<()> {
    ...
    let thread_id = whisply_protocol::ThreadId::from_string(&thread.id)?;
    store
        .append_items(AppendThreadItemsParams {
            thread_id,
...
async fn thread_list_includes_store_thread_without_rollout_path() -> Result<()> {
    let codex_home = TempDir::new()?;
    let thread_id =
        whisply_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000124")?;
    let store_id = Uuid::new_v4().to_string();
    write_managed_thread_store_config(codex_home.path(), &store_id)?;
    let store = InMemoryThreadStore::for_id(store_id.clone());
    let _in_memory_store = InMemoryThreadStoreId { store_id };
