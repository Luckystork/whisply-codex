...
/// A conversation shortened on request reports what was done to it.
///
/// The wire projection is the part a person's own tooling reads, so it is
/// tested for the same three things the record itself promises: the budget the
/// runtime used, an account of the shortening, and none of what was said.
#[cfg(target_os = "macos")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn whisply_compaction_read_reports_the_budget_and_what_the_shortening_did() -> Result<()> {
    let server = responses::start_mock_server().await;
    let sse1 = responses::sse(vec![
        responses::ev_assistant_message("m1", "The card ending 4417 has a balance."),
        responses::ev_completed_with_tokens("r1", /*total_tokens*/ 400),
    ]);
    let sse2 = responses::sse(vec![
        responses::ev_assistant_message("m2", "MANUAL_COMPACT_SUMMARY"),
    ...
    let before_id = mcp
        .send_whisply_compaction_read_request(WhisplyCompactionReadParams {
            thread_id: thread_id.clone(),
        })
        .await?;
    let before: WhisplyCompactionReadResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(before_id)).await??;
    assert_eq!(before.compaction_count, 0);
    assert!(
        before.records.is_empty(),
        "a thread that has not been shortened has nothing to account for"
    );

    send_turn_and_wait(&mut mcp, &thread_id, "what is on the card ending 4417").await?;
    ...
    let _: ThreadCompactStartResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(compact_id)).await??;
    wait_for_context_compaction_completed(&mut mcp).await?;
    ...
    let read_id = mcp
        ...
        .await?;
    let after: WhisplyCompactionReadResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(read_id)).await??;

    assert_eq!(after.compaction_count, 1);
    let record = after.records.last().expect("one shortening");
    assert_eq!(
        record.trigger,
        WhisplyCompactionTrigger::Requested,
        "this one was asked for, and someone who asked is owed a different \
         account than someone whose chat was shortened out from under them"
    );
    assert_eq!(record.mechanism, WhisplyCompactionMechanism::Summarized);
    assert_eq!(record.outcome, WhisplyCompactionOutcome::Completed);
    assert!(
        record.persisted,
    ...
    assert_eq!(
        record.summary_sha256.as_ref().map(String::len),
        Some(64),
        "the digest is what lets a resumed thread be shown to carry the same \
         summary"
    );

    let budget = after.budget.as_ref().expect("the window has been measured");
    assert_eq!(budget.auto_compact_limit_tokens, Some(AUTO_COMPACT_LIMIT));
    assert!(
        budget.used_basis_points.is_some(),
        "how full the conversation is is the first thing someone asks"
    );

    let reported = serde_json::to_string(&after)?;
    assert!(
        !reported.contains("4417") && !reported.contains("MANUAL_COMPACT_SUMMARY"),
        "reading this must not be a way to read the conversation: {reported}"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn whisply_compaction_read_rejects_unknown_thread_id() -> Result<()> {
    let codex_home = TempDir::new()?;
    write_managed_compaction_config(codex_home.path(), AUTO_COMPACT_LIMIT)?;
    ...
    assert!(
        ...
        "asking about a conversation this runtime does not have must not \
         invent an empty account of one: {}",
        error.error.message
    );
