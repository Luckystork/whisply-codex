...
"#,
    ...
    assert_eq!(
        marketplace
            .path
            .as_ref()
            .map(whisply_config::AbsolutePathBuf::as_path),
...
async fn external_agent_config_detects_and_imports_project_memory_files() -> Result<()> {
    ...
    let memory_diff = whisply_git_utils::diff_since_latest_init(&memory_root).await?;
    ...
    whisply_memories_write::workspace::reset_memory_workspace_baseline(&memory_root).await?;
    ...
    assert_eq!(
        ...
        vec![
            (
                whisply_git_utils::GitBaselineChangeStatus::Deleted,
                "extensions/external_agent_import/resources/project-a/MEMORY.md",
            ),
            (
                whisply_git_utils::GitBaselineChangeStatus::Deleted,
