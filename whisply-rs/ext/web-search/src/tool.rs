...
use whisply_api::SearchClient;
use whisply_api::SearchCommands;
use whisply_api::SearchQuery;
use whisply_api::SearchRequest;
use whisply_api::SearchSettings;
use whisply_core::X_CODEX_TURN_METADATA_HEADER;
use whisply_core::web_search_action_detail;
...
use whisply_extension_api::parse_tool_input_schema_without_compaction;
use whisply_extension_items::ExtensionItem;
use whisply_extension_items::web_search::WebSearchAction;
...
impl WebSearchTool {
    async fn handle_call(&self, call: ToolCall) -> Result<Box<dyn ToolOutput>, FunctionCallError> {
        ...
        if let Some(results) = results.as_ref()
            && let Some(metrics) = whisply_otel::global()
