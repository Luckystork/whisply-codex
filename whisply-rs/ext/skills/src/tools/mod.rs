...
impl SkillToolAuthority {
    ...
    /// The scope a per-turn read budget is accounted under. Two authorities
    /// never share a package namespace, so they never share a budget key.
    fn budget_scope(&self) -> String {
        match self {
