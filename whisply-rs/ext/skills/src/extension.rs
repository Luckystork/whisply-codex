...
use whisply_core_skills::injection::HostSkillsCatalogInWorldState;
...
use whisply_extension_api::ConfigContributor;
use whisply_extension_api::ContextContributor;
use whisply_extension_api::ContextualUserFragment;
...
use whisply_extension_api::SkillInvocationContributor;
use whisply_extension_api::SkillInvocationInput;
use whisply_extension_api::SkillInvocationKind;
...
use whisply_extension_api::ToolContributor;
use whisply_extension_api::ToolExecutor;
use whisply_extension_api::TurnInputContext;
use whisply_extension_api::TurnInputContributor;
    ...
    C: Send + Sync + 'static,
{
    fn contribute<'a>(
        ...
        Box::pin(async move {
            ...
            let mentions = collect_explicit_skill_mentions(&input.user_input, &catalog);
            let selected_entries = mentions.selected;
            // Same account of the same message as the non-extension path gives,
            // so turning this extension on cannot change what a person is told
            // their `$name` did.
            for unused in &mentions.unused {
                self.emit_warning(
            ...
            let shadow_selection_turn = if config.shadow_selection_enabled {
                ...
                let shadow_selected_entries =
                    collect_explicit_skill_mentions(&input.user_input, &shadow_catalog).selected;
