...
- How to use a skill (progressive disclosure):
  1) If this turn already includes a `<skill>` block for that skill, its `SKILL.md` is already loaded. Follow those instructions as the reply guidance. Do not open, cat, exec_command, or `skills.read` that path again. Only follow extra referenced files if the already-loaded `SKILL.md` names them and this turn actually needs them.
  2) Otherwise, after deciding to use a skill, the main agent must read its `SKILL.md` completely before taking task actions. For a `file` entry, open the listed path. For an `environment resource`, call `skills.list` with `{"authority":{"kind":"executor"}}`; for an `orchestrator resource`, use `{"authority":{"kind":"orchestrator"}}`. Select the matching package and pass its exact authority, package, and `main_resource` to `skills.read`. Follow `next_cursor`; if a read is paginated, continue until EOF.
  3) When `SKILL.md` references another resource, use the same access mechanism. Resolve relative references beneath an executor skill's returned package and call `skills.read` with the same authority and package. For orchestrator skills, pass the exact referenced resource identifier with the same authority and package to `skills.read`; do not treat `skill://` identifiers as filesystem paths.
  4) If `SKILL.md` points to extra folders such as `references/`, use its routing instructions to identify the resources required for the task. The main agent must read each required instruction or reference file itself before acting on it. Do not delegate reading, summarizing, or interpreting skill instructions to a subagent. Subagents may still perform task work when the selected skill allows it.
  5) For filesystem-backed skills, prefer running or patching provided scripts instead of retyping large code blocks. For environment and orchestrator skills, use `skills.read` and the available tools; do not invent a local path.
  6) Reuse provided assets or templates through the same source access mechanism instead of recreating them.
...
- How to use a skill (progressive disclosure):
  1) If this turn already includes a `<skill>` block for that skill, its `SKILL.md` is already loaded. Follow those instructions as the reply guidance. Do not open, cat, exec_command, or `skills.read` that path again. Only follow extra referenced files if the already-loaded `SKILL.md` names them and this turn actually needs them.
  2) Otherwise, after deciding to use a skill, the main agent must expand the listed short `path` with the matching alias from `### Skill roots`, then open and read its `SKILL.md` completely before taking task actions. If a read is truncated or paginated, continue until EOF.
  3) When `SKILL.md` references relative paths (e.g., `scripts/foo.py`), resolve them relative to the directory containing that expanded `SKILL.md` first, and only consider other paths if needed.
