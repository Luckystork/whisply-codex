...
use whisply_mcp::McpResourceClientCacheKey;
...
use crate::read_budget::SkillReadBudget;
...
impl SkillsThreadState {
    pub(crate) fn new(config: SkillsExtensionConfig, orchestrator_skills_available: bool) -> Self {
        Self {
            ...
            read_budget: SkillReadBudget::default(),
        }
    }

    /// What this thread's current turn is still allowed to pull in by
    /// following skill references.
    pub(crate) fn read_budget(&self) -> &SkillReadBudget {
        &self.read_budget
    }
