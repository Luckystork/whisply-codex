...
async fn snapshot_for_config_preserves_host_precedence_for_symlinked_plugin_root() {
    ...
    assert_eq!(
        outcome.skills,
        vec![whisply_skills::SkillMetadata {
            version: None,
...
async fn skills_for_cwd_loads_repo_and_user_roots_with_local_fs() {
    ...
    let other_file_system: Arc<dyn whisply_exec_server::ExecutorFileSystem> =
        Arc::new(whisply_exec_server::LocalFileSystem::unsandboxed());
