...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<whisply_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> whisply_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => whisply_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => whisply_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: whisply_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            whisply_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            whisply_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<whisply_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &whisply_state::StateRuntime,
    thread_id: ThreadId,
    goal: &whisply_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: whisply_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: whisply_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> whisply_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => whisply_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => whisply_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => whisply_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => whisply_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => whisply_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => whisply_state::ThreadGoalStatus::Complete,
    }
}
