...
use whisply_extension_api::ThreadIdleInput;
...
use whisply_extension_api::TokenUsageContributor;
...
use whisply_extension_api::ToolLifecycleContributor;
use whisply_extension_api::ToolLifecycleFuture;
use whisply_extension_api::TurnAbortInput;
use whisply_extension_api::TurnErrorInput;
use whisply_extension_api::TurnLifecycleContributor;
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                whisply_protocol::config_types::ModeKind::Plan
            ) {
    ...
    C: Send + Sync + 'static,
{
    fn tools(
        ...
        _session_store: &ExtensionData,
        thread_store: &ExtensionData,
    ) -> Vec<Arc<dyn whisply_extension_api::ToolExecutor<whisply_extension_api::ToolCall>>> {
        let Some(runtime) = goal_runtime_handle(thread_store) else {
            return Vec::new();
        };
