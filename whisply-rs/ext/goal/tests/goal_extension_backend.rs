...
use whisply_extension_api::ExtensionEventSink;
use whisply_extension_api::ExtensionRegistryBuilder;
use whisply_extension_api::ExtensionWarning;
use whisply_extension_api::FunctionCallError;
use whisply_extension_api::NoopTurnItemEmitter;
use whisply_extension_api::ThreadResumeInput;
use whisply_extension_api::ThreadStartInput;
use whisply_extension_api::ThreadStopInput;
use whisply_extension_api::ToolCall;
use whisply_extension_api::ToolCallOutcome;
use whisply_extension_api::ToolCallSource;
use whisply_extension_api::ToolExecutor;
use whisply_extension_api::ToolFinishInput;
use whisply_extension_api::ToolPayload;
use whisply_extension_api::TurnErrorInput;
use whisply_extension_api::TurnStartInput;
use whisply_extension_api::TurnStopInput;
use whisply_goal_extension::GoalObjectiveUpdate;
use whisply_goal_extension::GoalRuntimeHandle;
use whisply_goal_extension::GoalService;
use whisply_goal_extension::GoalSetRequest;
use whisply_goal_extension::GoalTokenBudgetUpdate;
use whisply_goal_extension::install_with_backend;
...
use whisply_protocol::protocol::ThreadGoalStatus;
use whisply_protocol::protocol::TokenUsage;
use whisply_protocol::protocol::TokenUsageInfo;
use whisply_protocol::protocol::TruncationPolicy;
...
async fn budget_limited_goal_keeps_accruing_until_turn_stop() -> anyhow::Result<()> {
    ...
    assert_eq!(whisply_state::ThreadGoalStatus::BudgetLimited, goal.status);
...
async fn turn_error_usage_limit_accounts_progress_and_clears_accounting() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(whisply_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(
        vec![
            CapturedGoalEvent {
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(whisply_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
...
async fn usage_limit_plan_turn_does_not_stop_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(whisply_state::ThreadGoalStatus::Active, goal.status);
...
async fn thread_resume_rehydrates_active_goal_idle_accounting() -> anyhow::Result<()> {
    ...
    runtime
        ...
        .replace_thread_goal(
            ...
            whisply_state::ThreadGoalStatus::Active,
            /*token_budget*/ None,
        )
        .await?;
    let harness = GoalExtensionHarness::new(runtime.clone(), thread_id).await?;
...
async fn installed_tools(
    runtime: Arc<whisply_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: whisply_extension_api::ExtensionRegistry<()>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish(&self, turn_id: &str, call_id: &str, tool_name: &str) {
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = whisply_extension_api::ToolName::plain(tool_name);
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall {
    ToolCall {
        ...
        tool_name: whisply_extension_api::ToolName::plain(tool_name),
        model: "gpt-test".to_string(),
        codex_turn_metadata: None,
        truncation_policy: TruncationPolicy::Bytes(1024),
        conversation_history: whisply_extension_api::ConversationHistory::default(),
...
async fn test_runtime() -> anyhow::Result<Arc<whisply_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    whisply_state::StateRuntime::init(
        whisply_state::SqliteConfig::new_for_testing(tempdir.keep().as_path().abs()),
...
async fn seed_thread_metadata(
    runtime: &whisply_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = whisply_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .sqlite()
            .home()
...
fn protocol_status(status: whisply_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
        whisply_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        whisply_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        whisply_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        whisply_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        whisply_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        whisply_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}
