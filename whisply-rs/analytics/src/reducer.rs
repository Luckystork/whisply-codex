...
fn item_review_summary_key(pending_review: &PendingReviewState) -> Option<ToolItemKey> {
    match pending_review.subject_kind {
        ...
        | ReviewSubjectKind::FirstPartyTool => Some(ToolItemKey {
...
fn guardian_review_result(
    ...
    match status {
        // Neither resolved the approval. For `AskUser` the person's own answer
        // follows and is attributed there; emitting a resolution here would
        // count one approval twice.
        GuardianApprovalReviewStatus::InProgress | GuardianApprovalReviewStatus::AskUser => None,
...
fn guardian_review_subject_metadata(
    ...
    match action {
        ...
        GuardianApprovalReviewAction::FirstPartyTool { tool_id, .. } => (
            ReviewSubjectKind::FirstPartyTool,
            tool_id.clone(),
            ReviewTrigger::Initial,
        ),
    }
...
fn guardian_review_requested_additional_permissions(action: &GuardianApprovalReviewAction) -> bool {
    match action {
        ...
        | GuardianApprovalReviewAction::FirstPartyTool { .. } => false,
    }
}
