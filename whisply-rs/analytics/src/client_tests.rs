...
async fn capture_file_writes_exact_serialized_request() {
    ...
    let auth = whisply_login::CodexAuth::create_dummy_chatgpt_auth_for_testing();
...
async fn api_key_auth_sends_only_plugin_events_to_codex_backend() {
    ...
    let auth_manager = whisply_login::AuthManager::from_auth_for_testing(
        whisply_login::CodexAuth::from_api_key("sk-test"),
    );
...
fn track_response_ignores_unserializable_thread_responses() {
    ...
    thread_start.cwd = whisply_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
