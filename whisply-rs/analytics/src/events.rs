...
use whisply_protocol::protocol::GuardianAssessmentOutcome;
...
pub enum GuardianReviewDecision {
    ...
    Denied,
    Aborted,
    /// The review declined to decide and handed the approval to the person.
    /// Counting these as denials would overstate how often the reviewer
    /// refuses work.
    AskedUser,
}
...
pub enum GuardianReviewTerminalStatus {
    ...
    TimedOut,
    FailedClosed,
    /// Distinct from `FailedClosed`: the same failure, but the action was not
    /// refused on the reviewer's behalf; the person was asked.
    AskedUser,
}
...
pub enum GuardianReviewFailureReason {
    ...
    SessionError,
    ParseError,
    /// The action could not be put to a reviewer at all, so no review of it
    /// was possible.
    UnsupportedAction,
    /// The review ran and finished, and the reviewer said it could not tell.
    /// Distinct from every other reason here, which describe a review that
    /// never produced a judgement.
    ReviewerUncertain,
}
...
pub enum GuardianReviewSessionKind {
    ...
    TrunkReused,
    EphemeralForked,
    /// A review spawned fresh because Whisply's `Auto` permission mode is in
    /// force. Distinct from `EphemeralForked`: no parent history is forked
    /// into it, so it must not be counted as a fork.
    StatelessAuto,
}
...
pub struct GuardianReviewEventParams {
    ...
    /// Digest of the exact policy this verdict was decided under.
    pub guardian_policy_revision: Option<String>,
...
impl GuardianReviewTrackContext {
    ...
    pub(crate) fn event_params(
        ...
        GuardianReviewEventParams {
            ...
            guardian_policy_revision: result.guardian_policy_revision,
...
impl GuardianReviewAnalyticsResult {
    ...
    pub fn from_session(params: GuardianReviewSessionAnalyticsParams) -> Self {
        Self {
            ...
            guardian_policy_revision: Some(params.guardian_policy_revision),
...
pub struct GuardianReviewSessionAnalyticsParams {
    ...
    pub guardian_policy_revision: String,
...
pub(crate) struct CodexGoalEventParams {
    ...
    pub(crate) goal_status: whisply_state::ThreadGoalStatus,
