...
impl PermissionMode {
    ...
    ///
    /// One definition because the same three modes are named on several
    /// surfaces, and a person moving between the app and the terminal has to be
    /// able to tell that they are looking at the same setting.
    ...
    /// What choosing this mode actually changes, in one line.
    ///
    /// `Auto` is described by what it does — a review of the pending action —
    /// rather than as a filter that only stops unsafe things, because the
    /// review can also answer `ask`, and a person told they would only be
    /// interrupted for something unsafe would read every prompt as an accusation.
    ///
    /// The limits of `Full access` are stated separately, in
    /// [`FULL_ACCESS_WARNING`], and both belong wherever it can be chosen.
    pub const fn description(self) -> &'static str {
        match self {
            Self::Ask => "Ask you whenever an action needs approval.",
            Self::Auto => "A fresh limited review decides allow, ask, or deny.",
            Self::FullAccess => "Skip approval prompts for this thread.",
        }
    }
}

/// Stated wherever `Full access` can be chosen.
///
/// Widening execution scope is the only thing this mode does, and it is easy to
/// read as an override of everything, so the limits are named rather than left
/// to a general caution.
pub const FULL_ACCESS_WARNING: &str = "Full access does not bypass subscriptions, Usage, site, \
     file, or app rules, macOS permissions, or required final confirmation.";
...
///
/// Not the live path. `Auto` reaches the reviewer as
/// `Config::stateless_auto_review`, which makes the guardian spawn a fresh
/// review per approval; this remains an unused sketch of a packet-shaped
/// alternative. Check the guardian, not this function, when asking whether
/// `Auto` is stateless.
