...
use whisply_exec_server::ExecBackend;
use whisply_exec_server::ExecOutputStream;
...
async fn remote_sandboxed_process_preserves_custom_arg0() -> Result<()> {
    if let Some(warning) = whisply_sandboxing::system_bwrap_warning(&PermissionProfile::read_only())
    {
