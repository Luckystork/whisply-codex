...
use whisply_exec_server::ExecServerTelemetry;
use whisply_exec_server::RequestDispatchMode;
...
use whisply_test_binary_support::TestBinaryDispatchGuard;
use whisply_test_binary_support::TestBinaryDispatchMode;
use whisply_test_binary_support::configure_test_binary_dispatch;
...
fn maybe_run_exec_server_from_test_binary(guard: Option<&TestBinaryDispatchGuard>) {
    ...
    let http_client_factory = match (
        ...
        (Ok(request_url), Ok(proxy_url)) => {
            whisply_http_client::cache_system_proxy_route_for_test(&request_url, proxy_url);
    ...
    let exit_code = match runtime.block_on(whisply_exec_server::run_main_with_telemetry(
        &listen_url,
        runtime_paths,
