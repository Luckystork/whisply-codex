...
use whisply_websocket_client::WebSocketTlsMode;
...
fn stdio_command_process(stdio_command: &StdioExecServerCommand) -> Command {
    ...
    // custom executor cannot inherit those descriptors, nor the handles that
    // name them: the FDs have close-on-exec set before this spawn, so passing
    // the numbers on would only describe descriptors that are no longer there.
