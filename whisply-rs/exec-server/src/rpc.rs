        ...
        T: DeserializeOwned,
    {
        ...
        if self
            ...
            .send(JSONRPCMessage::Request(JSONRPCRequest {
                ...
                method: method.to_string(),
                params: Some(params),
                trace: whisply_otel::current_span_w3c_trace_context(),
            }))
            .await
            .is_err()
        {
    ...
    async fn rpc_client_propagates_current_trace_context() {
        ...
        let subscriber = tracing_subscriber::registry().with(
            tracing_opentelemetry::layer()
                .with_tracer(tracer)
                .with_filter(filter_fn(whisply_otel::OtelProvider::trace_export_filter)),
        );
        let _subscriber_guard = tracing::subscriber::set_default(subscriber);
        tracing::callsite::rebuild_interest_cache();
        let parent_span = tracing::info_span!("outbound-parent");
        let expected_trace = whisply_otel::span_w3c_trace_context(&parent_span)
