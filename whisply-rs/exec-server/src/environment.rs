...
impl EnvironmentManager {
    ...
    pub async fn create_for_tests(
        ...
        match Self::from_snapshot(
            provider.snapshot_inner().unwrap_or_else(|err| {
    ...
    ///
    /// If `WHISPLY_HOME/environments.toml` is present, it defines the configured
    ...
    /// Builds a manager from `WHISPLY_HOME` with an explicit outbound HTTP policy.
...
-> Result<Option<NoiseRendezvousEnvironmentConfig>, ExecServerError> {
    noise_environment_config_from_values(
        /*registry_url*/ None, /*environment_id*/ None, /*auth_token*/ None,
...
mod tests {
    ...
    fn environment_add_rejects_non_loopback_websocket_urls_without_connecting() {
        ...
        for exec_server_url in ["ws://executor.example:8765", "wss://192.0.2.1:8765"] {
            let err = manager
                .upsert_environment(
                    "executor-a".to_string(),
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = whisply_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
            std::env::current_exe().expect("current exe").as_path(),
        )
        .expect("absolute current exe");
        let path = whisply_utils_path_uri::PathUri::from_abs_path(&path);
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            whisply_protocol::models::PermissionProfile::from_runtime_permissions(
                &whisply_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
                whisply_protocol::permissions::NetworkSandboxPolicy::Restricted,
            ),
        );
