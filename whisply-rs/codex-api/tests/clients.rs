...
use whisply_api::ResponsesClient;
use whisply_api::ResponsesOptions;
use whisply_client::HttpTransport;
use whisply_client::Request;
use whisply_client::RequestBody;
use whisply_client::Response;
use whisply_client::StreamResponse;
use whisply_client::TransportError;
...
struct FlakyTransportState {
    attempts: i64,
    requests: Vec<(RequestBody, HeaderMap, whisply_client::RequestCompression)>,
}
...
impl FlakyTransport {
    ...
    fn requests(&self) -> Vec<(RequestBody, HeaderMap, whisply_client::RequestCompression)> {
        self.state
            .lock()
            .expect("flaky transport state mutex should not be poisoned")
            .requests
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(requests[0].2, whisply_client::RequestCompression::None);
    Ok(())
}
