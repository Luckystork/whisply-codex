...
fn is_whisply_nonretryable_terminal_error(error: &Error) -> bool {
    matches!(
        error.code.as_deref(),
        Some(
...
mod tests {
    ...
    #[tokio::test]
    async fn managed_gateway_failure_frame_is_visible_to_responses_parser() {
        let failure = json!({
            ...
            "response": {
                ...
                "error": {
                    "type": "whisply_error",
                    "code": "whisply_malformed_stream",
                    "message": "The model response ended without a complete terminal result.",
                },
            },
        })
        .to_string();
