...
use whisply_protocol::config_types::ToolExposureSurface;
...
pub enum McpServerDisabledReason {
    ...
    /// A plugin's MCP server no longer launches what was approved when the
    /// plugin was installed. It is not started until the new definition is
    /// reviewed, because the process runs outside the sandbox.
    PluginLaunchChanged { plugin: String },
    /// A plugin's MCP server appeared after the plugin's launches were
    /// reviewed, so nobody has agreed to run it.
    PluginServerAdded { plugin: String },
    /// The plugin's own files are no longer the ones that were recorded. The
    /// command line can be unchanged and still start a different program, so
    /// none of the plugin's servers are started until the package is reviewed.
    PluginPackageChanged {
        plugin: String,
        origin: PluginPackageOrigin,
    },
    /// A package hash was recorded for the plugin and cannot be computed again,
    /// so there is no way to tell whether the program is the recorded one.
    PluginPackageUnreadable {
        plugin: String,
        origin: PluginPackageOrigin,
    },
}

/// How a plugin's recorded package hash came to be there, which is the most a
/// refusal can honestly claim about it.
///
/// Only some plugins were ever put in front of a person. The curated cache and
/// remote bundles are installed by the product without anyone reading them, and
/// telling someone their files "changed after they approved them" when nobody
/// approved anything is the kind of small lie that teaches people to stop
/// reading the message.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PluginPackageOrigin {
    /// The plugin was shown to a person who agreed to it.
    Approved,
    /// Whisply installed the tree and measured it; nobody reviewed it.
    Installed,
}

impl PluginPackageOrigin {
    /// The moment the recorded files are being compared against, phrased to sit
    /// inside a sentence about a change.
    fn recorded_when(self) -> &'static str {
        match self {
            Self::Approved => "it was approved",
            Self::Installed => "Whisply installed them",
        }
    }
}
...
impl fmt::Display for McpServerDisabledReason {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            McpServerDisabledReason::PluginLaunchChanged { plugin } => write!(
                f,
                "the launch definition changed after {plugin} was installed; \
                 review it and run `whisply plugin trust {plugin}` to start it again"
            ),
            McpServerDisabledReason::PluginServerAdded { plugin } => write!(
                f,
                "this server was added after {plugin} was reviewed; \
                 review it and run `whisply plugin trust {plugin}` to start it"
            ),
            McpServerDisabledReason::PluginPackageChanged { plugin, origin } => write!(
                f,
                "the files {plugin} installed changed after {}; \
                 review it and run `whisply plugin trust {plugin}` to start it again",
                origin.recorded_when()
            ),
            McpServerDisabledReason::PluginPackageUnreadable { plugin, origin } => write!(
                f,
                "the files {plugin} installed can no longer be read, so they cannot be \
                 checked against what was recorded {}; review it and run \
                 `whisply plugin trust {plugin}` to start it again",
                origin.recorded_when()
            ),
        }
    }
...
/// The environment namespace that carries Whisply's own authority handles.
///
/// The gateway auth and endpoint descriptors, the native broker socket path
/// and capability descriptor, and the managed home all live here. Credentials
/// themselves are deliberately kept out of the environment -- the gateway token
/// arrives on an inherited read-once descriptor and the variable holds only its
/// number -- so what this namespace exposes is the set of handles that say
/// where the product's authority can be reached.
///
/// That is still not something a third-party subprocess has any business
/// receiving, and the namespace is reserved so that a future member is covered
/// the day it is added rather than the day someone remembers it.
pub const WHISPLY_RESERVED_ENV_PREFIX: &str = codex_whisply::RESERVED_ENV_PREFIX;

/// Whether an MCP server may read `name` from the host environment.
///
/// One rule, defined next to the handles it protects, so a server declaring a
/// variable and a helper process inheriting one cannot answer this differently.
///
/// This governs reading a value *out of the host process*. A server declaring
/// a literal value for such a name is only configuring its own child and is
/// left alone.
pub fn host_env_var_is_reserved(name: &str) -> bool {
    codex_whisply::env_var_is_reserved(name)
}

impl McpServerEnvVar {
    /// Whether this entry would pull a reserved host value into the server.
    pub fn reads_reserved_host_env(&self) -> bool {
        host_env_var_is_reserved(self.name())
    }
}

/// What a server declares it will read out of the user's environment.
///
/// Split rather than merged, because the two answer different questions. The
/// forwarded names are what the user is being asked to share. The refused ones
/// are what the server asked for and will not get, which is worth showing
/// precisely because a server reaching for Whisply's own namespace is a fact
/// about that server the user should see before installing it.
#[derive(Debug, Default, Clone, PartialEq, Eq)]
pub struct DeclaredHostEnvUsage {
    pub forwarded: Vec<String>,
    pub refused: Vec<String>,
}

impl McpServerConfig {
    /// Collects every declaration that turns a host variable into a value the
    /// server receives, across all transports.
    ///
    /// Literal `env` values are deliberately excluded: those are the server
    /// configuring its own child with values it already has, not a request for
    /// anything of the user's.
    pub fn declared_host_env_usage(&self) -> DeclaredHostEnvUsage {
        let mut names: Vec<&str> = Vec::new();
        match &self.transport {
            McpServerTransportConfig::Stdio { env_vars, .. } => {
                names.extend(env_vars.iter().map(McpServerEnvVar::name));
            }
            McpServerTransportConfig::StreamableHttp {
                ...
                if let Some(headers) = env_http_headers {
                    names.extend(headers.values().map(String::as_str));
                }
            }
        }

        let mut usage = DeclaredHostEnvUsage::default();
        for name in names {
            let bucket = if host_env_var_is_reserved(name) {
                &mut usage.refused
            } else {
                &mut usage.forwarded
            };
        ...
        for bucket in [&mut usage.forwarded, &mut usage.refused] {
