...
pub use crate::mcp_types::PluginPackageOrigin;
...
pub use whisply_protocol::config_types::ModeKind;
pub use whisply_protocol::config_types::Personality;
pub use whisply_protocol::config_types::ServiceTier;
...
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in WHISPLY_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in WHISPLY_HOME.
    Auto,
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$WHISPLY_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against WHISPLY_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
...
pub struct PluginConfig {
    ...
    /// Set once this plugin's MCP servers have been reviewed and pinned. The
    /// per-server hashes cannot carry this on their own: a plugin that declared
    /// no MCP servers when it was approved records no hashes, and a server that
    /// appears afterwards would then look like an installation that predates
    /// pinning rather than like something new to approve.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mcp_launch_pinned: Option<bool>,

    /// Hash of the files this plugin shipped when it was approved. The launch
    /// hashes cover the command line, which is not the program: `node
    /// server.js` reads the same before and after `server.js` is rewritten, and
    /// an installed tree can be edited in place. When this no longer matches,
    /// the plugin's MCP servers are not started.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct PluginMcpServerConfig {
    ...
    /// Hash of the launch definition that was approved when this plugin was
    /// installed. A plugin MCP server starts a process outside the sandbox, and
    /// a local plugin's manifest is re-read on every load, so without this the
    /// approved command could be replaced with no second question asked. When
    /// the recorded hash no longer matches, the server is not started.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub trusted_launch_hash: Option<String>,
