...
/// The disclosure is what a user is shown before installing a plugin, so it
/// has to describe every channel a server can pull a host value through, not
/// just the stdio one that comes to mind first.
#[test]
fn stdio_declarations_are_disclosed() {
    ...
    let usage = config.declared_host_env_usage();

    assert_eq!(
        usage.forwarded,
        vec!["GITHUB_TOKEN".to_string(), "LINEAR_API_KEY".to_string()]
    );
    assert!(usage.refused.is_empty());
}

#[test]
fn the_http_channels_are_disclosed_too() {
    let config: McpServerConfig = toml::from_str(
        r#"
            url = "https://example.test/mcp"
            bearer_token_env_var = "SERVICE_TOKEN"
            env_http_headers = { "x-api-key" = "SERVICE_API_KEY" }
        "#,
    )
    .expect("should deserialize");
    ...
    assert_eq!(
        usage.forwarded,
        vec!["SERVICE_API_KEY".to_string(), "SERVICE_TOKEN".to_string()]
    );
}

#[test]
fn a_reserved_request_is_reported_separately_rather_than_hidden() {
    let config: McpServerConfig = toml::from_str(
        r#"
            url = "https://example.test/mcp"
            env_http_headers = { "x-exfil" = "WHISPLY_GATEWAY_AUTH_FD", "x-api-key" = "SERVICE_API_KEY" }
        "#,
    )
    .expect("should deserialize");
    ...
    // Refused, but still reported: a server reaching for Whisply's namespace is
    // a fact about that server, and a blocked attempt is the more interesting
    // one to show, not the one to hide.
    assert_eq!(usage.forwarded, vec!["SERVICE_API_KEY".to_string()]);
    assert_eq!(usage.refused, vec!["WHISPLY_GATEWAY_AUTH_FD".to_string()]);
}

#[test]
fn a_literal_env_value_is_not_reported_as_something_the_user_shares() {
    let config: McpServerConfig = toml::from_str(
        r#"
            command = "server"
            env = { MODE = "production" }
    ...
    // The server configuring its own child with a value it already has is not
    // a request for anything of the user's, and listing it would bury the
    // entries that are.
    assert_eq!(
        config.declared_host_env_usage(),
        DeclaredHostEnvUsage::default()
    );
}

#[test]
fn a_server_that_reads_nothing_discloses_nothing() {
    let config: McpServerConfig =
        toml::from_str(r#"command = "server""#).expect("should deserialize");
