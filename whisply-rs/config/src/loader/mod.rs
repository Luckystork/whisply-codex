...
use whisply_file_system::ExecutorFileSystem;
...
async fn project_trust_context(
    ...
    let project_root = find_project_root(fs, cwd, project_root_markers, config_base_dir).await?;
    ...
    let checkout_root = find_git_checkout_root(fs, cwd, config_base_dir).await;
    // Same bound as the two searches above, applied to the shared git helper
    // rather than reaching into it: a thread running inside the app's own home
    // has no repository, so it cannot borrow the trust decision of one.
    let repo_root = if is_inside_runtime_home(cwd, config_base_dir) {
        None
    } else {
...
/// Whether this working directory belongs to the app rather than to a project.
///
/// A `No directory` thread runs in an app-owned root inside the Whisply home
/// precisely so that no project is inferred for it. Everything above that root
/// is the app's own state or the person's home directory, and neither is the
/// thread's project — so no upward search for a project marker, a config layer
/// or a repository may leave the runtime home.
///
/// Home directories being git checkouts is ordinary; a dotfiles repository is
/// the common case. Without this bound the first marker found above the neutral
/// root is the person's entire home, and it arrives carrying their `AGENTS.md`,
/// their project config layer, and their repository identity — for a thread
/// whose whole promise is that none of that was inferred.
///
/// An ordinary thread's cwd is never inside the Whisply home, so for every real
/// project this changes nothing.
pub fn is_inside_runtime_home(cwd: &AbsolutePathBuf, whisply_home: &Path) -> bool {
    // Resolving links matters: the runtime home is reached through a symlink on
    // some installs, and a purely textual comparison would miss it.
    if let Some((cwd, home)) = normalize_path(cwd.as_path())
        .ok()
        .zip(normalize_path(whisply_home).ok())
    {
        return cwd.starts_with(&home);
    }
    // `dunce::canonicalize` needs both paths to exist. When one does not — the
    // home is still being created, a volume is briefly unavailable — the
    // question is still answerable textually, and answering it is safer than
    // declining to: the callers read a `false` here as permission to search
    // above the runtime home, which is exactly what must not happen by
    // accident.
    lexically_normalized(cwd.as_path()).starts_with(lexically_normalized(whisply_home))
}

/// A path with `.` and `..` resolved textually, without consulting the
/// filesystem.
...
fn ancestors_within_runtime_home(
    cwd: &AbsolutePathBuf,
    whisply_home: &Path,
) -> Vec<AbsolutePathBuf> {
    if is_inside_runtime_home(cwd, whisply_home) {
        return vec![cwd.clone()];
    }
...
async fn find_project_root(
    ...
    for ancestor in ancestors_within_runtime_home(cwd, whisply_home) {
...
async fn find_git_checkout_root(
    ...
    for dir in ancestors_within_runtime_home(&base, whisply_home) {
