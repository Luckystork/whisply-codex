...
pub enum ImageProcessingError {
    ...
    #[error("image at {path} needs more room to decode than one image is given: {source}")]
    ImageTooLargeToDecode {
        path: PathBuf,
        #[source]
        source: image::ImageError,
    },
...
impl ImageProcessingError {
    pub fn decode_error(path: &std::path::Path, source: image::ImageError) -> Self {
        // A refusal on size is not a decoding failure and not an unsupported
        // format. Reporting it as either sends someone to convert a file that
        // is already a supported format, when what they need is a smaller one.
        if matches!(source, ImageError::Limits(_)) {
            return ImageProcessingError::ImageTooLargeToDecode {
