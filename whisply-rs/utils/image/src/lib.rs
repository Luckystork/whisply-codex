...
use whisply_utils_cache::BlockingLruCache;
use whisply_utils_cache::sha1_digest;
...
///
/// Comfortably above any real screenshot, photograph, or diagram -- an
/// uncompressed 6000x6000 PNG lands around 30 MB -- and far below the point
/// where holding the encoded bytes, the base64 form of them, and the decoded
/// pixels at once matters on the machine the product runs on.
pub const MAX_PROMPT_IMAGE_INPUT_BYTES: usize = 64 * 1024 * 1024;
/// Largest source width or height this will decode.
///
/// Independent of the output limits, which describe what is sent to a model.
/// This one is about what is read into memory before anything is resized.
pub const MAX_DECODED_IMAGE_DIMENSION: u32 = 20_000;
/// Most memory one image decode may ask for.
///
/// The encoded size says almost nothing about this: a few hundred kilobytes of
/// PNG can describe a 40,000 x 40,000 image, which is six gigabytes of pixels
/// the moment it is decoded. Whisply runs alongside everything else on
/// someone's Mac, so an image that needs more than this is refused rather than
/// attempted.
pub const MAX_IMAGE_DECODE_BYTES: u64 = 256 * 1024 * 1024;
const MAX_IMAGE_CACHE_BYTES: usize = 64 * 1024 * 1024;

/// What a single decode is allowed to cost.
...
pub fn load_for_prompt_bytes(
    ...
    let image = (move || {
        ...
        // Set before the decoder is made: the PNG decoder takes its limits at
        // construction and cannot be told afterwards, and PNG is the format a
        // pixel bomb arrives as.
