...
pub(crate) struct EventProcessorWithHumanOutput {
    ...
    account_is_metered_by_whisply: bool,
}

/// What a finished run says about what it consumed.
#[derive(Debug, PartialEq)]
pub(crate) enum RunUsageReport {
    /// Nothing was consumed, or nothing true can be said about it.
    Nothing,
    /// A blended token total, for a run Whisply does not meter.
    Tokens(i64),
    /// The account's cost-based Usage, or the reason it could not be read.
    Account(String),
}

/// Chooses what to report at the end of a run.
///
/// A Whisply-metered account is billed by cost, not by tokens, so a token total
/// would name a unit the account is not charged in; when the read fails, the
/// reason is reported instead of falling back to tokens, because falling back
/// would answer a question the user did not ask with the wrong unit. A local or
/// bring-your-own-key run is not metered by Whisply at all, and for it the
/// token total is the only true thing there is to say.
pub(crate) fn run_usage_report(
    account_is_metered_by_whisply: bool,
    token_usage: Option<&ThreadTokenUsage>,
    read_account_usage: impl FnOnce() -> Result<ContextualUsageSnapshot, String>,
) -> RunUsageReport {
    if account_is_metered_by_whisply {
        return RunUsageReport::Account(match read_account_usage() {
            Ok(snapshot) => codex_whisply::format_account_usage(&snapshot),
            Err(message) => message,
        });
    }

    match token_usage {
        Some(usage) => RunUsageReport::Tokens(blended_total(usage)),
        None => RunUsageReport::Nothing,
    }
}

fn read_account_usage_from_environment() -> Result<ContextualUsageSnapshot, String> {
    NativeBrokerClient::from_environment()
        .ok()
        .flatten()
        .ok_or_else(|| codex_whisply::ACCOUNT_USAGE_UNAVAILABLE.to_string())
        .and_then(|broker| codex_whisply::read_account_usage(&broker))
}
...
impl EventProcessorWithHumanOutput {
    pub(crate) fn create_with_ansi(
        ...
        Self {
            ...
            account_is_metered_by_whisply: config.model_provider_id == WHISPLY_PROVIDER_ID,
        }
    }

    fn print_usage_summary(&self) {
        match run_usage_report(
            self.account_is_metered_by_whisply,
            self.last_total_token_usage.as_ref(),
            read_account_usage_from_environment,
        ) {
            RunUsageReport::Nothing => {}
            RunUsageReport::Tokens(total) => eprintln!(
                "{}\n{}",
                "tokens used".style(self.dimmed),
                format_with_separators(total)
            ),
            RunUsageReport::Account(text) => eprintln!("{}", text.style(self.dimmed)),
        }
    }
