...
use codex_whisply::WhisplyTerminalStatus;
...
use event_processor_with_jsonl_output::account_usage_from_snapshot;
...
use whisply_app_server_client::EnvironmentManager;
use whisply_app_server_client::ExecServerRuntimePaths;
use whisply_app_server_client::InProcessAppServerClient;
use whisply_app_server_client::InProcessClientStartArgs;
use whisply_app_server_client::InProcessServerEvent;
use whisply_arg0::Arg0DispatchPaths;
use whisply_config::CloudConfigBundleLoader;
use whisply_config::ConfigLoadError;
use whisply_config::ConfigLoadOptions;
use whisply_config::LoaderOverrides;
use whisply_config::format_config_error_with_source;
use whisply_core::StateDbHandle;
use whisply_core::check_execpolicy_for_warnings;
use whisply_core::config::Config;
use whisply_core::config::ConfigBuilder;
use whisply_core::config::ConfigOverrides;
use whisply_core::config::ConfigTomlLoadResult;
use whisply_core::config::find_codex_home;
use whisply_core::config::load_config_toml_with_layer_stack;
use whisply_core::config::resolve_oss_provider;
use whisply_core::config::resolve_profile_v2_config_path;
use whisply_core::find_thread_meta_by_name_str;
use whisply_core::format_exec_policy_error_with_source;
use whisply_core::path_utils;
use whisply_core::read_session_meta_line;
...
use whisply_login::default_client::set_default_originator;
...
use whisply_otel::set_parent_from_context;
use whisply_otel::traceparent_context_from_env;
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    let cli_kv_overrides = match config_overrides.parse_overrides() {
        ...
        Err(e) => {
            ...
            std::process::exit(WhisplyTerminalStatus::InvalidConfiguration.exit_code());
        }
    };
    ...
    let codex_home = match find_codex_home() {
        ...
        Err(err) => {
            ...
            std::process::exit(WhisplyTerminalStatus::EnvironmentUnavailable.exit_code());
        }
    };
    ...
    } = resolve_cli_working_root(&codex_home, cwd.clone(), no_directory)?;
    ...
    let overrides = ConfigOverrides {
        ...
        // No selector and nobody to ask, so exec does not start in `Auto`.
        default_approvals_reviewer: None,
        sandbox_mode,
        permission_profile: None,
        default_permissions: None,
        cwd: resolved_cwd,
        // An explicit `--cd` is the user's directory choice; falling back to the
        // directory exec was launched from is not, and `--no-directory` is the
        // explicit opposite of one.
        workspace_directory_selected: Some(workspace_directory_selected),
        // Whisply's `Auto` permission mode is a native-app control; exec keeps
        // the upstream reviewer behavior.
    ...
    if let Err(err) = enforce_login_restrictions(&config.auth_config()).await {
        eprintln!("{err}");
        std::process::exit(WhisplyTerminalStatus::AuthenticationRequired.exit_code());
    }

    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        whisply_core::otel_init::build_provider(
            &config,
    ...
    whisply_core::otel_init::record_process_start(otel.as_ref(), "whisply_exec");
    whisply_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "whisply_exec");
    ...
    let state_db = whisply_core::init_state_db(&config).await;
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let mut event_processor: Box<dyn EventProcessor> = match json_mode {
        true => {
            let processor = EventProcessorWithJsonOutput::new(last_message_file.clone());
            // Only a Whisply-metered account has cost to report; a local or
            // bring-your-own-key run is billed elsewhere, and reading Whisply
            // Usage for it would report a number that means nothing.
            let processor = match config.model_provider_id == WHISPLY_PROVIDER_ID {
                true => processor.with_account_usage_reader(Box::new(|| {
                    let broker = NativeBrokerClient::from_environment().ok().flatten()?;
                    let snapshot = codex_whisply::read_account_usage(&broker).ok()?;
                    Some(account_usage_from_snapshot(&snapshot))
                })),
                false => processor,
            };
            Box::new(processor)
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = whisply_core::review_prompts::user_facing_hint(&review_request.target);
        ...
        && get_git_repo_root(&default_cwd).is_none()
    {
        ...
        std::process::exit(WhisplyTerminalStatus::PreconditionUnmet.exit_code());
    }
    ...
    if !json_mode
        && let Some(message) =
            whisply_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
...
fn session_configured_from_thread_response(
    ...
    approvals_reviewer: whisply_protocol::config_types::ApprovalsReviewer,
    permission_profile: PermissionProfile,
    active_permission_profile: Option<whisply_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
async fn handle_server_request(
    ...
    let handle_result = match request {
        ...
        | ServerRequest::WhisplyToolAdmittedExecute { request_id, .. }
        | ServerRequest::WhisplyToolAdmittedCancel { request_id, .. } => {
