    ...
    match decision {
        ...
        EscalationDecision::Escalate(execution) => {
            ...
            command
                ...
                // `PreparedExec::env` is the whole environment for this
                // command, not an overlay on this process's. Without clearing
                // first, the escalated command -- the one that runs with the
                // sandbox stepped out of the way -- would inherit everything
                // this runtime holds: the handles naming its gateway and
                // native broker, the managed-proxy variables the preparation
                // step deliberately strips, and whatever the person's shell
                // environment policy excluded.
                .env_clear()
                .envs(&env)
                .current_dir(&cwd)
                .stdin(Stdio::null())
...
mod tests {
    ...
    use whisply_protocol::approvals::EscalationPermissions;
    ...
    ///
    /// The escalate server runs inside the runtime process, so "what this
    /// process holds" is what the test process holds. These tests already
    /// serialize on `ESCALATE_SERVER_TEST_LOCK`.
    struct EnvVarGuard {
        key: &'static str,
    ...
    /// WCD-718: the escalated command is the one running with the sandbox
    /// stepped out of the way, and `PreparedExec::env` is the whole environment
    /// it was prepared to receive. Inheriting this runtime's environment on top
    /// of it would hand that command the handles naming the gateway and native
    /// broker, put back the managed-proxy variables preparation deliberately
    /// strips, and undo whatever the person's shell environment policy
    /// excluded.
    #[tokio::test]
    async fn handle_escalate_session_gives_the_escalated_command_only_its_prepared_environment()
    -> anyhow::Result<()> {
        let _guard = ESCALATE_SERVER_TEST_LOCK.acquire().await?;
        let _broker =
            EnvVarGuard::set("WHISPLY_NATIVE_BROKER_SOCKET", "/tmp/should-not-cross.sock");
        let _excluded = EnvVarGuard::set("ESCALATION_EXCLUDED_SECRET", "should-not-cross");
        ...
        client
            .send(EscalateRequest {
                ...
                argv: vec![
                    "sh".to_string(),
                    "-c".to_string(),
                    r#"if [ -n "$WHISPLY_NATIVE_BROKER_SOCKET" ]; then exit 3; fi
                       if [ -n "$ESCALATION_EXCLUDED_SECRET" ]; then exit 4; fi
                       if [ "$KEY" = VALUE ]; then exit 42; fi
                       exit 1"#
                        .to_string(),
                ],
                workdir: AbsolutePathBuf::current_dir()?,
        ...
        match result.exit_code {
            3 => {
                panic!("the escalated command was told where this runtime's broker can be reached")
            }
            4 => panic!("the escalated command inherited a variable it was not prepared with"),
            42 => {}
            other => panic!("escalated command exited {other}, expected 42"),
        }
