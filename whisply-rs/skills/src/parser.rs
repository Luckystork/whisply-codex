...
struct SkillFrontmatter {
    ...
    /// Widely used across skill ecosystems, and until now parsed and dropped.
    /// Read as an opaque string rather than a semantic version: packages in the
    /// wild use dates, commit shorthands, and bare integers, and refusing those
    /// would report no version for a package that plainly declares one.
    #[serde(default)]
...
pub struct ParsedSkillFrontmatter {
    ...
    /// Whatever the package declared, sanitized and bounded. Absent when the
    /// package declared nothing, which must stay distinguishable from a package
    /// that declared an empty value.
...
pub fn parse_skill_frontmatter_metadata(
    ...
    // A malformed version must not make an otherwise valid skill unloadable, so
    // an over-long one is dropped rather than rejected.
    let version = version.filter(|value| value.chars().count() <= MAX_VERSION_LEN);
...
/// YAML happily types `version: 1.0` as a float and `version: 2026-01-05` as a
/// date. Reading only strings would report no version for packages that are
/// perfectly explicit about theirs.
